...
```
MyEngine/
├── xmake.lua                 # 仅工程元数据与 includes 编排
├── xmake/
│   ├── module.lua            # myengine.module 自动发现与三库声明规则
│   ├── packages.lua          # 唯一 add_requires 来源
│   ├── build_config.lua      # BuildConfig、Runtime API、Editor API
│   └── scripts/architecture.lua
├── design.md                 # 本文档
├── Content/                  # 游戏内容（构建后复制到输出目录）
├── tests/                    # 测试 target、本地 xmake.lua 与架构检查夹具
├── thirdparty/               # MyEngine.ThirdParty：Lua、ImGuizmo、AngelScript addons
└── src/
    ├── Apps/                 # 每个产品目录拥有入口、资源与本地 xmake.lua
    ├── Runtime/
    │   ├── Math|Core|Project|Input|Assets|Scene|... # Runtime 逻辑目录
    │   ├── Renderer/
    │   │   ├── RHI/          # 平台无关接口
    │   │   └── Backends/     # D3DCommon、D3D11、D3D12、Vulkan、Metal
    │   └── RuntimeModule/    # 唯一组合根，生成 runtime.dll
    └── Editor/
        ├── Cooking/          # Publisher、Validator、CookDependencyGraph
        └── Backends/         # Editor-only ImGui/Vulkan/Metal 适配
```
...
| 目标 | 类型 | 说明 |
|------|------|------|
| `MyEngine.ThirdParty` | `static` | Lua、ImGuizmo 与 AngelScript addons 编译一次；不依赖 Runtime 或 Editor |
| `MyEngineRuntime` | `shared`（basename `runtime`） | 直接编译全部 Runtime 与当前平台后端源码；对外仍只有 `runtime.dll` |
| `MyEngine.Editor` | `static` | Editor、Cooking 与 Editor backend 源码只编译一次；Editor、Cooker 与 Tests 复用 |
| `MyEngineEditor` / `MyEnginePlayer` / `MyEngineCooker` | `binary` | 各自在 `src/Apps/<Product>/` 声明，产品名与发布布局保持兼容 |
| `MyEngineIconTool` / `MyEngineEditorPackager` | `binary` | 工具入口也由各自目录拥有 |
| `MyEngineTests` | `binary` | 链接 `runtime.dll` 与 Editor static lib，不重复编译 Editor/Cooking 源码 |
| `MyEngine.Architecture` | `phony` | 构建期检查三库 DAG、源码归属、平台过滤、fragment、feature/define 与边界退化 |

仅 `thirdparty/xmake.lua`、`src/Runtime/xmake.lua` 与 `src/Editor/xmake.lua` 声明编译库。`myengine.module` 从一个或多个目录递归发现 `.c/.cpp` 与头文件；`.rc` 仅在 Windows、`.m/.mm` 仅在 macOS 编译，D3D/Vulkan/Metal 目录按平台和 feature 过滤。`fragment_dirs` 内源码不作为 translation unit，且必须由同库的非 fragment translation unit 恰好 include 一次。内部 Runtime/Editor 目录不再维护 library target 或源码数组。
...
| **imgui / imnodes** | Editor UI 与 Shader Graph |
| **nlohmann_json** | `SceneSerializer` 与测试 |
| **stb** | 图像加载等 |
| **RmlUi** | Runtime UI |
| **Jolt Physics** | Physics 实现（private） |
| **AngelScript** | Scripting 实现；Editor 调试域按需使用 |
| **Vulkan Headers** | Vulkan backend（private） |
| **miniaudio（vendored）** | 运行时音频设备、解码和播放 |
| **Slang / slangc（SDK/tool）** | 统一 Shader 编译入口；`.shader + .hlsl/.hlsli` 一次编写，cook/热编译生成后端产物 |

Xmake package 版本只在 `xmake/packages.lua` 声明。三库的本地脚本集中声明直接 package 与平台 syslink/framework：Windows 使用 D3D/DXGI，macOS 使用 Metal/MetalKit。
...
Runtime 内部仍遵守以下源码层次，但这些目录不再生成独立 build target；构建期强制 DAG 为 `Editor → Runtime → ThirdParty`，Editor 也可直接依赖 ThirdParty：

| 层级 | 模块 |
|------|------|
| 基础 | `Math`; `Core → Math`; `Project → Core/Math`; `Renderer.RHI → Core/Math` |
| 数据 | `Input → Core/Project/Math`; `Assets → Core/Project/RHI/Math`; `Scene → Assets/Core/Project/Math` |
| 系统 | `Physics → Scene`; `Navigation → Physics/Scene/Assets`; `Audio → Scene/Assets`; `Camera → Physics/Scene`; `Animation → Scene/Assets` |
| 渲染 | `UI → RHI/Scene/Assets/Input`; `DebugDraw → Physics/Scene/Assets`; `Renderer → RHI/UI/Scene/Assets/Animation/Camera/DebugDraw` |
| 上层 | `Gameplay → Renderer/Navigation/Physics/Scene`; `Game → Gameplay/Renderer/UI/Audio/Input/Scene`; `Scripting → Game/Gameplay/Renderer/UI/Physics/Scene`; `Miscs → Renderer/Core` |

`Assets` 持有动画、音频、脚本、NavMesh 与 Prefab 的纯数据契约；`Scene` 持有通用 Actor/Component、调度器及 Physics/Navigation 子系统接口。具体子系统、组件、启用后端和 D3D shader compiler 都由 `RuntimeModule` 组合根按确定顺序显式注册，注册过程幂等，不依赖 static lib 全局初始化顺序。UI 通过生命周期回调连接 Scripting。
...
flowchart TB
    subgraph apps [可执行入口]
        EditorExe[src/Apps/Editor/EditorMain.cpp]
        PlayerExe[src/Apps/Player/PlayerMain.cpp]
    ...
    subgraph runtime_dll [MyEngineRuntime 共享库]
        ...
        R[Renderer]
        RHI[Renderer.RHI]
        Backend[D3D/Vulkan/Metal backend sources]
        Cam[Camera]
        Sc[Scene + Serializer]
        Ast[Assets]
        Core[Core]
    ...
    TestsExe --> Editor
    Editor --> runtime_dll
    Editor --> ThirdParty[MyEngine.ThirdParty]
    runtime_dll --> ThirdParty
    Editor --> Game
    Editor --> Core
    Game --> Sc
    Game --> R
...
|------|------------------|-------------|--------------|
| **编辑器**（`src/Apps/Editor/EditorMain.cpp`） | 通过已注册 backend factory 创建 | 先 `SceneRenderLayer`，后 `EditorLayer` | `SceneRenderLayer::SetPresentEnabled(false)`，由编辑器在 ImGui 之后统一 `EndFrame` |
| **玩家**（`src/Apps/Player/PlayerMain.cpp`） | 同上 | 仅 `SceneRenderLayer` | `SetPresentEnabled(true)`，场景层内 Present |
...
- **macOS**：`MetalContext`；Editor Metal 源码目录通过本地 `.mm` wrapper 编译 ImGui Metal 实现，不存在跨目录手写源码数组或辅助 target。
...
- **行主序** `Mat4`，**左手坐标系**，**Y 向上**，与 D3D 深度 0..1 及 HLSL `mul(vector, matrix)` 风格一致（所有权在 `Math/EngineMath.h`；`Core/EngineMath.h` 为兼容转发头）。
