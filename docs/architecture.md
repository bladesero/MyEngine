...
| `MyEngineEditor` | binary | 编译 `main.cpp` 和 `src/Editor`，链接 `MyEngineRuntime`，推入 `SceneRenderLayer` 与 `EditorLayer`。 |
| `MyEnginePlayer` | binary | 编译 `player_main.cpp`，链接 `MyEngineRuntime`，只推入 `SceneRenderLayer`，用于无编辑器运行。 |
...
MyEngine/
  xmake.lua
  main.cpp                 # Editor 入口
  player_main.cpp          # Player 入口
  cooker_main.cpp          # Cooker 入口
  icon_tool_main.cpp       # 图标生成入口
  editor_packager_main.cpp
  src/
    Runtime/
      Animation/
      Assets/
...
```mermaid
flowchart TB
    EditorExe["MyEngineEditor / main.cpp"] --> Editor
    PlayerExe["MyEnginePlayer / player_main.cpp"] --> Game
