...
void TriangleLayer::OnAttach() {
    ...
    m_VB = m_Renderer->CreateVertexBuffer(vertices, sizeof(vertices), sizeof(Vertex));

    // ---- Shader ------------------------------------------------------------
    ShaderManager::Get().SetDevice(m_Renderer);
    m_ShaderHandle = ShaderManager::Get().GetOrCreate("Content/Engine/Shaders/Triangle.shader", k_Layout, 2);
    ...
    m_Camera.SetPerspective(60.0f, static_cast<float>(m_VpW) / static_cast<float>(m_VpH), 0.1f, 1000.0f);
}
...
void TriangleLayer::OnUpdate(float dt) {
    ...
    if (m_RmbDown) {
        ...
        if (relX != 0 || relY != 0) {
            m_Camera.Orbit(static_cast<float>(relX) * 0.3f, static_cast<float>(relY) * 0.3f);
        }
    }
    ...
    ++frames;
    if (acc >= 1.0f) {
        Logger::Info("FPS=", frames, "  frame=", Time::FrameCount(), "  total=", Time::TotalSeconds(), "s");
        acc = 0.0f;
        frames = 0;
    }
}
