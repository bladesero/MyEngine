...
SceneManager::~SceneManager() {
    if (m_WorkerTask.Valid())
        m_WorkerTask.Cancel();
    for (auto& task : m_StaleTasks)
        task.Cancel();
...
SceneLoadRequestID SceneManager::RequestLoad(std::string path, SceneLoadOptions options) {
    if (!IsSafeScenePath(path)) {
        ...
        m_State = SceneLoadState::Failed;
        return 0;
    }
    if (m_WorkerTask.Valid()) {
        m_WorkerTask.Cancel();
        m_StaleTasks.push_back(std::move(m_WorkerTask));
    }
    ReleasePinnedDependencies();
    ++m_RequestID;
...
void SceneManager::CancelLoad() {
    if (m_State == SceneLoadState::Ready || m_State == SceneLoadState::Failed || m_State == SceneLoadState::Idle)
        return;
    if (m_WorkerTask.Valid()) {
        m_WorkerTask.Cancel();
        m_StaleTasks.push_back(std::move(m_WorkerTask));
    ...
    m_PreloadStarted = false;
    m_UploadFence = 0;
    m_State = SceneLoadState::Cancelled;
    m_Progress = 0.0f;
}

bool SceneManager::RetryLastLoad() {
    if (m_RequestedPath.empty() || m_State != SceneLoadState::Failed)
        return false;
...
void SceneManager::DismissFailure() {
    if (m_State != SceneLoadState::Failed)
        return;
...
bool SceneManager::Process(std::unique_ptr<Scene>& loadedScene) {
    ...
    if (m_State == SceneLoadState::Requested) {
            ...
            m_TaskScope, {"scene.read", TaskPriority::High}, [absolute, id](CancellationToken token) {
                ...
                if (!RuntimeFileSystem::Get().ReadText(absolute.string(), result.source, &result.error)) {
                    if (result.error.empty())
                        result.error = "cannot open scene file";
                    return result;
                }
                token.ThrowIfCancellationRequested();
                result.success = true;
        ...
        m_State = SceneLoadState::Reading;
        m_Progress = 0.05f;
        return false;
    }
    if (m_State == SceneLoadState::Reading) {
        if (!m_WorkerTask.Valid() || !m_WorkerTask.IsReady())
            return false;
        WorkerResult result;
        try {
            result = m_WorkerTask.Get();
        } catch (const TaskCancelled&) {
            m_State = SceneLoadState::Cancelled;
            return false;
        } catch (const std::exception& exception) {
            m_LastError = "scene read task failed: " + std::string(exception.what());
            m_State = SceneLoadState::Failed;
            return false;
        }
        m_WorkerTask = {};
        if (result.requestID != m_RequestID)
            return false;
        if (!result.success) {
            m_LastError = "failed to read scene '" + m_RequestedPath + "': " + result.error;
            m_State = SceneLoadState::Failed;
            return false;
        }
        m_Source = std::move(result.source);
        const SceneLoadRequestID id = m_RequestID;
        m_WorkerTask = TaskService::Get().Submit(m_TaskScope, {"scene.parse", TaskPriority::High},
                                                 [source = std::move(m_Source), id](CancellationToken token) mutable {
                                                     WorkerResult parsed;
                                                     parsed.requestID = id;
                                                     token.ThrowIfCancellationRequested();
                                                     parsed.success = SceneSerializer::BuildLoadPlan(
                                                         source, parsed.plan, &parsed.error);
                                                     token.ThrowIfCancellationRequested();
                                                     return parsed;
                                                 });
        m_State = SceneLoadState::Parsing;
        m_Progress = 0.15f;
        return false;
    }
    if (m_State == SceneLoadState::Parsing) {
        ...
        try {
            ...
            m_LastError = "scene parse task failed: " + std::string(exception.what());
            m_State = SceneLoadState::Failed;
            return false;
        }
        m_WorkerTask = {};
        if (result.requestID != m_RequestID)
            return false;
        if (!result.success) {
            m_LastError = "failed to parse scene '" + m_RequestedPath + "': " + result.error;
            m_State = SceneLoadState::Failed;
            return false;
        }
        m_Plan = std::move(result.plan);
        m_State = SceneLoadState::Preloading;
        m_Progress = 0.3f;
        return false;
    }
    if (m_State == SceneLoadState::Preloading) {
        if (!m_PreloadStarted) {
            ...
            for (const std::string& path : m_PinnedDependencies)
            ...
            for (const std::string& path : paths)
                m_PreloadTasks.push_back(AssetManager::Get().RequestAsync(path));
            m_PreloadStarted = true;
        }
        size_t ready = 0;
        for (auto& task : m_PreloadTasks)
            if (task.IsReady())
                ++ready;
        if (ready < m_PreloadTasks.size()) {
            m_Progress = 0.3f + 0.15f * static_cast<float>(ready) /
                                    static_cast<float>(std::max<size_t>(1, m_PreloadTasks.size()));
            return false;
        }
        if (m_Options.waitForCriticalAssets)
            for (auto& task : m_PreloadTasks)
                try {
                    if (!task.Get()) {
        ...
        m_Candidate = std::make_unique<Scene>();
        m_Instantiation = {};
        m_State = SceneLoadState::Instantiating;
        m_Progress = 0.45f;
        return false;
    }
    if (m_State == SceneLoadState::Uploading) {
        if (!GpuUploadQueue::Get().IsFenceComplete(m_UploadFence))
            return false;
        m_State = SceneLoadState::Activating;
        m_Progress = 0.98f;
        return false;
    ...
    if (m_State == SceneLoadState::Activating) {
        ...
        m_DependenciesPinned = false;
        loadedScene = std::move(m_Candidate);
        m_State = SceneLoadState::Ready;
        m_Progress = 1.0f;
        return true;
    }
    if (m_State != SceneLoadState::Instantiating || !m_Candidate)
        return false;
    ...
    } while (!complete && std::chrono::duration<double, std::milli>(std::chrono::steady_clock::now() - start).count() <
                              m_Options.instantiateBudgetMs);
    if (!complete)
        return false;
    m_UploadFence = GpuUploadQueue::Get().CaptureFence();
    m_State = SceneLoadState::Uploading;
    m_Progress = 0.9f;
    return false;
}

void SceneManager::SetPersistentValue(const std::string& key, nlohmann::json value) {
    if (!key.empty())
        m_PersistentData[key] = std::move(value);
}
nlohmann::json SceneManager::GetPersistentValue(const std::string& key, const nlohmann::json& fallback) const {
    const auto it = m_PersistentData.find(key);
    return it == m_PersistentData.end() ? fallback : *it;
}
