...
void SceneRenderLayer::OnUpdate(float dt)
{
    ...
    if(m_ResourceBudgetEnabled)m_ResourceBudget.Tick();
...
void SceneRenderLayer::OnEvent(Event& event)
{
    ...
    if(event.type==EventType::KeyDown&&!event.key.repeat){
        if(GetSceneManager().IsLoading()&&event.key.scancode==SDL_SCANCODE_ESCAPE){
            GetSceneManager().CancelLoad();event.handled=true;SyncSceneLoadOverlay();return;
        }
        if(loadState==SceneLoadState::Failed){
            if(event.key.scancode==SDL_SCANCODE_R||event.key.scancode==SDL_SCANCODE_RETURN){
                GetSceneManager().RetryLastLoad();event.handled=true;SyncSceneLoadOverlay();return;
            }
            if(event.key.scancode==SDL_SCANCODE_ESCAPE){
                GetSceneManager().DismissFailure();event.handled=true;SyncSceneLoadOverlay();return;
            }
        }
    }
    if (m_RuntimeScreensEnabled && !event.handled) {
        SyncRuntimeScreen();
        if (m_RuntimeScreens.Empty() && event.type == EventType::KeyDown &&
            !event.key.repeat && event.key.scancode == SDL_SCANCODE_ESCAPE &&
            GetGameFlowController().GetState() == GameFlowState::Gameplay) {
            GetGameFlowController().RequestPause(
                GamePauseReason::User, &GetSimulationScene());
            SyncRuntimeScreen();
            event.handled = true;
            return;
        }
        if (!m_RuntimeScreens.Empty()) {
            ...
            bool activate = false;
            if (m_UISystem.ProcessRuntimeScreenPointer(
                    event, m_UIInputViewport, pointerIndex, activate)) {
...
void SceneRenderLayer::SetRuntimeScreensEnabled(bool enabled)
{
    ...
    if (!enabled) m_RuntimeScreens.Clear();
    SyncRuntimeScreen();
}
...
{
    std::string text;
    if(!RuntimeFileSystem::Get().ReadText(path,text,error))return false;
    RuntimeUIScreenConfig config;
    if(!RuntimeUIScreenConfig::FromText(text,config,error))return false;
    if(!config.Apply(m_RuntimeScreens,error))return false;
    SyncRuntimeScreen();return true;
}
...
void SceneRenderLayer::SyncRuntimeScreen()
{
    ...
    case GameFlowState::Paused: desiredRoot = "pause"; break;
    case GameFlowState::MainMenu: desiredRoot = "mainMenu"; break;
    case GameFlowState::GameOver: desiredRoot = "gameOver"; break;
    default: break;
    }
...
{
    if (event.type == RuntimeUIStackEventType::None ||
        event.type == RuntimeUIStackEventType::FocusChanged) return;
    ...
    if (event.action == "settings") {
        ...
        if (!path.empty()) RequestSceneLoad(path);
        else GetGameFlowController().EnterGameplay(&scene);
...
void SceneRenderLayer::RefreshRuntimeSettingsLabels()
{
    ...
    m_RuntimeScreens.SetActionLabel("settings", "masterDown",
        "Master " + percent(audio.GetMasterVolume()) + "  -");
    m_RuntimeScreens.SetActionLabel("settings", "masterUp",
        "Master " + percent(audio.GetMasterVolume()) + "  +");
    for (const auto& entry : std::vector<std::pair<AudioBus, std::string>>{
             {AudioBus::Music, "music"}, {AudioBus::Effects, "effects"},
             {AudioBus::Voice, "voice"}}) {
        const std::string display = entry.second == "music" ? "Music" :
            (entry.second == "effects" ? "Effects" : "Voice");
...
void SceneRenderLayer::AdjustRuntimeSetting(const std::string& action)
{
    ...
    const bool accessibilityAction=action=="subtitles"||action=="highContrast"||
        action.rfind("uiScale",0)==0;
    if (!increase && !decrease && !accessibilityAction) return;
    ...
    if (action.rfind("master", 0) == 0) {
        ...
        if (action.rfind("music", 0) == 0) bus = AudioBus::Music;
        else if (action.rfind("effects", 0) == 0) bus = AudioBus::Effects;
        else if (action.rfind("voice", 0) == 0) bus = AudioBus::Voice;
        if (bus != AudioBus::Count) audio.SetBusVolume(bus, audio.GetBusVolume(bus) + delta);
    }
    ...
    if(action.rfind("uiScale",0)==0)ui.uiScale=std::clamp(ui.uiScale+delta,0.5f,2.0f);
    else if(action=="subtitles")ui.subtitles=!ui.subtitles;
    else if(action=="highContrast")ui.highContrast=!ui.highContrast;
    if(!m_UISystem.SetAccessibilitySettings(ui,&error)){
        Logger::Warn("[RuntimeUI] Could not apply accessibility settings: ",error);return;
    }
