...
GameLayer::GameLayer(SDL_Renderer* renderer)
    : Layer("GameLayer"), m_Renderer(renderer) {}
...
void GameLayer::OnEvent(Event& event) {
    ...
    case EventType::KeyDown:
        Logger::Info(Name(), " KeyDown scancode=", event.key.scancode,
                     " repeat=", event.key.repeat ? "yes" : "no");
        // ESC → quit
    ...
    case EventType::WindowResize:
        Logger::Info(Name(), " WindowResize ",
                     event.resize.width, "x", event.resize.height);
        break;
    default:
        break;
    }
