...
GameFlowController::~GameFlowController() {
    if (g_ActiveFlowController != this)
        return;
...
void GameFlowController::EnterBoot(Scene* scene) {
    SetBaseState(GameFlowState::Boot, scene);
}
void GameFlowController::EnterMainMenu(Scene* scene) {
    SetBaseState(GameFlowState::MainMenu, scene);
}
void GameFlowController::EnterGameplay(Scene* scene) {
    SetBaseState(GameFlowState::Gameplay, scene);
}
void GameFlowController::EnterGameOver(Scene* scene) {
    SetBaseState(GameFlowState::GameOver, scene);
}
...
void GameFlowController::RequestPause(GamePauseReason reason, Scene* scene) {
    if (reason == GamePauseReason::None)
        return;
...
void GameFlowController::Recompute(Scene* scene) {
    ...
    if (scene) {
        if (m_Snapshot.simulationPaused)
            scene->Pause();
        else
            scene->Resume();
    }
