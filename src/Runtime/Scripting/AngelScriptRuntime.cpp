...
public:
    explicit ActiveScriptComponentScope(ScriptComponent& component) : m_Previous(g_ActiveComponent) {
        g_ActiveComponent = &component;
    }

    ~ActiveScriptComponentScope() { g_ActiveComponent = m_Previous; }
...
std::vector<uint64_t> ToUInt64Values(const std::vector<ActorHandle>& handles) {
    ...
    for (const ActorHandle& handle : handles)
        values.push_back(handle.ToUInt64());
    return values;
}

class ScriptUInt64Array {
public:
    explicit ScriptUInt64Array(std::vector<uint64_t> values = {}) : m_Values(std::move(values)) {}
    ...
    uint64_t At(uint32_t index) const { return index < m_Values.size() ? m_Values[index] : 0; }
...
void UInt64ArrayAddRef(ScriptUInt64Array* array) {
    if (array)
        array->AddRef();
}
void UInt64ArrayRelease(ScriptUInt64Array* array) {
    if (array)
        array->Release();
}
uint32_t UInt64ArrayLength(const ScriptUInt64Array* array) {
...
bool TryParseIncludeDirective(const std::string& line, std::string& includePath) {
    ...
    if (!isHashInclude && !isImport)
        return false;
    const size_t firstQuote = trimmed.find('"');
    const size_t secondQuote = firstQuote == std::string::npos ? std::string::npos : trimmed.find('"', firstQuote + 1);
...
std::filesystem::path ResolveScriptIncludePath(const std::string& includePath,
    ...
    if (!IsSafeScriptIncludePath(includePath))
        return {};
    ...
    for (const auto& candidate : candidates) {
        ...
        if (error)
            continue;
        if (RuntimeFileSystem::Get().Exists(normalized.string()))
            return normalized;
    }
    return {};
}

bool PreprocessScriptSourceRecursive(const std::string& source, const std::string& chunkName,
                                     const std::filesystem::path& baseDirectory, std::string& outSource,
                                     std::vector<std::string>* dependencies,
                                     std::unordered_set<std::string>& includeStack, std::string& error, int depth) {
    if (depth > 32) {
    ...
    while (std::getline(input, line)) {
        ...
        if (resolved.empty()) {
            error = chunkName + ":" + std::to_string(lineNumber) + ": failed to resolve script include '" +
                    includePath + "'";
            return false;
        }
        const std::string key = resolved.string();
        if (includeStack.count(key)) {
            error = chunkName + ":" + std::to_string(lineNumber) + ": cyclic script include '" + includePath + "'";
            return false;
        }
        ...
        outSource += "\n";
        if (!PreprocessScriptSourceRecursive(includeSource, key, resolved.parent_path(), outSource, dependencies,
                                             includeStack, error, depth + 1)) {
            includeStack.erase(key);
            return false;
        }
...
Vec3 ActorGetForward() {
    Actor* actor = ActiveActor();
    return actor ? actor->GetWorldMatrix().TransformDir(Vec3::Forward()).Normalized() : Vec3::Forward();
}

Vec3 ActorGetRight() {
    Actor* actor = ActiveActor();
    return actor ? actor->GetWorldMatrix().TransformDir(Vec3::Right()).Normalized() : Vec3::Right();
}

Vec3 ParentGetForward() {
    Actor* actor = ActiveActor();
    Actor* parent = actor ? actor->GetParent() : nullptr;
    return parent ? parent->GetWorldMatrix().TransformDir(Vec3::Forward()).Normalized() : Vec3::Forward();
}

Vec3 ParentGetRight() {
    Actor* actor = ActiveActor();
    Actor* parent = actor ? actor->GetParent() : nullptr;
    return parent ? parent->GetWorldMatrix().TransformDir(Vec3::Right()).Normalized() : Vec3::Right();
}
...
void ActorSetPosition(const Vec3& value) {
    if (Actor* actor = ActiveActor())
        actor->GetTransform().position = value;
}

void ActorSetRotation(const Vec3& value) {
    if (Actor* actor = ActiveActor())
        actor->GetTransform().rotation = value;
}
...
void ActorSetParentRotation(const Vec3& value) {
    ...
    Actor* parent = actor ? actor->GetParent() : nullptr;
    if (parent)
        parent->GetTransform().rotation = value;
}
...
ActorHandle SceneGetSelf() {
    return ActiveActorHandle();
}
bool SceneIsValid(ActorHandle handle) {
...
ActorHandle SceneFindByTag(const std::string& tag) {
    ...
    scene->ForEach([&](Actor& actor) {
        if (!result && actor.HasTag(tag))
            result = actor.GetHandle();
    });
    return result;
}
...
ScriptUInt64Array* SceneGetChildren(ActorHandle handle) {
    ...
    if (Actor* actor = ResolveActor(handle)) {
        for (Actor* child : actor->GetChildren()) {
            if (child)
                values.push_back(child->GetHandle().ToUInt64());
        }
    }
    return new ScriptUInt64Array(std::move(values));
}
...
ColliderComponent* FirstCollider(Actor& actor) {
    ...
    actor.ForEachComponent([&](Component& component) {
        if (!collider)
            collider = dynamic_cast<ColliderComponent*>(&component);
    });
    return collider;
}
...
void SceneDestroyActor(ActorHandle handle) {
    try {
        if (Scene* scene = ActiveScene())
            scene->QueueDestroyActor(handle);
    } catch (const std::exception& e) {
...
void SceneSetActive(ActorHandle handle, bool active) {
    try {
        if (Scene* scene = ActiveScene())
            scene->QueueSetActive(handle, active);
    } catch (const std::exception& e) {
...
void SceneSetLayer(ActorHandle handle, uint32_t layer) {
    try {
        if (Scene* scene = ActiveScene())
            scene->QueueSetLayer(handle, layer);
    } catch (const std::exception& e) {
...
void SceneSetParent(ActorHandle child, ActorHandle parent) {
    try {
        if (Scene* scene = ActiveScene())
            scene->QueueSetParent(child, parent);
    } catch (const std::exception& e) {
...
void SceneMoveActor(ActorHandle child, ActorHandle parent, ActorHandle nextSibling) {
    try {
        if (Scene* scene = ActiveScene())
            scene->QueueMoveActor(child, parent, nextSibling);
    } catch (const std::exception& e) {
...
ActorHandle SceneFindNearestWithComponent(const std::string& type, const Vec3& center, float maxDistance) {
    if (type.empty() || maxDistance < 0.0f)
        return {};
    Scene* scene = ActiveScene();
    if (!scene)
        return {};
...
bool IsProjectRelativeContentPath(const std::string& path) {
    try {
        const std::filesystem::path input(path);
        if (path.empty() || input.is_absolute())
            return false;
        const std::string normalized = input.lexically_normal().generic_string();
        if (normalized == "Content" || normalized.rfind("Content/", 0) != 0)
            return false;
        for (const auto& part : input) {
            if (part == "..")
                return false;
...
ActorHandle SceneInstantiatePrefab(const std::string& path, const Vec3& position, const Vec3& rotation) {
    try {
        Scene* scene = ActiveScene();
        if (!scene || !IsProjectRelativeContentPath(path))
            return {};
        ...
        if (!handle && !error.empty())
            Logger::Warn("[Scripting] prefab instantiate failed: ", error);
        return handle;
    } catch (const std::exception& e) {
...
void ActorSetScaleHandle(ActorHandle handle, const Vec3& value) {
    if (Actor* actor = ResolveActor(handle))
        actor->GetTransform().scale = value;
}

Vec3 TransformGetPosition() {
    return ReadActorPosition();
}
Vec3 TransformGetRotation() {
    return ReadActorRotation();
}
Vec3 TransformGetScale() {
    Actor* actor = ActiveActor();
    return actor ? actor->GetTransform().scale : Vec3::One();
}
Vec3 TransformGetPositionHandle(ActorHandle handle) {
    return ActorGetPositionHandle(handle);
}
Vec3 TransformGetRotationHandle(ActorHandle handle) {
    return ActorGetRotationHandle(handle);
}
Vec3 TransformGetScaleHandle(ActorHandle handle) {
    return ActorGetScaleHandle(handle);
}
void TransformSetPosition(const Vec3& value) {
    ActorSetPosition(value);
}
void TransformSetRotation(const Vec3& value) {
...
void TransformSetPositionHandle(ActorHandle handle, const Vec3& value) {
    ActorSetPositionHandle(handle, value);
}
void TransformSetRotationHandle(ActorHandle handle, const Vec3& value) {
    ActorSetRotationHandle(handle, value);
}
void TransformSetScaleHandle(ActorHandle handle, const Vec3& value) {
    ActorSetScaleHandle(handle, value);
}
...
void TagsSet(const std::string& tag) {
    if (Actor* actor = ActiveActor())
        SceneSetTag(actor->GetHandle(), tag);
}
...
bool AudioListenerSetEnabled(ActorHandle handle, bool enabled) {
    Actor* actor = ResolveActorOrSelf(handle);
    auto* listener = actor ? actor->GetComponent<AudioListenerComponent>() : nullptr;
    if (!listener)
        return false;
    listener->SetEnabled(enabled);
    return true;
}
bool AudioListenerIsEnabled(ActorHandle handle) {
    Actor* actor = ResolveActorOrSelf(handle);
    auto* listener = actor ? actor->GetComponent<AudioListenerComponent>() : nullptr;
    return listener && listener->IsEnabled();
}
bool ParticlePlay(ActorHandle handle) {
    Actor* actor = ResolveActorOrSelf(handle);
    auto* particles = actor ? actor->GetComponent<ParticleSystemComponent>() : nullptr;
    return particles && particles->Play();
}
void ParticleStop(ActorHandle handle) {
    Actor* actor = ResolveActorOrSelf(handle);
    if (auto* particles = actor ? actor->GetComponent<ParticleSystemComponent>() : nullptr)
        particles->Stop();
}
bool ParticleIsPlaying(ActorHandle handle) {
    Actor* actor = ResolveActorOrSelf(handle);
    auto* particles = actor ? actor->GetComponent<ParticleSystemComponent>() : nullptr;
    return particles && particles->IsPlaying();
}
void ParticleEmit(ActorHandle handle, uint32_t count) {
    Actor* actor = ResolveActorOrSelf(handle);
    if (auto* particles = actor ? actor->GetComponent<ParticleSystemComponent>() : nullptr)
...
void BodySetVelocity(const Vec3& value) {
    if (auto* body = ActiveBody())
        body->SetVelocity(value);
}
void BodyAddForce(const Vec3& value) {
    if (auto* body = ActiveBody())
        body->AddForce(value);
}
void BodySetAngularVelocity(const Vec3& value) {
    if (auto* body = ActiveBody())
        body->SetAngularVelocity(value);
}
void BodyAddTorque(const Vec3& value) {
    if (auto* body = ActiveBody())
        body->AddTorque(value);
}
void BodyAddImpulse(const Vec3& value) {
    if (auto* body = ActiveBody())
        body->AddImpulse(value);
}
void BodyAddAngularImpulse(const Vec3& value) {
    if (auto* body = ActiveBody())
        body->AddAngularImpulse(value);
}
void BodyTeleport(const Vec3& position, const Vec3& rotation) {
    if (auto* body = ActiveBody())
        body->Teleport(position, rotation);
}
void BodySetKinematicTarget(const Vec3& position, const Vec3& rotation) {
    if (auto* body = ActiveBody())
        body->SetKinematicTarget(position, rotation);
}
Vec3 BodyGetVelocity(ActorHandle handle) {
    if (auto* body = ResolveBody(handle))
        return body->GetVelocity();
    return Vec3{};
}
void BodySetVelocityHandle(ActorHandle handle, const Vec3& value) {
    if (auto* body = ResolveBody(handle))
        body->SetVelocity(value);
}
void BodyAddForceHandle(ActorHandle handle, const Vec3& value) {
    if (auto* body = ResolveBody(handle))
        body->AddForce(value);
}
void BodySetAngularVelocityHandle(ActorHandle handle, const Vec3& value) {
    if (auto* body = ResolveBody(handle))
        body->SetAngularVelocity(value);
}
void BodyAddTorqueHandle(ActorHandle handle, const Vec3& value) {
    if (auto* body = ResolveBody(handle))
        body->AddTorque(value);
}
void BodyAddImpulseHandle(ActorHandle handle, const Vec3& value) {
    if (auto* body = ResolveBody(handle))
        body->AddImpulse(value);
}
void BodyAddAngularImpulseHandle(ActorHandle handle, const Vec3& value) {
    if (auto* body = ResolveBody(handle))
        body->AddAngularImpulse(value);
}
void BodyTeleportHandle(ActorHandle handle, const Vec3& position, const Vec3& rotation) {
    if (auto* body = ResolveBody(handle))
        body->Teleport(position, rotation);
}
void BodySetKinematicTargetHandle(ActorHandle handle, const Vec3& position, const Vec3& rotation) {
    if (auto* body = ResolveBody(handle))
        body->SetKinematicTarget(position, rotation);
}
void BodySetUseGravity(ActorHandle handle, bool value) {
    if (auto* body = ResolveBody(handle))
        body->SetUseGravity(value);
}
bool BodyUsesGravity(ActorHandle handle) {
    if (auto* body = ResolveBody(handle))
        return body->UsesGravity();
    return false;
}
...
void CharacterControllerMove(const Vec3& velocity) {
    if (auto* controller = ActiveCharacterController())
...
void CharacterControllerSetUseGravity(bool enabled) {
    if (auto* controller = ActiveCharacterController())
        controller->SetUseGravity(enabled);
}

void CharacterControllerMoveHandle(ActorHandle handle, const Vec3& velocity) {
    if (auto* controller = ResolveCharacterController(handle))
        controller->Move(velocity);
}
...
bool ColliderIsTrigger(ActorHandle handle) {
    if (auto* collider = ResolveCollider(handle))
        return collider->IsTrigger();
    return false;
}
void ColliderSetTrigger(ActorHandle handle, bool value) {
    if (auto* collider = ResolveCollider(handle))
        collider->SetTrigger(value);
}
uint32_t ColliderGetLayer(ActorHandle handle) {
    if (auto* collider = ResolveCollider(handle))
        return collider->GetLayer();
    return 0u;
}
void ColliderSetLayer(ActorHandle handle, uint32_t value) {
    if (Actor* actor = ResolveActorOrSelf(handle))
        ForEachCollider(*actor, [&](ColliderComponent& collider) { collider.SetLayer(value); });
}
uint32_t ColliderGetLayerMask(ActorHandle handle) {
    if (auto* collider = ResolveCollider(handle))
        return collider->GetLayerMask();
    return 0u;
}
void ColliderSetLayerMask(ActorHandle handle, uint32_t value) {
    if (Actor* actor = ResolveActorOrSelf(handle))
        ForEachCollider(*actor, [&](ColliderComponent& collider) { collider.SetLayerMask(value); });
}
...
bool ComponentsAdd(ActorHandle handle, const std::string& type) {
    try {
        Scene* scene = ActiveScene();
        if (!scene || !ComponentRegistry::Get().IsRegistered(type))
            return false;
...
bool AudioSourcePlay() {
    if (auto* source = ResolveAudioSource({}))
        return source->Play();
    return false;
}
bool AudioSourcePlayHandle(ActorHandle handle) {
    if (auto* source = ResolveAudioSource(handle))
        return source->Play();
    return false;
}
...
void AudioSourceStopHandle(ActorHandle handle) {
    if (auto* source = ResolveAudioSource(handle))
        source->Stop();
}
...
bool AudioSourceIsPlayingHandle(ActorHandle handle) {
    if (auto* source = ResolveAudioSource(handle))
        return source->IsPlaying();
    return false;
}
void AudioSourceSetClipPath(const std::string& path) {
    if (auto* source = ResolveAudioSource({}))
        source->SetClipPath(path);
}
void AudioSourceSetClipPathHandle(ActorHandle handle, const std::string& path) {
    if (auto* source = ResolveAudioSource(handle))
        source->SetClipPath(path);
}
void AudioSourceSetVolume(float value) {
    if (auto* source = ResolveAudioSource({}))
        source->SetVolume(value);
}
void AudioSourceSetVolumeHandle(ActorHandle handle, float value) {
    if (auto* source = ResolveAudioSource(handle))
        source->SetVolume(value);
}
void AudioSourceFadeVolume(float value, uint32_t milliseconds) {
    if (auto* source = ResolveAudioSource({}))
        source->FadeVolume(value, milliseconds);
}
void AudioSourceFadeVolumeHandle(ActorHandle handle, float value, uint32_t milliseconds) {
    if (auto* source = ResolveAudioSource(handle))
        source->FadeVolume(value, milliseconds);
}
void AudioSourceSetPitch(float value) {
    if (auto* source = ResolveAudioSource({}))
        source->SetPitch(value);
}
void AudioSourceSetPitchHandle(ActorHandle handle, float value) {
    if (auto* source = ResolveAudioSource(handle))
        source->SetPitch(value);
}
void AudioSourceSetLoop(bool value) {
    if (auto* source = ResolveAudioSource({}))
        source->SetLoop(value);
}
void AudioSourceSetLoopHandle(ActorHandle handle, bool value) {
    if (auto* source = ResolveAudioSource(handle))
        source->SetLoop(value);
}
void AudioSourceSetStreaming(bool value) {
    if (auto* source = ResolveAudioSource({}))
        source->SetStreaming(value);
}
void AudioSourceSetStreamingHandle(ActorHandle handle, bool value) {
    if (auto* source = ResolveAudioSource(handle))
        source->SetStreaming(value);
}
bool AudioSourceSetBus(const std::string& name) {
    AudioBus bus;
    if (!ParseAudioBus(name, bus))
        return false;
    if (auto* source = ResolveAudioSource({})) {
        source->SetBus(bus);
        return true;
...
bool AudioSourceSetBusHandle(ActorHandle handle, const std::string& name) {
    AudioBus bus;
    if (!ParseAudioBus(name, bus))
        return false;
    if (auto* source = ResolveAudioSource(handle)) {
...
void AudioSourceSetPriority(int value) {
    if (auto* source = ResolveAudioSource({}))
        source->SetPriority(value);
}
void AudioSourceSetPriorityHandle(ActorHandle handle, int value) {
    if (auto* source = ResolveAudioSource(handle))
        source->SetPriority(value);
}
void AudioSourceSetConcurrency(const std::string& group, uint32_t maxInstances) {
    if (auto* source = ResolveAudioSource({})) {
        source->SetConcurrencyGroup(group);
        source->SetMaxInstances(maxInstances);
    }
}
void AudioSourceSetConcurrencyHandle(ActorHandle handle, const std::string& group, uint32_t maxInstances) {
    if (auto* source = ResolveAudioSource(handle)) {
        source->SetConcurrencyGroup(group);
        source->SetMaxInstances(maxInstances);
    }
}
void AudioMixerSetMasterVolume(float value) {
    AudioEngine::Get().SetMasterVolume(value);
}
float AudioMixerGetMasterVolume() {
    return AudioEngine::Get().GetMasterVolume();
}
bool AudioMixerSetBusVolume(const std::string& name, float value) {
    AudioBus bus;
    if (!ParseAudioBus(name, bus))
        return false;
    AudioEngine::Get().SetBusVolume(bus, value);
    return true;
}
float AudioMixerGetBusVolume(const std::string& name) {
    AudioBus bus;
    return ParseAudioBus(name, bus) ? AudioEngine::Get().GetBusVolume(bus) : -1.0f;
}
bool AudioMixerSetBusMuted(const std::string& name, bool muted) {
    AudioBus bus;
    if (!ParseAudioBus(name, bus))
        return false;
    AudioEngine::Get().SetBusMuted(bus, muted);
    return true;
}
std::string AudioMixerGetDiagnosticsJson() {
    const AudioDiagnostics d = AudioEngine::Get().GetDiagnostics();
    nlohmann::json buses = nlohmann::json::object();
    for (uint8_t i = 0; i < static_cast<uint8_t>(AudioBus::Count); ++i) {
        const auto bus = static_cast<AudioBus>(i);
        buses[AudioBusName(bus)] = {{"volume", AudioEngine::Get().GetBusVolume(bus)},
                                    {"muted", AudioEngine::Get().IsBusMuted(bus)},
                                    {"voices", d.voicesByBus[i]}};
    }
    return nlohmann::json{{"activeVoices", d.activeVoices}, {"pausedVoices", d.pausedVoices},
                          {"stolenVoices", d.stolenVoices}, {"rejectedVoices", d.rejectedVoices},
                          {"maxVoices", d.maxVoices},       {"masterVolume", AudioEngine::Get().GetMasterVolume()},
...
void CameraSetMain(ActorHandle handle, bool value) {
    if (auto* camera = ResolveCamera(handle))
        camera->SetMainCamera(value);
}
bool CameraIsMain(ActorHandle handle) {
    if (auto* camera = ResolveCamera(handle))
        return camera->IsMainCamera();
    return false;
}
void CameraSetFovY(ActorHandle handle, float value) {
    if (auto* camera = ResolveCamera(handle))
        camera->SetFovYDegrees(value);
}
float CameraGetFovY(ActorHandle handle) {
    if (auto* camera = ResolveCamera(handle))
        return camera->GetFovYDegrees();
    return 0.0f;
}
...
void LightSetIntensity(ActorHandle handle, float value) {
    if (auto* light = ResolveLight(handle))
        light->SetIntensity(value);
}
float LightGetIntensity(ActorHandle handle) {
    if (auto* light = ResolveLight(handle))
        return light->GetIntensity();
    return 0.0f;
}
void LightSetColor(ActorHandle handle, const Vec3& value) {
    if (auto* light = ResolveLight(handle))
        light->SetColor(value);
}
Vec3 LightGetColor(ActorHandle handle) {
    if (auto* light = ResolveLight(handle))
...
bool AnimatorPlay(ActorHandle handle, const std::string& state, float transition) {
    if (auto* animator = ResolveAnimator(handle))
        return animator->Play(state, transition);
    return false;
}
...
float AnimatorGetNormalizedTime(ActorHandle handle) {
    if (auto* animator = ResolveAnimator(handle))
        return animator->GetNormalizedTime();
    return 0.0f;
}
...
bool ThirdPersonCameraSetTarget(ActorHandle camera, ActorHandle target) {
    ...
    if (!controller || !ResolveActor(target))
        return false;
...
void ThirdPersonCameraAddOrbit(ActorHandle handle, float yaw, float pitch) {
    if (auto* controller = ResolveThirdPersonCamera(handle))
        controller->AddOrbit(yaw, pitch);
}
...
bool CombatHeal(ActorHandle target, float amount) {
    if (auto* health = ResolveHealth(target))
        return health->Heal(amount);
    return false;
}
float CombatGetHealth(ActorHandle target) {
    if (auto* health = ResolveHealth(target))
        return health->GetHealth();
    return 0.0f;
}
float CombatGetMaxHealth(ActorHandle target) {
    if (auto* health = ResolveHealth(target))
        return health->GetMaxHealth();
    return 0.0f;
}
bool CombatIsDead(ActorHandle target) {
    if (auto* health = ResolveHealth(target))
        return health->IsDead();
    return false;
}
bool CombatBeginAttack(ActorHandle attacker, float damage) {
    Actor* actor = ResolveActorOrSelf(attacker);
    auto* hitbox = actor ? actor->GetComponent<HitboxComponent>() : nullptr;
    if (!hitbox)
        return false;
    hitbox->BeginAttack(damage);
    return true;
}
void CombatEndAttack(ActorHandle attacker) {
    Actor* actor = ResolveActorOrSelf(attacker);
    if (auto* hitbox = actor ? actor->GetComponent<HitboxComponent>() : nullptr)
        hitbox->EndAttack();
}

InteractionComponent* ResolveInteraction(ActorHandle handle) {
    Actor* actor = ResolveActor(handle);
    return actor ? actor->GetComponent<InteractionComponent>() : nullptr;
}

ActorHandle InteractionFindNearest(ActorHandle instigator, float maxDistance) {
    Scene* scene = ActiveScene();
    Actor* source = scene ? scene->TryGetActor(instigator) : nullptr;
    ...
    scene->ForEach([&](Actor& actor) {
        ...
        if (!interaction || !interaction->CanInteract())
            return;
        float distance = (actor.GetWorldPosition() - source->GetWorldPosition()).Length();
        if (distance <= bestDistance && distance <= interaction->GetRange()) {
            best = actor.GetHandle();
...
bool InteractionUse(ActorHandle target, ActorHandle instigator) {
    if (auto* interaction = ResolveInteraction(target))
        return interaction->Interact(instigator);
    return false;
}
std::string InteractionGetPrompt(ActorHandle target) {
    if (auto* interaction = ResolveInteraction(target))
        return interaction->GetPrompt();
    return {};
}
ActorHandle InteractionFindNearestSelf(float maxDistance) {
    return InteractionFindNearest(ActiveActorHandle(), maxDistance);
}
std::string InteractionGetNearestName(float maxDistance) {
    Scene* scene = ActiveScene();
    Actor* actor = scene ? scene->TryGetActor(InteractionFindNearestSelf(maxDistance)) : nullptr;
    return actor ? actor->GetName() : std::string{};
}
std::string InteractionGetNearestPrompt(float maxDistance) {
    return InteractionGetPrompt(InteractionFindNearestSelf(maxDistance));
}
std::string InteractionUseNearest(float maxDistance) {
    Scene* scene = ActiveScene();
    const ActorHandle target = InteractionFindNearestSelf(maxDistance);
    Actor* actor = scene ? scene->TryGetActor(target) : nullptr;
    auto* interaction = actor ? actor->GetComponent<InteractionComponent>() : nullptr;
    if (!actor || !interaction || !interaction->Interact(ActiveActorHandle()))
        return {};
    const std::string name = actor->GetName();
    if (interaction->GetDestroyOnUse())
        scene->QueueDestroyActor(target);
    return name;
}

GameplayFeedbackComponent* ResolveFeedback(ActorHandle handle) {
    Actor* actor = ResolveActorOrSelf(handle);
    return actor ? actor->GetComponent<GameplayFeedbackComponent>() : nullptr;
}
bool FeedbackShake(ActorHandle handle, float amplitude, float duration) {
    if (auto* feedback = ResolveFeedback(handle)) {
        feedback->Shake(amplitude, duration);
        return true;
    }
    return false;
}
bool FeedbackFlash(ActorHandle handle, float intensity, float duration) {
    if (auto* feedback = ResolveFeedback(handle)) {
        feedback->Flash(intensity, duration);
        return true;
    }
    return false;
}
bool FeedbackSlowMotion(ActorHandle handle, float scale, float duration) {
    if (auto* feedback = ResolveFeedback(handle)) {
        feedback->SlowMotion(scale, duration);
        return true;
    }
    return false;
}

NavAgentComponent* ResolveNavAgent(ActorHandle handle) {
    Actor* actor = ResolveActorOrSelf(handle);
    return actor ? actor->GetComponent<NavAgentComponent>() : nullptr;
}
bool NavigationSetDestination(ActorHandle handle, const Vec3& destination) {
    if (auto* agent = ResolveNavAgent(handle))
        return agent->SetDestination(destination);
    return false;
}
void NavigationStop(ActorHandle handle) {
    if (auto* agent = ResolveNavAgent(handle))
        agent->Stop();
}
bool NavigationHasPath(ActorHandle handle) {
    if (auto* agent = ResolveNavAgent(handle))
        return agent->HasPath();
    return false;
}
bool NavigationReached(ActorHandle handle) {
    if (auto* agent = ResolveNavAgent(handle))
        return agent->ReachedDestination();
    return false;
}
std::string NavigationFindPathJson(const Vec3& start, const Vec3& goal) {
    Scene* scene = ActiveScene();
    if (!scene)
        return "[]";
    std::vector<Vec3> path;
    if (!scene->GetNavigationWorld().FindPath(start, goal, path))
        return "[]";
    nlohmann::json value = nlohmann::json::array();
    for (const Vec3& p : path)
        value.push_back({p.x, p.y, p.z});
    return value.dump();
}
bool NavigationSetAreaBlocked(const Vec3& minimum, const Vec3& maximum, bool blocked) {
    Scene* scene = ActiveScene();
    return scene && scene->GetNavigationWorld().SetAreaWalkable({minimum, maximum}, !blocked);
}
void PerceptionEmitSound(const Vec3& position, float radius, ActorHandle source) {
    if (Scene* scene = ActiveScene())
        scene->GetNavigationWorld().EmitSound(position, radius, source);
}
bool PerceptionCanSee(ActorHandle observer, ActorHandle target, float range) {
    Scene* scene = ActiveScene();
    Actor* a = scene ? scene->TryGetActor(observer) : nullptr;
    Actor* b = scene ? scene->TryGetActor(target) : nullptr;
    if (!a || !b || range < 0)
        return false;
    Vec3 delta = b->GetWorldPosition() - a->GetWorldPosition();
    float distance = delta.Length();
    if (distance > range)
        return false;
    RaycastHit hit;
    return !scene->GetPhysicsWorld().Raycast(*scene, Ray{a->GetWorldPosition() + Vec3(0, 1, 0), delta.Normalized()},
                                             distance, 0xffffffffu, hit) ||
           hit.actor == b;
}
EnemyAIComponent* ResolveEnemyAI(ActorHandle handle) {
    Actor* actor = ResolveActorOrSelf(handle);
    return actor ? actor->GetComponent<EnemyAIComponent>() : nullptr;
}
bool EnemySetTarget(ActorHandle enemy, ActorHandle target) {
    if (!ResolveActor(target))
        return false;
    if (auto* ai = ResolveEnemyAI(enemy)) {
        ai->SetTarget(target);
        return true;
    }
    return false;
}
int EnemyGetState(ActorHandle enemy) {
    if (auto* ai = ResolveEnemyAI(enemy))
        return static_cast<int>(ai->GetState());
    return -1;
}
void EnemyStagger(ActorHandle enemy, float duration) {
    if (auto* ai = ResolveEnemyAI(enemy))
        ai->Stagger(duration);
}
bool ScenesLoad(const std::string& path) {
    Scene* scene = ActiveScene();
    SceneManager* manager = scene ? scene->GetSceneManager() : nullptr;
    if (!manager || !manager->RequestLoad(path))
        return false;
    if (auto* flow = scene->GetGameFlowController())
        flow->BeginLoading(scene);
    return true;
}
int ScenesGetLoadState() {
    Scene* scene = ActiveScene();
    SceneManager* manager = scene ? scene->GetSceneManager() : nullptr;
    return manager ? static_cast<int>(manager->GetState()) : static_cast<int>(SceneLoadState::Failed);
}
std::string ScenesGetLastError() {
    Scene* scene = ActiveScene();
    SceneManager* manager = scene ? scene->GetSceneManager() : nullptr;
    return manager ? manager->GetLastError() : std::string("scene manager unavailable");
}
bool ScenesSetPersistentJson(const std::string& key, const std::string& json) {
    Scene* scene = ActiveScene();
    SceneManager* manager = scene ? scene->GetSceneManager() : nullptr;
    if (!manager || key.empty())
        return false;
    try {
        manager->SetPersistentValue(key, nlohmann::json::parse(json));
        return true;
    } catch (...) {
        return false;
    }
}
std::string ScenesGetPersistentJson(const std::string& key) {
    Scene* scene = ActiveScene();
    SceneManager* manager = scene ? scene->GetSceneManager() : nullptr;
    return manager ? manager->GetPersistentValue(key, nlohmann::json::object()).dump() : "{}";
}
bool WorldStreamingRegisterDistance(const std::string& name, const std::string& path, const Vec3& center,
                                    const Vec3& halfExtents, float loadDistance, float unloadDistance, int priority) {
    Scene* scene = ActiveScene();
    if (!scene)
        return false;
    WorldZoneStreamDescriptor descriptor;
    descriptor.stableName = name;
    descriptor.sourcePath = path;
    descriptor.boundsCenter = center;
    descriptor.boundsHalfExtents = halfExtents;
    descriptor.loadDistance = loadDistance;
    descriptor.unloadDistance = unloadDistance;
    descriptor.triggerMode = WorldZoneTriggerMode::Distance;
    descriptor.priority = priority;
    return scene->GetZoneStreamer().Register(std::move(descriptor));
}
bool WorldStreamingRegisterPortal(const std::string& name, const std::string& path, bool open, int priority) {
    Scene* scene = ActiveScene();
    if (!scene)
        return false;
    WorldZoneStreamDescriptor descriptor;
    descriptor.stableName = name;
    descriptor.sourcePath = path;
    descriptor.triggerMode = WorldZoneTriggerMode::Portal;
    descriptor.portalInitiallyOpen = open;
    descriptor.priority = priority;
    return scene->GetZoneStreamer().Register(std::move(descriptor));
}
void WorldStreamingSetObserver(const Vec3& position) {
    if (Scene* scene = ActiveScene())
        scene->GetZoneStreamer().SetObserverPosition(position);
}
bool WorldStreamingSetPortalOpen(const std::string& name, bool open) {
    Scene* scene = ActiveScene();
    return scene && scene->GetZoneStreamer().SetPortalOpen(name, open);
}
bool WorldStreamingRetry(const std::string& name) {
    Scene* scene = ActiveScene();
    return scene && scene->GetZoneStreamer().Retry(name);
}
std::string WorldStreamingGetStatsJson() {
    Scene* scene = ActiveScene();
    if (!scene)
        return "{}";
    const auto stats = scene->GetZoneStreamer().GetStats();
    nlohmann::json entries = nlohmann::json::array();
    for (const auto& entry : scene->GetZoneStreamer().GetEntryStats())
        entries.push_back({{"name", entry.stableName},
                           {"state", static_cast<int>(entry.state)},
                           {"desired", entry.desired},
                           {"generation", entry.lifetimeGeneration},
                           {"actors", entry.instantiatedActors},
                           {"error", entry.lastError}});
    return nlohmann::json{{"registeredZones", stats.registeredZones},
                          {"loadedZones", stats.loadedZones},
                          {"loadingZones", stats.loadingZones},
                          {"failedZones", stats.failedZones},
                          {"instantiatedActorsThisFrame", stats.instantiatedActorsThisFrame},
                          {"actorBudgetBlockedFrames", stats.actorBudgetBlockedFrames},
                          {"entries", std::move(entries)}}
        .dump();
}

std::string ResourcesGetStatsJson() {
    const auto s = FrameStatsProvider::GetResourceStats();
    return nlohmann::json{{"assetCpuBytes", s.assetCpuBytes},
                          {"assetEvictedBytes", s.assetEvictedBytes},
                          {"assetBlockedBytes", s.assetBlockedBytes},
                          {"pendingUploadBytes", s.pendingUploadBytes},
                          {"peakPendingUploadBytes", s.peakPendingUploadBytes},
                          {"pendingUploadTasks", s.pendingUploadTasks},
                          {"liveActors", s.liveActors},
                          {"gpuResourceBytes", s.gpuResourceBytes},
                          {"peakGpuResourceBytes", s.peakGpuResourceBytes},
                          {"liveGpuDescriptors", s.liveGpuDescriptors},
                          ...
                          {"uploadPressure", s.uploadPressure},
                          {"actorPressure", s.actorPressure},
                          {"gpuResourcePressure", s.gpuResourcePressure},
                          {"descriptorPressure", s.descriptorPressure},
                          {"nativeDescriptorPressure", s.nativeDescriptorPressure},
                          {"transientPressure", s.transientPressure}}
        .dump();
}
bool SaveGameWrite(const std::string& slot, const std::string& checkpoint, const std::string& player,
                   const std::string& collected, const std::string& settings) {
    try {
        SaveGameData data;
        data.checkpoint = checkpoint;
        data.player = nlohmann::json::parse(player.empty() ? "{}" : player);
        data.collected = nlohmann::json::parse(collected.empty() ? "[]" : collected).get<std::vector<std::string>>();
        data.settings = nlohmann::json::parse(settings.empty() ? "{}" : settings);
        return SaveGame::Write(slot, data);
    } catch (...) {
        return false;
    }
}
std::string SaveGameReadJson(const std::string& slot) {
    SaveGameData data;
    if (!SaveGame::Read(slot, data))
        return "{}";
    return SaveGame::ToJson(data).dump();
}
std::string SaveGameReadBackupJson(const std::string& slot) {
    SaveGameData data;
    if (!SaveGame::ReadBackup(slot, data))
        return "{}";
    return SaveGame::ToJson(data).dump();
}
std::string SaveGameListSlotsJson() {
    nlohmann::json result = nlohmann::json::array();
    for (const auto& slot : SaveGame::ListSlots())
        result.push_back({{"slot", slot.slot},
                          {"valid", slot.valid},
                          {"hasBackup", slot.hasBackup},
                          {"error", slot.error},
                          {"metadata",
                           {{"displayName", slot.metadata.displayName},
                            {"scene", slot.metadata.scene},
                            {"savedAtUtc", slot.metadata.savedAtUtc},
                            {"playTimeSeconds", slot.metadata.playTimeSeconds},
                            {"screenshot", slot.metadata.screenshot},
                            {"buildId", slot.metadata.buildId}}}});
    return result.dump();
}
std::string SaveGameWriteAutosave(const std::string& checkpoint, const std::string& player,
                                  const std::string& collected, const std::string& settings, uint32_t slots) {
    try {
        SaveGameData data;
        data.checkpoint = checkpoint;
        data.player = nlohmann::json::parse(player.empty() ? "{}" : player);
        ...
        data.settings = nlohmann::json::parse(settings.empty() ? "{}" : settings);
        std::string slot;
        return SaveGame::WriteAutosave(data, slots, &slot) ? slot : "";
    } catch (...) {
        return "";
    }
}
std::string SaveGameWriteCheckpoint(const std::string& id, const std::string& player, const std::string& collected,
    ...
    try {
        ...
        data.settings = nlohmann::json::parse(settings.empty() ? "{}" : settings);
        std::string slot;
        return SaveGame::WriteCheckpoint(id, data, &slot) ? slot : "";
    } catch (...) {
        return "";
    }
}
std::string SaveGameLatestAutosaveJson() {
    SaveGameSlotInfo info;
    if (!SaveGame::FindLatestAutosave(info))
        return "{}";
    SaveGameData data;
    if (!SaveGame::Read(info.slot, data))
        return "{}";
    return SaveGame::ToJson(data).dump();
}
bool SaveGameExists(const std::string& slot) {
    return SaveGame::Exists(slot);
}
bool SaveGameRemove(const std::string& slot) {
    return SaveGame::Remove(slot);
}
bool SaveGameRestoreBackup(const std::string& slot) {
    return SaveGame::RestoreBackup(slot);
}
std::string UserSettingsReadJson() {
    RuntimeUserSettings settings;
    if (!RuntimeUserSettingsStore::Load(settings))
        settings = RuntimeUserSettingsStore::Defaults();
    return RuntimeUserSettingsStore::ToJson(settings).dump();
}
bool UserSettingsWriteJson(const std::string& source) {
    try {
        RuntimeUserSettings settings;
        if (!RuntimeUserSettingsStore::FromJson(nlohmann::json::parse(source), settings))
            return false;
        if (!RuntimeUserSettingsStore::Save(settings))
            return false;
        Input::SetRuntimePreferences(settings.input.mouseSensitivity, settings.input.invertY,
                                     settings.input.gamepadDeadZone, settings.input.gamepadSensitivity,
                                     settings.input.vibration);
        AudioEngine::Get().SetMasterVolume(settings.audio.master);
        AudioEngine::Get().SetBusVolume(AudioBus::Music, settings.audio.music);
        AudioEngine::Get().SetBusVolume(AudioBus::Effects, settings.audio.effects);
        AudioEngine::Get().SetBusVolume(AudioBus::Voice, settings.audio.voice);
        if (auto* ui = UISystemBinding()) {
            UIAccessibilitySettings value;
            value.uiScale = settings.accessibility.uiScale;
            value.subtitleScale = settings.accessibility.subtitleScale;
            value.subtitles = settings.accessibility.subtitles;
            value.reduceCameraShake = settings.accessibility.reduceCameraShake;
            value.highContrast = settings.accessibility.highContrast;
            value.colorVisionMode = settings.accessibility.colorVisionMode;
            if (!ui->SetAccessibilitySettings(value))
                return false;
        }
        return Input::ApplyActionMapOverrides(settings.input.actionMap);
    } catch (...) {
        return false;
    }
}
bool UserSettingsReset() {
    RuntimeUserSettings settings = RuntimeUserSettingsStore::Defaults();
    if (!RuntimeUserSettingsStore::Save(settings))
        return false;
    Input::SetRuntimePreferences(1.0f, false, 0.15f, 1.0f, 1.0f);
    Input::ResetActionMapOverrides();
    AudioEngine::Get().SetMasterVolume(1.0f);
    AudioEngine::Get().SetBusVolume(AudioBus::Music, 1.0f);
    AudioEngine::Get().SetBusVolume(AudioBus::Effects, 1.0f);
    AudioEngine::Get().SetBusVolume(AudioBus::Voice, 1.0f);
    if (auto* ui = UISystemBinding())
        ui->SetAccessibilitySettings({});
    return true;
}
void GamePause() {
    if (Scene* scene = ActiveScene()) {
...
bool GameIsPaused() {
    ...
    return scene->GetState() == SceneState::Paused;
}
void GameSetTimeScale(float scale) {
    if (Scene* scene = ActiveScene())
        scene->SetTimeScale(std::clamp(scale, 0.0f, 10.0f));
}
float GameGetTimeScale() {
    Scene* scene = ActiveScene();
...
std::string GameGetFlowState() {
    Scene* scene = ActiveScene();
    auto* flow = scene ? scene->GetGameFlowController() : nullptr;
    return flow ? GameFlowController::StateName(flow->GetState()) : "Unavailable";
}

void AnimatorSetPlaying(ActorHandle handle, bool playing) {
    if (auto* renderer = ResolveSkinnedMeshRenderer(handle))
...
float AnimatorGetTime(ActorHandle handle) {
    if (auto* renderer = ResolveSkinnedMeshRenderer(handle))
        return renderer->GetAnimationTime();
    return 0.0f;
}
...
float AnimatorGetBlendWeight(ActorHandle handle) {
    if (auto* renderer = ResolveSkinnedMeshRenderer(handle))
        return renderer->GetBlendWeight();
    return 0.0f;
}
...
UIElementComponent* ResolveUIElement(ActorHandle handle) {
    ...
    if (!actor)
        return nullptr;
    if (auto* value = actor->GetComponent<UITextComponent>())
        return value;
    if (auto* value = actor->GetComponent<UIImageComponent>())
        return value;
    if (auto* value = actor->GetComponent<UIButtonComponent>())
        return value;
    if (auto* value = actor->GetComponent<UISliderComponent>())
        return value;
    if (auto* value = actor->GetComponent<UIProgressBarComponent>())
        return value;
    if (auto* value = actor->GetComponent<UIScrollViewComponent>())
        return value;
    if (auto* value = actor->GetComponent<UIVerticalLayoutComponent>())
        return value;
    if (auto* value = actor->GetComponent<UIHorizontalLayoutComponent>())
        return value;
    if (auto* value = actor->GetComponent<UIGridLayoutComponent>())
        return value;
    return nullptr;
}

std::string UIElementGetId(ActorHandle handle) {
    if (auto* element = ResolveUIElement(handle))
        return element->GetElementID();
    return {};
}
void UIElementSetId(ActorHandle handle, const std::string& value) {
    if (auto* element = ResolveUIElement(handle))
...
bool AssetsExists(const std::string& path) {
    ...
    return AssetManager::Get().IsLoaded(path) || RuntimeFileSystem::Get().Exists(resolved) ||
           RuntimeFileSystem::Get().Exists(path);
}

std::string ComponentsGetPropertyJson(ActorHandle handle, const std::string& type, const std::string& property) {
    Actor* actor = ResolveActor(handle);
    Component* component = actor ? actor->GetComponentByTypeName(type) : nullptr;
    ...
    if (!component || !field || !HasPropertyFlag(field->flags, PropertyFlags::ScriptRead))
        return {};
    nlohmann::json value;
    return TypeRegistry::Get().GetProperty(*component, property, value) ? value.dump() : std::string{};
}

bool ComponentsSetPropertyJson(ActorHandle handle, const std::string& type, const std::string& property,
                               const std::string& json) {
    Actor* actor = ResolveActor(handle);
    Component* component = actor ? actor->GetComponentByTypeName(type) : nullptr;
    const TypeDescriptor* descriptor = TypeRegistry::Get().Find(type);
    const PropertyDescriptor* field = descriptor ? TypeRegistry::Get().FindProperty(*descriptor, property) : nullptr;
    if (!component || !field || !HasPropertyFlag(field->flags, PropertyFlags::ScriptWrite))
        return false;
    try {
        return TypeRegistry::Get().SetProperty(*component, property, nlohmann::json::parse(json));
    } catch (...) {
        return false;
    }
}
...
std::string AssetsGetType(const std::string& path) {
    ...
    if (path.size() >= prefabSuffix.size() && path.substr(path.size() - prefabSuffix.size()) == prefabSuffix)
        return "Prefab";
    return "Unknown";
}
...
void DebugWarning(const std::string& message) {
    Logger::Warn("[Script] ", message);
}
void DebugError(const std::string& message) {
    Logger::Error("[Script] ", message);
}
void DebugLogCategory(const std::string& category, const std::string& message) {
    Logger::Info(DebugPrefix(category), message);
}
void DebugWarningCategory(const std::string& category, const std::string& message) {
    Logger::Warn(DebugPrefix(category), message);
}
void DebugErrorCategory(const std::string& category, const std::string& message) {
    Logger::Error(DebugPrefix(category), message);
}
void DebugLogOnce(const std::string& key, const std::string& message) {
    ...
    if (g_DebugOnceKeys.insert(key).second)
        DebugLog(message);
}
void DebugLogThrottle(const std::string& key, const std::string& message, float seconds) {
    if (key.empty() || seconds <= 0.0f) {
        DebugLog(message);
        return;
    }
    const auto now = std::chrono::steady_clock::now();
    const auto found = g_DebugThrottleTimes.find(key);
    if (found != g_DebugThrottleTimes.end() && std::chrono::duration<float>(now - found->second).count() < seconds) {
        return;
    }
...
bool InputActionDown(const std::string& action) {
    return Input::IsActionDown(action);
}
bool InputActionPressed(const std::string& action) {
    return Input::IsActionPressed(action);
}
bool InputActionReleased(const std::string& action) {
    return Input::IsActionReleased(action);
}
float InputAxis(const std::string& action) {
    return Input::GetAxis1D(action);
}
Math::Vec2 InputAxis2(const std::string& action) {
    return Input::GetAxis2D(action);
}
bool InputKeyDown(int scancode) {
    return Input::IsKeyDown(scancode);
}
bool InputKeyPressed(int scancode) {
    return Input::IsKeyPressed(scancode);
}
bool InputKeyReleased(int scancode) {
    return Input::IsKeyReleased(scancode);
}
...
Math::Vec2 InputMousePosition() {
    return Math::Vec2{static_cast<float>(Input::GetMouseX()), static_cast<float>(Input::GetMouseY())};
}
Math::Vec2 InputMouseDelta() {
    return Math::Vec2{static_cast<float>(Input::GetMouseRelX()), static_cast<float>(Input::GetMouseRelY())};
}
int InputGamepadCount() {
    return Input::GetGamepadCount();
}
int InputPrimaryGamepadId() {
    return static_cast<int>(Input::GetPrimaryGamepadId());
}
bool InputGamepadConnected(int id) {
    return Input::IsGamepadConnected(static_cast<SDL_JoystickID>(id));
}
bool InputGamepadButtonDown(int id, int button) {
    return Input::IsGamepadButtonDown(static_cast<SDL_JoystickID>(id), static_cast<SDL_GamepadButton>(button));
}
bool InputGamepadButtonPressed(int id, int button) {
    return Input::IsGamepadButtonPressed(static_cast<SDL_JoystickID>(id), static_cast<SDL_GamepadButton>(button));
}
bool InputGamepadButtonReleased(int id, int button) {
    return Input::IsGamepadButtonReleased(static_cast<SDL_JoystickID>(id), static_cast<SDL_GamepadButton>(button));
}
float InputGamepadAxis(int id, int axis) {
    return Input::GetGamepadAxis(static_cast<SDL_JoystickID>(id), static_cast<SDL_GamepadAxis>(axis));
}
std::string InputGlyphSet() {
    return Input::GetGlyphSetName();
}
std::string InputGlyphFamily() {
    return Input::GetGlyphFamilyName();
}
std::string InputActionGlyphJson(const std::string& action) {
    return Input::GetActionGlyphJson(action);
}
std::string InputSourceGlyphJson(const std::string& source) {
    return Input::GetSourceGlyphJson(source);
}
void InputSetGlyphLocale(const std::string& locale) {
    Input::SetGlyphLocale(locale);
}
...
std::string InputBindingConflictsJson(const std::string& action, uint32_t index, const std::string& part,
...
bool InputRebind(const std::string& action, uint32_t index, const std::string& part, const std::string& source,
                 bool allowConflicts) {
...
void ConstructActorHandle(void* memory) {
    new (memory) ActorHandle();
}
void ConstructActorHandle(uint64_t value, void* memory) {
    new (memory) ActorHandle(ActorHandle::FromUInt64(value));
}
bool ActorHandleIsValid(const ActorHandle& handle) {
    return handle.IsValid();
}
uint64_t ActorHandleToUInt64(const ActorHandle& handle) {
    return handle.ToUInt64();
}
ActorHandle ActorHandleFromUInt64(uint64_t value) {
    return ActorHandle::FromUInt64(value);
}
bool ActorHandleEquals(const ActorHandle& a, const ActorHandle& b) {
    return a == b;
}
...
void SceneInstantiatePrefabGeneric(asIScriptGeneric* generic) {
    ...
    ReturnActorHandle(generic, SceneInstantiatePrefab(path ? *path : std::string{}, position ? *position : Vec3{},
...
void SceneFindNearestWithComponentGeneric(asIScriptGeneric* generic) {
    ...
    ReturnActorHandle(generic, SceneFindNearestWithComponent(type ? *type : std::string{}, center ? *center : Vec3{},
...
void InteractionFindNearestGeneric(asIScriptGeneric* generic) {
    ...
    ReturnActorHandle(generic,
                      InteractionFindNearest(instigator ? *instigator : ActorHandle{}, generic->GetArgFloat(1)));
}
...
bool InteractionUseRef(const ActorHandle& target, const ActorHandle& instigator) {
    return InteractionUse(target, instigator);
}
bool InteractionUseSelfRef(const ActorHandle& target) {
    return InteractionUse(target, ActiveActorHandle());
}
std::string InteractionGetPromptRef(const ActorHandle& target) {
    return InteractionGetPrompt(target);
}
...
ScriptUInt64Array* PhysicsOverlapSphere(const Vec3& center, float radius, uint32_t mask) {
    ...
    Scene* scene = actor ? actor->GetScene() : nullptr;
    if (scene)
        scene->GetPhysicsWorld().OverlapSphere(*scene, center, radius, mask, handles);
    std::vector<uint64_t> values;
    values.reserve(handles.size());
    for (const ActorHandle& handle : handles)
        values.push_back(handle.ToUInt64());
...
void ConstructUIEvent(void* memory) {
    new (memory) ScriptUIEvent();
}
void DestructUIEvent(ScriptUIEvent* event) {
    event->~ScriptUIEvent();
}

bool UISubscribe(const std::string& elementId, const std::string& eventName, const std::string& callbackName) {
    ScriptComponent* component = ActiveComponent();
...
void UISetVec2(const std::string& model, const std::string& key, const Vec2& value) {
    if (auto* uiSystem = UISystemBinding())
        uiSystem->CreateDataModel(model).SetVec2(key, value);
}

void UISetVec3(const std::string& model, const std::string& key, const Vec3& value) {
    if (auto* uiSystem = UISystemBinding())
        uiSystem->CreateDataModel(model).SetVec3(key, value);
}
...
void UISetJson(const std::string& model, const std::string& key, const std::string& json) {
    ...
    if (!uiSystem)
        return;
    try {
        uiSystem->CreateDataModel(model).SetJson(key, nlohmann::json::parse(json.empty() ? "[]" : json));
    } catch (...) {
...
void UINotify(const std::string& model, const std::string&) {
    if (auto* uiSystem = UISystemBinding())
        uiSystem->CreateDataModel(model).MarkDirty();
}

bool UISetSafeArea(float left, float top, float right, float bottom) {
    if (auto* uiSystem = UISystemBinding())
        return uiSystem->SetSafeAreaInsets({left, top, right, bottom});
    return false;
}

bool UIShowSubtitle(const std::string& stableId, const std::string& speaker, const std::string& text,
                    float durationSeconds, int priority) {
    if (auto* uiSystem = UISystemBinding())
        return uiSystem->ShowSubtitle({stableId, speaker, text, durationSeconds, priority});
    return false;
}

void UIClearSubtitles() {
    if (auto* uiSystem = UISystemBinding())
        uiSystem->ClearSubtitles();
}
...
std::string UIGetSubtitleJson() {
    ...
    return nlohmann::json{{"visible", state.visible},
                          {"id", state.stableId},
...
std::string UIGetDiagnosticsJson() {
    ...
    return nlohmann::json{{"viewportWidth", d.viewportWidth},
                          {"viewportHeight", d.viewportHeight},
                          {"safeWidth", d.safeWidth},
                          {"safeHeight", d.safeHeight},
                          {"effectiveScale", d.effectiveScale},
                          {"loadedFontFaces", d.loadedFontFaces},
                          {"failedFontFaces", d.failedFontFaces},
                          {"narrowLayout", d.narrowLayout},
                          {"safeAreaValid", d.safeAreaValid},
                          ...
                          {"glyphFamily", Input::GetGlyphFamilyName()},
                          {"glyphLocale", Input::GetGlyphLocale()},
                          {"lastFontError", d.lastFontError}}
        .dump();
}
...
void ConstructVec2(void* memory) {
    new (memory) Math::Vec2();
}
void ConstructVec2(float x, float y, void* memory) {
    new (memory) Math::Vec2{x, y};
}
void ConstructVec3(void* memory) {
    new (memory) Vec3();
}
void ConstructVec3(float x, float y, float z, void* memory) {
    new (memory) Vec3{x, y, z};
}
void ConstructRaycastHit(void* memory) {
    new (memory) ScriptRaycastHit();
}
void ConstructCollisionEvent(void* memory) {
    new (memory) ScriptCollisionEvent();
}
Math::Vec2 Vec2Add(const Math::Vec2& self, const Math::Vec2& other) {
    return self + other;
}
Math::Vec2 Vec2Sub(const Math::Vec2& self, const Math::Vec2& other) {
    return self - other;
}
Math::Vec2 Vec2Mul(const Math::Vec2& self, float scalar) {
    return self * scalar;
}
Vec3 Vec3Add(const Vec3& self, const Vec3& other) {
    return self + other;
}
Vec3 Vec3Sub(const Vec3& self, const Vec3& other) {
    return self - other;
}
Vec3 Vec3Mul(const Vec3& self, float scalar) {
    return self * scalar;
}
...
ScriptFieldType FieldTypeFromAngelScript(asIScriptEngine& engine, int typeId) {
    ...
    if (typeId == asTYPEID_INT8 || typeId == asTYPEID_INT16 || typeId == asTYPEID_INT32 || typeId == asTYPEID_INT64) {
        return ScriptFieldType::Int;
    }
    if (typeId == asTYPEID_UINT8 || typeId == asTYPEID_UINT16 || typeId == asTYPEID_UINT32 ||
    ...
    if (declaration.find("string") != std::string::npos)
        return ScriptFieldType::String;
    if (declaration == "Vec2")
        return ScriptFieldType::Vec2;
    if (declaration == "Vec3")
...
nlohmann::json ReadFieldJson(asIScriptObject& object, asITypeInfo& type, unsigned int index,
    ...
    case ScriptFieldType::Int:
        if (typeId == asTYPEID_INT8)
            return *static_cast<int8_t*>(address);
...
bool WriteFieldJson(asIScriptObject& object, asITypeInfo& type, unsigned int index, ScriptFieldType fieldType,
    ...
    try {
        ...
        case ScriptFieldType::Int:
            if (!value.is_number_integer())
                return false;
            if (typeId == asTYPEID_INT8)
                *static_cast<int8_t*>(address) = static_cast<int8_t>(value.get<int64_t>());
            else if (typeId == asTYPEID_INT16)
                *static_cast<int16_t*>(address) = static_cast<int16_t>(value.get<int64_t>());
            else if (typeId == asTYPEID_INT64)
                *static_cast<int64_t*>(address) = value.get<int64_t>();
            else
                *static_cast<int32_t*>(address) = static_cast<int32_t>(value.get<int64_t>());
            return true;
        case ScriptFieldType::UInt:
            if (!value.is_number_unsigned() && !value.is_number_integer())
                return false;
            if (typeId == asTYPEID_UINT8)
                *static_cast<uint8_t*>(address) = static_cast<uint8_t>(value.get<uint64_t>());
            else if (typeId == asTYPEID_UINT16)
                *static_cast<uint16_t*>(address) = static_cast<uint16_t>(value.get<uint64_t>());
            else if (typeId == asTYPEID_UINT64)
                *static_cast<uint64_t*>(address) = value.get<uint64_t>();
            else
                *static_cast<uint32_t*>(address) = static_cast<uint32_t>(value.get<uint64_t>());
            return true;
        case ScriptFieldType::Float:
            if (value.is_number()) {
...
std::vector<ScriptFieldInfo> ReflectFields(asIScriptEngine& engine, asITypeInfo& type, asIScriptObject* object) {
    ...
    for (unsigned int i = 0; i < count; ++i) {
        ...
        if (field.type == ScriptFieldType::Unsupported)
            continue;
        if (object)
            field.defaultValue = ReadFieldJson(*object, type, i, field.type);
...
void CaptureProperties(asIScriptObject& object, asITypeInfo& type, const std::vector<ScriptFieldInfo>& fields,
...
void ApplyProperties(asIScriptObject& object, asITypeInfo& type, const std::vector<ScriptFieldInfo>& fields,
...
void RegisterVec2(asIScriptEngine& engine) {
    Check(
        engine.RegisterObjectType("Vec2", sizeof(Math::Vec2),
    ...
    Check(engine.RegisterObjectBehaviour("Vec2", asBEHAVE_CONSTRUCT, "void f(float, float)",
                                         asFUNCTIONPR(ConstructVec2, (float, float, void*), void),
    ...
    Check(engine.RegisterObjectMethod("Vec2", "Vec2 opAdd(const Vec2 &in) const",
                                      asMETHODPR(Math::Vec2, operator+, (const Math::Vec2&) const, Math::Vec2),
                                      asCALL_THISCALL));
...
#else
    Check(engine.RegisterObjectMethod("Vec2", "Vec2 opAdd(const Vec2 &in) const", asFUNCTION(Vec2Add),
                                      asCALL_CDECL_OBJFIRST));
    Check(engine.RegisterObjectMethod("Vec2", "Vec2 opSub(const Vec2 &in) const", asFUNCTION(Vec2Sub),
                                      asCALL_CDECL_OBJFIRST));
    Check(engine.RegisterObjectMethod("Vec2", "Vec2 opMul(float) const", asFUNCTION(Vec2Mul), asCALL_CDECL_OBJFIRST));
#endif
}
...
void RegisterVec3(asIScriptEngine& engine) {
    ...
    Check(engine.RegisterObjectBehaviour("Vec3", asBEHAVE_CONSTRUCT, "void f(float, float, float)",
                                         asFUNCTIONPR(ConstructVec3, (float, float, float, void*), void),
                                         asCALL_CDECL_OBJLAST));
...
#else
    Check(engine.RegisterObjectMethod("Vec3", "Vec3 opAdd(const Vec3 &in) const", asFUNCTION(Vec3Add),
                                      asCALL_CDECL_OBJFIRST));
    Check(engine.RegisterObjectMethod("Vec3", "Vec3 opSub(const Vec3 &in) const", asFUNCTION(Vec3Sub),
                                      asCALL_CDECL_OBJFIRST));
    Check(engine.RegisterObjectMethod("Vec3", "Vec3 opMul(float) const", asFUNCTION(Vec3Mul), asCALL_CDECL_OBJFIRST));
#endif
}
...
void RegisterScriptTypes(asIScriptEngine& engine) {
    ...
    Check(
        engine.RegisterObjectType("ActorHandle", sizeof(ActorHandle),
    ...
    Check(engine.RegisterObjectBehaviour("ActorHandle", asBEHAVE_CONSTRUCT, "void f(uint64)",
                                         asFUNCTIONPR(ConstructActorHandle, (uint64_t, void*), void),
                                         asCALL_CDECL_OBJLAST));
    ...
    Check(engine.RegisterObjectMethod("ActorHandle", "bool IsValid() const", asFUNCTION(ActorHandleIsValid),
                                      asCALL_CDECL_OBJFIRST));
    Check(engine.RegisterObjectMethod("ActorHandle", "uint64 ToUInt64() const", asFUNCTION(ActorHandleToUInt64),
                                      asCALL_CDECL_OBJFIRST));
    ...
    Check(engine.RegisterObjectBehaviour("UInt64Array", asBEHAVE_ADDREF, "void f()", asFUNCTION(UInt64ArrayAddRef),
                                         asCALL_CDECL_OBJLAST));
    Check(engine.RegisterObjectBehaviour("UInt64Array", asBEHAVE_RELEASE, "void f()", asFUNCTION(UInt64ArrayRelease),
                                         asCALL_CDECL_OBJLAST));
    Check(engine.RegisterObjectMethod("UInt64Array", "uint Length() const", asFUNCTION(UInt64ArrayLength),
                                      asCALL_CDECL_OBJLAST));
    Check(engine.RegisterObjectMethod("UInt64Array", "uint64 At(uint index) const", asFUNCTION(UInt64ArrayAt),
                                      asCALL_CDECL_OBJLAST));

    Check(engine.RegisterObjectType("ActorHandleArray", 0, asOBJ_REF));
    Check(engine.RegisterObjectBehaviour("ActorHandleArray", asBEHAVE_ADDREF, "void f()", asFUNCTION(UInt64ArrayAddRef),
                                         asCALL_CDECL_OBJLAST));
    ...
    Check(engine.RegisterObjectMethod("ActorHandleArray", "uint Length() const", asFUNCTION(UInt64ArrayLength),
                                      asCALL_CDECL_OBJLAST));
    ...
    Check(engine.RegisterObjectProperty("RaycastHit", "uint64 actorHandle", asOFFSET(ScriptRaycastHit, actorHandle)));
    Check(engine.RegisterObjectProperty("RaycastHit", "float distance", asOFFSET(ScriptRaycastHit, distance)));
    Check(engine.RegisterObjectProperty("RaycastHit", "Vec3 point", asOFFSET(ScriptRaycastHit, point)));
    Check(engine.RegisterObjectProperty("RaycastHit", "Vec3 normal", asOFFSET(ScriptRaycastHit, normal)));
    Check(engine.RegisterObjectProperty("RaycastHit", "bool hit", asOFFSET(ScriptRaycastHit, hit)));
    ...
    Check(engine.RegisterObjectProperty("CollisionEvent", "Vec3 point", asOFFSET(ScriptCollisionEvent, point)));
    Check(engine.RegisterObjectProperty("CollisionEvent", "Vec3 normal", asOFFSET(ScriptCollisionEvent, normal)));
    Check(engine.RegisterObjectProperty("CollisionEvent", "float depth", asOFFSET(ScriptCollisionEvent, depth)));
    Check(engine.RegisterObjectProperty("CollisionEvent", "bool trigger", asOFFSET(ScriptCollisionEvent, trigger)));
    Check(engine.RegisterObjectProperty("CollisionEvent", "int phase", asOFFSET(ScriptCollisionEvent, phase)));

    Check(engine.RegisterObjectType("UIEvent", sizeof(ScriptUIEvent), asOBJ_VALUE | asGetTypeTraits<ScriptUIEvent>()));
    ...
    Check(engine.RegisterObjectBehaviour("UIEvent", asBEHAVE_DESTRUCT, "void f()", asFUNCTION(DestructUIEvent),
                                         asCALL_CDECL_OBJLAST));
    Check(engine.RegisterObjectProperty("UIEvent", "string elementId", asOFFSET(ScriptUIEvent, elementId)));
    Check(engine.RegisterObjectProperty("UIEvent", "string eventName", asOFFSET(ScriptUIEvent, eventName)));
    Check(engine.RegisterObjectProperty("UIEvent", "float value", asOFFSET(ScriptUIEvent, value)));
    Check(engine.RegisterObjectProperty("UIEvent", "bool hasValue", asOFFSET(ScriptUIEvent, hasValue)));
}
...
ScriptCollisionEvent ConvertCollisionEvent(const CollisionEvent& event) {
    ...
    result.phase = event.phase == CollisionEventPhase::Enter ? 1 : event.phase == CollisionEventPhase::Stay ? 2 : 3;
    return result;
}
...
struct AngelScriptRuntime::Impl {
    ...
    void RecordExecution(asIScriptFunction* function, int result, double elapsedMs, const std::string& error,
                         int line) {
        const std::string scriptClass = type && type->GetName() ? type->GetName() : "<unknown>";
        const std::string declaration =
            function && function->GetDeclaration() ? function->GetDeclaration() : "<unknown>";
    ...
    bool Execute(asIScriptFunction* function, float deltaSeconds, bool hasDelta, const ScriptCollisionEvent* collision,
                 std::string& error) {
        if (!function || !object || !engine)
            return true;
        ...
        if (collision)
            context->SetArgObject(0, const_cast<ScriptCollisionEvent*>(collision));
        ...
        if (result != asEXECUTION_FINISHED) {
            ...
            if (result == asEXECUTION_EXCEPTION) {
                ...
                if (asIScriptFunction* exceptionFunction = context->GetExceptionFunction()) {
                    stream << " at " << exceptionFunction->GetDeclaration() << ":" << context->GetExceptionLineNumber();
                }
            }
            error = stream.str();
    ...
    bool ExecuteUIEvent(asIScriptFunction* function, const UIEvent& event, std::string& error) {
        if (!function || !object || !engine)
            return true;
    ...
    bool ExecuteAnimationEvent(asIScriptFunction* function, const std::string& name, const std::string& payload,
                               std::string& error) {
        if (!function || !object || !engine)
            return true;
        asIScriptContext* context = engine->CreateContext();
    ...
    bool SubscribeUIEvent(const std::string& elementId, const std::string& eventName, const std::string& callbackName,
        ...
        // The UI bridge may dispatch after scene teardown has started. Acquire the captured generation before touching
        // the component or AngelScript function so an old callback cannot cross into a replacement Scene.
    ...
    void ClearUIEventSubscriptions() {
        if (UIEventBridgeBinding())
            UIEventBridgeBinding()->ClearOwner(this);
    }
    ...
    bool ExecuteStringArg(asIScriptFunction* function, const std::string& value, const char* label,
                          std::string& error) {
        if (!function || !object || !engine)
            return true;
        asIScriptContext* context = engine->CreateContext();
    ...
    bool SubscribeScriptEvent(const std::string& eventName, const std::string& callbackName, std::string& error) {
        ...
        // Subscriptions outlive the call that registers them, so both the routing record and callback capture the same
        // Scene generation. The guard prevents the captured component/function pair from surviving Scene replacement.
        g_ScriptEventSubscriptions.push_back(
            {this, component.GetOwner() ? component.GetOwner()->GetScene() : nullptr,
             component.GetOwner() && component.GetOwner()->GetScene()
                 ? component.GetOwner()->GetScene()->GetLifetimeToken()
                 : SceneLifetimeToken{},
             eventName,
             [this, function,
              lifetime = component.GetOwner() && component.GetOwner()->GetScene()
                             ? component.GetOwner()->GetScene()->GetLifetimeToken()
                             : SceneLifetimeToken{}](const std::string& payload, std::string& callbackError) {
                 SceneLifetimeGuard guard = lifetime.TryAcquire();
                 if (!guard)
                     return true;
    ...
    void ClearScriptEventSubscriptions() {
        g_ScriptEventSubscriptions.erase(
            std::remove_if(g_ScriptEventSubscriptions.begin(), g_ScriptEventSubscriptions.end(),
                           [&](const ScriptEventSubscription& subscription) { return subscription.owner == this; }),
            g_ScriptEventSubscriptions.end());
    }
    ...
    bool EmitScriptEvent(const std::string& eventName, const std::string& payload) {
        ...
        if (!scene || eventName.empty())
            return false;
        g_QueuedScriptEvents.push_back({scene, scene->GetLifetimeToken(), eventName, payload.empty() ? "{}" : payload});
        return true;
    }
    ...
    void CancelTimer(uint64_t timerID) {
        timers.erase(
            std::remove_if(timers.begin(), timers.end(), [&](const Timer& timer) { return timer.id == timerID; }),
            timers.end());
    }

    void CancelAllTimers() { timers.clear(); }
    ...
    bool TickTimers(float deltaSeconds, std::string& error) {
        ...
        timers.erase(std::remove_if(timers.begin(), timers.end(), [](const Timer& timer) { return timer.cancelled; }),
                     timers.end());
        for (asIScriptFunction* function : due) {
            if (!ExecuteNoArg(function, "timer", error))
                return false;
        }
        return true;
    }
    ...
    bool FlushScriptEvents(std::string& error) {
        Scene* scene = component.GetOwner() ? component.GetOwner()->GetScene() : nullptr;
        if (!scene || g_QueuedScriptEvents.empty())
            return true;
        ...
        for (const QueuedScriptEvent& event : pending) {
            if (event.scene != scene) {
                if (event.lifetime.IsAlive())
                    g_QueuedScriptEvents.push_back(event);
                continue;
            }
            // Hold the Scene generation while selecting and invoking callbacks; teardown cannot invalidate their
            // captured component state in the middle of this dispatch.
            ...
            for (const auto& callback : callbacks) {
                if (!callback(event.payload, error))
                    return false;
            }
        }
        return true;
...
AngelScriptRuntime::AngelScriptRuntime(ScriptComponent& component) : m_Impl(std::make_unique<Impl>(component)) {
}

AngelScriptRuntime::~AngelScriptRuntime() = default;

bool AngelScriptRuntime::Load(const std::string& source, const std::string& chunkName, const std::string& className,
    ...
    m_Impl->engine->SetMessageCallback(asMETHOD(Impl, MessageCallback), m_Impl.get(), asCALL_THISCALL);
    ...
    int result = module->AddScriptSection(chunkName.c_str(), source.data(), static_cast<unsigned int>(source.size()));
    if (result < 0 || module->Build() < 0) {
        error = m_Impl->messages.str();
        if (error.empty())
            error = "AngelScript compile failed";
        return false;
    }
    ...
    m_Impl->object = static_cast<asIScriptObject*>(m_Impl->engine->CreateScriptObject(m_Impl->type));
    if (!m_Impl->object) {
    ...
    if (!m_Impl->onCollision)
        m_Impl->onCollision = m_Impl->type->GetMethodByDecl("void OnCollision(CollisionEvent)");
    m_Impl->onAnimationEvent =
        m_Impl->type->GetMethodByDecl("void OnAnimationEvent(const string &in, const string &in)");
...
bool AngelScriptRuntime::Call(const char* methodDecl, float deltaSeconds, std::string& error) {
    asIScriptFunction* function = nullptr;
    if (std::string(methodDecl) == "Awake")
        function = m_Impl->awake;
    else if (std::string(methodDecl) == "OnEnable")
        function = m_Impl->onEnable;
    else if (std::string(methodDecl) == "Start")
        function = m_Impl->start;
    else if (std::string(methodDecl) == "FixedUpdate")
        function = m_Impl->fixedUpdate;
    else if (std::string(methodDecl) == "Update")
        function = m_Impl->update;
    else if (std::string(methodDecl) == "LateUpdate")
        function = m_Impl->lateUpdate;
    else if (std::string(methodDecl) == "OnDisable")
        function = m_Impl->onDisable;
    else if (std::string(methodDecl) == "OnDestroy")
        function = m_Impl->onDestroy;
    return m_Impl->Execute(function, deltaSeconds,
                           function == m_Impl->fixedUpdate || function == m_Impl->update ||
                               function == m_Impl->lateUpdate,
                           nullptr, error);
}
...
bool AngelScriptRuntime::CallAnimationEvent(const std::string& name, const std::string& payload, std::string& error) {
    return m_Impl->ExecuteAnimationEvent(m_Impl->onAnimationEvent, name, payload, error);
}

bool AngelScriptRuntime::SubscribeUIEvent(const std::string& elementId, const std::string& eventName,
                                          const std::string& callbackName, std::string& error) {
    return m_Impl->SubscribeUIEvent(elementId, eventName, callbackName, error);
}

void AngelScriptRuntime::UnsubscribeUIEvent(const std::string& elementId, const std::string& eventName) {
...
bool AngelScriptRuntime::SubscribeScriptEvent(const std::string& eventName, const std::string& callbackName,
                                              std::string& error) {
    return m_Impl->SubscribeScriptEvent(eventName, callbackName, error);
}

bool AngelScriptRuntime::EmitScriptEvent(const std::string& eventName, const std::string& jsonPayload) {
...
uint64_t AngelScriptRuntime::ScheduleTimer(float seconds, bool repeat, const std::string& callbackName,
                                           std::string& error) {
...
std::vector<ScriptClassInfo> AngelScriptRuntime::DiscoverClasses(const std::string& source,
    ...
    const int addResult =
        module->AddScriptSection(chunkName.c_str(), source.data(), static_cast<unsigned int>(source.size()));
...
bool AngelScriptRuntime::PreprocessSource(const std::string& source, const std::string& chunkName,
                                          std::string& outSource, std::vector<std::string>* outDependencies,
                                          std::string& error) {
    outSource.clear();
    if (outDependencies)
    ...
    if (!chunkName.empty() && chunkName != "InlineScript") {
        ...
        if (chunkPath.has_parent_path())
            baseDirectory = chunkPath.parent_path();
    }
    std::unordered_set<std::string> includeStack;
    return PreprocessScriptSourceRecursive(source, chunkName.empty() ? "InlineScript" : chunkName, baseDirectory,
                                           outSource, outDependencies, includeStack, error, 0);
}
