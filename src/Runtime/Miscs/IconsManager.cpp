...
std::string WithSvgExtension(std::string_view iconName)
{
    ...
    if (name.size() < 4 || Lower(name.substr(name.size() - 4)) != ".svg") name += ".svg";
    return name;
}
...
{
    if (value.empty() || value == "currentColor") return currentColor;
...
SvgStyle ParseStyle(const std::string& tag, const SvgStyle& inherited, IconColor currentColor)
{
    ...
    if (!strokeWidth.empty()) style.strokeWidth = ParseFloat(strokeWidth, style.strokeWidth);
    return style;
}
...
bool IsPathCommand(char c)
{
    ...
    case 'V': case 'v': case 'C': case 'c': case 'A': case 'a':
    case 'Z': case 'z':
        return true;
    default:
        return false;
...
void SkipSeparators(const std::string& text, size_t& pos)
{
    while (pos < text.size()) {
        ...
        if (std::isspace(static_cast<unsigned char>(c)) || c == ',') ++pos;
        else break;
    }
}
...
bool ReadPathNumber(const std::string& text, size_t& pos, float& out)
{
    ...
    if (pos >= text.size() || IsPathCommand(text[pos])) return false;
...
{
    if (current.points.size() >= 2) shapes.push_back(current);
...
std::vector<SvgShape> ParsePath(const std::string& path, SvgStyle style)
{
    ...
    while (pos < path.size()) {
        ...
        if (IsPathCommand(path[pos])) command = path[pos++];
        if (!command) break;
        ...
        case 'M': case 'm': {
            float x = 0.0f, y = 0.0f;
            if (!ReadPathNumber(path, pos, x) || !ReadPathNumber(path, pos, y)) break;
        ...
        case 'H': case 'h': {
            float x = 0.0f;
            if (!ReadPathNumber(path, pos, x)) break;
        ...
        case 'V': case 'v': {
            float y = 0.0f;
            if (!ReadPathNumber(path, pos, y)) break;
        ...
        case 'C': case 'c': {
            ...
            if (!ReadPathNumber(path, pos, x1) || !ReadPathNumber(path, pos, y1) ||
                !ReadPathNumber(path, pos, x2) || !ReadPathNumber(path, pos, y2) ||
                !ReadPathNumber(path, pos, x) || !ReadPathNumber(path, pos, y)) break;
        ...
        case 'A': case 'a': {
            ...
            if (!ReadPathNumber(path, pos, rx) || !ReadPathNumber(path, pos, ry) ||
                ...
                !ReadPathNumber(path, pos, sweep) || !ReadPathNumber(path, pos, x) ||
                !ReadPathNumber(path, pos, y)) break;
            (void)rx; (void)ry; (void)rotation; (void)largeArc; (void)sweep;
            cursor = relative ? Vec2f{cursor.x + x, cursor.y + y} : Vec2f{x, y};
            currentShape.points.push_back(cursor);
            break;
        }
...
SvgDocument ParseSvg(const std::string& svg, IconColor color)
{
    ...
    if (svgStart != std::string::npos) {
        ...
        const std::string tag = svgEnd == std::string::npos
            ? std::string{} : svg.substr(svgStart, svgEnd - svgStart + 1);
...
{
    if (x < 0 || y < 0 || x >= pixels.width || y >= pixels.height || color.a == 0) return;
    ...
    for (int i = 0; i <= steps; ++i) {
        ...
        for (int oy = -radius; oy <= radius; ++oy) {
            for (int ox = -radius; ox <= radius; ++ox) {
                if (ox * ox + oy * oy <= radius * radius) BlendPixel(pixels, cx + ox, cy + oy, color);
            }
        }
    }
}
...
void FillPolygon(IconPixels& pixels, const std::vector<Vec2f>& points, IconColor color)
{
    ...
    for (int y = iy0; y <= iy1; ++y) {
        for (int x = ix0; x <= ix1; ++x) {
            if (PointInPolygon(static_cast<float>(x) + 0.5f,
...
{
    if (size <= 0 || svg.width <= 0.0f || svg.height <= 0.0f) return nullptr;
    ...
    for (const SvgShape& shape : svg.shapes) {
        ...
        if (shape.style.fill && shape.closed) FillPolygon(*pixels, points, shape.style.fillColor);
        if (shape.style.stroke && points.size() >= 2) {
            for (size_t i = 1; i < points.size(); ++i)
                DrawLine(*pixels, points[i - 1], points[i],
                         (std::max)(1.0f, shape.style.strokeWidth * scale),
...
std::vector<uint8_t> MakeIcoDib(const IconPixels& pixels)
{
    ...
    append32(0); append32(0); append32(0); append32(0);
...
fs::path IconsManager::FindDefaultIconRoot() const
{
    if (const char* basePath = SDL_GetBasePath()) {
        ...
        std::error_code ec;
        if (fs::is_directory(candidate, ec) && !ec) return candidate.lexically_normal();
    }
...
fs::path IconsManager::ResolveIconPath(std::string_view iconName) const
{
    ...
    std::error_code ec;
    if (fs::is_regular_file(path, ec) && !ec) return path.lexically_normal();
    return {};
}

std::string IconsManager::MakePixelKey(std::string_view iconName, int size, IconColor color) const
{
    return Lower(std::string(iconName)) + "|" + std::to_string(size) + "|" +
        std::to_string(color.r) + "," + std::to_string(color.g) + "," +
        std::to_string(color.b) + "," + std::to_string(color.a);
}

std::string IconsManager::MakeUploadKey(RHIBackend backend, std::string_view iconName,
                                        int size, IconColor color) const
{
    return std::to_string(static_cast<int>(backend)) + "|" + MakePixelKey(iconName, size, color);
}

std::shared_ptr<IconPixels> IconsManager::Rasterize(std::string_view iconName,
                                                    int size,
                                                    IconColor color)
{
    if (size <= 0) return nullptr;
    const std::string key = MakePixelKey(iconName, size, color);
    if (auto it = m_PixelCache.find(key); it != m_PixelCache.end()) return it->second;
...
GpuTextureView* IconsManager::GetOrUpload(IRHIDevice& device,
...
bool IconsManager::WriteIco(std::string_view iconName,
                            const fs::path& output,
                            const std::vector<int>& sizes,
                            IconColor color)
{
    if (sizes.empty()) return false;
    struct Entry { int size = 0; std::vector<uint8_t> dib; };
    ...
    for (const Entry& entry : entries)
        out.write(reinterpret_cast<const char*>(entry.dib.data()),
                  static_cast<std::streamsize>(entry.dib.size()));
    return out.good();
}
