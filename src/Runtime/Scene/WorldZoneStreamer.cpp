...
struct WorldZoneStreamer::Impl {
    ...
    bool DistanceDesired(const Entry& entry) const {
        const bool active =
            entry.state != WorldZoneStreamState::Unloaded && entry.state != WorldZoneStreamState::Failed;
        const float threshold = active ? entry.descriptor.unloadDistance : entry.descriptor.loadDistance;
        return DistanceToBounds(observer, entry.descriptor.boundsCenter, entry.descriptor.boundsHalfExtents) <=
               threshold;
    }
    bool Desired(const Entry& entry) const {
        const bool distance = DistanceDesired(entry);
        switch (entry.descriptor.triggerMode) {
        case WorldZoneTriggerMode::Distance:
            return distance;
        case WorldZoneTriggerMode::Portal:
            return entry.portalOpen;
        case WorldZoneTriggerMode::DistanceOrPortal:
            return distance || entry.portalOpen;
        case WorldZoneTriggerMode::DistanceAndPortal:
            return distance && entry.portalOpen;
        }
        return false;
    }
...
bool WorldZoneStreamer::Register(WorldZoneStreamDescriptor descriptor, std::string* error) {
    if (descriptor.stableName.empty() || !IsSafeContentPath(descriptor.sourcePath)) {
        if (error)
            *error = "zone requires a stable name and safe Content path";
        return false;
    }
    if (descriptor.loadDistance < 0.0f || descriptor.unloadDistance < descriptor.loadDistance) {
        if (error)
            *error = "zone unload distance must be greater than or equal to load distance";
        return false;
    }
    if (m_Impl->entries.count(descriptor.stableName)) {
        if (error)
            *error = "duplicate streamed zone: " + descriptor.stableName;
        return false;
    }
    Impl::Entry entry;
...
bool WorldZoneStreamer::Unregister(Scene& scene, const std::string& stableName) {
    ...
    if (found == m_Impl->entries.end())
        return false;
    if (found->second.zoneID)
        scene.DestroyZone(found->second.zoneID);
...
bool WorldZoneStreamer::Retry(const std::string& stableName) {
    const auto found = m_Impl->entries.find(stableName);
    if (found == m_Impl->entries.end() || found->second.state != WorldZoneStreamState::Failed)
        return false;
...
void WorldZoneStreamer::SetObserverPosition(const Vec3& position) {
    m_Impl->observer = position;
}
const Vec3& WorldZoneStreamer::GetObserverPosition() const {
...
void WorldZoneStreamer::Tick(Scene& scene, float deltaSeconds) {
    ...
    std::sort(ordered.begin(), ordered.end(), [this](const auto* left, const auto* right) {
        ...
        const float ld =
            DistanceToBounds(m_Impl->observer, left->descriptor.boundsCenter, left->descriptor.boundsHalfExtents);
        const float rd =
            DistanceToBounds(m_Impl->observer, right->descriptor.boundsCenter, right->descriptor.boundsHalfExtents);
        if (ld != rd)
            return ld < rd;
    ...
    for (Impl::Entry* entry : ordered) {
        if (transitions >= m_Impl->maxTransitionsPerFrame)
            break;
        if (entry->desired || entry->state == WorldZoneStreamState::Unloaded)
            continue;
        if (entry->zoneID)
            scene.DestroyZone(entry->zoneID);
        entry->zoneID = 0;
        entry->loadTask = {};
        entry->plan = {};
        entry->instantiation = {};
    ...
    for (Impl::Entry* entry : ordered) {
        ...
        entry->loadTask = scene.SubmitZoneTask(
            entry->zoneID, {"zone.read_parse." + entry->descriptor.stableName, TaskPriority::Normal},
            [sourcePath](CancellationToken cancellation, WorldZoneLifetimeToken lifetime) {
                ...
                if (source.empty()) {
                    if (result.error.empty())
                        result.error = "zone source is empty";
                    return result;
                }
    ...
    if (actorBudget) {
        ...
        remainingActors =
            static_cast<size_t>(std::min<uint64_t>(remainingActors, live < actorBudget ? actorBudget - live : 0));
        if (remainingActors == 0)
            ++m_Impl->actorBudgetBlockedFrames;
    }
    for (Impl::Entry* entry : ordered) {
        if (!entry->desired)
            continue;
        ...
        if (entry->state != WorldZoneStreamState::Instantiating || remainingActors == 0)
            continue;
        ...
        const bool instantiated = SceneSerializer::InstantiateLoadPlanAdditive(
            scene, entry->plan, entry->instantiation, remainingActors, created, complete, &instantiateError);
        ...
        if (complete)
            entry->state = WorldZoneStreamState::Loaded;
    }
}
...
WorldZoneStreamerStats WorldZoneStreamer::GetStats() const {
    ...
    for (const auto& pair : m_Impl->entries) {
        ...
        case WorldZoneStreamState::Loaded:
            ++result.loadedZones;
            break;
        case WorldZoneStreamState::Loading:
        case WorldZoneStreamState::Instantiating:
            ++result.loadingZones;
            break;
        case WorldZoneStreamState::Failed:
            ++result.failedZones;
            break;
        default:
            break;
...
std::vector<WorldZoneStreamEntryStats> WorldZoneStreamer::GetEntryStats() const {
    ...
    for (const auto& pair : m_Impl->entries) {
        ...
        result.push_back({entry.descriptor.stableName, entry.state, entry.desired, entry.lifetimeGeneration,
                          entry.instantiatedActors, entry.lastError});
    }
    std::sort(result.begin(), result.end(),
              [](const auto& left, const auto& right) { return left.stableName < right.stableName; });
    return result;
}
