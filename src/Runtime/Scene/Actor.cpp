...
Actor::Actor(std::string name, uint64_t id, ActorHandle handle) : m_ID(id), m_Handle(handle), m_Name(std::move(name)) {
}
...
void Component::SetEnabled(bool enabled) {
    if (m_Enabled == enabled)
        return;
    if (m_Owner)
        m_Owner->OnComponentEnabledChanged(*this, enabled);
    else
        m_Enabled = enabled;
}
...
void Actor::OnComponentEnabledChanged(Component& component, bool enabled) {
    ...
    if (effective && !component.m_EffectiveEnabled) {
        component.OnEnable();
        component.m_EffectiveEnabled = true;
    } else if (!effective && component.m_EffectiveEnabled) {
        component.OnDisable();
        component.m_EffectiveEnabled = false;
    }
}
...
void Actor::QueueComponentAdd(const std::string& type, const nlohmann::json& data) {
    if (m_Scene)
        m_Scene->QueueAddComponent(m_Handle, type, data);
}
...
Vec3 Actor::GetWorldPosition() const {
    return m_Parent ? m_Parent->GetWorldMatrix().TransformPoint(m_Transform.position) : m_Transform.position;
}
...
Component* Actor::GetComponentByTypeName(const std::string& typeName) const {
    for (const auto& entry : m_Components) {
        if (entry.component && typeName == entry.component->GetTypeName())
...
bool Actor::RemoveComponentByTypeName(const std::string& typeName) {
    for (size_t i = 0; i < m_Components.size(); ++i) {
        if (typeName == m_Components[i].component->GetTypeName())
            return RemoveComponentAt(i);
    }
    return false;
}

Component* Actor::AddComponentObject(std::type_index type, std::unique_ptr<Component> component, bool finalizeNow) {
    if (!component)
        return nullptr;
    const auto existing = m_ComponentLookup.find(type);
    if (existing != m_ComponentLookup.end())
        return m_Components[existing->second].component.get();
    ...
    if (finalizeNow) {
        ...
        raw->m_Attached = true;
        raw->OnInitialize();
        raw->m_Initialized = true;
        if (m_Scene && m_Scene->IsPlaying()) {
            raw->OnBeginPlay();
            raw->m_BeganPlay = true;
            if (IsActive() && raw->IsEnabled()) {
                raw->OnEnable();
                raw->m_EffectiveEnabled = true;
            }
            raw->OnStart();
            raw->m_Started = true;
        }
...
void Actor::SetParent(Actor* parent) {
    ...
    for (Actor* ancestor = parent; ancestor; ancestor = ancestor->m_Parent) {
        if (ancestor == this) {
            Logger::Warn("[Scene] rejected cyclic actor parenting");
            return;
        }
    }
    if (m_Parent)
...
void Actor::RefreshActiveInHierarchy(bool parentActive, bool playing) {
    const bool next =
        m_ActiveSelf && parentActive && m_State != ActorState::PendingDestroy && m_State != ActorState::Destroyed;
    ...
    if (changed && playing) {
        for (Component* comp : OrderedComponents(!next)) {
            ...
            if (effective && !comp->m_EffectiveEnabled) {
                comp->OnEnable();
                comp->m_EffectiveEnabled = true;
            } else if (!effective && comp->m_EffectiveEnabled) {
                comp->OnDisable();
                comp->m_EffectiveEnabled = false;
            }
        }
    ...
    if (next)
        for (Actor* child : m_Children)
            child->RefreshActiveInHierarchy(true, playing);
}
