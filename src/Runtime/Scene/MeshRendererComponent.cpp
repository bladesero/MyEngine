...
void MeshRendererComponent::Serialize(nlohmann::json& data) const {
    ...
    if (!m_Materials.empty()) {
        ...
        for (const MaterialHandle& material : m_Materials) {
            data["materials"].push_back(material.Get()
                ? AssetManager::Get().MakeProjectRelativePath(material->GetPath())
                : std::string{});
        }
    }
    ...
    if (slot0.Get()) {
        const std::string materialPath =
            AssetManager::Get().MakeProjectRelativePath(slot0->GetPath());
...
void MeshRendererComponent::Deserialize(const nlohmann::json& data) {
        ...
        for (const auto& entry : *itMaterials) {
            if (entry.is_string()) {
                m_Materials.push_back(
                    AssetManager::Get().ResolveMaterialReference(entry.get<std::string>(), meshPath));
            } else {
                m_Materials.push_back({});
            }
        }
    ...
    if (!m_Materials.empty() && hasEmbeddedMaterial) {
        ...
        } else if (itMaterials != data.end() && itMaterials->is_array() &&
                   !itMaterials->empty() && (*itMaterials)[0].is_string()) {
