...
enum class PropertyKind { Bool, Int32, UInt32, UInt64, Float, String, Vec2, Vec3, Vec4, Color, Enum, AssetPath, ActorHandle, Json };
enum class PropertyFlags : uint32_t {
    None = 0, Serialize = 1u << 0, Inspector = 1u << 1, ScriptRead = 1u << 2,
    ScriptWrite = 1u << 3, PrefabOverride = 1u << 4, Transient = 1u << 5,
    ReadOnly = 1u << 6
};
inline PropertyFlags operator|(PropertyFlags a, PropertyFlags b) { return static_cast<PropertyFlags>(static_cast<uint32_t>(a) | static_cast<uint32_t>(b)); }
inline bool HasPropertyFlag(PropertyFlags value, PropertyFlags flag) { return (static_cast<uint32_t>(value) & static_cast<uint32_t>(flag)) != 0; }
...
public:
    ...
    bool Serialize(const Component& component, nlohmann::json& data, uint32_t& version, std::string* error = nullptr) const;
    bool Deserialize(Component& component, const std::string& stableName, uint32_t version,
                     const nlohmann::json& data, std::string* error = nullptr) const;
...
#define MYENGINE_BEGIN_COMPONENT_TYPE(Type, StableName, Version) \
    TypeDescriptor _myengine_type_descriptor; using _MyEngineRegisteredType = Type; \
    _myengine_type_descriptor.stableName = StableName; _myengine_type_descriptor.schemaVersion = Version; \
...
#define MYENGINE_PROPERTY_ACCESSOR(StableName, Kind, Getter, Setter, Flags, DefaultValue, Hints) \
    do { PropertyDescriptor _p; _p.stableName = StableName; _p.kind = Kind; _p.flags = Flags; \
         _p.defaultValue = DefaultValue; _p.hints = Hints; _p.getter = Getter; _p.setter = Setter; \
         _myengine_type_descriptor.properties.push_back(std::move(_p)); } while (false)
