...
// Runtime/Math owns this contract; Core/EngineMath.h remains a forwarding shim.
...
struct Mat4 {
    ...
    Vec3 TransformDir(const Vec3& v) const { return Transform(Vec4::FromVec3(v, 0.0f)).XYZ(); }

    // Transform a point (w=1).
    Vec3 TransformPoint(const Vec3& v) const { return Transform(Vec4::FromVec3(v, 1.0f)).XYZ(); }
...
inline Vec4 Mat4::TransformSimd(const Vec4& v) const {
    ...
    const __m128 r = _mm_add_ps(_mm_add_ps(_mm_mul_ps(_mm_shuffle_ps(vx, vx, _MM_SHUFFLE(0, 0, 0, 0)), c0),
