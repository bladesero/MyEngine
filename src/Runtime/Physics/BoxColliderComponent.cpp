...
Vec3 BoxColliderComponent::GetWorldHalfExtents() const {
    ...
    return {
        std::fabs(world.m[0][0]) * m_HalfExtents.x + std::fabs(world.m[1][0]) * m_HalfExtents.y +
            std::fabs(world.m[2][0]) * m_HalfExtents.z,
        std::fabs(world.m[0][1]) * m_HalfExtents.x + std::fabs(world.m[1][1]) * m_HalfExtents.y +
            std::fabs(world.m[2][1]) * m_HalfExtents.z,
        std::fabs(world.m[0][2]) * m_HalfExtents.x + std::fabs(world.m[1][2]) * m_HalfExtents.y +
...
OrientedBox BoxColliderComponent::GetWorldShape() const {
    ...
    for (size_t axis = 0; axis < 3; ++axis) {
        ...
        box.axes[axis] = scale > 1e-6f ? transformedAxes[axis] / scale
                                       : (axis == 0 ? Vec3::Right() : (axis == 1 ? Vec3::Up() : Vec3::Forward()));
        if (axis == 0)
            box.halfExtents.x *= scale;
        else if (axis == 1)
            box.halfExtents.y *= scale;
        else
            box.halfExtents.z *= scale;
    }
    return box;
}
...
void BoxColliderComponent::Serialize(nlohmann::json& data) const {
    ...
    data["halfExtents"] = nlohmann::json::array({m_HalfExtents.x, m_HalfExtents.y, m_HalfExtents.z});
}
