...
bool JoltAssertFailed(const char* expression, const char* message, const char* file, JPH::uint line) {
    Logger::Error("[Jolt] Assertion failed: ", expression, " at ", file, ":", line, message ? " - " : "",
                  message ? message : "");
    return false;
}
...
Quat WorldRotation(const Actor& actor) {
    ...
    for (int row = 0; row < 3; ++row) {
        const float len = std::sqrt(world.m[row][0] * world.m[row][0] + world.m[row][1] * world.m[row][1] +
...
void SetActorWorldPose(Actor& actor, const Vec3& position, const Quat& rotation) {
    ...
    if (actor.GetParent()) {
        Mat4 inverseParent;
        if (Mat4Invert(actor.GetParent()->GetWorldMatrix(), inverseParent))
            world *= inverseParent;
    }
...
bool SamePose(const Vec3& position, const Quat& rotation, const Vec3& otherPosition, const Quat& otherRotation) {
    if ((position - otherPosition).LengthSq() > 1e-8f)
        return false;
    const float dot = rotation.x * otherRotation.x + rotation.y * otherRotation.y + rotation.z * otherRotation.z +
                      rotation.w * otherRotation.w;
...
public:
    ...
    bool ShouldCollide(JPH::ObjectLayer a, JPH::ObjectLayer b) const override {
        if (a >= m_Layers.size() || b >= m_Layers.size())
            return false;
        const LayerState& lhs = m_Layers[a];
        const LayerState& rhs = m_Layers[b];
...
public:
    ...
    bool ShouldCollide(JPH::ObjectLayer layer, JPH::BroadPhaseLayer broad) const override {
        if (layer >= m_Layers.size() || !m_Layers[layer].valid)
            return false;
...
public:
    Impl()
        : layers(kMaxBodies + 1), broad(layers), pair(layers), objectBroad(layers), tempAllocator(64 * 1024 * 1024),
          jobs(JPH::cMaxPhysicsJobs, JPH::cMaxPhysicsBarriers, std::max(1u, std::thread::hardware_concurrency()) - 1) {
    ...
    JPH::ObjectLayer AllocateLayer(uint32_t layer, uint32_t mask, bool moving) {
        ...
        if (!freeSlots.empty()) {
            ...
            freeSlots.pop_back();
        } else if (nextSlot < layers.size())
            result = nextSlot++;
        if (result == JPH::cObjectLayerInvalid)
            return result;
    ...
    static ColliderComponent* FindCollider(Actor& actor) {
        if (auto* value = actor.GetComponent<BoxColliderComponent>(); value && value->IsEnabled())
            return value;
        if (auto* value = actor.GetComponent<SphereColliderComponent>(); value && value->IsEnabled())
            return value;
        if (auto* value = actor.GetComponent<CapsuleColliderComponent>(); value && value->IsEnabled())
            return value;
        return nullptr;
    }
    ...
    static JPH::RefConst<JPH::Shape> CreateShape(Actor& actor, ColliderComponent& collider) {
        ...
        if (auto* box = dynamic_cast<BoxColliderComponent*>(&collider)) {
            ...
            return new JPH::BoxShape(JPH::Vec3(std::max(0.001f, half.x * scale.x), std::max(0.001f, half.y * scale.y),
                                               std::max(0.001f, half.z * scale.z)));
        }
        if (auto* sphere = dynamic_cast<SphereColliderComponent*>(&collider)) {
            return new JPH::SphereShape(std::max(0.001f, sphere->GetRadius() * std::max({scale.x, scale.y, scale.z})));
        }
    ...
    JPH::EAllowedDOFs AllowedDOFs(const RigidBodyComponent& body) {
        ...
        if (linear.x < 0.5f)
            result |= D::TranslationX;
        if (linear.y < 0.5f)
            result |= D::TranslationY;
        if (linear.z < 0.5f)
            result |= D::TranslationZ;
        if (angular.x < 0.5f)
            result |= D::RotationX;
        if (angular.y < 0.5f)
            result |= D::RotationY;
    ...
    bool CreateRegular(Actor& actor, ColliderComponent& collider, RigidBodyComponent* body, uint64_t signature) {
        ...
        if (slot == JPH::cObjectLayerInvalid)
            return false;
        const JPH::EMotionType motion = !body || body->GetBodyType() == BodyType::Static
                                            ? JPH::EMotionType::Static
                                            : (body->GetBodyType() == BodyType::Kinematic ? JPH::EMotionType::Kinematic
        ...
        if (body)
            body->m_SettingsDirty = false;
        return true;
    }

    bool CreateCharacter(Actor& actor, CapsuleColliderComponent& capsule, CharacterControllerComponent& controller,
                         uint64_t signature) {
        ...
        JPH::Ref<JPH::Character> character = new JPH::Character(&settings, ToJoltPosition(actor.GetWorldPosition()),
                                                                ToJolt(WorldRotation(actor)), actor.GetID(), &system);
    ...
    void ApplyCommands(Actor& actor, BodyInfo& info, RigidBodyComponent& body, float dt) {
        ...
        if (info.hasLastPose &&
            !SamePose(actorPosition, actorRotation, info.lastEnginePosition, info.lastEngineRotation) &&
        ...
        if (body.m_LinearVelocityDirty) {
            api.SetLinearVelocity(info.body, ToJolt(body.m_Velocity));
            body.m_LinearVelocityDirty = false;
        }
        if (body.m_AngularVelocityDirty) {
            api.SetAngularVelocity(info.body, ToJolt(body.m_AngularVelocity));
            body.m_AngularVelocityDirty = false;
        }
        if (body.m_AccumulatedForce.LengthSq() > 0.0f)
            api.AddForce(info.body, ToJolt(body.m_AccumulatedForce));
        if (body.m_AccumulatedTorque.LengthSq() > 0.0f)
            api.AddTorque(info.body, ToJolt(body.m_AccumulatedTorque));
        if (body.m_AccumulatedImpulse.LengthSq() > 0.0f)
            api.AddImpulse(info.body, ToJolt(body.m_AccumulatedImpulse));
        ...
        if (body.m_TeleportPending) {
            api.SetPositionAndRotation(
                info.body, ToJoltPosition(body.m_TeleportPosition),
                ToJolt(Quat::FromEulerRad(body.m_TeleportRotation.x * kDeg2Rad, body.m_TeleportRotation.y * kDeg2Rad,
                                          body.m_TeleportRotation.z * kDeg2Rad)),
                JPH::EActivation::Activate);
        ...
        if (body.IsKinematic() && body.m_KinematicTargetPending) {
            api.MoveKinematic(info.body, ToJoltPosition(body.m_KinematicTargetPosition),
                              ToJolt(Quat::FromEulerRad(body.m_KinematicTargetRotation.x * kDeg2Rad,
                                                        body.m_KinematicTargetRotation.y * kDeg2Rad,
                                                        body.m_KinematicTargetRotation.z * kDeg2Rad)),
                              dt);
            body.m_KinematicTargetPending = false;
        }
        if (body.m_WakeRequested) {
            api.ActivateBody(info.body);
            body.m_WakeRequested = false;
        }
    }
    ...
    void Reconcile(Scene& scene, float dt) {
        ...
        scene.ForEach([&](Actor& actor) {
            ...
            if (activeBody && controller && controller->IsEnabled()) {
                Logger::Warn("[Physics] Actor '", actor.GetName(),
                             "' cannot use RigidBody and CharacterController together");
            }
            if ((activeBody || isCharacter) && HasNonUniformParentScale(actor)) {
                Logger::Error("[Physics] Actor '", actor.GetName(),
                              "' has a moving physics body under non-uniform parent scale");
                return;
            }
            ...
            if (found == bodies.end()) {
                if (isCharacter)
                    CreateCharacter(actor, *static_cast<CapsuleColliderComponent*>(collider), *controller, signature);
                else
                    CreateRegular(actor, *collider, activeBody, signature);
            ...
            if (isCharacter) {
                ...
                const JPH::CharacterBase::EGroundState groundState = found->second.characterObject->GetGroundState();
                const bool onGround = groundState == JPH::CharacterBase::EGroundState::OnGround;
                const Vec3 groundVelocity =
                    onGround ? FromJolt(found->second.characterObject->GetGroundVelocity()) : Vec3::Zero();
                if (!onGround) {
                ...
                if (!info.hasLastPose ||
                    !SamePose(position, rotation, info.lastEnginePosition, info.lastEngineRotation)) {
                    system.GetBodyInterface().SetPositionAndRotation(info.body, ToJoltPosition(position),
                                                                     ToJolt(rotation), JPH::EActivation::DontActivate);
    ...
    void PullTransforms(Scene& scene) {
        ...
        for (auto& [actorID, info] : bodies) {
            ...
            if (info.character) {
                ...
                JPH::RVec3 p;
                JPH::Quat q;
                info.characterObject->GetPositionAndRotation(p, q);
            ...
            if (body->GetBodyType() != BodyType::Static) {
                JPH::RVec3 p;
                JPH::Quat q;
                api.GetPositionAndRotation(info.body, p, q);
                SetActorWorldPose(*actor, FromJolt(p), FromJolt(q));
            }
            info.lastEnginePosition = actor->GetWorldPosition();
            info.lastEngineRotation = WorldRotation(*actor);
    ...
    void Dispatch(Scene& scene) {
        ...
        for (const auto& [key, old] : activePairs)
            if (!current.count(key))
                Emit(scene, old, CollisionEventPhase::Exit);
        for (const auto& key : touched)
            Emit(scene, current[key], activePairs.count(key) ? CollisionEventPhase::Stay : CollisionEventPhase::Enter);
        activePairs = std::move(current);
    }

    static void Emit(Scene& scene, const ContactRecord& record, CollisionEventPhase phase) {
        Actor* a = scene.FindByID(record.a);
        Actor* b = scene.FindByID(record.b);
        if (!a || !b)
            return;
    ...
    void QueueContact(const JPH::Body& a, const JPH::Body& b, const JPH::ContactManifold& manifold,
        ...
        record.a = a.GetUserData();
        record.b = b.GetUserData();
        record.normal = FromJolt(manifold.mWorldSpaceNormal);
        record.depth = manifold.mPenetrationDepth;
        record.trigger = settings.mIsSensor || a.IsSensor() || b.IsSensor();
        if (!manifold.mRelativeContactPointsOn1.empty())
            record.point = FromJolt(manifold.GetWorldSpaceContactPointOn1(0));
        std::lock_guard<std::mutex> lock(contactMutex);
    ...
    void OnContactRemoved(const JPH::SubShapeIDPair& pairValue) override {
        ...
        if (a == bodyActors.end() || b == bodyActors.end())
            return;
        ContactRecord record;
        record.a = a->second;
        record.b = b->second;
...
void PhysicsWorld::StepFixed(Scene& scene, float fixedDeltaSeconds) {
    ...
    const JPH::EPhysicsUpdateError error =
        m_Impl->system.Update(fixedDeltaSeconds, 1, &m_Impl->tempAllocator, &m_Impl->jobs);
    if (error != JPH::EPhysicsUpdateError::None)
        Logger::Error("[Physics] Jolt update error: ", static_cast<uint32_t>(error));
    m_Impl->PullTransforms(scene);
    m_Impl->Dispatch(scene);
}

bool PhysicsWorld::Raycast(const Scene& scene, const Ray& ray, float maxDistance, uint32_t layerMask,
                           RaycastHit& hit) const {
    if (maxDistance <= 0.0f || ray.direction.LengthSq() < 1e-8f)
        return false;
    ...
    if (!m_Impl->system.GetNarrowPhaseQuery().CastRay(cast, result, {}, filter))
        return false;
    ...
    hit.normal =
        lock.Succeeded()
            ? FromJolt(lock.GetBody().GetWorldSpaceSurfaceNormal(result.mSubShapeID2, ToJoltPosition(hit.point)))
            : Vec3::Up();
    return true;
}

bool PhysicsWorld::OverlapSphere(const Scene& scene, const Vec3& center, float radius, uint32_t layerMask,
    ...
    scene.ForEach([&](Actor& actor) {
        ...
        if (!collider || (collider->GetLayer() & layerMask) == 0)
            return;
        ...
        if (overlapping)
            outActors.push_back(actor.GetHandle());
    });
