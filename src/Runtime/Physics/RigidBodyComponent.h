...
public:
    ...
    void AddAngularImpulse(const Vec3& impulse) {
        m_AccumulatedAngularImpulse += impulse;
        WakeUp();
    }
    ...
    void SetLinearAxisLocks(const Vec3& locks) {
        m_LinearAxisLocks = locks;
        m_SettingsDirty = true;
    }
    const Vec3& GetAngularAxisLocks() const { return m_AngularAxisLocks; }
    void SetAngularAxisLocks(const Vec3& locks) {
        m_AngularAxisLocks = locks;
        m_SettingsDirty = true;
    }
...
private:
    ...
    void SetSleepingFromPhysics(bool value) {
        m_Sleeping = value;
