...
void CharacterControllerComponent::Serialize(nlohmann::json& data) const {
    data["velocity"] = nlohmann::json::array({m_DesiredVelocity.x, m_DesiredVelocity.y, m_DesiredVelocity.z});
...
void CharacterControllerComponent::Deserialize(const nlohmann::json& data) {
    if (data.contains("velocity") && data["velocity"].is_array() && data["velocity"].size() == 3) {
        m_DesiredVelocity = {data["velocity"][0].get<float>(), data["velocity"][1].get<float>(),
