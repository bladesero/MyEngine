...
float ProjectRadius(const OrientedBox& box, const Vec3& axis) {
    return std::fabs(axis.Dot(box.axes[0])) * box.halfExtents.x + std::fabs(axis.Dot(box.axes[1])) * box.halfExtents.y +
           std::fabs(axis.Dot(box.axes[2])) * box.halfExtents.z;
}

bool TestAxis(const OrientedBox& a, const OrientedBox& b, const Vec3& rawAxis, const Vec3& delta, float& bestDepth,
              Vec3& bestNormal) {
...
void ClosestSegmentPoints(const Vec3& p1, const Vec3& q1, const Vec3& p2, const Vec3& q2, Vec3& c1, Vec3& c2) {
    const Vec3 d1 = q1 - p1;
    const Vec3 d2 = q2 - p2;
    const Vec3 r = p1 - p2;
...
bool CollideSpheres(const Vec3& centerA, float radiusA, const Vec3& centerB, float radiusB, ContactManifold& contact) {
...
AABB ComputeBounds(const OrientedBox& box) {
    const Vec3 extent = {
        std::fabs(box.axes[0].x) * box.halfExtents.x + std::fabs(box.axes[1].x) * box.halfExtents.y +
            std::fabs(box.axes[2].x) * box.halfExtents.z,
        std::fabs(box.axes[0].y) * box.halfExtents.x + std::fabs(box.axes[1].y) * box.halfExtents.y +
            std::fabs(box.axes[2].y) * box.halfExtents.z,
        std::fabs(box.axes[0].z) * box.halfExtents.x + std::fabs(box.axes[1].z) * box.halfExtents.y +
...
bool Collide(const OrientedBox& a, const OrientedBox& b, ContactManifold& contact) {
    ...
    for (size_t i = 0; i < 3; ++i) {
        if (!TestAxis(a, b, a.axes[i], delta, bestDepth, bestNormal) ||
            !TestAxis(a, b, b.axes[i], delta, bestDepth, bestNormal))
            return false;
    }
    for (size_t i = 0; i < 3; ++i) {
        for (size_t j = 0; j < 3; ++j) {
            if (!TestAxis(a, b, a.axes[i].Cross(b.axes[j]), delta, bestDepth, bestNormal))
                return false;
        }
    }
...
bool Collide(const SphereShape& sphere, const OrientedBox& box, ContactManifold& contact) {
    ...
    if (CollideSpheres(sphere.center, sphere.radius, closest, 0.0f, contact))
        return true;
    if ((closest - sphere.center).LengthSq() > 1e-12f)
        return false;
...
bool Collide(const CapsuleShape& capsule, const SphereShape& sphere, ContactManifold& contact) {
    const Vec3 point = ClosestPointOnSegment(capsule.pointA, capsule.pointB, sphere.center);
