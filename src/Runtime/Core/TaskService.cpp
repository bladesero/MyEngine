...
void TaskScope::ReapCompleted() {
    std::lock_guard<std::mutex> lock(m_Mutex);
    m_Tasks.erase(std::remove_if(m_Tasks.begin(), m_Tasks.end(), [](const auto& task) { return task->IsReady(); }),
                  m_Tasks.end());
}
...
TaskService::TaskService(size_t workerCount) {
    if (workerCount == 0) {
        ...
        workerCount = std::min<size_t>(8, std::max<size_t>(1, hardware > 1 ? hardware - 1 : 1));
    }
...
void TaskService::Enqueue(TaskPriority priority, WorkItem work, const std::shared_ptr<TaskStateBase>& state) {
    {
        std::lock_guard<std::mutex> lock(m_Mutex);
        if (m_Stopping)
            throw std::runtime_error("TaskService is shutting down");
