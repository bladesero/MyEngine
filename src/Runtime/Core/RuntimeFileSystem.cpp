...
{
    static std::shared_ptr<IReadOnlyFileSystem> fileSystem =
        std::make_shared<PhysicalFileSystem>();
    return fileSystem;
}
...
bool IReadOnlyFileSystem::ReadText(const std::string& path, std::string& out,
...
bool PhysicalFileSystem::Stat(const std::string& path, FileStat& out,
                              std::string* error) const
{
    std::error_code ec;
    ...
    if (ec) out.normalizedPath = file.lexically_normal().generic_string();
    return true;
}
...
std::vector<std::string> PhysicalFileSystem::ListFiles(const std::string& prefix) const
{
    ...
    if (!fs::is_directory(root, ec) || ec) return files;
...
bool PhysicalFileSystem::ReadAllBytes(const std::string& path,
...
std::unique_ptr<std::istream> PhysicalFileSystem::OpenRead(
...
{
    if (m_Root.empty() || path.empty()) return {};
    fs::path input(path);
    ...
    if (input.has_parent_path() || !IsPackageRootFileName(input.generic_string())) return {};
...
bool PackageRootFileSystem::Stat(const std::string& path, FileStat& out,
                                 std::string* error) const
{
    const fs::path resolved = ResolveRootFile(path);
    if (resolved.empty()) {
    ...
    if (!physical.Stat(resolved.string(), out, error)) return false;
...
std::vector<std::string> PackageRootFileSystem::ListFiles(const std::string& prefix) const
{
    ...
    for (const char* name : {"MyEngine.project.json", "CookManifest.json",
                             "RuntimeDependencies.json", "Content.pak"}) {
        if (!prefix.empty() && std::string(name).rfind(normalizedPrefix, 0) != 0) continue;
        if (Exists(name)) files.emplace_back(name);
    }
    return files;
}

bool PackageRootFileSystem::ReadAllBytes(const std::string& path,
                                         std::vector<uint8_t>& out,
                                         std::string* error) const
{
    const fs::path resolved = ResolveRootFile(path);
...
std::unique_ptr<std::istream> PackageRootFileSystem::OpenRead(
    const std::string& path,
    std::string* error) const
{
    const fs::path resolved = ResolveRootFile(path);
...
{
    std::error_code ec;
    m_ProjectRoot = root.empty()
        ? fs::path{}
        : fs::absolute(std::move(root), ec).lexically_normal();
    if (ec) m_ProjectRoot.clear();
}

void MountedFileSystem::AddMount(std::shared_ptr<IReadOnlyFileSystem> mount)
{
    if (mount) m_Mounts.push_back(std::move(mount));
}
...
bool MountedFileSystem::Exists(const std::string& path) const
{
    ...
    for (const auto& mount : m_Mounts) {
        if (mount->Exists(normalized) || mount->Exists(path)) return true;
    }
    return false;
}
...
uint64_t MountedFileSystem::FileSize(const std::string& path) const
{
    const std::string normalized = Normalize(path);
    for (const auto& mount : m_Mounts) {
        if (mount->Exists(normalized)) return mount->FileSize(normalized);
        if (mount->Exists(path)) return mount->FileSize(path);
    }
    return 0;
}

bool MountedFileSystem::Stat(const std::string& path, FileStat& out,
                             std::string* error) const
{
    const std::string normalized = Normalize(path);
    std::string lastError;
    for (const auto& mount : m_Mounts) {
        if (mount->Stat(normalized, out, &lastError)) return true;
        if (normalized != path && mount->Stat(path, out, &lastError)) return true;
    }
    SetError(error, "file not found: requested=" + path +
...
bool MountedFileSystem::ReadAllBytes(const std::string& path,
                                     std::vector<uint8_t>& out,
                                     std::string* error) const
{
    const std::string normalized = Normalize(path);
    for (const auto& mount : m_Mounts) {
        ...
        if (mount->Exists(path)) return mount->ReadAllBytes(path, out, error);
    }
    SetError(error, "file not found: requested=" + path + " normalized=" + normalized);
    return false;
}

std::unique_ptr<std::istream> MountedFileSystem::OpenRead(
    const std::string& path,
    std::string* error) const
{
    const std::string normalized = Normalize(path);
    for (const auto& mount : m_Mounts) {
        if (mount->Exists(normalized)) return mount->OpenRead(normalized, error);
        if (mount->Exists(path)) return mount->OpenRead(path, error);
    }
    SetError(error, "file not found: requested=" + path + " normalized=" + normalized);
    return {};
}
...
void RuntimeFileSystem::Set(std::shared_ptr<IReadOnlyFileSystem> fileSystem)
{
    ...
    ActiveFileSystem() = fileSystem ? std::move(fileSystem)
                                    : std::make_shared<PhysicalFileSystem>();
}
