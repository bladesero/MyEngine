...
uint32_t R(uint32_t x, uint32_t n) {
    return (x >> n) | (x << (32 - n));
}
} // namespace
void Sha256::Transform(const uint8_t b[64]) {
    ...
    for (int i = 0; i < 16; ++i)
        w[i] =
            (uint32_t(b[i * 4]) << 24) | (uint32_t(b[i * 4 + 1]) << 16) | (uint32_t(b[i * 4 + 2]) << 8) | b[i * 4 + 3];
    for (int i = 16; i < 64; ++i) {
        uint32_t s0 = R(w[i - 15], 7) ^ R(w[i - 15], 18) ^ (w[i - 15] >> 3);
        uint32_t s1 = R(w[i - 2], 17) ^ R(w[i - 2], 19) ^ (w[i - 2] >> 10);
        w[i] = w[i - 16] + s0 + w[i - 7] + s1;
    }
    uint32_t a = m_State[0], b0 = m_State[1], c = m_State[2], d = m_State[3], e = m_State[4], f = m_State[5],
...
void Sha256::Update(const void* data, size_t size) {
    ...
    m_TotalBytes += size;
    while (size) {
        size_t n = (std::min)(size, 64 - m_BufferSize);
        std::copy(p, p + n, m_Buffer.begin() + m_BufferSize);
        m_BufferSize += n;
        p += n;
        size -= n;
        if (m_BufferSize == 64) {
...
Sha256::Digest Sha256::Final() {
    if (!m_Finalized) {
        ...
        while (m_BufferSize < 56)
            m_Buffer[m_BufferSize++] = 0;
        for (int i = 7; i >= 0; --i)
            m_Buffer[m_BufferSize++] = uint8_t(bits >> (i * 8));
        Transform(m_Buffer.data());
        m_Finalized = true;
    }
    Digest out{};
...
bool Sha256::FromHex(const std::string& s, Digest& d) {
    if (s.size() != 64)
        return false;
    auto v = [](char c) -> int {
        if (c >= '0' && c <= '9')
...
std::string Sha256::HashFile(const std::filesystem::path& path, std::string* error) {
    if (error)
        error->clear();
    ...
    while (in) {
        ...
        hash.Update(b.data(), static_cast<size_t>(in.gcount()));
    }
    if (!in.eof()) {
        if (error)
