...
void AddExceeded(std::vector<std::string>& violations,
                 const char* metric, double observed, double limit)
{
...
RuntimePerformanceGate::RuntimePerformanceGate(RuntimePerformanceBudget budget)
...
RuntimePerformanceReport RuntimePerformanceGate::Evaluate() const
{
    ...
    for (auto it = begin; it != m_Samples.end(); ++it) {
        if (!std::isfinite(it->frameMs) || it->frameMs < 0.0 ||
            !std::isfinite(it->gpuMs) || it->gpuMs < 0.0) {
            report.violations.push_back("sample contains a non-finite or negative duration");
            continue;
        }
        frameTimes.push_back(it->frameMs);
        if (it->gpuTimingAvailable) gpuTimes.push_back(it->gpuMs);
    ...
    report.summary.workingSetGrowthBytes = maxWorkingSet >= baselineWorkingSet
        ? maxWorkingSet - baselineWorkingSet
        : 0;
    ...
    if (report.summary.gpuSampleCount > 0 &&
        report.summary.p95GpuMs > m_Budget.maxP95GpuMs)
    ...
    if (report.summary.droppedFixedTicks > m_Budget.maxDroppedFixedTicks)
        AddExceeded(report.violations, "droppedFixedTicks",
                    static_cast<double>(report.summary.droppedFixedTicks),
