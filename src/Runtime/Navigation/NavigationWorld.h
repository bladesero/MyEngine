...
public:
    struct BakeSettings { AABB bounds{{-10,0,-10},{10,2,10}}; float cellSize=0.5f; float agentRadius=0.4f; };
    struct SoundEvent { Vec3 position; float radius=0.0f; ActorHandle source; float remaining=0.0f; };
    ...
    uint32_t GetWidth()const{return m_Width;}uint32_t GetHeight()const{return m_Height;}
    ...
    bool SetAreaWalkable(const AABB& bounds,bool walkable);uint64_t GetRevision()const{return m_Revision;}
private:
    bool ToCell(const Vec3& position,int& x,int& z)const;
    Vec3 CellCenter(int x,int z)const;
    BakeSettings m_Settings;uint32_t m_Width=0,m_Height=0;std::vector<uint8_t> m_Walkable;std::vector<SoundEvent> m_Sounds;uint64_t m_Revision=0;
};
