...
bool RuntimePerformanceProfile::FromJson(nlohmann::json value,
                                         RuntimePerformanceProfile& out,
                                         std::string* error)
{
    if (!value.is_object()) {
    ...
    migrations.Register(0, [](nlohmann::json& json, std::string*) {
        if (!json.contains("budget")) {
            ...
            for (const char* field : {"warmupSamples", "minimumSamples",
                                      "maxP95FrameMs", "maxP99FrameMs", "maxFrameMs",
                                      "maxP95GpuMs", "maxWorkingSetGrowthBytes",
                                      "maxDroppedFixedTicks"}) {
                if (json.contains(field)) budget[field] = json[field];
            }
    ...
    migrations.Register(1, [](nlohmann::json& json, std::string*) {
        if(!json.contains("scenario"))json["scenario"]="warm-gameplay";
        if(!json.contains("resourceBudgetScale"))json["resourceBudgetScale"]=1.0;
        if(!json.contains("sceneReloadCount"))json["sceneReloadCount"]=0;
        return true;
    });
    migrations.Register(2, [](nlohmann::json& json, std::string*) {
        if(!json.contains("maxInitialSceneReadyMs"))json["maxInitialSceneReadyMs"]=10000.0;
        if(!json.contains("maxSceneReloadMs"))json["maxSceneReloadMs"]=5000.0;
        return true;
    });
    if (!migrations.Migrate(value, error)) return false;
    ...
    parsed.budget.maxWorkingSetGrowthBytes = budgetJson.value(
        "maxWorkingSetGrowthBytes", parsed.budget.maxWorkingSetGrowthBytes);
    parsed.budget.maxDroppedFixedTicks = budgetJson.value(
        "maxDroppedFixedTicks", parsed.budget.maxDroppedFixedTicks);
    ...
    if (parsed.name.empty() || parsed.hardwareClass.empty() ||!validScenario||
        ...
        !PositiveFinite(parsed.maxInitialSceneReadyMs)||
        !PositiveFinite(parsed.maxSceneReloadMs)||
        parsed.sceneReloadCount>1000||
        parsed.budget.minimumSamples == 0 || parsed.budget.minimumSamples > 1000000 ||
        parsed.budget.warmupSamples > 1000000 ||
        !PositiveFinite(parsed.budget.maxP95FrameMs) ||
        !PositiveFinite(parsed.budget.maxP99FrameMs) ||
        !PositiveFinite(parsed.budget.maxFrameMs) ||
        !PositiveFinite(parsed.budget.maxP95GpuMs) ||
        parsed.budget.maxP95FrameMs > parsed.budget.maxP99FrameMs ||
...
bool RuntimePerformanceProfile::FromText(const std::string& text,
                                         RuntimePerformanceProfile& out,
                                         std::string* error)
{
    try {
