...
public:
    ...
    static bool WriteAutosave(const SaveGameData& data, size_t slotCount = 3, std::string* writtenSlot = nullptr,
    ...
    static bool Remove(const std::string& slot, std::string* error = nullptr);
    ...
    static nlohmann::json ToJson(const SaveGameData& data);
    static bool FromJson(nlohmann::json value, SaveGameData& data, std::string* error = nullptr);

private:
    static bool Resolve(const std::string& slot, std::string& path, std::string* error);
};
