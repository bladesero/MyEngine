...
public:
    explicit MemoryReadBuffer(std::vector<uint8_t> bytes)
...
std::string ContentArchiveReader::NormalizeEntryPath(const std::string& path)
{
    ...
    if (generic.rfind("Content/", 0) == 0) generic = generic.substr(8);
    if (!SafeRelativePath(fs::path(generic))) return {};
    return generic;
}
...
bool ContentArchiveReader::Open(const fs::path& archivePath, std::string* error)
{
    ...
    input.read(magic.data(), static_cast<std::streamsize>(magic.size()));
    uint32_t fileCount = 0;
    if (!input.good() || magic != kMagic || !ReadValue(input, fileCount) ||
        fileCount > 1000000) {
        SetError(error, "invalid Content archive header");
        return false;
    }
...
bool ContentArchiveReader::Stat(const std::string& path, FileStat& out,
...
std::vector<std::string> ContentArchiveReader::ListFiles(const std::string& prefix) const
{
    ...
    if (!prefix.empty()) {
        const std::string genericPrefix =
            fs::path(prefix).lexically_normal().generic_string();
        if (genericPrefix != "Content" && genericPrefix != "Content/") {
            normalizedPrefix = NormalizeEntryPath(prefix);
            if (normalizedPrefix.empty()) return files;
        }
    }
    for (const auto& [key, entry] : m_Entries) {
        (void)entry;
        if (!normalizedPrefix.empty() && key.rfind(normalizedPrefix, 0) != 0) continue;
...
bool ContentArchiveReader::ReadFile(const std::string& path,
...
bool ContentArchiveReader::ReadFileUncheckedAfterManifestValidated(
...
bool ContentArchiveReader::ReadFileInternal(const std::string& path,
                                            std::vector<uint8_t>& out,
                                            bool verifyHash,
                                            std::string* error) const
{
    ...
    if (verifyHash) {
        Sha256 hash;
        if (!out.empty()) hash.Update(out.data(), out.size());
...
bool ContentArchiveReader::ValidateAgainstManifest(const CookManifest& manifest,
                                                   std::string* error) const
{
    if (error) error->clear();
    ...
    for (const CookedContentEntry& manifestEntry : manifest.files) {
        ...
        const auto found = m_Entries.find(key);
        if (found == m_Entries.end()) {
            SetError(error, "Cook manifest entry missing from Content archive: " +
                manifestEntry.path);
            return false;
        }
        if (found->second.size != manifestEntry.size ||
            found->second.hash != manifestEntry.hash) {
...
PakFileSystem::PakFileSystem(std::shared_ptr<ContentArchiveReader> reader)
...
bool PakFileSystem::Stat(const std::string& path, FileStat& out,
                         std::string* error) const
{
    if (!m_Reader) {
...
bool PakFileSystem::ReadAllBytes(const std::string& path,
                                 std::vector<uint8_t>& out,
                                 std::string* error) const
{
    if (!m_Reader) {
...
std::unique_ptr<std::istream> PakFileSystem::OpenRead(
    const std::string& path,
    std::string* error) const
{
    std::vector<uint8_t> bytes;
    if (!ReadAllBytes(path, bytes, error)) return {};
