...
{
    if (m_CurrentState.empty()) m_CurrentState = m_Controller.GetEntryState();
    ApplyState(0.0f);
}
...
{
    if (!m_Controller.FindState(stateName)) return false;
...
bool AnimatorComponent::ConditionsPass(const AnimatorTransition& transition) const
{
    for (const AnimatorCondition& condition : transition.conditions) {
        ...
        case AnimatorConditionMode::Greater: if (!(value > condition.threshold)) return false; break;
        case AnimatorConditionMode::Less: if (!(value < condition.threshold)) return false; break;
        case AnimatorConditionMode::Equals: if (std::fabs(value - condition.threshold) > 0.0001f) return false; break;
        case AnimatorConditionMode::NotEquals: if (std::fabs(value - condition.threshold) <= 0.0001f) return false; break;
        case AnimatorConditionMode::If: if (!GetBool(condition.parameter)) return false; break;
        case AnimatorConditionMode::IfNot: if (GetBool(condition.parameter)) return false; break;
        case AnimatorConditionMode::Trigger: if (!m_Triggers.count(condition.parameter)) return false; break;
        }
    }
    return true;
}
...
void AnimatorComponent::ApplyState(float transitionSeconds)
{
    ...
    else renderer->SetAnimation(state->clip);
    if (!state->blendTree.empty()) {
        const float parameter = GetFloat(state->blendParameter);
        auto samples = state->blendTree;
        std::sort(samples.begin(), samples.end(), [](const auto& a, const auto& b) { return a.threshold < b.threshold; });
        const auto upper = std::lower_bound(samples.begin(), samples.end(), parameter,
            [](const AnimatorBlendSample& sample, float value) { return sample.threshold < value; });
        if (upper == samples.begin()) renderer->SetAnimation(upper->clip);
        else if (upper == samples.end()) renderer->SetAnimation(samples.back().clip);
        else {
...
{
    if (state.clip.duration <= 0.0f) return;
        ...
        if (crossed) {
            ...
            if (Actor* owner = GetOwner()) {
                ...
                owner->ForEachComponent([&](Component& component) {
                    if (&component != this && component.IsEnabled()) component.OnAnimationEvent(dispatched);
                });
            }
        }
    }
...
{
    const AnimatorState* state = m_Controller.FindState(m_CurrentState);
    if (!state) { Play(m_Controller.GetEntryState()); state = m_Controller.FindState(m_CurrentState); }
    if (!state) return;
    ...
    if (state->clip.looping && state->clip.duration > 0.0f) m_StateTime = std::fmod(m_StateTime, state->clip.duration);
    UpdateEvents(*state, previous, m_StateTime);
    for (const AnimatorTransition& transition : state->transitions) {
        if (transition.hasExitTime && GetNormalizedTime() < transition.exitTime) continue;
        if (!ConditionsPass(transition)) continue;
        for (const AnimatorCondition& condition : transition.conditions)
            if (condition.mode == AnimatorConditionMode::Trigger) m_Triggers.erase(condition.parameter);
...
{
    if (data.contains("controller")) m_Controller.Deserialize(data["controller"]);
