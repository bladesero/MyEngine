...
Quat JsonToQuat(const nlohmann::json& value, const Quat& fallback) {
    if (!value.is_array() || value.size() != 4)
        return fallback;
    return Quat{value[0].get<float>(), value[1].get<float>(), value[2].get<float>(), value[3].get<float>()}
        .Normalized();
}
...
void SkinnedMeshRendererComponent::SetSkeleton(std::vector<Bone> bones, std::vector<SkinWeight> weights) {
...
void SkinnedMeshRendererComponent::SetAnimationTime(float time) {
    ...
    if (m_Clip.duration > 0.0f) {
        m_Time = m_Clip.looping ? std::fmod(m_Time, m_Clip.duration) : std::min(m_Time, m_Clip.duration);
    }
    RebuildPose();
}

BoneKeyframe SkinnedMeshRendererComponent::SampleTrack(const BoneTrack& track, const Bone& bone, float time) const {
...
BoneKeyframe SkinnedMeshRendererComponent::SampleClip(const AnimationClip& clip, const Bone& bone, size_t boneIndex,
                                                      float time) const {
    for (const BoneTrack& track : clip.tracks) {
        if (track.boneIndex == boneIndex)
            return SampleTrack(track, bone, time);
    }
...
void SkinnedMeshRendererComponent::RebuildPose() {
    ...
    for (size_t i = 0; i < m_Bones.size(); ++i) {
        ...
        if (m_BlendWeight > 0.0f && !m_BlendClip.tracks.empty()) {
            const BoneKeyframe blendPose = SampleClip(m_BlendClip, bone, i, m_BlendTime);
            pose.translation = Vec3::Lerp(pose.translation, blendPose.translation, m_BlendWeight);
            pose.rotation = Slerp(pose.rotation, blendPose.rotation, m_BlendWeight);
        ...
        globalPose[i] = bone.parent >= 0 && static_cast<size_t>(bone.parent) < i
                            ? local * globalPose[static_cast<size_t>(bone.parent)]
                            : local;
...
void SkinnedMeshRendererComponent::RebuildGpuMesh() {
    if (!m_SourceMesh || !m_DeformedMesh)
        return;
    ...
    if (m_Weights.size() == source.size()) {
        for (size_t vertexIndex = 0; vertexIndex < vertices.size(); ++vertexIndex) {
            for (size_t influence = 0; influence < 4; ++influence) {
                ...
                vertices[vertexIndex].boneWeights[influence] = m_Weights[vertexIndex].weights[influence];
            }
        }
    }
    m_DeformedMesh->SetGeometry(std::move(vertices), m_SourceMesh->GetIndices(), m_SourceMesh->GetSubMeshes());
}
...
void SkinnedMeshRendererComponent::Serialize(nlohmann::json& data) const {
    data["mesh"] = m_SourceMesh ? AssetManager::Get().MakeProjectRelativePath(m_SourceMesh->GetPath()) : "";
    data["material"] = m_Material ? AssetManager::Get().MakeProjectRelativePath(m_Material->GetPath()) : "";
    if (m_Material) {
        const std::string materialPath = AssetManager::Get().MakeProjectRelativePath(m_Material->GetPath());
    ...
    data["time"] = m_Time;
    data["clip"] = {{"name", m_Clip.name}, {"duration", m_Clip.duration}, {"looping", m_Clip.looping}};
    ...
    if (data.contains("bones")) {
        for (const auto& value : data["bones"]) {
            ...
            if (value.contains("translation"))
                bone.bindTranslation = JsonToVec(value["translation"], Vec3::Zero());
            if (value.contains("rotation"))
                bone.bindRotation = JsonToQuat(value["rotation"], Quat::Identity());
            if (value.contains("scale"))
                bone.bindScale = JsonToVec(value["scale"], Vec3::One());
            if (value.contains("inverseBind"))
                bone.inverseBind = JsonToMat(value["inverseBind"]);
            ...
            if (trackJson.contains("keys")) {
                for (const auto& keyJson : trackJson["keys"]) {
                    ...
                    if (keyJson.contains("translation"))
                        key.translation = JsonToVec(keyJson["translation"], Vec3::Zero());
                    if (keyJson.contains("rotation"))
                        key.rotation = JsonToQuat(keyJson["rotation"], Quat::Identity());
                    if (keyJson.contains("scale"))
                        key.scale = JsonToVec(keyJson["scale"], Vec3::One());
