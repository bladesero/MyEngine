...
bool ParseAudioBus(const std::string& value, AudioBus& bus) {
    for (uint8_t index = 0; index < static_cast<uint8_t>(AudioBus::Count); ++index) {
        ...
        if (value == AudioBusName(candidate)) {
            bus = candidate;
            return true;
        }
    }
...
bool AudioEngine::IsSilent() const {
    return m_Impl->silent;
}
...
AudioEngine::SoundID AudioEngine::Play(const AudioPlayDesc& desc) {
    if (!m_Impl->initialized || !desc.clip || !desc.clip->IsReady())
        return 0;
    ...
    for (const auto& pair : m_Impl->sounds)
        active.push_back({pair.first, pair.second.priority, pair.second.sequence, pair.second.concurrencyGroup});
    const AudioVoiceAdmission admission =
        EvaluateVoiceAdmission(active, m_Impl->maxVoices, desc.concurrencyGroup, desc.maxInstances, desc.priority);
    ...
    const ma_result result =
        ma_sound_init_from_file(&m_Impl->engine, desc.clip->GetPath().c_str(), flags, nullptr, nullptr, sound.get());
    if (result != MA_SUCCESS) {
    ...
    const float effectiveVolume =
        (std::max)(0.0f, desc.volume) * m_Impl->masterVolume * bus.volume * (bus.muted ? 0.0f : 1.0f);
...
void AudioEngine::SetPaused(bool paused) {
    ...
    for (auto& pair : m_Impl->sounds) {
        ...
        if (!m_Impl->buses[static_cast<size_t>(entry.bus)].pauseWithGame)
            continue;
        if (paused) {
            ...
            if (!ma_sound_at_end(entry.sound.get()))
...
void AudioEngine::SetMasterVolume(float volume) {
    ...
    for (auto& pair : m_Impl->sounds) {
        ...
        ma_sound_set_volume(entry.sound.get(),
                            entry.baseVolume * m_Impl->masterVolume * bus.volume * (bus.muted ? 0.0f : 1.0f));
    }
}
...
void AudioEngine::SetBusVolume(AudioBus bus, float volume) {
    ...
    for (auto& pair : m_Impl->sounds) {
        ...
        if (entry.bus != bus)
            continue;
        ma_sound_set_volume(entry.sound.get(),
                            entry.baseVolume * m_Impl->masterVolume * state.volume * (state.muted ? 0.0f : 1.0f));
    }
}
...
void AudioEngine::SetBusPauseWithGame(AudioBus bus, bool enabled) {
    if (bus == AudioBus::Count)
        return;
    auto& state = m_Impl->buses[static_cast<size_t>(bus)];
    if (state.pauseWithGame == enabled)
        return;
...
bool AudioEngine::GetBusPauseWithGame(AudioBus bus) const {
    return bus != AudioBus::Count && m_Impl->buses[static_cast<size_t>(bus)].pauseWithGame;
}
...
void AudioEngine::SetMaxVoices(uint32_t maxVoices) {
    ...
    while (m_Impl->sounds.size() > m_Impl->maxVoices) {
        ...
        for (auto it = m_Impl->sounds.begin(); it != m_Impl->sounds.end(); ++it) {
            if (victim == m_Impl->sounds.end() || it->second.priority < victim->second.priority ||
                (it->second.priority == victim->second.priority && it->second.sequence < victim->second.sequence))
                victim = it;
        }
        ma_sound_stop(victim->second.sound.get());
        ma_sound_uninit(victim->second.sound.get());
...
AudioDiagnostics AudioEngine::GetDiagnostics() const {
    ...
    for (const auto& pair : m_Impl->sounds) {
        ...
        if (entry.pausedByGame)
            ++diagnostics.pausedVoices;
    }
    return diagnostics;
}

AudioVoiceAdmission AudioEngine::EvaluateVoiceAdmission(const std::vector<AudioVoiceCandidate>& active,
    ...
    if (maxInstances > 0 && !concurrencyGroup.empty()) {
        ...
        for (const auto& candidate : active) {
            if (candidate.concurrencyGroup != concurrencyGroup)
                continue;
            ++count;
            prefer(candidate);
        }
    ...
    if (!victim)
        return {};
    if (victim->priority > newPriority)
        return {false, 0};
...
bool AudioEngine::IsPlaying(SoundID id) const {
    const auto it = m_Impl->sounds.find(id);
    return it != m_Impl->sounds.end() && (it->second.pausedByGame || ma_sound_is_playing(it->second.sound.get()));
}
...
void AudioEngine::SetSoundVolume(SoundID id, float volume) {
    ...
    if (it != m_Impl->sounds.end()) {
        ...
        ma_sound_set_volume(it->second.sound.get(),
                            it->second.baseVolume * m_Impl->masterVolume * bus.volume * (bus.muted ? 0.0f : 1.0f));
    }
}
...
void AudioEngine::SetSoundPitch(SoundID id, float pitch) {
    const auto it = m_Impl->sounds.find(id);
    if (it != m_Impl->sounds.end())
        ma_sound_set_pitch(it->second.sound.get(), (std::max)(0.01f, pitch));
}
...
void AudioEngine::FadeSoundVolume(SoundID id, float targetVolume, uint32_t milliseconds) {
    ...
    const auto& bus = m_Impl->buses[static_cast<size_t>(entry.bus)];
    const float effectiveTarget = entry.baseVolume * m_Impl->masterVolume * bus.volume * (bus.muted ? 0.0f : 1.0f);
...
void AudioEngine::SetListenerTransform(const Vec3& position, const Vec3& forward, const Vec3& up) {
    ...
    ma_engine_listener_set_direction(&m_Impl->engine, 0, m_Impl->listenerForward.x, m_Impl->listenerForward.y,
                                     m_Impl->listenerForward.z);
    ma_engine_listener_set_world_up(&m_Impl->engine, 0, m_Impl->listenerUp.x, m_Impl->listenerUp.y,
                                    m_Impl->listenerUp.z);
}

const Vec3& AudioEngine::GetListenerPosition() const {
    return m_Impl->listenerPosition;
}
const Vec3& AudioEngine::GetListenerForward() const {
    return m_Impl->listenerForward;
}
const Vec3& AudioEngine::GetListenerUp() const {
    return m_Impl->listenerUp;
}
