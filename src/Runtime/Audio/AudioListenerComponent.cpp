...
void AudioListenerComponent::OnLateUpdate(float) {
    ...
    if (!owner || !m_Primary || !owner->IsActive())
        return;
    Scene* scene = owner->GetScene();
    if (scene) {
        bool earlierPrimary = false;
        scene->ForEach([&](Actor& actor) {
            if (earlierPrimary || actor.GetID() >= owner->GetID())
                return;
    ...
    AudioEngine::Get().SetListenerTransform(owner->GetWorldPosition(), world.TransformDir(Vec3::Forward()).Normalized(),
                                            world.TransformDir(Vec3::Up()).Normalized());
}

void AudioListenerComponent::Serialize(nlohmann::json& data) const {
    data["primary"] = m_Primary;
}
void AudioListenerComponent::Deserialize(const nlohmann::json& data) {
    m_Primary = data.value("primary", true);
}
