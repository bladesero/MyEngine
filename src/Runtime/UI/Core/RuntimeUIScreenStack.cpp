...
bool RuntimeUIScreenStack::Register(RuntimeUIScreenDescriptor descriptor, std::string* error) {
    ...
    for (const auto& action : descriptor.actions) {
        if (action.stableName.empty() || action.label.empty() || !actions.insert(action.stableName).second) {
...
bool RuntimeUIScreenStack::SetActionLabel(const std::string& screen, const std::string& action, std::string label) {
    if (label.empty())
        return false;
    ...
    for (auto& descriptor : found->second.actions) {
        if (descriptor.stableName != action)
            continue;
...
bool RuntimeUIScreenStack::ApplyProjectOverride(const std::string& screen, std::string title, std::string documentPath,
                                                const std::unordered_map<std::string, std::string>& actionLabels,
                                                std::string* error) {
    const auto found = m_Descriptors.find(screen);
    if (found == m_Descriptors.end()) {
        SetError(error, "unknown standard runtime screen: " + screen);
        return false;
    }
    if (title.empty() || documentPath.empty()) {
        SetError(error, "screen title and document path must not be empty: " + screen);
        return false;
    }
    auto descriptor = found->second;
    descriptor.title = std::move(title);
    descriptor.documentPath = std::move(documentPath);
    for (const auto& [action, label] : actionLabels) {
        ...
        if (item == descriptor.actions.end() || label.empty()) {
            SetError(error, "unknown action or empty label on runtime screen: " + screen + "." + action);
            return false;
        }
        item->label = label;
    }
...
RuntimeUIScreenStack RuntimeUIScreenStack::CreateStandard() {
    ...
    stack.Register({"mainMenu", "Main Menu", true, false, {{"play", "Play"}, {"settings", "Settings"}}});
    stack.Register(
        {"pause", "Paused", true, true, {{"resume", "Resume"}, {"settings", "Settings"}, {"mainMenu", "Main Menu"}}});
    stack.Register({"settings",
                    ...
                    {{"masterDown", "Master -"},
                     ...
                     {"voiceDown", "Voice -"},
                     {"voiceUp", "Voice +"},
                     {"uiScaleDown", "UI Scale -"},
                     {"uiScaleUp", "UI Scale +"},
                     {"subtitles", "Subtitles: On"},
                     {"highContrast", "High Contrast: Off"},
                     {"back", "Back"}}});
    stack.Register({"gameOver", "Game Over", true, false, {{"retry", "Retry"}, {"mainMenu", "Main Menu"}}});
    return stack;
}

bool RuntimeUIScreenStack::Push(const std::string& stableName) {
    if (!m_Descriptors.count(stableName))
        return false;
    if (!m_Stack.empty() && m_Stack.back().screen == stableName)
        return true;
...
void RuntimeUIScreenStack::Clear() {
    m_Stack.clear();
}
...
bool RuntimeUIScreenStack::SetFocusedIndex(size_t index) {
    const auto* descriptor = TopDescriptor();
    if (!descriptor || index >= descriptor->actions.size())
        return false;
...
RuntimeUIStackEvent RuntimeUIScreenStack::MoveFocus(int delta) {
    const auto* descriptor = TopDescriptor();
    if (!descriptor || descriptor->actions.empty() || delta == 0)
        return {};
...
RuntimeUIStackEvent RuntimeUIScreenStack::ActivateFocused() const {
    const auto* descriptor = TopDescriptor();
    if (!descriptor || descriptor->actions.empty())
        return {};
    const size_t index = (std::min)(m_Stack.back().focusedIndex, descriptor->actions.size() - 1);
    return {RuntimeUIStackEventType::Activated, descriptor->stableName, descriptor->actions[index].stableName};
}

RuntimeUIStackEvent RuntimeUIScreenStack::Activate(size_t index) {
    if (!SetFocusedIndex(index))
        return {};
...
RuntimeUIStackEvent RuntimeUIScreenStack::Dismiss() {
    const auto* descriptor = TopDescriptor();
    if (!descriptor || !descriptor->dismissible)
        return {};
