...
{
    return "rgba(" + std::to_string(ColorByte(color.r)) + "," +
           std::to_string(ColorByte(color.g)) + "," +
           std::to_string(ColorByte(color.b)) + "," +
           std::to_string(ColorByte(color.a)) + ")";
}
...
std::string RectStyle(const UIRectTransformComponent* rect)
{
    ...
    if (r.anchorMin.x == 0.0f && r.anchorMax.x == 1.0f) {
        ...
        if (width > 0.0f) style += "width:" + Dp(width) + ";";
    }
    if (r.anchorMin.y == 0.0f && r.anchorMax.y == 1.0f) {
        ...
        if (height > 0.0f) style += "height:" + Dp(height) + ";";
    }
    return style;
}
...
void AppendLayoutStyle(const Actor& actor, std::string& style, std::string& className)
{
    ...
    if (auto* layout = actor.GetComponent<UIVerticalLayoutComponent>()) {
        ...
        if (!layout->GetClassName().empty()) className += " " + layout->GetClassName();
        ...
        for (int i = 0; i < grid->columns; ++i) style += " 1fr";
        style += ";gap:" + Dp(grid->gap) + ";padding:" + Dp(grid->padding) + ";";
        if (!grid->GetClassName().empty()) className += " " + grid->GetClassName();
    }
}

const UIElementComponent* FindElementComponent(const Actor& actor)
{
    if (auto* component = actor.GetComponent<UITextComponent>()) return component;
    if (auto* component = actor.GetComponent<UIImageComponent>()) return component;
    if (auto* component = actor.GetComponent<UIButtonComponent>()) return component;
    if (auto* component = actor.GetComponent<UISliderComponent>()) return component;
    if (auto* component = actor.GetComponent<UIProgressBarComponent>()) return component;
    if (auto* component = actor.GetComponent<UIScrollViewComponent>()) return component;
    if (auto* component = actor.GetComponent<UIVerticalLayoutComponent>()) return component;
    if (auto* component = actor.GetComponent<UIHorizontalLayoutComponent>()) return component;
    if (auto* component = actor.GetComponent<UIGridLayoutComponent>()) return component;
    return nullptr;
}
...
std::string ElementIDFor(const Actor& actor, std::unordered_set<std::string>& usedIDs)
{
    ...
    if (usedIDs.insert(id).second) return id;
    Logger::Warn("[UI] Duplicate UI element id '", id, "' on actor ", actor.GetName(),
                 "; using ", fallback);
    usedIDs.insert(fallback);
    return fallback;
}

void AppendActorElement(const Actor& actor, std::ostringstream& out,
                        std::unordered_set<std::string>& usedIDs)
{
    if (!actor.IsActiveSelf()) return;
    ...
    if (auto* text = actor.GetComponent<UITextComponent>()) {
        ...
        if (button->disabled) extraAttrs += " disabled=\"disabled\"";
        ...
        extraAttrs += " data-ui-widget=\"slider\" data-min=\"" + Float(slider->min) +
            "\" data-max=\"" + Float(slider->max) + "\" data-step=\"" + Float(slider->step) +
            "\" data-value=\"" + Float(slider->value) + "\"";
        style += "display:block;width:200dp;height:24dp;";
        EnsurePositioned(style);
        content = "<div class=\"ui-slider-track\" style=\"position:absolute;left:0dp;right:0dp;top:9dp;height:6dp;"
                  "background-color:#1f2937;border-radius:3dp;\"></div>"
                  "<div class=\"ui-slider-fill\" style=\"position:absolute;left:0dp;top:9dp;width:" + Percent(valueRatio) +
                  ";height:6dp;background-color:#3b82f6;border-radius:3dp;\"></div>"
                  "<div class=\"ui-slider-handle\" style=\"position:absolute;left:" + Percent(valueRatio) +
        ...
        extraAttrs += " data-ui-widget=\"progress\" data-value=\"" + Float(progress->value) +
            "\" data-max=\"" + Float(progress->max) + "\"";
        style += "display:block;width:200dp;height:16dp;background-color:#202938;"
                 "border-radius:5dp;overflow:hidden;";
        EnsurePositioned(style);
        content = "<div class=\"ui-progress-fill\" style=\"position:absolute;left:0dp;top:0dp;width:" +
                  Percent(valueRatio) +
    ...
    if (!className.empty()) out << " class=\"" << EscapeAttr(className) << "\"";
    if (!style.empty()) out << " style=\"" << EscapeAttr(style) << "\"";
    out << extraAttrs;
    ...
    out << ">";
    out << content;
    if (!actor.GetChildren().empty()) out << "\n";
    for (const Actor* child : actor.GetChildren()) {
        if (child) AppendActorElement(*child, out, usedIDs);
    }
    out << "</" << tag << ">\n";
}
...
void HashActor(const Actor& actor, std::size_t& seed)
{
    ...
    for (const Actor* child : actor.GetChildren()) {
        if (child) HashActor(*child, seed);
    }
}

} // namespace

bool UIActorTreeBuilder::BuildDocument(const Actor& canvasActor, const UICanvasComponent& canvas,
                                       std::string& outRml, std::string* error)
{
    (void)error;
    std::ostringstream out;
    out << "<rml>\n<head>\n<title>" << EscapeText(canvasActor.GetName()) << "</title>\n";
    for (const std::string& style : canvas.GetGeneratedStylePaths()) {
        if (!style.empty()) out << "<link type=\"text/rcss\" href=\"" << EscapeAttr(style) << "\" />\n";
    }
