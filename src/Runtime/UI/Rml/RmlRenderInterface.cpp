...
RmlRenderInterface::RmlRenderInterface(IRHIDevice* device) : m_Device(device) {
}
...
Rml::CompiledGeometryHandle RmlRenderInterface::NextGeometryHandle() {
    while (m_NextGeometry == 0 || m_Geometry.count(m_NextGeometry) != 0)
        ++m_NextGeometry;
    return m_NextGeometry++;
}

Rml::TextureHandle RmlRenderInterface::NextTextureHandle() {
    while (m_NextTexture == 0 || m_Textures.count(m_NextTexture) != 0)
        ++m_NextTexture;
...
Rml::CompiledGeometryHandle RmlRenderInterface::CompileGeometry(Rml::Span<const Rml::Vertex> vertices,
    ...
    for (int index : indices)
        uiIndices.push_back(static_cast<uint32_t>(std::max(0, index)));

    Geometry geometry;
    geometry.vertexBuffer = m_Device->CreateVertexBuffer(
        uiVertices.data(), static_cast<uint32_t>(uiVertices.size() * sizeof(UIVertex)), sizeof(UIVertex));
    geometry.indexBuffer =
        m_Device->CreateIndexBuffer(uiIndices.data(), static_cast<uint32_t>(uiIndices.size() * sizeof(uint32_t)));
...
void RmlRenderInterface::RenderGeometry(Rml::CompiledGeometryHandle geometryHandle, Rml::Vector2f translation,
                                        Rml::TextureHandle textureHandle) {
    if (!m_DrawList)
        return;
    const auto geometryIt = m_Geometry.find(geometryHandle);
    if (geometryIt == m_Geometry.end())
        return;
...
std::shared_ptr<GpuTexture> RmlRenderInterface::CreateTextureFromRgba(const uint8_t* rgba, int width, int height) {
...
Rml::TextureHandle RmlRenderInterface::LoadTexture(Rml::Vector2i& textureDimensions, const Rml::String& source) {
    ...
    auto gpuTexture = CreateTextureFromRgba(texture->GetPixelData().data(), texture->GetWidth(), texture->GetHeight());
    if (!gpuTexture)
        return 0;
...
Rml::TextureHandle RmlRenderInterface::GenerateTexture(Rml::Span<const Rml::byte> source,
    ...
    if (source.empty() || width <= 0 || height <= 0)
        return 0;
    auto gpuTexture = CreateTextureFromRgba(reinterpret_cast<const uint8_t*>(source.data()), width, height);
    if (!gpuTexture)
        return 0;
    const auto handle = NextTextureHandle();
