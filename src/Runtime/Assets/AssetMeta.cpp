...
AssetMeta AssetMeta::LoadOrCreate(const std::string& sourcePath) {
    if (auto existing = Load(sourcePath))
        return *existing;
...
std::optional<AssetMeta> AssetMeta::Load(const std::string& sourcePath, std::string* error) {
    const std::filesystem::path normalized = std::filesystem::absolute(sourcePath).lexically_normal();
    ...
    if (error)
        *error = "metadata is missing or invalid: " + metaPath;
    return std::nullopt;
}
...
bool AssetMeta::Save(const AssetMeta& meta) {
    if (meta.uuid.empty() || meta.sourcePath.empty())
        return false;
    try {
        nlohmann::json json;
        json["uuid"] = meta.uuid;
