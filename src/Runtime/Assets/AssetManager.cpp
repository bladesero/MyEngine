...
size_t EstimateAssetCpuBytes(const Asset& a) {
    ...
    case AssetType::Mesh: {
        ...
        size_t bytes = m.GetVertices().size() * sizeof(MeshVertex) + m.GetIndices().size() * sizeof(uint32_t) + 512;
        for (const MeshLod& lod : m.GetLods()) {
            bytes += lod.indices.size() * sizeof(uint32_t);
        }
        bytes +=
            m.GetColliderData().vertices.size() * sizeof(Vec3) + m.GetColliderData().indices.size() * sizeof(uint32_t);
        return bytes;
    }
    ...
    case AssetType::Shader: {
        ...
        for (size_t backend = 0; backend < kShaderBackendCount; ++backend)
            for (size_t stage = 0; stage < kShaderStageCount; ++stage)
                bytes +=
                    shader.GetBytecode(static_cast<ShaderBackend>(backend), static_cast<ShaderStage>(stage)).size();
        return bytes;
    }
...
void AssetManager::RegisterPersistentIdentity(const std::string& path, const std::string& uuid) {
    if (uuid.empty())
        return;
    std::lock_guard<std::recursive_mutex> lock(m_Mutex);
...
std::string AssetManager::ResolvePath(const std::string& path) const {
    ...
    const std::string sourcePath = fragmentPosition == std::string::npos ? path : path.substr(0, fragmentPosition);
    const std::string fragment = fragmentPosition == std::string::npos ? std::string{} : path.substr(fragmentPosition);
    const std::filesystem::path input(sourcePath);
    std::filesystem::path resolved;
    const std::string generic = input.generic_string();
    std::error_code error;
    if (generic.rfind("Content/Engine/", 0) == 0 && RuntimeFileSystem::Get().Exists(generic)) {
        return generic + fragment;
    }
    if (!m_EngineContentRoot.empty() && generic.rfind("Content/Engine/", 0) == 0) {
        resolved = std::filesystem::absolute(m_EngineContentRoot / std::filesystem::path(generic.substr(15)), error)
                       .lexically_normal();
...
std::string AssetManager::MakeProjectRelativePath(const std::string& path) const {
    ...
    const std::string sourcePath =
        fragmentPosition == std::string::npos ? resolvedPath : resolvedPath.substr(0, fragmentPosition);
    const std::string fragment =
        fragmentPosition == std::string::npos ? std::string{} : resolvedPath.substr(fragmentPosition);
    if (!fragment.empty()) {
    ...
    if (!m_EngineContentRoot.empty()) {
        const auto engineRelative = std::filesystem::relative(absolutePath, m_EngineContentRoot, error);
        if (!error && !engineRelative.empty() && !engineRelative.is_absolute() && *engineRelative.begin() != "..") {
    ...
    const std::filesystem::path contentRelative = std::filesystem::relative(absolutePath, contentRoot, error);
    if (error || contentRelative.empty() || contentRelative.is_absolute() ||
        (contentRelative.begin() != contentRelative.end() && *contentRelative.begin() == "..")) {
...
MaterialHandle AssetManager::ResolveMaterialReference(const std::string& path, const std::string& meshPath) {
    if (path.empty())
        return {};
...
void AssetManager::ApplyPersistentIdentity(Asset& asset) {
    ...
    const std::string sourcePath = fragment == std::string::npos ? path : path.substr(0, fragment);
    const auto registered = m_RegisteredIdentities.find(NormalizePath(sourcePath));
    if (registered != m_RegisteredIdentities.end()) {
        const std::string uuid =
            fragment == std::string::npos ? registered->second : registered->second + path.substr(fragment);
    ...
    if (!meta)
        return;
    const std::string uuid = fragment == std::string::npos ? meta->uuid : meta->uuid + path.substr(fragment);
    asset.SetPersistentIdentity(MakeAssetID(uuid), uuid);
}

std::string AssetManager::ResolveImportedArtifactPath(const std::string& normalizedPath, std::string* outUuid) const {
    if (m_ProjectRoot.empty())
        return {};
    const std::string extension = GetExtension(normalizedPath);
    if (extension != "gltf" && extension != "glb" && extension != "shader")
        return {};
    const std::string expectedType = extension == "shader" ? "shader" : "model";
    const std::string expectedArtifactExtension = extension == "shader" ? "shader" : "modelbin";
    ...
    if (!record || record->state != AssetImportState::Ready || record->artifactPath.empty() ||
        record->type != expectedType) {
        return {};
    }

    std::error_code ec;
    const std::filesystem::path artifact = std::filesystem::absolute(record->artifactPath, ec).lexically_normal();
    if (ec || !std::filesystem::is_regular_file(artifact, ec) || ec) {
        Logger::Warn("[AssetCache] imported artifact stale type=", expectedType, " source=", normalizedPath);
        return {};
    }
    if (GetExtension(artifact.string()) != expectedArtifactExtension)
        return {};
    if (outUuid)
        *outUuid = record->uuid;
...
std::shared_ptr<Asset> AssetManager::LoadAsset(const std::string& path) {
    ...
    if (pathIt != m_PathToID.end()) {
        ...
        if (cached != m_Cache.end()) {
            m_LastUsed[pathIt->second] = ++m_UseClock;
    ...
    if (fragment != std::string::npos) {
        ...
        const std::string artifactPath = ResolveImportedArtifactPath(sourcePath, &artifactUuid);
        if (!artifactPath.empty()) {
            const std::string artifactSubAssetPath = artifactPath + normalizedPath.substr(fragment);
    ...
    std::string artifactUuid;
    const std::string artifactPath = ResolveImportedArtifactPath(normalizedPath, &artifactUuid);
    if (!artifactPath.empty()) {
        ...
        if (artifactLoader != m_Loaders.end()) {
            std::shared_ptr<Asset> artifact = InvokeLoader(artifactPath, artifactLoader->second);
            if (artifact) {
                ...
                Logger::Info("[AssetCache] imported artifact cache hit type=", AssetTypeToString(artifact->GetType()),
    ...
    Logger::Info("[AssetManager] Loaded [", AssetTypeToString(asset->GetType()), "] '", asset->GetName(),
                 "' uuid=", asset->GetUuid());
    PublishAssetChanged({AssetChangeType::Imported, normalizedPath, id, {}});
    return asset;
}
...
TaskHandle<std::shared_ptr<Asset>> AssetManager::LoadAsync(const std::string& path) {
    return TaskService::Get().Submit(m_TaskScope, {"asset.load", TaskPriority::Normal},
                                     [this, path](CancellationToken token) {
...
bool AssetManager::Reload(const std::string& path) {
    ...
    if (cachedIt == m_Cache.end() || !cachedIt->second)
        return false;
    ...
    if (!replacement || replacement->GetType() != cachedIt->second->GetType()) {
        ...
        PublishAssetChanged({AssetChangeType::Failed, normalizedPath, cachedIt->second->GetID(), "reload failed"});
        return false;
    }
    ...
    Logger::Info("[AssetManager] Reloaded '", existing.GetName(), "' version=", existing.GetVersion());
    PublishAssetChanged({AssetChangeType::Reloaded, normalizedPath, existing.GetID(), {}});
    return true;
}

AssetManager::ListenerID AssetManager::SubscribeAssetChanged(AssetChangedCallback callback) {
    std::lock_guard<std::recursive_mutex> lock(m_Mutex);
...
void AssetManager::PublishAssetChanged(AssetChangedEvent event) {
    ...
    for (const auto& [id, callback] : m_AssetChangedListeners)
...
size_t AssetManager::PollHotReload() {
    ...
    for (const auto& [id, previousWriteTime] : m_SourceWriteTimes) {
        ...
        if (assetIt == m_Cache.end() || !assetIt->second)
            continue;
...
void AssetManager::RefreshDependencies(Asset& asset) {
    ...
    if (asset.GetType() == AssetType::Material) {
        ...
        if (material.GetShaderAsset())
            dependencies.push_back(material.GetShaderAsset()->GetID());
    } else if (asset.GetType() == AssetType::Model) {
        const auto& model = static_cast<const ModelAsset&>(asset);
        if (model.GetMesh())
            dependencies.push_back(model.GetMesh()->GetID());
        for (const MaterialHandle& material : model.GetMaterials()) {
            if (material)
                dependencies.push_back(material->GetID());
        }
    }
...
TextureHandle AssetManager::GetWhiteTexture() {
    ...
    if (IsLoaded(path))
        return GetByPath<TextureAsset>(path);
...
MeshHandle AssetManager::GetTriangleMesh() {
    ...
    if (IsLoaded(path))
        return GetByPath<MeshAsset>(path);
...
MaterialHandle AssetManager::GetDefaultMaterial() {
    ...
    if (IsLoaded(path))
        return GetByPath<MaterialAsset>(path);
...
AssetGarbageCollectionReport AssetManager::CollectGarbageDetailed(const AssetCacheBudget& budget) {
    ...
    if (!report.budgetExceeded) {
        report.targetBytes = high;
        report.bytesAfter = m_AssetCpuTotalBytes;
        return report;
    }
    ++m_BudgetCollections;
    const size_t target =
        static_cast<size_t>(static_cast<double>(high) * std::clamp(budget.lowWatermarkRatio, 0.0f, 1.0f));
    ...
    for (const auto& entry : m_Cache) {
        ...
        if (entry.second.use_count() != 1) {
            ++report.blockedReferencedAssets;
            report.blockedBytes += bytes;
            continue;
        }
        bool pinned = false, builtin = false;
        for (const auto& path : m_PathToID)
            if (path.second == entry.first) {
                pinned |= m_PinCounts.count(path.first) != 0;
                builtin |= path.first.rfind("__builtin__/", 0) == 0;
            }
        if (pinned) {
            ++report.blockedPinnedAssets;
            report.blockedBytes += bytes;
            continue;
        }
        if (builtin) {
            ++report.blockedBuiltinAssets;
            report.blockedBytes += bytes;
            continue;
        }
    ...
    for (const auto& candidate : candidates) {
        if (m_AssetCpuTotalBytes <= target || report.evictions.size() >= budget.maxEvictionsPerCollect)
            break;
        const auto found = m_Cache.find(candidate.second);
        if (found == m_Cache.end())
            continue;
        AssetEvictionRecord record;
        record.id = candidate.second;
        record.path = found->second->GetPath();
        record.estimatedCpuBytes = EstimateAssetCpuBytes(*found->second);
        Unload(candidate.second);
        report.evictions.push_back(std::move(record));
    }
    report.bytesAfter = m_AssetCpuTotalBytes;
    report.targetReached = m_AssetCpuTotalBytes <= target;
    m_EvictedAssets += report.evictions.size();
    for (const auto& record : report.evictions)
        m_EvictedBytes += record.estimatedCpuBytes;
    return report;
}
...
AssetCacheStats AssetManager::GetCacheStats() const {
    ...
    stats.cachedAssets = m_Cache.size();
    stats.estimatedCpuBytes = m_AssetCpuTotalBytes;
    stats.pinnedAssets = m_PinCounts.size();
    stats.loadingAssets = m_AsyncRequests.size();
    stats.collections = m_GarbageCollections;
    stats.budgetCollections = m_BudgetCollections;
    stats.evictedAssets = m_EvictedAssets;
    stats.evictedBytes = m_EvictedBytes;
    return stats;
}
...
void AssetManager::Clear() {
    ...
    m_GarbageCollections = 0;
    m_BudgetCollections = 0;
    m_EvictedAssets = 0;
    m_EvictedBytes = 0;
}
