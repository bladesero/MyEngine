...
uint64_t HashText(const std::string& text, uint64_t hash) { return HashBytes(text.data(), text.size(), hash); }

bool SafeRelative(const std::string& value) {
    const std::filesystem::path path(value);
    if (value.empty() || path.is_absolute() || path.has_root_name()) return false;
...
std::filesystem::path ShaderAsset::ResolveSource(ShaderStage stage) const {
    return (std::filesystem::path(GetPath()).parent_path() /
            GetStage(stage).source).lexically_normal();
}
...
void ShaderAsset::SetDescription(uint32_t mask, std::array<ShaderStageSource, 3> sources,
    ...
    m_Cooked = false; m_StageMask = mask; m_Sources = std::move(sources);
...
bool ShaderAsset::ReloadFrom(const Asset& source) {
    ...
    m_Cooked = shader->m_Cooked; m_StageMask = shader->m_StageMask; m_SourceHash = shader->m_SourceHash;
    m_Sources = shader->m_Sources; m_Defines = shader->m_Defines; m_Bytecode = shader->m_Bytecode;
    SetState(AssetState::Ready); return true;
}
...
std::shared_ptr<ShaderAsset> LoadShaderAssetFromFile(const std::string& path) {
    try {
        ...
        for (auto it = stages->begin(); it != stages->end(); ++it)
            if (it.key() != "vertex" && it.key() != "pixel" && it.key() != "compute") return {};
        std::array<ShaderStageSource, 3> sources{}; uint32_t mask = 0;
        const std::pair<const char*, ShaderStage> names[] = {{"vertex",ShaderStage::Vertex},{"pixel",ShaderStage::Pixel},{"compute",ShaderStage::Compute}};
        uint64_t hash = 14695981039346656037ULL;
        for (const auto& [name, stage] : names) {
            const auto it = stages->find(name); if (it == stages->end()) continue;
            if (!it->is_object()) return {};
            auto& dst = sources[static_cast<size_t>(stage)];
            dst.source = it->value("source", std::string()); dst.entry = it->value("entry", std::string());
            if (!SafeRelative(dst.source) || dst.entry.empty()) return {};
            mask |= 1u << static_cast<uint32_t>(stage);
            hash = HashText(name, hash); hash = HashText(dst.source, hash); hash = HashText(dst.entry, hash);
        }
        if (mask != ShaderAsset::kComputeMask && mask != (ShaderAsset::kVertexMask | ShaderAsset::kPixelMask)) return {};
        std::vector<std::string> defines;
        if (data.contains("defines")) {
            if (!data["defines"].is_array()) return {};
            for (const auto& item : data["defines"]) { if (!item.is_string()) return {}; defines.push_back(item.get<std::string>()); hash = HashText(defines.back(), hash); }
        }
        auto asset = std::make_shared<ShaderAsset>(path);
        asset->SetDescription(mask, std::move(sources), std::move(defines), hash); asset->MarkReady(); return asset;
    } catch (const std::exception& e) { Logger::Error("[ShaderAsset] ", path, ": ", e.what()); return {}; }
}
