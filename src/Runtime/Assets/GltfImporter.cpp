...
struct GltfImportContext {
    ...
    ptrdiff_t ImageIndex(const cgltf_image* image) const {
        if (!image || !data.images || image < data.images || image >= data.images + data.images_count) {
            return -1;
        }
    ...
    uintptr_t TextureKey(const cgltf_image* image, bool srgb) const {
        return (static_cast<uintptr_t>(ImageIndex(image) + 1) << 1) | static_cast<uintptr_t>(srgb ? 1 : 0);
    }
};
...
void GenerateTangents(std::vector<MeshVertex>& vertices, const std::vector<uint32_t>& indices) {
    ...
    for (size_t i = 0; i < vertices.size(); ++i) {
        Vec3 tangent = accumulated[i] - vertices[i].normal * accumulated[i].Dot(vertices[i].normal);
        vertices[i].tangent = tangent.LengthSq() > 1e-8f ? tangent.Normalized() : Vec3::Right();
    }
}
...
bool GetImageBytes(const cgltf_image& image, const std::filesystem::path& sourcePath, std::vector<uint8_t>& bytes) {
...
TextureHandle ImportTexture(GltfImportContext& context, const cgltf_texture_view& view, bool srgb) {
    ...
    if (encodedIt == context.encodedImages.end()) {
        ...
        if (!GetImageBytes(*image, context.sourcePath, encoded))
            return {};
    ...
    if (decodedIt == context.decodedImages.end()) {
        ...
        stbi_uc* pixels = stbi_load_from_memory(encodedIt->second.data(), static_cast<int>(encodedIt->second.size()),
        ...
        decoded.rgba.assign(pixels, pixels + static_cast<size_t>(width) * static_cast<size_t>(height) * 4);
    ...
    const std::string suffix =
        image->name ? image->name : (image->uri ? std::filesystem::path(image->uri).stem().string() : "embedded");
    auto texture = std::make_shared<TextureAsset>(context.sourcePath.string() + "#texture-" +
...
std::vector<MaterialHandle> ImportMaterials(GltfImportContext& context) {
    std::vector<MaterialHandle> materials;
    const cgltf_data& data = context.data;
    for (cgltf_size i = 0; i < data.materials_count; ++i) {
        const cgltf_material& source = data.materials[i];
        const std::string materialName = source.name ? source.name : "Material" + std::to_string(i);
        ...
        if (source.has_pbr_metallic_roughness) {
            ...
            material->SetParam("BaseColor",
                               MaterialParam::FromVec4(pbr.base_color_factor[0], pbr.base_color_factor[1],
        ...
        material->SetParam("Emissive", MaterialParam::FromVec3(source.emissive_factor[0], source.emissive_factor[1],
...
void ImportSkinAndAnimations(const cgltf_data& data, const std::vector<SkinWeight>& weights, ModelAsset& model) {
    ...
    for (cgltf_size i = 0; i < skin.joints_count; ++i) {
        ...
        if (node->has_rotation) {
            bone.bindRotation =
                Quat{node->rotation[0], node->rotation[1], node->rotation[2], node->rotation[3]}.Normalized();
        }
        if (node->has_scale) {
    ...
    for (cgltf_size animationIndex = 0; animationIndex < data.animations_count; ++animationIndex) {
        ...
        AnimationClip clip;
        clip.name = source.name ? source.name : "Animation" + std::to_string(animationIndex);
        ...
        for (auto& entry : keys) {
            ...
            for (auto& key : entry.second)
                track.keys.push_back(std::move(key.second));
...
std::shared_ptr<ModelAsset> LoadModelAssetFromGltf(const std::string& path) {
    ...
    for (cgltf_size meshIndex = 0; meshIndex < data->meshes_count; ++meshIndex) {
        ...
        for (cgltf_size primitiveIndex = 0; primitiveIndex < mesh.primitives_count; ++primitiveIndex) {
            ...
            for (cgltf_size i = 0; i < positions->count; ++i) {
                ...
                if (joints && weights) {
                    ...
                    for (size_t influence = 0; influence < 4; ++influence) {
                        ...
                        skinWeights[vertexBase + i].weights[influence] = weightValues[influence];
                        vertex.boneIndices[influence] = static_cast<float>(jointValues[influence]);
            ...
            const std::string meshName = mesh.name ? mesh.name : "Mesh" + std::to_string(meshIndex);
    ...
    Logger::Info("[glTF] Imported '", model->GetName(), "' vertices=", model->GetMesh()->VertexCount(),
                 " materials=", model->MaterialCount(), " bones=", model->GetBones().size(),
...
public:
    ...
    std::string GetArtifactExtension(const std::filesystem::path&) const override { return ".modelbin"; }

    ImportResult Import(const ImportRequest& request) const override {
        ImportResult result;
        std::shared_ptr<ModelAsset> model = LoadModelAssetFromGltf(request.sourcePath.string());
        if (!model) {
