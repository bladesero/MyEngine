...
{
    uint32_t size = 0;
    if (!ReadPod(in, size) || size > kMaxStringBytes) return false;
    value.assign(size, '\0');
...
{
    static_assert(std::is_trivially_copyable_v<T>);
    if (values.size() > kMaxArrayItems) return false;
...
bool ReadVector(std::istream& in, std::vector<T>& values)
{
    static_assert(std::is_trivially_copyable_v<T>);
    uint32_t size = 0;
    if (!ReadPod(in, size) || size > kMaxArrayItems) return false;
    values.resize(size);
    if (values.empty()) return true;
...
{
    return WriteVector(out, texture.GetPixelData()) &&
           WriteMipChain(out, texture.GetMips());
}

bool WriteTexture(std::ostream& out, const TextureAsset& texture,
                  ...
                  uint32_t& texturePayloadIndex)
{
    ...
    if (SaveTexturePayloadToFile(texture, payloadPath.string())) {
        ...
        std::filesystem::path relative =
            std::filesystem::relative(payloadPath, modelCacheDirectory, ec);
        if (ec || relative.empty()) relative = payloadPath.filename();
        return WritePod(out, storage) &&
               WriteString(out, relative.generic_string());
    }
...
bool ReadExternalTexturePayload(std::istream& in,
                                const std::filesystem::path& modelCacheDirectory,
                                const TextureDesc& desc,
                                std::shared_ptr<TextureAsset>& texture,
                                ModelCacheLoadStats* stats)
{
    ...
    if (!ReadString(in, relativePayloadPath) || relativePayloadPath.empty()) return false;
    const std::filesystem::path payloadPath =
...
bool ReadTexture(std::istream& in, const std::string& fallbackPath,
                 const std::filesystem::path& modelCacheDirectory,
                 std::shared_ptr<TextureAsset>& texture,
                 ModelCacheLoadStats* stats,
                 uint32_t cacheVersion)
{
    ...
    std::string name;
    if (!ReadString(in, originalPath) || !ReadString(in, name)) return false;
    TextureDesc desc;
    if (!ReadTextureDesc(in, desc)) return false;

    texture = std::make_shared<TextureAsset>(
        originalPath.empty() ? fallbackPath : originalPath);
    texture->SetName(name);

    if (cacheVersion >= 3) {
        TextureCacheStorage storage = TextureCacheStorage::Embedded;
        if (!ReadPod(in, storage)) return false;
        switch (storage) {
            ...
            case TextureCacheStorage::ExternalPayload:
                return ReadExternalTexturePayload(
                    in, modelCacheDirectory, desc, texture, stats);
            default:
                return false;
        }
    }
...
bool WriteMaterial(std::ostream& out, const MaterialAsset& material,
                   const std::filesystem::path& modelCacheDirectory,
                   const std::filesystem::path& payloadDirectory,
                   uint32_t& texturePayloadIndex)
{
    if (!WriteString(out, material.GetPath()) ||
        !WriteString(out, material.GetName()) ||
        !WritePod(out, material.GetBlendMode()) ||
        !WritePod(out, material.IsTwoSided()) ||
        !WritePod(out, material.IsWireframe()) ||
        !WritePod(out, material.GetAlphaThreshold())) {
        return false;
    }

    const auto& params = material.GetParams();
    if (params.size() > kMaxArrayItems ||
        !WritePod(out, static_cast<uint32_t>(params.size()))) {
        return false;
    }
    for (const auto& [name, param] : params) {
        if (!WriteString(out, name) ||
            !WritePod(out, param.type) ||
            !WritePod(out, param.data)) {
            return false;
        }
    }

    const auto& textures = material.GetTextures();
    if (textures.size() > kMaxArrayItems ||
        !WritePod(out, static_cast<uint32_t>(textures.size()))) {
        return false;
    }
    for (const auto& [slot, texture] : textures) {
        if (!WriteString(out, slot)) return false;
        const bool hasTexture = texture.IsValid();
        if (!WritePod(out, hasTexture)) return false;
        if (hasTexture && !WriteTexture(out, *texture.Get(),
                                        modelCacheDirectory,
                                        payloadDirectory,
                                        texturePayloadIndex)) {
            return false;
...
bool ReadMaterial(std::istream& in, const std::string& modelPath,
                  const std::filesystem::path& modelCacheDirectory,
                  uint32_t materialIndex, MaterialHandle& material,
                  ModelCacheLoadStats* stats,
                  uint32_t cacheVersion)
{
    std::string originalPath;
    ...
    if (!ReadString(in, originalPath) ||
        !ReadString(in, name) ||
        !ReadPod(in, blendMode) ||
        !ReadPod(in, twoSided) ||
        !ReadPod(in, wireframe) ||
        !ReadPod(in, alphaThreshold)) {
        return false;
    }
    ...
    if (!ReadPod(in, paramCount) || paramCount > kMaxArrayItems) return false;
    for (uint32_t i = 0; i < paramCount; ++i) {
        std::string paramName;
        MaterialParam param;
        if (!ReadString(in, paramName) ||
            !ReadPod(in, param.type) ||
            !ReadPod(in, param.data)) {
            return false;
        }
    ...
    if (!ReadPod(in, textureCount) || textureCount > kMaxArrayItems) return false;
    for (uint32_t i = 0; i < textureCount; ++i) {
        std::string slot;
        bool hasTexture = false;
        if (!ReadString(in, slot) || !ReadPod(in, hasTexture)) return false;
        if (!hasTexture) continue;
        std::shared_ptr<TextureAsset> texture;
        const std::string texturePath = modelPath + "#texture-" +
            std::to_string(materialIndex) + "-" + slot;
        if (!ReadTexture(in, texturePath, modelCacheDirectory,
                         texture, stats, cacheVersion)) {
            return false;
        }
...
bool WriteModelNode(std::ostream& out, const ModelNode& node)
{
    ...
    if (!WriteString(out, node.name) ||
        !WritePod(out, node.localTransform) ||
        !WritePod(out, hasMesh) ||
...
bool ReadModelNode(std::istream& in, ModelNode& node, MeshHandle mesh)
{
    ...
    if (!ReadString(in, node.name) ||
        !ReadPod(in, node.localTransform) ||
        !ReadPod(in, hasMesh) ||
        !ReadPod(in, node.materialSlot) ||
        !ReadVector(in, node.children)) {
        return false;
    }
    if (hasMesh) node.mesh = mesh;
    return true;
}

bool WriteBones(std::ostream& out, const std::vector<Bone>& bones)
{
    if (bones.size() > kMaxArrayItems ||
        !WritePod(out, static_cast<uint32_t>(bones.size()))) {
        return false;
    }
    for (const Bone& bone : bones) {
        if (!WriteString(out, bone.name) ||
            !WritePod(out, bone.parent) ||
            !WritePod(out, bone.inverseBind) ||
            !WritePod(out, bone.bindTranslation) ||
            !WritePod(out, bone.bindRotation) ||
...
{
    uint32_t count = 0;
    if (!ReadPod(in, count) || count > kMaxArrayItems) return false;
    bones.resize(count);
    for (Bone& bone : bones) {
        if (!ReadString(in, bone.name) ||
            !ReadPod(in, bone.parent) ||
            !ReadPod(in, bone.inverseBind) ||
            !ReadPod(in, bone.bindTranslation) ||
            !ReadPod(in, bone.bindRotation) ||
            !ReadPod(in, bone.bindScale)) {
            return false;
        }
    }
    return true;
...
bool SaveModelCacheAssetToFile(const ModelAsset& model, const std::string& path)
{
    ...
    try {
        ...
        const std::filesystem::path payloadDirectory =
            outputDirectory / (outputPath.filename().string() + "_textures");
        ...
        uint32_t texturePayloadIndex = 0;
        {
            ...
            if (!out) return false;
            if (!WritePod(out, kModelCacheMagic) ||
                !WritePod(out, kModelCacheVersion) ||
                !WriteString(out, model.GetPath()) ||
                !WriteString(out, model.GetName()) ||
                !WriteVector(out, mesh->GetVertices()) ||
                !WriteVector(out, mesh->GetIndices())) {
                return false;
            }
            const auto& subMeshes = mesh->GetSubMeshes();
            if (subMeshes.size() > kMaxArrayItems ||
                !WritePod(out, static_cast<uint32_t>(subMeshes.size()))) {
                return false;
            }
            for (const SubMesh& subMesh : subMeshes) {
                if (!WritePod(out, subMesh.indexOffset) ||
                    !WritePod(out, subMesh.indexCount) ||
                    !WritePod(out, subMesh.vertexOffset) ||
                    !WritePod(out, subMesh.materialSlot) ||
            ...
            if (materials.size() > kMaxArrayItems ||
                !WritePod(out, static_cast<uint32_t>(materials.size()))) {
                return false;
            }
            for (const MaterialHandle& material : materials) {
                const bool hasMaterial = material.IsValid();
                if (!WritePod(out, hasMaterial)) return false;
                if (hasMaterial &&
                    !WriteMaterial(out, *material.Get(), outputDirectory,
                                   payloadDirectory, texturePayloadIndex)) {
                    return false;
                }
            }

            const auto& nodes = model.GetNodes();
            if (nodes.size() > kMaxArrayItems ||
                !WritePod(out, static_cast<uint32_t>(nodes.size())) ||
                !WritePod(out, model.GetRootIndex())) {
                return false;
            }
            for (const ModelNode& node : nodes) {
                if (!WriteModelNode(out, node)) return false;
            }

            if (!WriteBones(out, model.GetBones()) ||
                !WriteVector(out, model.GetSkinWeights())) {
                return false;
            }
            const auto& clips = model.GetAnimations();
            if (clips.size() > kMaxArrayItems ||
                !WritePod(out, static_cast<uint32_t>(clips.size()))) {
                return false;
            }
            for (const AnimationClip& clip : clips) {
                if (!WriteString(out, clip.name) ||
                    !WritePod(out, clip.duration) ||
                    !WritePod(out, clip.looping)) {
                    return false;
                }
                const auto& tracks = clip.tracks;
                if (tracks.size() > kMaxArrayItems ||
                    !WritePod(out, static_cast<uint32_t>(tracks.size()))) {
                    return false;
                }
                for (const BoneTrack& track : tracks) {
                    if (!WritePod(out, track.boneIndex) ||
                        !WriteVector(out, track.keys)) {
                        return false;
                    }
                }
            }
...
std::shared_ptr<ModelAsset> LoadModelCacheAssetFromFile(const std::string& path)
{
    try {
        ...
        if (!ReadPod(in, magic) || !ReadPod(in, version) ||
            magic != kModelCacheMagic || version == 0 || version > kModelCacheVersion) {
            return {};
        }
        ...
        if (!ReadPod(in, subMeshCount) || subMeshCount > kMaxArrayItems) return {};
        std::vector<SubMesh> subMeshes(subMeshCount);
        for (SubMesh& subMesh : subMeshes) {
            if (!ReadPod(in, subMesh.indexOffset) ||
                !ReadPod(in, subMesh.indexCount) ||
                !ReadPod(in, subMesh.vertexOffset) ||
                !ReadPod(in, subMesh.materialSlot) ||
        ...
        if (!ReadPod(in, materialCount) || materialCount > kMaxArrayItems) return {};
        materials.reserve(materialCount);
        for (uint32_t i = 0; i < materialCount; ++i) {
            bool hasMaterial = false;
            if (!ReadPod(in, hasMaterial)) return {};
            if (!hasMaterial) {
                materials.push_back({});
                continue;
            }
            MaterialHandle material;
            if (!ReadMaterial(in, path, modelCacheDirectory, i,
                              material, &stats, version)) {
                return {};
            }
        ...
        int rootIndex = 0;
        if (!ReadPod(in, nodeCount) || nodeCount > kMaxArrayItems ||
            !ReadPod(in, rootIndex)) {
            return {};
        }
        std::vector<ModelNode> nodes(nodeCount);
        for (ModelNode& node : nodes) {
            if (!ReadModelNode(in, node, meshHandle)) return {};
        }

        std::vector<Bone> bones;
        std::vector<SkinWeight> skinWeights;
        if (!ReadBones(in, bones) || !ReadVector(in, skinWeights)) return {};

        uint32_t clipCount = 0;
        if (!ReadPod(in, clipCount) || clipCount > kMaxArrayItems) return {};
        std::vector<AnimationClip> clips;
        clips.reserve(clipCount);
        for (uint32_t i = 0; i < clipCount; ++i) {
            AnimationClip clip;
            if (!ReadString(in, clip.name) ||
                !ReadPod(in, clip.duration) ||
                !ReadPod(in, clip.looping)) {
                return {};
            }
            uint32_t trackCount = 0;
            if (!ReadPod(in, trackCount) || trackCount > kMaxArrayItems) return {};
            clip.tracks.resize(trackCount);
            for (BoneTrack& track : clip.tracks) {
                if (!ReadPod(in, track.boneIndex) ||
                    !ReadVector(in, track.keys)) {
                    return {};
                }
            }
        ...
        Logger::Info("[AssetCache] model cache hit: ", path,
                     " version=", version,
                     " fileMB=", fileBytes / (1024.0 * 1024.0),
                     " loadMs=", ElapsedMs(loadStart),
                     " textures=", stats.textures,
                     " deferredTextures=", stats.deferredTextures,
                     " textureMB=", stats.textureBytes / (1024.0 * 1024.0),
                     " textureRebuildMs=", stats.textureRebuildMs,
                     " vertices=", model->GetMesh()->VertexCount(),
