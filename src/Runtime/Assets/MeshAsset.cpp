...
void MeshAsset::RebuildSubMeshBounds() {
    for (SubMesh& subMesh : m_SubMeshes) {
        ...
        if (!m_Indices.empty()) {
            const uint32_t end =
                (std::min)(subMesh.indexOffset + subMesh.indexCount, static_cast<uint32_t>(m_Indices.size()));
            for (uint32_t index = subMesh.indexOffset; index < end; ++index) {
                expandVertex(m_Indices[index]);
            }
        } else {
            const uint32_t end =
                (std::min)(subMesh.vertexOffset + subMesh.indexCount, static_cast<uint32_t>(m_Vertices.size()));
        ...
        for (size_t stride : {size_t{2}, size_t{4}}) {
            ...
            for (size_t triangle = 0; triangle < triangleCount; triangle += stride) {
                ...
                lod.indices.insert(lod.indices.end(),
                                   {m_Indices[firstIndex], m_Indices[firstIndex + 1], m_Indices[firstIndex + 2]});
            }
...
// --------------------------------------------------------------------------
static void PushQuadIndices(std::vector<uint32_t>& idx, uint32_t a, uint32_t b, uint32_t c, uint32_t d) {
    // a-b-c, a-c-d
    idx.push_back(a);
    idx.push_back(b);
    idx.push_back(c);
...
std::shared_ptr<MeshAsset> MeshAsset::CreateCube(const std::string& name) {
    ...
    // -Z back
    addFace({0, 0, -1}, {-1, 0, 0}, {0.5f, -0.5f, -0.5f}, {-0.5f, -0.5f, -0.5f}, {-0.5f, 0.5f, -0.5f},
            {0.5f, 0.5f, -0.5f});
    // +X right
    addFace({1, 0, 0}, {0, 0, -1}, {0.5f, -0.5f, 0.5f}, {0.5f, -0.5f, -0.5f}, {0.5f, 0.5f, -0.5f}, {0.5f, 0.5f, 0.5f});
    // -X left
    addFace({-1, 0, 0}, {0, 0, 1}, {-0.5f, -0.5f, -0.5f}, {-0.5f, -0.5f, 0.5f}, {-0.5f, 0.5f, 0.5f},
            {-0.5f, 0.5f, -0.5f});
    // +Y top
    addFace({0, 1, 0}, {1, 0, 0}, {-0.5f, 0.5f, 0.5f}, {0.5f, 0.5f, 0.5f}, {0.5f, 0.5f, -0.5f}, {-0.5f, 0.5f, -0.5f});
    // -Y bottom
    addFace({0, -1, 0}, {1, 0, 0}, {-0.5f, -0.5f, -0.5f}, {0.5f, -0.5f, -0.5f}, {0.5f, -0.5f, 0.5f},
            {-0.5f, -0.5f, 0.5f});
