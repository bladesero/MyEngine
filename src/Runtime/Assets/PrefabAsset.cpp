...
nlohmann::json PrefabNodeToJson(const PrefabNode& node) {
    ...
    for (const auto& component : node.components)
        components.push_back({{"type", component.type},
                              {"enabled", component.enabled},
                              {"version", component.version},
                              {"data", component.data}});
    nlohmann::json result = {{"localId", node.localId},
                             {"parentLocalId", node.parentLocalId},
                             {"name", node.name},
                             {"active", node.activeSelf},
                             {"transform",
                              {{"position", Vec3Json(node.transform.position)},
                               {"rotation", Vec3Json(node.transform.rotation)},
                               {"scale", Vec3Json(node.transform.scale)}}},
                             {"components", std::move(components)}};
    if (node.editorFlags != 0)
        result["editorFlags"] = node.editorFlags;
    return result;
}
...
nlohmann::json NestedInstanceToJson(const PrefabNestedInstance& instance) {
    return {{"instanceLocalId", instance.instanceLocalId},
            ...
            {"uuid", instance.assetUuid},
            {"sourceRevision", instance.sourceRevision},
            {"rootTransform",
             {{"position", Vec3Json(instance.rootTransform.position)},
              {"rotation", Vec3Json(instance.rootTransform.rotation)},
              {"scale", Vec3Json(instance.rootTransform.scale)}}},
            {"overrides", instance.overrides}};
}

bool NestedInstanceFromJson(const nlohmann::json& json, PrefabNestedInstance& instance, std::string* error) {
    if (!json.is_object()) {
        SetError(error, "nested prefab instance is not an object");
        return false;
    }
    ...
    if (const auto transform = json.find("rootTransform"); transform != json.end() && transform->is_object()) {
    ...
    if (instance.instanceLocalId.empty() || instance.parentLocalId.empty() || instance.assetPath.empty() ||
        instance.assetUuid.empty() || !instance.overrides.is_array()) {
        SetError(error, "nested prefab instance header is incomplete");
        return false;
    }
    return true;
}
...
bool PrefabNodeFromJson(const nlohmann::json& json, PrefabNode& node, std::string* error) {
    try {
        if (!json.is_object()) {
            SetError(error, "prefab node is not an object");
            return false;
        }
        node = {};
        ...
        if (const auto transform = json.find("transform"); transform != json.end() && transform->is_object()) {
            node.transform.position =
                ReadVec3(transform->value("position", nlohmann::json::array()), node.transform.position);
            node.transform.rotation =
                ReadVec3(transform->value("rotation", nlohmann::json::array()), node.transform.rotation);
        ...
        if (node.localId.empty()) {
            SetError(error, "prefab node localId is empty");
            return false;
        }
        return true;
...
bool PrefabAsset::Validate(std::string* error) const {
    ...
    if (uuid.empty() || rootLocalId.empty() || nodes.empty()) {
        SetError(error, "prefab header is incomplete");
        return false;
    }
    ...
    for (const auto& node : nodes) {
        if (node.localId.empty() || !ids.insert(node.localId).second) {
            SetError(error, "duplicate prefab localId");
            return false;
        }
        ...
        for (const auto& component : node.components)
            if (!componentTypes.insert(component.type).second) {
                SetError(error, "duplicate prefab component type");
                return false;
            }
    }
    if (roots != 1 || !ids.count(rootLocalId)) {
        SetError(error, "prefab must contain exactly one root");
        return false;
    }
    for (const auto& node : nodes)
        if (!node.parentLocalId.empty() && !ids.count(node.parentLocalId)) {
            SetError(error, "prefab parent localId is missing");
            return false;
        }
    for (const auto& node : nodes) {
        std::unordered_set<std::string> path;
        std::string current = node.localId;
        while (!current.empty()) {
            if (!path.insert(current).second) {
                SetError(error, "prefab hierarchy contains a cycle");
                return false;
            }
            current = parents[current];
        }
        if (!path.count(rootLocalId)) {
            SetError(error, "prefab node is disconnected from root");
            return false;
        }
    }
    ...
    for (const auto& nested : nestedInstances) {
        if (nested.instanceLocalId.empty() || !nestedIds.insert(nested.instanceLocalId).second ||
            ...
            SetError(error, "invalid nested prefab reference");
            return false;
        }
    }
    return true;
...
bool PrefabAsset::Save(const std::filesystem::path& path, std::string* error) const {
    ...
    try {
        ...
        for (const auto& node : nodes)
            nodeArray.push_back(PrefabNodeToJson(node));
        nlohmann::json nestedArray = nlohmann::json::array();
        for (const auto& nested : nestedInstances)
            nestedArray.push_back(NestedInstanceToJson(nested));
        nlohmann::json root = {{"version", version},
                               {"uuid", uuid},
                               {"revision", revision},
        ...
        options.validator = [](const std::filesystem::path& candidate, std::string* validationError) {
            PrefabAsset ignored;
            return PrefabAsset::Load(candidate, ignored, validationError);
        };
...
bool PrefabAsset::Load(const std::filesystem::path& path, PrefabAsset& result, std::string* error) {
    try {
        ...
        if (!json.contains("nodes") || !json["nodes"].is_array()) {
            SetError(error, "prefab nodes are missing");
            return false;
        }
        for (const auto& value : json["nodes"]) {
            PrefabNode node;
            if (!PrefabNodeFromJson(value, node, error))
                return false;
            asset.nodes.push_back(std::move(node));
        }
        if (const auto nested = json.find("nestedInstances"); nested != json.end()) {
            if (!nested->is_array()) {
                SetError(error, "nestedInstances must be an array");
                return false;
            }
            for (const auto& value : *nested) {
                PrefabNestedInstance instance;
                if (!NestedInstanceFromJson(value, instance, error))
                    return false;
