...
public:
    static void Draw(MainPass& mainPass, GpuCommandList& commands, const Camera& camera,
                     const std::vector<SceneRenderItem>& items, const ForwardRenderContext& context,
                     bool allowInstancing) {
        if (!mainPass.Device() || !context.sceneLights || !context.postProcess)
            return;
        ...
        for (size_t itemIndex = 0; itemIndex < items.size();) {
            ...
            if (mainPass.m_ShaderMode == MainPass::ShaderMode::ShadowedPbr) {
                ...
                const char* mapSlots[] = {"NormalMap", "MetallicRoughnessMap", "OcclusionMap", "EmissiveMap"};
                float mapFlags[4] = {};
                ...
                std::memcpy(constants.viewProj, viewProj.Data(), sizeof(constants.viewProj));
                std::memcpy(constants.world, world.Data(), sizeof(constants.world));
                std::memcpy(constants.lightViewProj, mainPass.m_LightViewProj.Data(), sizeof(constants.lightViewProj));
                ...
                std::memcpy(constants.cascadeSplits, mainPass.m_CascadeSplits, sizeof(constants.cascadeSplits));
                ...
                constants.shadowInfo[0] = (mainPass.m_DirectionalShadowEnabled && mainPass.m_ShadowMap) ? 1.0f : 0.0f;
                ...
                if (mainPass.m_SpotShadowIndex >= 0 &&
                    ...
                    constants.shadowIntensity[1] =
                        sceneLights.spotLights[static_cast<size_t>(mainPass.m_SpotShadowIndex)].shadowIntensity;
                } else {
                ...
                if (mainPass.m_PointShadowIndex >= 0 &&
                    ...
                    constants.shadowIntensity[2] =
                        sceneLights.pointLights[static_cast<size_t>(mainPass.m_PointShadowIndex)].shadowIntensity;
                } else {
                ...
                for (size_t instance = 0; instance < instanceCount; ++instance) {
                    const Mat4 instanceMatrix = items[itemIndex + instance].actor->GetWorldMatrix();
                ...
                mainPass.EnsureNamedBindingDefaults();
                auto bindings =
                    mainPass.GetOrCreateMaterialBindGroup(drawShader, *mat, true, namedTextures, namedTextureAssets);
                if (bindings) {
                    ...
                    if (!mainPass.m_LoggedEnvironmentState) {
                        Logger::Info("[MainPass] Environment IBL state: cube=",
                                     (mainPass.m_EnvironmentCubemap && mainPass.m_EnvironmentCubemap->IsCube()) ? 1 : 0,
                ...
                if (mesh->GetIndexBuffer()) {
                    commands.BindIndexBuffer(mesh->GetIndexBuffer());
                    commands.DrawIndexedInstanced(subMesh->indexCount, static_cast<uint32_t>(instanceCount),
                                                  subMesh->indexOffset, static_cast<uint32_t>(subMesh->vertexOffset));
                    ++mainPass.m_LastStats.drawCalls;
                } else {
                    commands.BindIndexBuffer(nullptr);
                    commands.DrawInstanced(subMesh->indexCount, static_cast<uint32_t>(instanceCount),
...
SkyPass::SkyPass(MainPass& mainPass) : m_MainPass(mainPass) {
}
...
ForwardOpaquePass::ForwardOpaquePass(MainPass& mainPass) : m_MainPass(mainPass) {
}

void ForwardOpaquePass::Execute(GpuCommandList& commands, const Scene&, const Camera& camera,
                                const std::vector<SceneRenderItem>& items, const ForwardRenderContext& context) {
    ForwardDrawExecutor::Draw(m_MainPass, commands, camera, items, context, true);
}

ForwardTransparentPass::ForwardTransparentPass(MainPass& mainPass) : m_MainPass(mainPass) {
}

void ForwardTransparentPass::Execute(GpuCommandList& commands, const Scene&, const Camera& camera,
                                     const std::vector<SceneRenderItem>& items, const ForwardRenderContext& context) {
