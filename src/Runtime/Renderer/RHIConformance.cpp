...
std::string RHIConformanceReport::Summary() const {
    ...
    for (const auto& stage : completedStages) stream << " " << stage;
    if (!failure.empty()) stream << " failure=" << failure;
    return stream.str();
}
...
RHIConformanceReport RunRHIConformance(IRHIContext& context) {
    ...
    if (!require(context.GetBackend() != RHIBackend::Unknown &&
                 capabilities.maxTextureDimension2D >= 64 &&
                 capabilities.maxColorAttachments >= 1,
                 "capabilities", "invalid backend capability baseline")) return report;

    std::array<uint32_t, 16> initial{};
    for (uint32_t i = 0; i < initial.size(); ++i) initial[i] = i;
    ...
    if (!require(buffer && bufferView &&
                 context.UpdateBuffer(buffer, sizeof(uint32_t),
                                      &replacement, sizeof(replacement)),
                 "buffer", "create, structured view, or partial update failed")) return report;

    const std::array<uint32_t, 16> pixels = {
        0xff0000ff,0xff00ff00,0xffff0000,0xffffffff,
        0xff202020,0xff404040,0xff606060,0xff808080,
        0xff112233,0xff223344,0xff334455,0xff445566,
        0xffabcdef,0xff123456,0xff654321,0xfffedcba};
    RHITextureDesc uploadDesc;
    uploadDesc.width = uploadDesc.height = 4;
    uploadDesc.usage = RHIResourceUsage::ShaderResource |
                       RHIResourceUsage::CopySource;
    ...
    if (!require(uploaded && uploadedView, "upload", "texture upload or SRV failed")) return report;

    RHITextureDesc colorDesc;
    colorDesc.width = colorDesc.height = 32;
    colorDesc.usage = RHIResourceUsage::RenderTarget |
    ...
    depthDesc.usage = RHIResourceUsage::DepthStencil |
                      RHIResourceUsage::ShaderResource;
    ...
    if (!require(color && colorView && depth && depthView,
                 "render-targets", "color/depth texture or view failed")) return report;
...
float4 PSMain(VSOutput input) : SV_TARGET { return float4(0.2, 0.6, 0.9, 1.0); }
    ...
    if (!shader || !pipeline || !vertexBuffer || !commands) {
        ...
        failure << "shader=" << static_cast<bool>(shader)
                << " pipeline=" << static_cast<bool>(pipeline)
                << " vertexBuffer=" << static_cast<bool>(vertexBuffer)
                << " commands=" << static_cast<bool>(commands);
    ...
    context.EndFrame();
    if (!require(!context.IsDeviceLost(), "pipeline-bind",
                 "pipeline binding/draw transitioned to device-lost state")) return report;
    ...
    if (!require(context.CreateSampler(samplerDesc) != nullptr,
                 "sampler", "sampler creation failed")) return report;
    ...
    if (!require(swapChain && originalWidth > 0 && originalHeight > 0 &&
                 !swapChain->Resize(0, 0) &&
                 swapChain->GetWidth() == originalWidth &&
                 swapChain->GetHeight() == originalHeight &&
                 swapChain->Resize(64, 64) &&
                 swapChain->Resize(originalWidth, originalHeight),
                 "swapchain", "minimize rejection or resize/restore failed")) return report;

    context.BeginFrame(0.02f, 0.03f, 0.04f, 1.0f);
    context.EndFrame();
    const RHIDeviceLossInfo loss = context.GetDeviceLossInfo();
    if (!require(!context.IsDeviceLost() && loss.reason == RHIDeviceLossReason::None,
                 "frame", "frame/present transitioned to device-lost state")) return report;
    ...
    const bool bufferReadbackOk = waitReadback(bufferReadback) &&
        bufferReadback->Read(bufferBytes) && bufferBytes.size() >= sizeof(initial) &&
        std::memcmp(bufferBytes.data() + sizeof(uint32_t), &replacement,
                    sizeof(replacement)) == 0;
    std::vector<uint8_t> textureBytes;
    const bool textureReadbackOk = waitReadback(textureReadback) &&
        textureReadback->Read(textureBytes) && textureBytes.size() >= 64;
    if (!bufferReadbackOk || !textureReadbackOk) {
        std::ostringstream failure;
        uint32_t actualBufferValue = 0;
        if (bufferBytes.size() >= sizeof(uint32_t) * 2)
            std::memcpy(&actualBufferValue, bufferBytes.data() + sizeof(uint32_t),
                        sizeof(actualBufferValue));
        failure << "bufferOk=" << bufferReadbackOk
                << " bufferBytes=" << bufferBytes.size()
                << " actualBufferValue=" << actualBufferValue
                << " textureOk=" << textureReadbackOk
    ...
    context.BeginFrame(0.02f, 0.03f, 0.04f, 1.0f);
    context.EndFrame();
    if (!require(exhaustedGracefully && context.CreateBufferView(buffer, bufferViewDesc),
                 "descriptor-pressure",
                 "descriptor pressure caused device loss or leases did not recycle")) return report;
