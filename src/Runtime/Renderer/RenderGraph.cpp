...
const char* UsageName(RHIResourceUsage usage) {
    switch (usage) {
    case RHIResourceUsage::ShaderResource: return "ShaderResource";
    case RHIResourceUsage::RenderTarget: return "RenderTarget";
    case RHIResourceUsage::DepthStencil: return "DepthStencil";
    case RHIResourceUsage::UnorderedAccess: return "UnorderedAccess";
    case RHIResourceUsage::CopySource: return "CopySource";
    case RHIResourceUsage::CopyDestination: return "CopyDestination";
...
RHITextureViewDesc MakeViewDesc(RGTextureSubresource subresource,
...
bool SameSubresourceView(const RHITextureViewDesc& a, const RHITextureViewDesc& b) {
    return a.firstMip == b.firstMip && a.mipCount == b.mipCount &&
           a.firstLayer == b.firstLayer && a.layerCount == b.layerCount &&
           a.usage == b.usage;
}
...
std::string TexturePoolKey(const RHITextureDesc& desc) {
    ...
    out << desc.width << 'x' << desc.height
        << ":m" << desc.mipLevels
        << ":l" << desc.arrayLayers
        << ":s" << desc.sampleCount
        << ":q" << desc.sampleQuality
        << ":f" << static_cast<int>(desc.format)
        << ":u" << static_cast<uint32_t>(desc.usage)
        << ":c" << desc.cube;
    return out.str();
}
...
void RenderGraphBuilder::ReadTexture(RGTextureHandle h, RGTextureSubresource subresource) {
    Access access{h, RHIResourceState::ShaderResource, RHILoadOp::Load, RHIStoreOp::Store,
                  {}, 1.0f, true, false};
...
void RenderGraphBuilder::WriteColor(RGTextureHandle h, RGTextureSubresource subresource,
                                    RHILoadOp load, RHIStoreOp store, ClearColor clear) {
...
void RenderGraphBuilder::WriteDepth(RGTextureHandle h, RGTextureSubresource subresource,
                                    RHILoadOp load, RHIStoreOp store, float clearDepth) {
...
void RenderGraphBuilder::ReadWriteUAV(RGTextureHandle h, RGTextureSubresource subresource) {
    Access access{h, RHIResourceState::UnorderedAccess, RHILoadOp::Load, RHIStoreOp::Store,
                  {}, 1.0f, true, true};
...
void RenderGraph::AddPass(const std::string& name, SetupCallback setup,
                          ExecuteCallback execute, PassFlags flags) {
    Pass pass;
    pass.name = name;
...
bool RenderGraph::ValidateTextureAccess(const Pass& pass,
                                        const RenderGraphBuilder::Access& access) {
    if (!ValidateHandle(access.handle, pass.name)) return false;
    ...
    const auto requireUsage = [&](RHIResourceUsage usage) {
        if (HasUsage(resource.desc.usage, usage)) return true;
        return SetError(ErrorCode::TextureUsageMismatch,
                        "RenderGraph pass '" + pass.name + "' accesses texture '" +
    ...
    case RHIResourceState::RenderTarget:
        if (!requireUsage(RHIResourceUsage::RenderTarget)) return false;
        if (IsDepthFormat(resource.desc.format)) {
            return SetError(ErrorCode::AttachmentFormatMismatch,
                            "RenderGraph pass '" + pass.name +
    ...
    case RHIResourceState::DepthRead:
        if (!requireUsage(RHIResourceUsage::DepthStencil)) return false;
        if (!IsDepthFormat(resource.desc.format)) {
            return SetError(ErrorCode::AttachmentFormatMismatch,
                            "RenderGraph pass '" + pass.name +
...
bool RenderGraph::ValidateBufferAccess(const Pass& pass,
                                       const RenderGraphBuilder::BufferAccess& access) {
    ...
    const auto requireUsage = [&](RHIResourceUsage usage) {
        if (HasUsage(resource.desc.usage, usage)) return true;
        return SetError(ErrorCode::BufferUsageMismatch,
                        "RenderGraph pass '" + pass.name + "' accesses buffer '" +
                        resource.name + "' without " + UsageName(usage) + " usage");
    };
    switch (access.state) {
    case RHIResourceState::ShaderResource:
...
bool RenderGraph::Compile() {
    ...
    for (uint32_t p = 0; p < passCount; ++p) {
        ...
        for (const auto& access : m_Passes[p].accesses) {
            if (!ValidateTextureAccess(m_Passes[p], access)) return false;
            ...
            const RHITextureViewDesc accessView = access.hasViewDesc
                ? access.viewDesc : fullView(access.handle.index);
            for (const auto& seenAccess : seenTextureAccesses) {
                if (seenAccess.handle.index != access.handle.index) continue;
                const RHITextureViewDesc seenView = seenAccess.hasViewDesc
                    ? seenAccess.viewDesc : fullView(seenAccess.handle.index);
            ...
            const bool attachment = access.state == RHIResourceState::RenderTarget ||
                                    access.state == RHIResourceState::DepthWrite;
            if (attachment) {
                ...
                if (attachmentWidth == 0 || attachmentHeight == 0) {
                    return SetError(ErrorCode::AttachmentSizeMismatch,
                                    "RenderGraph pass '" + m_Passes[p].name +
                                    "' uses zero-sized attachment '" + texture.name + "'");
                }
                if (renderWidth == 0 && renderHeight == 0) {
                    renderWidth = attachmentWidth;
                    renderHeight = attachmentHeight;
                } else if (renderWidth != attachmentWidth ||
                           renderHeight != attachmentHeight) {
                    return SetError(ErrorCode::AttachmentSizeMismatch,
                                    "RenderGraph pass '" + m_Passes[p].name +
                                    "' declares attachments with mismatched sizes");
                }
                if (access.state == RHIResourceState::RenderTarget) ++colorAttachmentCount;
            }
            if (access.read && !access.write && lastWriter[r] < 0 && !m_Textures[r].imported) {
                return SetError(ErrorCode::UninitializedTextureRead,
                                "RenderGraph pass '" + m_Passes[p].name +
        ...
        const uint32_t maxColorAttachments = (std::max)(
            1u, m_Device.GetCapabilities().maxColorAttachments);
        if (colorAttachmentCount > maxColorAttachments) {
            std::ostringstream out;
            out << "RenderGraph pass '" << m_Passes[p].name << "' declares "
                << colorAttachmentCount << " color attachments, but device supports "
                << maxColorAttachments;
        ...
        for (const auto& access : m_Passes[p].bufferAccesses) {
            if (!ValidateBufferAccess(m_Passes[p], access)) return false;
            if (!seenBuffers.insert(access.handle.index).second) {
                return SetError(ErrorCode::DuplicateResourceAccess,
                                "RenderGraph pass '" + m_Passes[p].name +
                                "' declares the same buffer more than once");
            }
            const uint32_t r = access.handle.index;
            if (access.read && !access.write && lastBufferWriter[r] < 0 && !m_Buffers[r].imported) {
                return SetError(ErrorCode::UninitializedBufferRead,
                                "RenderGraph pass '" + m_Passes[p].name +
    ...
    for (uint32_t i = 0; i < m_Buffers.size(); ++i) {
        ...
        Pass finalPass;
        finalPass.name = "FinalState";
        if (!ValidateBufferAccess(finalPass, access)) return false;
    }
    ...
    while (changed) {
        ...
        for (int32_t p = static_cast<int32_t>(passCount) - 1; p >= 0; --p) {
            ...
            for (const auto& access : m_Passes[p].accesses) hasWrites = hasWrites || access.write;
            for (const auto& access : m_Passes[p].bufferAccesses) hasWrites = hasWrites || access.write;
            bool passLive = livePass[p] != 0 ||
                            HasPassFlag(m_Passes[p].flags, PassFlags::AllowNoResourceAccess) ||
                            !hasWrites;
            for (const auto& access : m_Passes[p].accesses) {
    ...
    for (uint32_t p = 0; p < passCount; ++p) {
        ...
        for (uint32_t next : edges[p]) {
            if (livePass[next]) ++liveIndegree[next];
        }
    }

    std::queue<uint32_t> ready;
    for (uint32_t p = 0; p < passCount; ++p) {
        if (livePass[p] && liveIndegree[p] == 0) ready.push(p);
    }
    while (!ready.empty()) {
        ...
        for (uint32_t next : edges[p]) {
            if (!livePass[next]) continue;
            if (--liveIndegree[next] == 0) ready.push(next);
        }
    }
    const uint32_t livePassCount = static_cast<uint32_t>(
        std::count(livePass.begin(), livePass.end(), uint8_t{1}));
    if (m_ExecutionOrder.size() != livePassCount) {
        return SetError(ErrorCode::DependencyCycle,
...
bool RenderGraph::EnsureResources() {
    ...
    for (uint32_t index = 0; index < m_Textures.size(); ++index) {
        ...
        resource.subresourceStates.assign(
            resource.desc.mipLevels * resource.desc.arrayLayers,
            resource.initialState);
    }
    for (uint32_t passIndex = 0; passIndex < m_Passes.size(); ++passIndex) {
        if (std::find(m_ExecutionOrder.begin(), m_ExecutionOrder.end(), passIndex) ==
            m_ExecutionOrder.end()) continue;
        auto& pass = m_Passes[passIndex];
        for (auto& access : pass.accesses) {
            if (!access.hasViewDesc || access.view) continue;
            const bool manualAttachment =
                HasPassFlag(pass.flags, PassFlags::ManualRenderingScope) &&
                (access.state == RHIResourceState::RenderTarget ||
                 access.state == RHIResourceState::DepthWrite);
            if (manualAttachment) continue;
            auto& resource = m_Textures[access.handle.index];
            access.view = m_Device.CreateTextureView(resource.texture, access.viewDesc);
            if (!access.view) {
                return SetError(ErrorCode::ResourceCreationFailed,
                                "RHI failed to create RenderGraph subresource view '" +
                                resource.name + "'");
            }
        }
    }
    for (uint32_t index = 0; index < m_Buffers.size(); ++index) {
        if (index < m_LiveBuffers.size() && !m_LiveBuffers[index]) continue;
...
bool RenderGraph::Execute(GpuCommandList& commandList) {
    ...
    for (uint32_t passIndex : m_ExecutionOrder) {
        ...
        const bool manualTransitions =
            HasPassFlag(pass.flags, PassFlags::ManualResourceTransitions);
        for (const auto& access : pass.accesses) {
            auto& resource = m_Textures[access.handle.index];
            if (manualTransitions) {
                resource.currentState = resource.hasFinalState ? resource.finalState : access.state;
                std::fill(resource.subresourceStates.begin(), resource.subresourceStates.end(),
                ...
                for (uint32_t layer = 0; layer < access.viewDesc.layerCount; ++layer) {
                    for (uint32_t mip = 0; mip < access.viewDesc.mipCount; ++mip) {
                        ...
                        const uint32_t subresource = SubresourceIndex(
                            resource.desc, absoluteMip, absoluteLayer);
                        RHIResourceState before = subresource < resource.subresourceStates.size()
                            ? resource.subresourceStates[subresource] : resource.currentState;
                        if (before == access.state) continue;
                        RHITextureViewDesc range = access.viewDesc;
                        range.firstMip = absoluteMip;
                        range.mipCount = 1;
                        range.firstLayer = absoluteLayer;
                        range.layerCount = 1;
                        commandList.TransitionTexture(resource.texture.get(), range,
                                                      before, access.state);
...
GpuTextureView* RenderGraphResources::GetView(RGTextureHandle h,
                                              RGTextureSubresource subresource) const {
    if (h.index >= m_Graph.m_Textures.size()) return nullptr;
    ...
    for (const auto& pass : m_Graph.m_Passes) {
        for (const auto& access : pass.accesses) {
            if (access.handle.index != h.index || !access.hasViewDesc || !access.view) continue;
            if (SameSubresourceView(access.viewDesc, target)) return access.view.get();
        }
    }
    return nullptr;
}
