...
public:
    ...
    std::shared_ptr<GpuTexture> CreateDepthTexture(int width, int height, bool cube, uint32_t arraySize = 1);

    void PushRenderTarget(const D3D12_CPU_DESCRIPTOR_HANDLE* colorRtv, D3D12_CPU_DESCRIPTOR_HANDLE depthDsv);
    ...
    D3D12_CPU_DESCRIPTOR_HANDLE AllocDsvSlot(std::shared_ptr<D3D12DescriptorLease>* lease = nullptr);
    D3D12_CPU_DESCRIPTOR_HANDLE AllocRtvSlot(std::shared_ptr<D3D12DescriptorLease>* lease = nullptr);
    ...
    void BindPSTextureDescriptors(uint32_t slot, D3D12_GPU_DESCRIPTOR_HANDLE srvGpu,
                                  D3D12_GPU_DESCRIPTOR_HANDLE sampGpu);
    std::shared_ptr<GpuShader> CreateFullscreenShaderFromBytecode(const void* vsBytecode, size_t vsSize,
    ...
    D3D12_CPU_DESCRIPTOR_HANDLE GetFontSrvCpuHandle() const { return m_FontSrvCpuHandle; }
    D3D12_GPU_DESCRIPTOR_HANDLE GetFontSrvGpuHandle() const { return m_FontSrvGpuHandle; }
...
private:
    ...
    struct SamplerCacheKey {
        ...
        bool operator==(const SamplerCacheKey& other) const {
            return desc.filter == other.desc.filter && desc.addressU == other.desc.addressU &&
                   desc.addressV == other.desc.addressV && desc.addressW == other.desc.addressW;
        }
    };
