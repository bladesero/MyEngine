...
D3D12_CPU_DESCRIPTOR_HANDLE D3D12Context::OffsetHandle(D3D12_CPU_DESCRIPTOR_HANDLE h, uint32_t index, uint32_t inc) {
    h.ptr += static_cast<SIZE_T>(index) * inc;
    return h;
}

D3D12_GPU_DESCRIPTOR_HANDLE D3D12Context::OffsetHandle(D3D12_GPU_DESCRIPTOR_HANDLE h, uint32_t index, uint32_t inc) {
    h.ptr += static_cast<SIZE_T>(index) * inc;
    return h;
}
...
static const char* SamplerFilterName(RHIFilter filter) {
    ...
    case RHIFilter::Linear:
        return "Linear";
    case RHIFilter::ComparisonLinear:
...
public:
    ...
    bool Wait(uint64_t value, uint32_t timeoutMs) override {
        if (!native || GetCompletedValue() >= value)
        ...
        const DWORD result = WaitForSingleObject(eventHandle, timeoutMs == UINT32_MAX ? INFINITE : timeoutMs);
        CloseHandle(eventHandle);
...
public:
    ...
    bool Submit(GpuCommandList* commands, GpuFence* fence, uint64_t value) override {
        if (commands)
            return false; // Immediate list submission occurs in EndFrame.
...
public:
    ...
    bool ReadResults(uint32_t first, uint32_t resultCount, std::vector<uint64_t>& ticks) override {
        if (!readback || first + resultCount > count)
            return false;
        void* mapped = nullptr;
        D3D12_RANGE range{static_cast<SIZE_T>(first) * 8, static_cast<SIZE_T>(first + resultCount) * 8};
...
public:
    explicit D3D12ImmediateCommandList(D3D12Context& owner) : m_Owner(owner) {}
    ...
    void WriteTimestamp(GpuTimestampQueryPool* pool, uint32_t index) override {
        ...
        if (native && index < native->count)
            m_Owner.GetCommandList()->EndQuery(native->heap.Get(), D3D12_QUERY_TYPE_TIMESTAMP, index);
    }
    void ResolveTimestamps(GpuTimestampQueryPool* pool, uint32_t first, uint32_t count) override {
        auto* native = dynamic_cast<D3D12TimestampPool*>(pool);
        if (native && first + count <= native->count)
            m_Owner.GetCommandList()->ResolveQueryData(native->heap.Get(), D3D12_QUERY_TYPE_TIMESTAMP, first, count,
                                                       native->readback.Get(),
    ...
    void SetGraphicsPipeline(GpuGraphicsPipeline* pipeline) override {
        ...
        auto* shader =
            pipeline && pipeline->desc.shader ? dynamic_cast<D3D12Shader*>(pipeline->desc.shader.get()) : nullptr;
        if (!nativePipeline || !nativePipeline->pipelineState || !shader || !shader->rootSignature)
            return;
        m_ComputeBinding = false;
        ...
        m_Owner.GetCommandList()->IASetPrimitiveTopology(ToD3D12Topology(pipeline->desc.topology));
        m_Owner.GetCommandList()->OMSetBlendFactor(pipeline->desc.blend.blendConstants);
        m_Owner.GetCommandList()->OMSetStencilRef(pipeline->desc.depthStencil.stencilReference);
    }
    void SetComputePipeline(GpuComputePipeline* pipeline) override {
        auto* shader =
            pipeline && pipeline->desc.shader ? dynamic_cast<D3D12Shader*>(pipeline->desc.shader.get()) : nullptr;
        if (!shader || !shader->computeRootSignature || !shader->computePipelineState)
            return;
    ...
    void CopyBuffer(GpuBuffer* dst, uint32_t dstOffset, GpuBuffer* src, uint32_t srcOffset,
        ...
        if (d && s)
            m_Owner.GetCommandList()->CopyBufferRegion(d->resource.Get(), dstOffset, s->resource.Get(), srcOffset,
                                                       byteSize);
    }
    ...
    void CopyTexture(GpuTexture* dst, const RHITextureRegion& dr, GpuTexture* src,
        ...
        auto* d = dynamic_cast<D3D12Texture*>(dst);
        auto* s = dynamic_cast<D3D12Texture*>(src);
        if (!d || !s)
            return;
        D3D12_TEXTURE_COPY_LOCATION dl{};
        dl.pResource = d->resource.Get();
    ...
    void UAVBarrier(GpuResource* resource) override {
        ...
        ID3D12Resource* native = buffer ? buffer->resource.Get() : texture ? texture->resource.Get() : nullptr;
        if (!native)
            return;
    ...
    void Transition(GpuResource* resource, RHIResourceState before, RHIResourceState after) override {
        auto* texture = dynamic_cast<D3D12Texture*>(resource);
        auto* buffer = dynamic_cast<D3D12Buffer*>(resource);
        ID3D12Resource* native = texture ? texture->resource.Get() : buffer ? buffer->resource.Get() : nullptr;
        if (!native || before == after)
            return;
        D3D12_RESOURCE_BARRIER barrier{};
    ...
    void BeginRendering(const RenderingInfo& info) override {
        ...
        if (depth.ptr && info.depth->loadOp == RHILoadOp::Clear)
            m_Owner.GetCommandList()->ClearDepthStencilView(depth, D3D12_CLEAR_FLAG_DEPTH, info.depth->clearDepth,
                                                            info.depth->clearStencil, 0, nullptr);
...
public:
    D3D12ReadbackTicket(ComPtr<ID3D12Resource> resource, ComPtr<ID3D12Fence> fence, uint64_t fenceValue, uint32_t size,
                        std::shared_ptr<D3D12DeferredReleaseQueue> deferredReleaseQueue)
        : m_Resource(std::move(resource)), m_Fence(std::move(fence)),
          m_DeferredReleaseQueue(std::move(deferredReleaseQueue)), m_FenceValue(fenceValue), m_Size(size) {}
    ...
    bool Read(std::vector<uint8_t>& data) override {
        if (!IsReady() || !m_Resource)
            return false;
        void* mapped = nullptr;
        D3D12_RANGE range{0, m_Size};
        if (FAILED(m_Resource->Map(0, &range, &mapped)) || !mapped)
            return false;
        data.resize(m_Size);
        std::memcpy(data.data(), mapped, m_Size);
        D3D12_RANGE written{0, 0};
...
public:
    explicit D3D12SwapChain(D3D12Context& owner) : m_Owner(owner) {}
...
static std::string FormatHRESULT(HRESULT hr) {
    ...
    stream << "0x" << std::hex << std::uppercase << std::setw(8) << std::setfill('0') << static_cast<uint32_t>(hr);
    return stream.str();
}
...
public:
    ...
    void AbandonSubmittedFrame() {
        ...
        if (m_Shutdown)
            return;
        m_RecordingFrame = false;
        for (auto& object : m_CurrentFrame)
            m_Abandoned.push_back(std::move(object));
        for (auto& action : m_CurrentFrameActions)
    ...
    void Retire(ComPtr<IUnknown> object) {
        ...
        if (m_RecordingFrame) {
            ...
            if (!m_Batches.empty() && m_Batches.back().fenceValue == m_LastSubmittedFenceValue) {
    ...
    void RetireAction(std::function<void()> action) {
        ...
        bool executeNow = false;
        {
            ...
            } else if (!m_Batches.empty() && m_Batches.back().fenceValue == m_LastSubmittedFenceValue) {
    ...
    void Collect(uint64_t completedFenceValue) {
        ...
        {
            std::lock_guard<std::mutex> lock(m_Mutex);
            while (!m_Batches.empty() && m_Batches.front().fenceValue <= completedFenceValue) {
    ...
    void ShutdownAndRelease() {
        std::vector<std::function<void()>> actions;
        {
            ...
            for (auto& action : m_AbandonedActions)
                actions.push_back(std::move(action));
            m_CurrentFrame.clear();
            m_CurrentFrameActions.clear();
            m_Batches.clear();
...
public:
    D3D12TextureReadbackTicket(ComPtr<ID3D12Resource> resource, ComPtr<ID3D12Fence> fence, uint64_t fenceValue,
                               uint32_t width, uint32_t height, uint32_t rowSize, uint32_t gpuRowPitch,
                               RHIFormat format, std::shared_ptr<D3D12DeferredReleaseQueue> releaseQueue)
        : m_Resource(std::move(resource)), m_Fence(std::move(fence)), m_ReleaseQueue(std::move(releaseQueue)),
          m_FenceValue(fenceValue), m_Width(width), m_Height(height), m_RowSize(rowSize), m_GpuRowPitch(gpuRowPitch),
          m_Format(format) {}
    ...
    bool Read(std::vector<uint8_t>& data) override {
        ...
        if (FAILED(mapHr) || !mapped) {
            Logger::Error("[RHI] D3D12 texture readback Map failed hr=", static_cast<long>(mapHr), " mapped=", mapped);
            return false;
        }
        data.resize(static_cast<size_t>(m_RowSize) * m_Height);
        for (uint32_t y = 0; y < m_Height; ++y)
            std::memcpy(data.data() + static_cast<size_t>(y) * m_RowSize,
                        static_cast<const uint8_t*>(mapped) + static_cast<size_t>(y) * m_GpuRowPitch, m_RowSize);
        D3D12_RANGE written{0, 0};
        m_Resource->Unmap(0, &written);
        return true;
    }
...
private:
    ...
    uint32_t m_RowSize = 0, m_GpuRowPitch = 0;
    RHIFormat m_Format = RHIFormat::Unknown;
};

class D3D12DescriptorPool : public std::enable_shared_from_this<D3D12DescriptorPool> {
public:
    D3D12DescriptorPool(D3D12_CPU_DESCRIPTOR_HANDLE cpuBase, D3D12_GPU_DESCRIPTOR_HANDLE gpuBase, uint32_t increment,
                        uint32_t capacity, uint32_t firstIndex, RHINativeDescriptorKind kind)
        : m_CpuBase(cpuBase), m_GpuBase(gpuBase), m_Increment(increment), m_Capacity(capacity), m_NextIndex(firstIndex),
          m_Kind(kind), m_ReservedSlots(firstIndex) {
        if (m_ReservedSlots)
            RHIResourceStatsProvider::AddNativeDescriptorSlots(m_Kind, m_ReservedSlots);
    }
    ~D3D12DescriptorPool() {
        if (m_ReservedSlots)
            RHIResourceStatsProvider::ReleaseNativeDescriptorSlots(m_Kind, m_ReservedSlots);
    }

    bool Allocate(const std::shared_ptr<D3D12DeferredReleaseQueue>& queue, D3D12_CPU_DESCRIPTOR_HANDLE& cpu,
                  D3D12_GPU_DESCRIPTOR_HANDLE& gpu, std::shared_ptr<D3D12DescriptorLease>* lease);
...
public:
    D3D12DescriptorLease(std::shared_ptr<D3D12DescriptorPool> pool, std::shared_ptr<D3D12DeferredReleaseQueue> queue,
                         uint32_t index)
    ...
    ~D3D12DescriptorLease() {
        ...
        if (m_Queue) {
            m_Queue->RetireAction([pool = std::move(pool), index]() { pool->Release(index); });
        } else {
            pool->Release(index);
        }
    }
...
bool D3D12DescriptorPool::Allocate(const std::shared_ptr<D3D12DeferredReleaseQueue>& queue,
    ...
    gpu = m_GpuBase;
    if (gpu.ptr != 0)
        gpu.ptr += static_cast<UINT64>(index) * m_Increment;
    if (lease) {
        *lease = std::make_shared<D3D12DescriptorLease>(shared_from_this(), queue, index);
    }
...
template <typename T>
static void RetireD3D12Object(const std::shared_ptr<D3D12DeferredReleaseQueue>& queue, ComPtr<T>& object) {
    if (!object)
        return;
    if (!queue) {
        object.Reset();
...
static bool IsEnvironmentFlagEnabled(const char* name) {
    char value[8] = {};
    return GetEnvironmentVariableA(name, value, static_cast<DWORD>(sizeof(value))) > 0 && std::strcmp(value, "0") != 0;
}

static void EnableD3D12Diagnostics() {
#if !defined(NDEBUG)
    const bool enableGpuValidation = IsEnvironmentFlagEnabled("MYENGINE_D3D12_GPU_VALIDATION");
    const bool enableDebugLayer = enableGpuValidation || IsEnvironmentFlagEnabled("MYENGINE_D3D12_DEBUG_LAYER");
    if (enableDebugLayer) {
...
bool D3D12Context::Init(IWindow* window) {
    ...
    if (FAILED(hr)) {
        Logger::Error("CreateCommandQueue failed: 0x", reinterpret_cast<void*>(static_cast<uintptr_t>(hr)));
        return false;
    }
    ...
    if (FAILED(m_Device->CreateCommandSignature(&indirectDesc, nullptr, IID_PPV_ARGS(&m_DrawIndirectSignature))))
        return false;
    ...
    ComPtr<IDXGIFactory4> factory;
    {
        ...
        if (FAILED(hr)) {
            Logger::Error("CreateDXGIFactory2 failed: 0x", reinterpret_cast<void*>(static_cast<uintptr_t>(hr)));
            return false;
        }
    }
    ...
    if (FAILED(hr)) {
        Logger::Error("CreateSwapChainForHwnd failed: 0x", reinterpret_cast<void*>(static_cast<uintptr_t>(hr)));
        return false;
    }

    hr = swapChain1.As(&m_SwapChain);
    if (FAILED(hr)) {
        Logger::Error("swapChain1.As<IDXGISwapChain3> failed: 0x", reinterpret_cast<void*>(static_cast<uintptr_t>(hr)));
        return false;
    }
    ...
    // RTV heap (swapchain + offscreen)
    {
        ...
        if (FAILED(hr)) {
            Logger::Error("CreateDescriptorHeap(RTV) failed: 0x", reinterpret_cast<void*>(static_cast<uintptr_t>(hr)));
            return false;
        }
        ...
        for (uint32_t i = 0; i < kFrameCount; ++i) {
            ...
            if (FAILED(hr)) {
                Logger::Error("GetBuffer failed: 0x", reinterpret_cast<void*>(static_cast<uintptr_t>(hr)));
                return false;
            }
    ...
    // Offscreen RTV heap (PostProcessPass)
    {
        ...
        m_RtvDescriptorPool = std::make_shared<D3D12DescriptorPool>(
            m_OffscreenRtvHeap->GetCPUDescriptorHandleForHeapStart(), D3D12_GPU_DESCRIPTOR_HANDLE{},
            m_OffscreenRtvDescriptorSize, kOffscreenRtvCount, 0, RHINativeDescriptorKind::RenderTarget);
    }
    ...
    // DSV heap (PostProcessPass depth)
    {
        ...
        if (FAILED(hr)) {
            Logger::Error("CreateDescriptorHeap(DSV) failed: 0x", reinterpret_cast<void*>(static_cast<uintptr_t>(hr)));
            return false;
        }
        m_DsvDescriptorSize = m_Device->GetDescriptorHandleIncrementSize(D3D12_DESCRIPTOR_HEAP_TYPE_DSV);
        m_DsvDescriptorPool = std::make_shared<D3D12DescriptorPool>(
            m_DsvHeap->GetCPUDescriptorHandleForHeapStart(), D3D12_GPU_DESCRIPTOR_HANDLE{}, m_DsvDescriptorSize,
    ...
    // SRV heap (shader-visible, required by ImGui dx12 backend for fonts)
    {
        ...
        if (FAILED(hr)) {
            Logger::Error("CreateDescriptorHeap(SRV) failed: 0x", reinterpret_cast<void*>(static_cast<uintptr_t>(hr)));
            return false;
        }
    ...
    m_SrvDescriptorPool = std::make_shared<D3D12DescriptorPool>(
        ...
        m_SrvDescriptorSize, kDefaultSrvDescriptorCount, 1, RHINativeDescriptorKind::Resource);
    ...
    // Sampler heap (samplers must be in a separate heap type in D3D12)
    {
        ...
        m_SamplerDescriptorPool = std::make_shared<D3D12DescriptorPool>(
            m_SamplerHeap->GetCPUDescriptorHandleForHeapStart(), m_SamplerHeap->GetGPUDescriptorHandleForHeapStart(),
            m_SamplerDescriptorSize, kDefaultSamplerDescriptorCount, 0, RHINativeDescriptorKind::Sampler);
    }
    ...
    // Upload command allocator + list (one-shot copies for texture uploads)
    {
        ...
        hr = m_Device->CreateCommandList(0, D3D12_COMMAND_LIST_TYPE_DIRECT, m_UploadCommandAllocator.Get(), nullptr,
                                         IID_PPV_ARGS(&m_UploadCommandList));
        if (FAILED(hr)) {
            Logger::Error("CreateCommandList(upload) failed: 0x", reinterpret_cast<void*>(static_cast<uintptr_t>(hr)));
            return false;
        }
    ...
    for (uint32_t i = 0; i < kFrameCount; ++i) {
        ...
        if (FAILED(hr)) {
            Logger::Error("CreateCommandAllocator failed: 0x", reinterpret_cast<void*>(static_cast<uintptr_t>(hr)));
            return false;
        }
        const std::wstring allocatorName = L"MyEngine D3D12 Frame Allocator " + std::to_wstring(i);
    ...
    if (FAILED(hr)) {
        Logger::Error("CreateCommandList failed: 0x", reinterpret_cast<void*>(static_cast<uintptr_t>(hr)));
        return false;
    }
    ...
    for (uint32_t i = 0; i < kFrameCount; ++i) {
        ...
        hr = m_Device->CreateCommittedResource(&props, D3D12_HEAP_FLAG_NONE, &desc, D3D12_RESOURCE_STATE_GENERIC_READ,
                                               nullptr, IID_PPV_ARGS(&fr.constantBufferUpload));
        if (FAILED(hr)) {
        ...
        hr = fr.constantBufferUpload->Map(0, nullptr, reinterpret_cast<void**>(&fr.constantBufferMapped));
        if (FAILED(hr) || !fr.constantBufferMapped) {
            Logger::Error("Map(constantBufferUpload) failed: 0x", reinterpret_cast<void*>(static_cast<uintptr_t>(hr)));
            return false;
        }
    }
    ...
    if (FAILED(hr)) {
        Logger::Error("CreateFence failed: 0x", reinterpret_cast<void*>(static_cast<uintptr_t>(hr)));
        return false;
    }
    ...
    ++m_DeviceGeneration;
    m_DeviceLossInfo = {};
    Logger::Info("D3D12Context initialised (", w, "x", h, ") generation=", m_DeviceGeneration);
    return true;
}
...
void D3D12Context::Shutdown() {
    ...
    if (m_IsRecording && m_CommandList) {
        ...
        if (m_DeferredReleaseQueue)
            m_DeferredReleaseQueue->AbortUnsubmittedFrame();
        m_IsRecording = false;
    }

    if (m_DeferredReleaseQueue)
        m_DeferredReleaseQueue->ShutdownAndRelease();
...
void D3D12Context::WaitForGpuIdle() {
    ...
    if (!CheckDeviceResult(m_CommandQueue->Signal(m_Fence.Get(), fenceValue), "Signal(wait for GPU idle)"))
        return;
    if (m_Fence->GetCompletedValue() < fenceValue) {
        if (!CheckDeviceResult(m_Fence->SetEventOnCompletion(fenceValue, m_FenceEvent),
                               "SetEventOnCompletion(wait for GPU idle)"))
            return;
        WaitForSingleObject(m_FenceEvent, INFINITE);
    }
    for (auto& frame : m_Frames) {
...
bool D3D12Context::UploadBufferData(ID3D12Resource* destination, ID3D12Resource* uploadBuffer, uint64_t byteSize,
                                    D3D12_RESOURCE_STATES finalState) {
    if (!destination || !uploadBuffer || !m_UploadCommandAllocator || !m_UploadCommandList || !m_CommandQueue ||
        !m_Fence || !m_FenceEvent) {
        return false;
    }

    if (!CanUseDevice("UploadBufferData"))
        return false;
    ...
    const uint64_t fenceValue = m_NextFenceValue++;
    if (!CheckDeviceResult(m_CommandQueue->Signal(m_Fence.Get(), fenceValue), "Signal(buffer upload)"))
        return false;
    if (m_Fence->GetCompletedValue() < fenceValue) {
        if (!CheckDeviceResult(m_Fence->SetEventOnCompletion(fenceValue, m_FenceEvent),
                               "SetEventOnCompletion(buffer upload)"))
            return false;
        WaitForSingleObject(m_FenceEvent, INFINITE);
    }
    return true;
...
void D3D12Context::TransitionToRenderTarget(ID3D12GraphicsCommandList* cmdList, uint32_t frameIndex) {
...
void D3D12Context::TransitionToPresent(ID3D12GraphicsCommandList* cmdList, uint32_t frameIndex) {
    auto* resource = m_BackBuffers[frameIndex].Get();
    D3D12_RESOURCE_BARRIER barrier = {};
    barrier.Type = D3D12_RESOURCE_BARRIER_TYPE_TRANSITION;
    barrier.Flags = D3D12_RESOURCE_BARRIER_FLAG_NONE;
...
void D3D12Context::BeginFrame(float r, float g, float b, float a) {
    ...
    if (!CanUseDevice("BeginFrame") || !m_SwapChain)
        return;
    ...
    if (!m_FrameCommandListClosed && m_CommandList) {
        ...
        if (SUCCEEDED(closeHr)) {
            ...
            Logger::Warn("Close(stale frame command list) failed during recovery: ", FormatHRESULT(closeHr));
        }
    }

    if (!CheckDeviceResult(fr.commandAllocator->Reset(), "Reset(frame command allocator)"))
        return;
    HRESULT resetHr = m_CommandList ? m_CommandList->Reset(fr.commandAllocator.Get(), nullptr) : E_POINTER;
    if (SUCCEEDED(resetHr)) {
        m_FrameCommandListClosed = false;
    } else {
        Logger::Warn("Reset(frame command list) failed during recovery: ", FormatHRESULT(resetHr),
                     "; recreating command list");
        if (!RecreateFrameCommandList(fr.commandAllocator.Get()))
            return;
    }
    if (m_DeferredReleaseQueue) {
    ...
    if (m_DeviceLost) {
        CheckDeviceResult(m_CommandList->Close(), "Close(frame command list after device removal)");
        m_FrameCommandListClosed = true;
        if (m_DeferredReleaseQueue)
            m_DeferredReleaseQueue->AbortUnsubmittedFrame();
        m_IsRecording = false;
    ...
    const D3D12_CPU_DESCRIPTOR_HANDLE* depthDsvPtr = depthDsv.ptr != 0 ? &depthDsv : nullptr;
    ...
    if (depthDsv.ptr != 0) {
        m_CommandList->ClearDepthStencilView(depthDsv, D3D12_CLEAR_FLAG_DEPTH | D3D12_CLEAR_FLAG_STENCIL, 1.0f, 0, 0,
                                             nullptr);
    }
...
void D3D12Context::EndFrame() {
    ...
    const uint64_t fenceValue = m_NextFenceValue++;
    if (!CheckDeviceResult(m_CommandQueue->Signal(m_Fence.Get(), fenceValue), "Signal(frame)")) {
        if (m_DeferredReleaseQueue)
            m_DeferredReleaseQueue->AbandonSubmittedFrame();
        m_IsRecording = false;
        return;
    ...
    if (m_DeferredReleaseQueue)
        m_DeferredReleaseQueue->EndFrame(fenceValue);
...
bool D3D12Context::RecreateFrameCommandList(ID3D12CommandAllocator* allocator) {
    ...
    const HRESULT hr =
        m_Device->CreateCommandList(0, D3D12_COMMAND_LIST_TYPE_DIRECT, allocator, nullptr, IID_PPV_ARGS(&commandList));
    if (!CheckDeviceResult(hr, "CreateCommandList(frame recovery)"))
        return false;
...
bool D3D12Context::CheckDeviceResult(HRESULT hr, const char* operation) {
    ...
    Logger::Error(error);
    if (!m_DeviceLost)
        m_LastDeviceError = error;
    ReportDeviceRemovedReason(operation);
    if (!m_DeviceLost &&
        (hr == DXGI_ERROR_DEVICE_REMOVED || hr == DXGI_ERROR_DEVICE_RESET || hr == DXGI_ERROR_DEVICE_HUNG)) {
        m_DeviceLost = true;
...
bool D3D12Context::CanUseDevice(const char* operation) {
    if (m_Device && !m_DeviceLost)
        return true;
    if (m_DeviceLost && !m_DeviceLossSuppressionLogged) {
        Logger::Warn("D3D12: skipping ", operation,
                     " because the device was already removed; first error: ", m_LastDeviceError);
...
void D3D12Context::ReportDeviceRemovedReason(const char* operation) {
    ...
    if (!m_DeviceLost) {
        m_DeviceLost = true;
        m_LastDeviceError = std::string(operation) + " detected device removal: " + FormatHRESULT(reason);
        Logger::Error(m_LastDeviceError);
    } else {
        Logger::Error("D3D12 device removed reason after ", operation, ": ", FormatHRESULT(reason));
    }
    RHIDeviceLossReason stableReason = RHIDeviceLossReason::Unknown;
    if (reason == DXGI_ERROR_DEVICE_REMOVED)
        stableReason = RHIDeviceLossReason::Removed;
    else if (reason == DXGI_ERROR_DEVICE_RESET)
        stableReason = RHIDeviceLossReason::Reset;
    else if (reason == DXGI_ERROR_DEVICE_HUNG)
        stableReason = RHIDeviceLossReason::Hung;
    else if (reason == DXGI_ERROR_DRIVER_INTERNAL_ERROR)
        stableReason = RHIDeviceLossReason::DriverInternalError;
    m_DeviceLossInfo = {stableReason, static_cast<int64_t>(reason), m_DeviceGeneration, m_LastDeviceError};
    DumpDredDiagnostics();
}

void D3D12Context::DumpDredDiagnostics() {
    if (m_DredDumped || !m_Device)
        return;
    m_DredDumped = true;
    ...
    if (SUCCEEDED(breadcrumbsHr)) {
        ...
        for (const D3D12_AUTO_BREADCRUMB_NODE1* node = breadcrumbs.pHeadAutoBreadcrumbNode; node;
             node = node->pNext, ++nodeIndex) {
            const UINT completed = node->pLastBreadcrumbValue ? *node->pLastBreadcrumbValue : 0;
            const std::string queueName = node->pCommandQueueDebugNameA ? node->pCommandQueueDebugNameA
                                                                        : WideToUtf8(node->pCommandQueueDebugNameW);
            const std::string listName =
                node->pCommandListDebugNameA ? node->pCommandListDebugNameA : WideToUtf8(node->pCommandListDebugNameW);
            Logger::Error("DRED breadcrumb node #", nodeIndex, " queue=", queueName.empty() ? "<unnamed>" : queueName,
                          " list=", listName.empty() ? "<unnamed>" : listName, " completed=", completed, "/",
                          node->BreadcrumbCount);
            if (node->pCommandHistory && completed > 0 && completed <= node->BreadcrumbCount) {
                Logger::Error("DRED last completed op=", static_cast<uint32_t>(node->pCommandHistory[completed - 1]));
            }
            if (node->pCommandHistory && completed < node->BreadcrumbCount) {
                Logger::Error("DRED next pending op=", static_cast<uint32_t>(node->pCommandHistory[completed]));
            }
    ...
    if (SUCCEEDED(pageFaultHr)) {
        ...
        auto logAllocations = [](const char* label, const D3D12_DRED_ALLOCATION_NODE1* head) {
            ...
            for (auto* node = head; node && count < 64; node = node->pNext, ++count) {
                const std::string objectName = node->ObjectNameA ? node->ObjectNameA : WideToUtf8(node->ObjectNameW);
            ...
            if (!head)
                Logger::Error("DRED ", label, " allocations: none reported");
        };
...
std::shared_ptr<GpuBuffer> D3D12Context::CreateVertexBuffer(const void* data, uint32_t byteSize, uint32_t strideBytes) {
    if (!CanUseDevice("CreateVertexBuffer"))
        return nullptr;
    ...
    HRESULT hr = m_Device->CreateCommittedResource(&props, D3D12_HEAP_FLAG_NONE, &desc, D3D12_RESOURCE_STATE_COPY_DEST,
                                                   nullptr, IID_PPV_ARGS(&buf->resource));
    if (!CheckDeviceResult(hr, "CreateCommittedResource(vertex buffer)"))
        return nullptr;

    ComPtr<ID3D12Resource> uploadBuffer;
    D3D12_HEAP_PROPERTIES uploadProps = {};
    uploadProps.Type = D3D12_HEAP_TYPE_UPLOAD;
    hr = m_Device->CreateCommittedResource(&uploadProps, D3D12_HEAP_FLAG_NONE, &desc, D3D12_RESOURCE_STATE_GENERIC_READ,
                                           nullptr, IID_PPV_ARGS(&uploadBuffer));
    if (!CheckDeviceResult(hr, "CreateCommittedResource(vertex upload buffer)"))
        return nullptr;
    ...
    if (!CheckDeviceResult(hr, "Map(vertex upload buffer)") || !mapped) {
        if (!mapped)
            ReportDeviceRemovedReason("Map(vertex upload buffer) returned null");
        return nullptr;
    }
...
std::shared_ptr<GpuBuffer> D3D12Context::CreateIndexBuffer(const void* data, uint32_t byteSize) {
    if (!CanUseDevice("CreateIndexBuffer"))
        return nullptr;
    if (!data || byteSize == 0)
        return nullptr;
    ...
    HRESULT hr = m_Device->CreateCommittedResource(&props, D3D12_HEAP_FLAG_NONE, &desc, D3D12_RESOURCE_STATE_COPY_DEST,
                                                   nullptr, IID_PPV_ARGS(&buf->resource));
    if (!CheckDeviceResult(hr, "CreateCommittedResource(index buffer)"))
        return nullptr;
    ...
    hr = m_Device->CreateCommittedResource(&uploadProps, D3D12_HEAP_FLAG_NONE, &desc, D3D12_RESOURCE_STATE_GENERIC_READ,
                                           nullptr, IID_PPV_ARGS(&uploadBuffer));
    if (!CheckDeviceResult(hr, "CreateCommittedResource(index upload buffer)"))
        return nullptr;
    ...
    if (!CheckDeviceResult(hr, "Map(index upload buffer)") || !mapped) {
        if (!mapped)
            ReportDeviceRemovedReason("Map(index upload buffer) returned null");
        return nullptr;
    }
    std::memcpy(mapped, data, byteSize);
    uploadBuffer->Unmap(0, nullptr);
...
std::shared_ptr<GpuBuffer> D3D12Context::CreateBuffer(const RHIBufferDesc& desc, const void* initialData) {
    if (!CanUseDevice("CreateBuffer") || desc.size == 0)
        return nullptr;
    ...
    const bool copyDestinationOnly = HasUsage(desc.usage, RHIResourceUsage::CopyDestination) &&
                                     ...
                                     !HasUsage(desc.usage, RHIResourceUsage::ShaderResource) && !unorderedAccess;
    ...
    const D3D12_RESOURCE_STATES initialState = initialData && !readback ? D3D12_RESOURCE_STATE_COPY_DEST : finalState;

    HRESULT hr = m_Device->CreateCommittedResource(&heap, D3D12_HEAP_FLAG_NONE, &native, initialState, nullptr,
    ...
    if (initialData && !readback) {
        ...
        if (!CheckDeviceResult(hr, "Map(generic upload buffer)") || !mapped) {
            if (!mapped)
                ReportDeviceRemovedReason("Map(generic upload buffer) returned null");
            return nullptr;
        }
...
std::shared_ptr<GpuBufferView> D3D12Context::CreateBufferView(const std::shared_ptr<GpuBuffer>& buffer,
                                                              const RHIBufferViewDesc& desc) {
    if (!CanUseDevice("CreateBufferView"))
        return nullptr;
    auto nativeBuffer = std::dynamic_pointer_cast<D3D12Buffer>(buffer);
    if (!nativeBuffer || !nativeBuffer->resource || nativeBuffer->desc.stride == 0)
        return nullptr;
    auto view = std::make_shared<D3D12BufferView>();
    view->buffer = buffer;
    view->desc = desc;
    ...
    if (HasUsage(desc.usage, RHIResourceUsage::ShaderResource)) {
        ...
        if (view->srvCpu.ptr == 0 || view->srvGpu.ptr == 0)
            return nullptr;
    ...
    if (HasUsage(desc.usage, RHIResourceUsage::UnorderedAccess)) {
        ...
        if (view->uavCpu.ptr == 0 || view->uavGpu.ptr == 0)
            return nullptr;
...
bool CreateTextureRootSignature(ID3D12Device* device, ID3D12RootSignature** outRootSig) {
    ...
    HRESULT hr = D3D12SerializeRootSignature(&rsDesc, D3D_ROOT_SIGNATURE_VERSION_1, &serializedRootSig, &errorBlob);
    if (FAILED(hr)) {
        if (errorBlob) {
...
bool D3D12Context::CreateMainDepthBuffer() {
    if (!CanUseDevice("CreateMainDepthBuffer") || m_SwapChainWidth == 0 || m_SwapChainHeight == 0) {
        return false;
    }
    ...
    HRESULT hr =
        m_Device->CreateCommittedResource(&heapProps, D3D12_HEAP_FLAG_NONE, &desc, D3D12_RESOURCE_STATE_DEPTH_WRITE,
                                          &clearValue, IID_PPV_ARGS(&m_MainDepthBuffer));
    if (!CheckDeviceResult(hr, "CreateCommittedResource(main depth buffer)"))
        return false;

    m_MainDsvHandle = AllocDsvSlot(&m_MainDsvLease);
    if (m_MainDsvHandle.ptr == 0)
        return false;
...
void D3D12Context::PushRenderTarget(const D3D12_CPU_DESCRIPTOR_HANDLE* colorRtv, D3D12_CPU_DESCRIPTOR_HANDLE depthDsv) {
    if (!m_IsRecording || !m_CommandList)
        return;
...
void D3D12Context::PopRenderTarget() {
    if (!m_IsRecording || !m_CommandList || m_RenderTargetStack.empty())
        return;
    ...
    if (restore.hasColorTarget && restore.colorRtv.ptr != 0) {
        const D3D12_CPU_DESCRIPTOR_HANDLE* dsv = restore.depthDsv.ptr != 0 ? &restore.depthDsv : nullptr;
...
std::shared_ptr<GpuTexture> D3D12Context::CreateDepthTexture(int width, int height, bool cube, uint32_t arraySize) {
    if (!CanUseDevice("CreateDepthTexture") || width <= 0 || height <= 0)
        return nullptr;
    ...
    if (!CheckDeviceResult(hr, "CreateCommittedResource(depth texture)"))
        return nullptr;

    tex->srvCpu = AllocSrvSlot(tex->srvGpu, &tex->srvLease);
    if (tex->srvCpu.ptr == 0 || tex->srvGpu.ptr == 0)
        return nullptr;
    ...
    if (cube) {
        ...
        for (uint32_t face = 0; face < 6; ++face) {
            ...
            m_Device->CreateDepthStencilView(tex->resource.Get(), &dsvDesc, tex->dsvFaces[face]);
        }
        ...
        for (uint32_t slice = 0; slice < arraySize; ++slice) {
            ...
            dsvDesc.Texture2DArray.MipSlice = 0;
            m_Device->CreateDepthStencilView(tex->resource.Get(), &dsvDesc, tex->dsvFaces[slice]);
        }
        tex->dsvCpu = tex->dsvFaces[0];
    } else {
...
bool D3D12Context::BuildShaderPipelines(D3D12Shader& shader, const D3D12_SHADER_BYTECODE& vs,
                                        const D3D12_SHADER_BYTECODE& ps, const VertexElement* layout,
...
std::shared_ptr<GpuShader> D3D12Context::CreateShader(const std::string& hlslSource, const std::string& vsEntry,
                                                      const std::string& psEntry, const VertexElement* layout,
                                                      uint32_t layoutCount) {
    if (!CanUseDevice("CreateShader"))
        return nullptr;
    if (layoutCount > 0 && !layout)
        return nullptr;

    auto sh = std::make_shared<D3D12Shader>();
    sh->deferredReleaseQueue = m_DeferredReleaseQueue;
    if (layout && layoutCount)
        sh->vertexLayout.assign(layout, layout + layoutCount);
    ...
    if (!compileShader(psEntry, "ps_5_0", psBlob))
        return nullptr;
    sh->vertexBytecode.assign(static_cast<const uint8_t*>(vsBlob->GetBufferPointer()),
    ...
    const D3D12_SHADER_BYTECODE vs = {vsBlob->GetBufferPointer(), vsBlob->GetBufferSize()};
    const D3D12_SHADER_BYTECODE ps = {psBlob->GetBufferPointer(), psBlob->GetBufferSize()};
...
std::shared_ptr<GpuShader> D3D12Context::CreateShaderFromBytecode(const void* vsBytecode, size_t vsSize,
                                                                  const void* psBytecode, size_t psSize,
                                                                  const VertexElement* layout, uint32_t layoutCount) {
    if (!CanUseDevice("CreateShaderFromBytecode"))
        return nullptr;
...
std::shared_ptr<GpuGraphicsPipeline> D3D12Context::CreateGraphicsPipeline(const GraphicsPipelineDesc& desc) {
    if (!CanUseDevice("CreateGraphicsPipeline"))
        return nullptr;
    auto shader = std::dynamic_pointer_cast<D3D12Shader>(desc.shader);
    if (!m_Device || !shader || !shader->rootSignature || shader->vertexBytecode.empty() ||
        shader->pixelBytecode.empty())
        return nullptr;
    ...
    for (size_t i = 0; i < shader->vertexLayout.size(); ++i) {
        ...
        inputElements[i] = {element.semantic,
                            element.index,
    ...
    native.RasterizerState.FillMode =
        desc.rasterizer.fillMode == RHIFillMode::Wireframe ? D3D12_FILL_MODE_WIREFRAME : D3D12_FILL_MODE_SOLID;
    native.RasterizerState.CullMode = desc.rasterizer.cullMode == RHICullMode::None    ? D3D12_CULL_MODE_NONE
                                      : desc.rasterizer.cullMode == RHICullMode::Front ? D3D12_CULL_MODE_FRONT
                                                                                       : D3D12_CULL_MODE_BACK;
    native.RasterizerState.FrontCounterClockwise = desc.rasterizer.frontFace == RHIFrontFace::CounterClockwise;
    ...
    native.RasterizerState.MultisampleEnable = desc.rasterizer.multisampleEnable || desc.multisample.sampleCount > 1;
    ...
    native.DSVFormat = desc.depthFormat == RHIFormat::Unknown ? DXGI_FORMAT_UNKNOWN : ToDxgiRHIFormat(desc.depthFormat);
    ...
    const HRESULT pipelineHr = m_Device->CreateGraphicsPipelineState(&native, IID_PPV_ARGS(&pipeline->pipelineState));
    if (!CheckDeviceResult(pipelineHr, "CreateGraphicsPipelineState(RHI pipeline)"))
        return nullptr;
    return pipeline;
}

void D3D12Context::ApplyBoundPipelineState() {
    if (!m_IsRecording || !m_BoundShader || m_DepthOnlyBound)
        return;
...
void D3D12Context::SetVSConstants(const void* data, uint32_t byteSize) {
    ...
    if (alignedOffset + alignedSize > fr.constantBufferCapacity) {
        ...
        while (newCap < alignedOffset + alignedSize)
            newCap *= 2;
        ...
        HRESULT hr = m_Device->CreateCommittedResource(
            &props, D3D12_HEAP_FLAG_NONE, &desc, D3D12_RESOURCE_STATE_GENERIC_READ, nullptr, IID_PPV_ARGS(&newRes));
...
void D3D12Context::DrawIndexed(uint32_t indexCount, uint32_t startIndex, uint32_t baseVertex) {
    if (!m_IsRecording)
        return;
    m_CommandList->DrawIndexedInstanced(indexCount, 1, startIndex, baseVertex, 0);
}

void D3D12Context::DrawInstanced(uint32_t vertexCount, uint32_t instanceCount, uint32_t startVertex) {
    if (!m_IsRecording)
        return;
    m_CommandList->DrawInstanced(vertexCount, instanceCount, startVertex, 0);
}

void D3D12Context::DrawIndexedInstanced(uint32_t indexCount, uint32_t instanceCount, uint32_t startIndex,
                                        uint32_t baseVertex) {
    if (!m_IsRecording)
        return;
    m_CommandList->DrawIndexedInstanced(indexCount, instanceCount, startIndex, baseVertex, 0);
}
...
void D3D12Context::EnsureDefaultResources() {
    if (m_DefaultTexture || !CanUseDevice("EnsureDefaultResources"))
        return;
    ...
    // --- Default 1x1 black texture ---
    {
        ...
        if (!CheckDeviceResult(hr, "CreateCommittedResource(default texture)"))
            return;
    }
...
D3D12_CPU_DESCRIPTOR_HANDLE D3D12Context::AllocSrvSlot(D3D12_GPU_DESCRIPTOR_HANDLE& outGpu,
    ...
    if (!m_SrvDescriptorPool || !m_SrvDescriptorPool->Allocate(m_DeferredReleaseQueue, cpu, outGpu, lease)) {
...
void D3D12Context::PushRenderTargets(uint32_t colorCount, const D3D12_CPU_DESCRIPTOR_HANDLE* colorRtvs,
...
D3D12_CPU_DESCRIPTOR_HANDLE D3D12Context::AllocRtvSlot(std::shared_ptr<D3D12DescriptorLease>* lease) {
    D3D12_CPU_DESCRIPTOR_HANDLE cpu{};
    D3D12_GPU_DESCRIPTOR_HANDLE gpu{};
    if (!m_RtvDescriptorPool || !m_RtvDescriptorPool->Allocate(m_DeferredReleaseQueue, cpu, gpu, lease)) {
...
D3D12_CPU_DESCRIPTOR_HANDLE D3D12Context::AllocDsvSlot(std::shared_ptr<D3D12DescriptorLease>* lease) {
    D3D12_CPU_DESCRIPTOR_HANDLE cpu{};
    D3D12_GPU_DESCRIPTOR_HANDLE gpu{};
    if (!m_DsvDescriptorPool || !m_DsvDescriptorPool->Allocate(m_DeferredReleaseQueue, cpu, gpu, lease)) {
...
void D3D12Context::BindPSTextureDescriptors(uint32_t slot, D3D12_GPU_DESCRIPTOR_HANDLE srvGpu,
    ...
    const D3D12_GPU_DESCRIPTOR_HANDLE srv = srvGpu.ptr ? srvGpu : m_DefaultTexSrvGpu;
    const D3D12_GPU_DESCRIPTOR_HANDLE samp = sampGpu.ptr ? sampGpu : m_DefaultSampGpu;
...
std::shared_ptr<GpuShader> D3D12Context::CreateFullscreenShaderFromBytecode(const void* vsBytecode, size_t vsSize,
                                                                            const void* psBytecode, size_t psSize,
                                                                            DXGI_FORMAT rtvFormat) {
    (void)rtvFormat;
    return CreateShaderFromBytecode(vsBytecode, vsSize, psBytecode, psSize, nullptr, 0);
}
D3D12_CPU_DESCRIPTOR_HANDLE D3D12Context::AllocSampSlot(D3D12_GPU_DESCRIPTOR_HANDLE& outGpu,
                                                        std::shared_ptr<D3D12DescriptorLease>* lease) {
    D3D12_CPU_DESCRIPTOR_HANDLE cpu{};
    if (!m_SamplerDescriptorPool || !m_SamplerDescriptorPool->Allocate(m_DeferredReleaseQueue, cpu, outGpu, lease)) {
        Logger::Error("[RHI] D3D12 sampler descriptor heap exhausted unique=", m_UniqueSamplerDescriptorCount,
                      " capacity=", kDefaultSamplerDescriptorCount);
        outGpu = {};
        return {};
    }
    return cpu;
...
std::shared_ptr<GpuTexture> D3D12Context::UploadTexture2D(const void* rgba8Data, int width, int height) {
    ...
    ComPtr<ID3D12Resource> texResource;
    {
        ...
        HRESULT hr = m_Device->CreateCommittedResource(&hp, D3D12_HEAP_FLAG_NONE, &desc, D3D12_RESOURCE_STATE_COPY_DEST,
    ...
    ComPtr<ID3D12Resource> uploadBuffer;
    {
        ...
        HRESULT hr = m_Device->CreateCommittedResource(
            &hp, D3D12_HEAP_FLAG_NONE, &desc, D3D12_RESOURCE_STATE_GENERIC_READ, nullptr, IID_PPV_ARGS(&uploadBuffer));
...

    {
        ...
        if (!CheckDeviceResult(mapHr, "Map(texture upload buffer)") || !mapped) {
            if (!mapped)
                ReportDeviceRemovedReason("Map(texture upload buffer) returned null");
            return nullptr;
        }
    ...
    {
        if (!CheckDeviceResult(m_UploadCommandAllocator->Reset(), "Reset(texture upload command allocator)"))
            return nullptr;
        if (!CheckDeviceResult(m_UploadCommandList->Reset(m_UploadCommandAllocator.Get(), nullptr),
        ...
        if (!CheckDeviceResult(m_UploadCommandList->Close(), "Close(texture upload command list)"))
            return nullptr;
        ...
        if (!CheckDeviceResult(m_CommandQueue->Signal(m_Fence.Get(), fenceVal), "Signal(texture upload)"))
            return nullptr;
        if (m_Fence->GetCompletedValue() < fenceVal) {
            if (!CheckDeviceResult(m_Fence->SetEventOnCompletion(fenceVal, m_FenceEvent),
                                   "SetEventOnCompletion(texture upload)"))
                return nullptr;
            WaitForSingleObject(m_FenceEvent, INFINITE);
        }
    }
...
bool D3D12Context::UpdateBuffer(const std::shared_ptr<GpuBuffer>& buffer, uint64_t offset, const void* data,
                                uint64_t size) {
    auto destination = std::dynamic_pointer_cast<D3D12Buffer>(buffer);
    if (!destination || !destination->resource || !data || !size || offset + size > destination->desc.size ||
        !CanUseDevice("UpdateBuffer"))
        return false;
    D3D12_HEAP_PROPERTIES heap{};
    heap.Type = D3D12_HEAP_TYPE_UPLOAD;
    D3D12_RESOURCE_DESC desc{};
    desc.Dimension = D3D12_RESOURCE_DIMENSION_BUFFER;
    desc.Width = (size + 255ull) & ~255ull;
    desc.Height = 1;
    desc.DepthOrArraySize = 1;
    desc.MipLevels = 1;
    desc.SampleDesc.Count = 1;
    ...
    if (FAILED(m_Device->CreateCommittedResource(&heap, D3D12_HEAP_FLAG_NONE, &desc, D3D12_RESOURCE_STATE_GENERIC_READ,
...
std::shared_ptr<GpuTexture> D3D12Context::UploadTexture(const RHITextureDesc& desc,
                                                        const RHITextureSubresourceData* data, uint32_t count) {
    const uint32_t subresourceCount = desc.mipLevels * desc.arrayLayers;
    if (!data || count != subresourceCount || desc.sampleCount > 1)
        return nullptr;
    ...
    UINT64 totalSize = 0;
    m_Device->GetCopyableFootprints(&nativeDesc, 0, subresourceCount, 0, footprints.data(), rows.data(),
    ...
    for (uint32_t i = 0; i < count; ++i) {
        ...
        if (!src.data || index >= subresourceCount || uploaded[index] || src.rowPitch < rowSizes[index]) {
            upload->Unmap(0, nullptr);
            return nullptr;
        }
        uploaded[index] = true;
        ...
        for (UINT row = 0; row < rows[index]; ++row)
            std::memcpy(dstSlice + static_cast<size_t>(row) * footprints[index].Footprint.RowPitch,
                        srcSlice + static_cast<size_t>(row) * src.rowPitch, static_cast<size_t>(rowSizes[index]));
    }
    upload->Unmap(0, nullptr);
    if (FAILED(m_UploadCommandAllocator->Reset()) ||
        FAILED(m_UploadCommandList->Reset(m_UploadCommandAllocator.Get(), nullptr)))
...
bool D3D12Context::IsFormatSupported(RHIFormat format, RHIResourceUsage usage) const {
    ...
    if (FAILED(m_Device->CheckFeatureSupport(D3D12_FEATURE_FORMAT_SUPPORT, &support, sizeof(support))))
        return false;
    if (HasUsage(usage, RHIResourceUsage::ShaderResource) && !(support.Support1 & D3D12_FORMAT_SUPPORT1_SHADER_SAMPLE))
        return false;
    if (HasUsage(usage, RHIResourceUsage::RenderTarget) && !(support.Support1 & D3D12_FORMAT_SUPPORT1_RENDER_TARGET))
        return false;
    if (HasUsage(usage, RHIResourceUsage::DepthStencil) && !(support.Support1 & D3D12_FORMAT_SUPPORT1_DEPTH_STENCIL))
        return false;
...
std::shared_ptr<GpuFence> D3D12Context::CreateFence(uint64_t initialValue) {
    ...
    if (FAILED(m_Device->CreateFence(initialValue, D3D12_FENCE_FLAG_NONE, IID_PPV_ARGS(&fence))))
        return nullptr;
...
std::shared_ptr<GpuTimestampQueryPool> D3D12Context::CreateTimestampQueryPool(uint32_t count) {
    if (!m_Device || !m_CommandQueue || count == 0)
        return nullptr;
    auto pool = std::make_shared<D3D12TimestampPool>();
    pool->count = count;
    D3D12_QUERY_HEAP_DESC query{};
    query.Count = count;
    query.Type = D3D12_QUERY_HEAP_TYPE_TIMESTAMP;
    if (FAILED(m_Device->CreateQueryHeap(&query, IID_PPV_ARGS(&pool->heap))))
        return nullptr;
    D3D12_HEAP_PROPERTIES heap{};
    heap.Type = D3D12_HEAP_TYPE_READBACK;
    D3D12_RESOURCE_DESC desc{};
    desc.Dimension = D3D12_RESOURCE_DIMENSION_BUFFER;
    desc.Width = static_cast<uint64_t>(count) * sizeof(uint64_t);
    desc.Height = 1;
    desc.DepthOrArraySize = 1;
    desc.MipLevels = 1;
    desc.SampleDesc.Count = 1;
    ...
    if (FAILED(m_Device->CreateCommittedResource(&heap, D3D12_HEAP_FLAG_NONE, &desc, D3D12_RESOURCE_STATE_COPY_DEST,
                                                 nullptr, IID_PPV_ARGS(&pool->readback))))
        return nullptr;
...
bool CreateComputeRootSignature(ID3D12Device* device, ID3D12RootSignature** outRootSig) {
    ...
    for (uint32_t slot = 0; slot < storageSlots; ++slot) {
        auto& uav = ranges[textureSlots * 2 + slot];
...
std::shared_ptr<GpuTexture> D3D12Context::CreateTexture(const RHITextureDesc& desc) {
    ...
    if (FAILED(m_Device->CheckFeatureSupport(D3D12_FEATURE_MULTISAMPLE_QUALITY_LEVELS, &msaa, sizeof(msaa))) ||
    ...
    clear.Format = ToDxgiRHIFormat(desc.format);
    if (HasUsage(desc.usage, RHIResourceUsage::DepthStencil))
        clear.DepthStencil.Depth = 1.0f;
    auto result = std::make_shared<D3D12Texture>();
    result->desc = desc;
    result->deferredReleaseQueue = m_DeferredReleaseQueue;
    result->arraySize = desc.arrayLayers;
    result->isCube = desc.cube;
    const D3D12_CLEAR_VALUE* clearPtr =
        (native.Flags & (D3D12_RESOURCE_FLAG_ALLOW_RENDER_TARGET | D3D12_RESOURCE_FLAG_ALLOW_DEPTH_STENCIL)) ? &clear
                                                                                                             : nullptr;
    const HRESULT textureHr = m_Device->CreateCommittedResource(
        &heap, D3D12_HEAP_FLAG_NONE, &native, D3D12_RESOURCE_STATE_COMMON, clearPtr, IID_PPV_ARGS(&result->resource));
    if (!CheckDeviceResult(textureHr, "CreateCommittedResource(texture)"))
        return nullptr;
    if (HasUsage(desc.usage, RHIResourceUsage::ShaderResource)) {
        ...
        if (!view)
            return nullptr;
        result->srvCpu = view->srvCpu;
        result->srvGpu = view->srvGpu;
        ...
        result->sampCpu = sampler->cpu;
        result->sampGpu = sampler->gpu;
...
std::shared_ptr<GpuTextureView> D3D12Context::CreateTextureView(const std::shared_ptr<GpuTexture>& texture,
                                                                const RHITextureViewDesc& desc) {
    if (!CanUseDevice("CreateTextureView"))
        return nullptr;
    auto nativeTexture = std::dynamic_pointer_cast<D3D12Texture>(texture);
    if (!nativeTexture || !nativeTexture->resource)
        return nullptr;
    auto view = std::make_shared<D3D12TextureView>();
    ...
    if (HasUsage(desc.usage, RHIResourceUsage::RenderTarget)) {
        ...
        if (view->rtvCpu.ptr == 0)
            return nullptr;
        D3D12_RENDER_TARGET_VIEW_DESC d{};
        d.Format = ToDxgiRHIFormat(td.format);
        if (td.sampleCount > 1 && td.arrayLayers > 1) {
...
std::shared_ptr<GpuSampler> D3D12Context::CreateSampler(const RHISamplerDesc& desc) {
    ...
    auto result = std::make_shared<D3D12Sampler>();
    result->desc = desc;
    result->cpu = AllocSampSlot(result->gpu, &result->lease);
    if (result->cpu.ptr == 0 || result->gpu.ptr == 0) {
        Logger::Error("[RHI] D3D12 failed to allocate sampler descriptor desc=(", SamplerFilterName(desc.filter), ",",
                      SamplerAddressName(desc.addressU), ",", SamplerAddressName(desc.addressV), ",",
                      SamplerAddressName(desc.addressW), ") unique=", m_UniqueSamplerDescriptorCount,
                      " capacity=", kDefaultSamplerDescriptorCount);
        return nullptr;
    }
    D3D12_SAMPLER_DESC d{};
    d.Filter = desc.filter == RHIFilter::Point              ? D3D12_FILTER_MIN_MAG_MIP_POINT
               : desc.filter == RHIFilter::ComparisonLinear ? D3D12_FILTER_COMPARISON_MIN_MAG_LINEAR_MIP_POINT
                                                            : D3D12_FILTER_MIN_MAG_MIP_LINEAR;
    auto address = [](RHIAddressMode m) {
        return m == RHIAddressMode::Clamp    ? D3D12_TEXTURE_ADDRESS_MODE_CLAMP
               : m == RHIAddressMode::Border ? D3D12_TEXTURE_ADDRESS_MODE_BORDER
...
std::shared_ptr<GpuReadbackTicket> D3D12Context::ReadbackBufferAsync(const std::shared_ptr<GpuBuffer>& buffer) {
    if (!CanUseDevice("ReadbackBufferAsync"))
        return nullptr;
    auto source = std::dynamic_pointer_cast<D3D12Buffer>(buffer);
    if (!source || !source->resource || source->desc.size == 0 || !m_IsRecording)
        return nullptr;
    D3D12_HEAP_PROPERTIES heap{};
    heap.Type = D3D12_HEAP_TYPE_READBACK;
    D3D12_RESOURCE_DESC desc{};
    ...
    if (!CheckDeviceResult(readbackHr, "CreateCommittedResource(readback buffer)"))
        return nullptr;
    m_CommandList->CopyBufferRegion(readback.Get(), 0, source->resource.Get(), 0, source->desc.size);
    return std::make_shared<D3D12ReadbackTicket>(std::move(readback), m_Fence, m_NextFenceValue, source->desc.size,
                                                 m_DeferredReleaseQueue);
}

std::shared_ptr<GpuTextureReadbackTicket> D3D12Context::ReadbackTextureAsync(const std::shared_ptr<GpuTexture>& texture,
    ...
    D3D12_BOX box{requested.x, requested.y, requested.z, requested.x + width, requested.y + height, requested.z + 1};
    const D3D12_RESOURCE_STATES sourceState = HasUsage(source->desc.usage, RHIResourceUsage::ShaderResource)
                                                  ? D3D12_RESOURCE_STATE_PIXEL_SHADER_RESOURCE
                                                  : D3D12_RESOURCE_STATE_COMMON;
    ...
    m_CommandList->ResourceBarrier(1, &toCopy);
    return std::make_shared<D3D12TextureReadbackTicket>(std::move(readback), m_Fence, m_NextFenceValue, width, height,
                                                        width * bpp, footprint.Footprint.RowPitch, source->desc.format,
                                                        m_DeferredReleaseQueue);
}
