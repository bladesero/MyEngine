...
MYENGINE_RUNTIME_API bool CollectDependencies(const std::filesystem::path& source,
                                              const std::filesystem::path& allowedRoot,
                                              std::vector<std::string>& outDependencies, std::string* error = nullptr);
MYENGINE_RUNTIME_API std::string BuildCacheKey(const std::filesystem::path& source,
                                               const std::filesystem::path& allowedRoot,
                                               const std::vector<ShaderBackend>& backends,
                                               const std::string& targetPlatform, const std::string& settingsJson,
