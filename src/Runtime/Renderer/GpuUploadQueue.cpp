...
GpuUploadFence GpuUploadQueue::Enqueue(UploadTask task, uint64_t estimatedBytes) {
    ...
    ++impl->stats.enqueuedTasks;
    impl->stats.enqueuedBytes += estimatedBytes;
    impl->stats.pendingTasks = impl->tasks.size();
    impl->stats.pendingBytes += estimatedBytes;
...
size_t GpuUploadQueue::Process(IRHIDevice& device, size_t maxTasks) {
    GpuUploadBudget budget;
    budget.maxTasks = maxTasks;
    budget.maxBytes = static_cast<uint64_t>(-1);
    budget.maxMilliseconds = 0.0;
...
size_t GpuUploadQueue::Process(IRHIDevice& device, const GpuUploadBudget& budget) {
    ...
    while (processed < budget.maxTasks) {
        Impl::Pending pending;
        {
            ...
            if (impl->tasks.empty())
                break;
            if (processed > 0 && bytes + impl->tasks.front().bytes > budget.maxBytes) {
                ++impl->stats.byteBudgetDeferrals;
                break;
            }
            pending = std::move(impl->tasks.front());
            impl->tasks.pop_front();
            impl->stats.pendingTasks = impl->tasks.size();
            impl->stats.pendingBytes =
                impl->stats.pendingBytes >= pending.bytes ? impl->stats.pendingBytes - pending.bytes : 0;
        }
        try {
            pending.task(device);
        } catch (...) {
        ...
        }
        {
            std::lock_guard<std::mutex> lock(impl->mutex);
            impl->completed = std::max(impl->completed, pending.sequence);
            ++impl->stats.processedTasks;
            impl->stats.processedBytes += pending.bytes;
        }
        bytes += pending.bytes;
        ++processed;
        if (budget.maxMilliseconds > 0.0 &&
            std::chrono::duration<double, std::milli>(std::chrono::steady_clock::now() - start).count() >=
                budget.maxMilliseconds) {
            std::lock_guard<std::mutex> lock(impl->mutex);
            if (!impl->tasks.empty())
                ++impl->stats.timeBudgetDeferrals;
            break;
        }
    }
    {
        std::lock_guard<std::mutex> lock(impl->mutex);
        if (processed >= budget.maxTasks && !impl->tasks.empty())
            ++impl->stats.taskBudgetDeferrals;
    }
    return processed;
}
...
void GpuUploadQueue::Clear() {
    ...
    std::lock_guard<std::mutex> lock(impl->mutex);
    impl->tasks.clear();
    impl->stats.pendingTasks = 0;
    impl->stats.pendingBytes = 0;
...
void GpuUploadQueue::ResetStatistics() {
    Impl* impl = Get().m_Impl;
    std::lock_guard<std::mutex> lock(impl->mutex);
    const size_t tasks = impl->tasks.size();
    uint64_t bytes = 0;
    for (const auto& task : impl->tasks)
        bytes += task.bytes;
    impl->stats = {};
    impl->stats.pendingTasks = tasks;
    impl->stats.pendingBytes = bytes;
    impl->stats.peakPendingTasks = tasks;
    impl->stats.peakPendingBytes = bytes;
}
