...
public:
    ...
    virtual bool Submit(GpuCommandList* commandList, GpuFence* signalFence, uint64_t signalValue) = 0;
...
public:
    ...
    virtual bool ReadResults(uint32_t first, uint32_t count, std::vector<uint64_t>& ticks) = 0;
};
