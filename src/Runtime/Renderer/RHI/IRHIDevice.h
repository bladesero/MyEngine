...
public:
    ...
    virtual std::shared_ptr<GpuBuffer> CreateBuffer(
        const RHIBufferDesc&, const void* initialData = nullptr) { return nullptr; }
    virtual std::shared_ptr<GpuBufferView> CreateBufferView(
        const std::shared_ptr<GpuBuffer>&, const RHIBufferViewDesc&) { return nullptr; }
    ...
    virtual std::shared_ptr<GpuShader> CreateShaderFromBytecode(
        ...
        const VertexElement*, uint32_t) { return nullptr; }
    virtual std::shared_ptr<GpuShader> CreateComputeShaderFromBytecode(
    ...
    virtual std::shared_ptr<GpuBindGroup> CreateBindGroup(
        const std::shared_ptr<GpuShader>& shader) {
        return shader ? std::make_shared<GpuBindGroup>(shader) : nullptr;
    }
    virtual std::shared_ptr<GpuGraphicsPipeline> CreateGraphicsPipeline(
        const GraphicsPipelineDesc& desc) {
        if (!desc.shader) return nullptr;
        auto pipeline = std::make_shared<GpuGraphicsPipeline>(); pipeline->desc = desc;
        return pipeline;
    }
    virtual std::shared_ptr<GpuComputePipeline> CreateComputePipeline(
