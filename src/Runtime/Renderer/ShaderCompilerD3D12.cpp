...
static bool CompileOneStageFromFile(const std::wstring& widePath, const std::string& filePath, const std::string& entry,
    ...
    const HRESULT hr =
        D3DCompileFromFile(widePath.c_str(), defines.empty() ? nullptr : macros.data(),
                           D3D_COMPILE_STANDARD_FILE_INCLUDE, entry.c_str(), target, flags, 0, &shaderBlob, &errBlob);
...
bool ShaderCompilerD3D12::CompileProgramFromFile(const std::string& filePath, const std::string& vsEntry,
...
bool ShaderCompilerD3D12::CompileStageFromFile(const std::string& filePath, const std::string& entry,
