...
public:
    explicit ScopedProjectRoot(const fs::path& root)
        : m_Previous(AssetManager::Get().GetProjectRoot())
    {
...
void Add(ProjectValidationReport& report, ProjectValidationSeverity severity,
         ProjectValidationCode code, std::string path, std::string referrer,
         std::string message)
{
    report.issues.push_back(
        {severity, code, std::move(path), std::move(referrer), std::move(message)});
}
}

size_t ProjectValidationReport::ErrorCount() const
{
    return static_cast<size_t>(std::count_if(issues.begin(), issues.end(),
...
std::string ProjectValidationReport::Summary() const
{
    ...
    result += " (" + std::to_string(ErrorCount()) + " errors, " +
    ...
    if (!Passed()) {
        ...
        if (first != issues.end()) result += ": " + first->message;
    }
    return result;
}

bool ProjectValidator::Validate(const ProjectConfig& project,
                                ...
                                const ProjectValidationOptions& options)
{
    ...
    if (root.empty() || !fs::is_directory(root, ec) || ec) {
        Add(report, ProjectValidationSeverity::Error,
            ProjectValidationCode::InvalidProject, root.generic_string(), {},
    ...
    if (!startupResolved || !fs::is_regular_file(startupScene, ec) || ec) {
        if (startupResolved)
            startupError = "startup scene does not exist inside the project: " +
                startupScene.generic_string();
        Add(report, ProjectValidationSeverity::Error,
            ProjectValidationCode::InvalidStartupScene, project.GetStartupScene(),
            project.GetManifestPath().generic_string(), startupError);
    }
    ...
    for (const PublishIssue& issue : report.preflight.errors) {
        Add(report, ProjectValidationSeverity::Error,
            ProjectValidationCode::CookDependency, issue.path, issue.referrer,
            issue.message);
    }
    ...
    if (!ContentPathPolicy::Enumerate(root / "Content", files, enumeratedBytes,
                                      &enumerationError)) {
        Add(report, ProjectValidationSeverity::Error,
            ProjectValidationCode::InvalidProject, "Content", {}, enumerationError);
    } else {
        ...
        for (const ContentFileInfo& file : files) {
            const std::string logical =
                (fs::path("Content") / file.relative).generic_string();
            if (options.oversizedAssetWarningBytes > 0 &&
                file.size > options.oversizedAssetWarningBytes) {
                Add(report, ProjectValidationSeverity::Warning,
                    ProjectValidationCode::OversizedAsset, logical, {},
                    "asset is larger than the configured warning threshold (" +
            ...
            if (extension != ".as" ||
                file.relative.generic_string().rfind("Editor/", 0) == 0) continue;
            const std::shared_ptr<ScriptAsset> script =
                LoadScriptAssetFromFile(file.absolute.string());
            if (!script || !script->GetLastError().empty()) {
                Add(report, ProjectValidationSeverity::Error,
                    ProjectValidationCode::ScriptCompile, logical, {},
    ...
    std::sort(report.issues.begin(), report.issues.end(),
        [](const ProjectValidationIssue& left, const ProjectValidationIssue& right) {
            ...
            if (left.referrer != right.referrer) return left.referrer < right.referrer;
