...
std::vector<RenderBackend> AvailableEditorBackends()
{
    ...
    for (RenderBackend backend : {RenderBackend::D3D11, RenderBackend::D3D12,
        ...
        if (IsBackendCompiled(backend)) backends.push_back(backend);
    }
    return backends;
}
...
int BackendIndexFromProjectValue(const std::string& value)
{
    ...
    for (size_t i = 0; i < backends.size(); ++i) {
        if (requested && backends[i] == *requested) return static_cast<int>(i);
    }
    for (size_t i = 0; i < backends.size(); ++i) {
        if (backends[i] == kDefaultRenderBackend) return static_cast<int>(i);
    }
    return 0;
}
}
...
void EditorProjectSettingsController::DrawGraphicsSettingsTab(EditorLayer& layer)
{
#if defined(MYENGINE_ENABLE_IMGUI)
    const auto backends = AvailableEditorBackends();
    if (layer.m_GraphicsBackendIndex < 0 ||
        layer.m_GraphicsBackendIndex >= static_cast<int>(backends.size())) {
        layer.m_GraphicsBackendIndex = 0;
    }
    const RenderBackend selectedBackend = backends.empty()
        ? kDefaultRenderBackend
        : backends[static_cast<size_t>(layer.m_GraphicsBackendIndex)];
    ...
    if (layer.m_RenderPathIndex >= kRenderPathCount) layer.m_RenderPathIndex = 0;
    ImGui::Combo("Render Path", &layer.m_RenderPathIndex, kRenderPaths, kRenderPathCount);
    const RHIBackend active = layer.m_RenderContext
        ? layer.m_RenderContext->GetBackend()
        : RHIBackend::Unknown;
    const RenderBackend activeBackend =
        active == RHIBackend::Vulkan ? RenderBackend::Vulkan :
        active == RHIBackend::D3D12 ? RenderBackend::D3D12 :
        active == RHIBackend::Metal ? RenderBackend::Metal :
        RenderBackend::D3D11;
    ImGui::LabelText("Active backend", "%s",
                     active == RHIBackend::Unknown
                         ? "Unknown"
                         : RenderBackendToLabel(activeBackend));
    const RenderPath activeRenderPath = layer.m_SceneLayer
        ? layer.m_SceneLayer->GetRenderPath()
        : RenderPath::Forward;
    ...
    if (ImGui::Button("Save Graphics")) {
        ...
        editable.GetGraphicsSettings().backend =
            RenderBackendToProjectValue(selectedBackend);
        editable.GetGraphicsSettings().renderPath =
            ProjectValueFromRenderPathIndex(layer.m_RenderPathIndex);
        std::string error;
        if (editable.Save(&error)) {
...
void EditorProjectSettingsController::DrawGameplayInputSettingsTab(EditorLayer& layer)
{
    ...
    if (ImGui::Button("Create Default Input Config")) layer.CreateDefaultInputConfig();
    ImGui::SameLine();
...
void EditorProjectSettingsController::DrawLayoutSettingsTab(EditorLayer& layer)
{
    ...
    if (ImGui::Button("Save Current")) layer.SaveEditorLayout();
    ImGui::SameLine();
    if (ImGui::Button("Reset To Default")) layer.ResetEditorLayoutToDefault();
    ImGui::SameLine();
    if (ImGui::Button("Reveal Config Path")) layer.RevealEditorLayoutConfig();
    ImGui::Separator();
...
void EditorProjectSettingsController::DrawAppearanceSettingsTab(EditorLayer& layer)
{
    ...
    ImGui::LabelText("Font root", "%s",
                     layer.m_UIScaleManager.GetFontManager().GetFontRoot().string().c_str());
    ...
    if (ImGui::SliderFloat("UI Scale", &userScale,
        ...
        std::string error;
        if (!layer.m_Workspace.Save(&error)) Logger::Warn("[Editor] ", error);
    }
    ...
    if (ImGui::Button("Reset Appearance")) {
        ...
        layer.m_ThemeManager.Apply(layer.m_UIScaleManager.GetEffectiveScale());
        std::string error;
        if (layer.m_Workspace.Save(&error)) layer.ShowProjectResult("Appearance reset.", false);
        else layer.ShowProjectResult("Failed to save appearance: " + error, true);
    }
    ImGui::Separator();
...
{
    if (!layer.m_ProjectOpen) return;
    const auto& config = layer.m_Project.GetConfig();
    std::strncpy(layer.m_ProjectName.data(), config.GetName().c_str(),
                 layer.m_ProjectName.size() - 1);
    layer.m_ProjectName.back() = '\0';
    std::strncpy(layer.m_PublishOutput.data(),
                 config.GetPublishSettings().outputDirectory.c_str(),
                 layer.m_PublishOutput.size() - 1);
    layer.m_PublishOutput.back() = '\0';
    std::strncpy(layer.m_InputConfigPath.data(),
                 config.GetInputSettings().config.c_str(),
                 layer.m_InputConfigPath.size() - 1);
    layer.m_InputConfigPath.back() = '\0';
    layer.m_GraphicsBackendIndex =
        BackendIndexFromProjectValue(config.GetGraphicsSettings().backend);
    layer.m_RenderPathIndex =
        config.GetGraphicsSettings().renderPath == "deferred" ? 1 : 0;
