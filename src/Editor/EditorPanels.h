...
public:SceneHierarchyPanel();
    ...
    bool CanHandleEditorAction(const EditorContext& context,
                               std::string_view actionID) const override;
protected:void DrawContent() override;
private:
    void DrawToolbar();
...
public:AssetBrowserPanel();void OnAttach(EditorContext& context) override;void OnDetach() override;void OnUpdate(float dt) override;bool HandleEditorAction(EditorContext& context, std::string_view actionID) override;bool CanHandleEditorAction(const EditorContext& context, std::string_view actionID) const override;
protected:void DrawContent() override;
private:
    ...
    std::vector<std::string> CollectAssetPathsForFolder(
    ...
    bool MoveSelectedAssetsToFolder(EditorContext& context,
    ...
    void SelectAssetRow(EditorContext& context,
                        const std::vector<EditorAssetInfo>& visibleAssets,
                        size_t index, bool toggle, bool range);
    void SelectVisibleAssets(EditorContext& context,
                             const std::vector<EditorAssetInfo>& visibleAssets);
    ...
    bool RequestCreateFolderInFolder(EditorContext& context,
                                     const std::string& parentFolder,
                                     bool selectCreated);
    bool RequestCreateAssetFromTemplateInFolder(EditorContext& context,
                                                const std::string& folderPath,
                                                const char* templateID);
    bool RequestPasteAssetsToFolder(EditorContext& context,
                                    const std::string& targetFolder);
    ...
    std::vector<PendingAssetRetarget> BuildPendingRetargetsForFolder(
    ...
    std::filesystem::path FolderPathToAbsolute(const std::string& folderPath,
                                               const char* fallback) const;
    std::filesystem::path CurrentContentDirectory(const char* fallback) const;
    std::filesystem::path TemplateTargetDirectoryInFolder(
