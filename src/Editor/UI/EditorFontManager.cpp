...
bool EditorFontManager::RebuildIfNeeded(float effectiveScale, EditorImGuiBackend* backend) {
#if defined(MYENGINE_ENABLE_IMGUI)
    m_LastRebuilt = false;
    if (!m_Dirty || !ImGui::GetCurrentContext())
        return false;
    ...
    m_Fonts[Index(EditorFontRole::UI)] = LoadFont(EditorFontRole::UI, Resolve(m_Config.uiRegularPath), uiSize, false);
    ...
    if (!m_Fonts[Index(EditorFontRole::Icon)]) {
        ...
        SetWarning(m_LastWarning.empty() ? "Icon font missing; using text icon fallback"
    ...
    if (!m_Fonts[Index(EditorFontRole::Log)]) {
        ...
        SetWarning(m_LastWarning.empty() ? "Log font missing; using UI font"
...
std::filesystem::path EditorFontManager::Resolve(const std::filesystem::path& path) const {
    if (path.is_absolute() || m_FontRoot.empty())
        return path;
    return (m_FontRoot / path).lexically_normal();
}

ImFont* EditorFontManager::LoadFont(EditorFontRole role, const std::filesystem::path& path, float sizePixels,
                                    bool mergeIcons) {
#if defined(MYENGINE_ENABLE_IMGUI)
    std::error_code ec;
    ...
    std::snprintf(config.Name, sizeof(config.Name), "%s", name.empty() ? "EditorFont" : name.c_str());
    ImFont* font = ImGui::GetIO().Fonts->AddFontFromFileTTF(path.string().c_str(), sizePixels, &config,
                                                            mergeIcons ? kFontAwesomeRanges : nullptr);
    if (!font) {
        const char* roleName = role == EditorFontRole::Log          ? "log"
                               : role == EditorFontRole::Icon       ? "icon"
                               : role == EditorFontRole::UIEmphasis ? "UI emphasis"
                                                                    : "UI";
        SetWarning(m_LastWarning.empty() ? std::string("Failed to load ") + roleName + " font: " + path.string()
