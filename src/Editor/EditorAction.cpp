...
LambdaEditorAction::LambdaEditorAction(std::string id, std::string label,
                                       Function execute, Predicate canExecute)
    : m_ID(std::move(id)),
...
{
    if (m_Execute) m_Execute(context);
}
...
{
    EditorAction* action = Find(id);
    if (!action || !action->CanExecute(context)) return false;
