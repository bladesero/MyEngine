...
std::string ToProjectContentReference(const fs::path& projectRoot, const std::string& sourcePath) {
    ...
    if (!IsWithin(source, contentRoot))
        return {};
...
std::unordered_map<std::string, std::string> BuildArtifactUuidReferenceMap(const fs::path& projectRoot) {
    std::unordered_map<std::string, std::string> result;
    AssetDatabase database;
    std::string error;
    if (!database.Open(projectRoot / ".myengine" / "AssetDatabase.json", &error))
...
void ValidateAssetDatabase(const fs::path& projectRoot, PublishPreflightReport& report) {
    ...
    const bool hasImportState =
        fs::exists(databasePath) || fs::exists(projectRoot / "SourceAssets") || fs::exists(projectRoot / "Library");
    if (!hasImportState)
        return;
    if (!fs::exists(databasePath)) {
        report.errors.push_back(
            {PublishIssueCode::MissingDependency, databasePath.generic_string(), {}, {}, "asset database is missing"});
        return;
    }
    AssetDatabase database;
    std::string error;
    if (!database.Open(databasePath, &error)) {
        report.errors.push_back({PublishIssueCode::InvalidAsset, databasePath.generic_string(), {}, {}, error});
        return;
    }
    AssetDatabaseValidationReport validation;
    database.ValidateAgainstProject(projectRoot, validation);
    for (const auto& issue : validation.issues) {
        report.errors.push_back({ToPublishIssue(issue.code), issue.path, {}, {issue.uuid}, issue.message});
    }
    for (const AssetRecord& record : database.GetAll()) {
...
struct Scanner {
    ...
    void Error(PublishIssueCode code, const fs::path& path, const fs::path& from, const std::vector<std::string>& chain,
               std::string message) {
        report->errors.push_back({code, path.generic_string(), from.generic_string(), chain, std::move(message)});
    }
    bool Resolve(const std::string& value, const fs::path& from, fs::path& out, const std::vector<std::string>& chain) {
        if (value.empty() || value.rfind("__builtin__/", 0) == 0 || value.rfind("data:", 0) == 0)
            return false;
        ...
        if (logical.is_absolute() || logical.has_root_name()) {
            ...
            if (const std::optional<std::string> uuid = ExtractProjectLibraryArtifactUuid(projectRoot, logical)) {
            ...
            Error(PublishIssueCode::UnsafePath, logical, from, chain, "absolute asset reference is forbidden");
            return false;
        }
        const std::string generic = logical.generic_string();
        if (generic.rfind("Content/Engine/", 0) == 0)
            return false;
        relative = generic.rfind("Content/", 0) == 0 ? fs::path(generic.substr(8))
        ...
        if (!ContentPathPolicy::ResolveContained(contentRoot, relative, out, &error, true)) {
            Error(error.find("missing") != std::string::npos ? PublishIssueCode::MissingDependency
                                                             : PublishIssueCode::UnsafePath,
                  logical, from, chain, error);
            return false;
        }
        return true;
    }
    void ScanReference(const std::string& value, const fs::path& from, std::vector<std::string> chain) {
        fs::path resolved;
        chain.push_back(value);
        if (Resolve(value, from, resolved, chain))
    ...
    void ScanPrefabReference(const nlohmann::json& reference, const fs::path& from, std::vector<std::string> chain) {
        if (!reference.is_object()) {
            Error(PublishIssueCode::InvalidAsset, from, from, chain, "prefab reference is not an object");
            return;
        }
        const std::string value = reference.value("asset", std::string{}),
                          expected = reference.value("uuid", std::string{});
        fs::path resolved;
        chain.push_back(value);
        if (!Resolve(value, from, resolved, chain))
            return;
        try {
            std::ifstream assetInput(resolved), metaInput(resolved.string() + ".meta");
            nlohmann::json assetJson, metaJson;
            assetInput >> assetJson;
            if (!metaInput) {
                Error(PublishIssueCode::MissingDependency, resolved.string() + ".meta", from, chain,
                      "prefab metadata is missing");
                return;
            }
            metaInput >> metaJson;
            const std::string assetUuid = assetJson.value("uuid", std::string{}),
                              metaUuid = metaJson.value("uuid", std::string{});
            if (expected.empty() || expected != assetUuid || assetUuid != metaUuid) {
                Error(PublishIssueCode::Compatibility, resolved, from, chain, "prefab UUID mismatch");
                return;
            }
        } catch (const std::exception& e) {
            Error(PublishIssueCode::InvalidAsset, resolved, from, chain, e.what());
            return;
        }
        Scan(resolved, std::move(chain));
    }
    void VisitJson(const nlohmann::json& node, const fs::path& from, const std::vector<std::string>& chain,
                   const std::string& key = {}) {
        static const std::unordered_set<std::string> direct = {"mesh",
                                                               "material",
                                                               "scriptPath",
                                                               "shader",
                                                               "uri",
        ...
        if (node.is_object()) {
            for (auto it = node.begin(); it != node.end(); ++it) {
                if (it.key() == "prefabInstance")
                    ScanPrefabReference(it.value(), from, chain);
                else if (it.key() == "nestedInstances" && it.value().is_array()) {
                    for (const auto& nested : it.value())
                        ScanPrefabReference(nested, from, chain);
                    ...
                    for (auto texture = it.value().begin(); texture != it.value().end(); ++texture)
                        if (texture.value().is_string())
                            ScanReference(texture.value().get<std::string>(), from, chain);
                } else
                    VisitJson(it.value(), from, chain, it.key());
            }
    ...
    void ScanTextDependencies(const fs::path& path, std::vector<std::string> chain) {
        ...
        while (std::getline(input, line)) {
            ...
            while (words >> token)
                value = token;
            if (!value.empty())
                ScanReference(value, path, chain);
        }
    }
    void Scan(const fs::path& path, std::vector<std::string> chain) {
        const std::string key = path.generic_string();
        if (active.count(key)) {
            Error(PublishIssueCode::InvalidAsset, path, path, chain, "prefab dependency cycle");
            return;
        }
        ...
        try {
            if (ext == ".json" || ext == ".mat" || ext == ".shader" || ext == ".gltf") {
                ...
                if (filename.size() >= 13 && filename.rfind(".profile.json") == filename.size() - 13) {
                ...
                if (filename == "RuntimeScreens.ui.json") {
                    ...
                    if (!config.Apply(stack, &configError))
                        throw std::runtime_error(configError);
                }
                if (filename == "InputGlyphs.glyph.json") {
                    InputGlyphAtlas atlas;
                    std::string atlasError;
                    if (!InputGlyphAtlas::FromJson(json, atlas, &atlasError))
                        throw std::runtime_error(atlasError);
                }
                if (IsPrefab(path)) {
                    if (json.value("version", 0u) != 1u || !json.contains("nodes") || !json["nodes"].is_array())
                        throw std::runtime_error("invalid prefab asset header");
                    std::ifstream meta(path.string() + ".meta");
                    nlohmann::json metaJson;
                    if (!meta)
                        throw std::runtime_error("prefab metadata is missing");
                    meta >> metaJson;
            ...
            Error(PublishIssueCode::InvalidAsset, path, {}, chain, e.what());
        }
        active.erase(key);
    }
};
...
bool CookDependencyGraph::Validate(const fs::path& projectRoot, PublishPreflightReport& report) {
    report = {};
    Scanner scanner{projectRoot, projectRoot / "Content", &report};
    ...
    if (!ContentPathPolicy::Enumerate(scanner.contentRoot, files, report.totalContentBytes, &error)) {
        report.errors.push_back({PublishIssueCode::ContentLimit, {}, {}, {}, error});
        return false;
    }
    for (const auto& file : files) {
        const std::string name = file.relative.generic_string();
        const bool scene =
            name.rfind("Scenes/", 0) == 0 && name.size() >= 11 && name.rfind(".scene.json") == name.size() - 11;
        const bool config = name.rfind("Config/", 0) == 0 && name.size() >= 5 && name.rfind(".json") == name.size() - 5;
        if (scene || config)
            scanner.Scan(file.absolute, {"Content/" + name});
    }
    if (report.visitedAssets.empty())
        report.errors.push_back(
            {PublishIssueCode::MissingDependency, "Content/Scenes", {}, {}, "no scene files found"});
