...
bool EditorSelectionOperator::SelectActor(EditorContext& context, uint64_t actorID,
    ...
    if (!actor)
        return false;
    const EditorSelectionWorldKind world = context.GetSceneViewMode() == EditorWorldViewMode::PlayWorldInspect
                                               ? EditorSelectionWorldKind::Play
                                               : EditorSelectionWorldKind::Editor;
    context.GetSelection().Select(EditorSelectObject::MakeActor(actor->GetHandle(), actor->GetID(), world),
                                  ToSelectionMode(mode));
    return true;
}

bool EditorSelectionOperator::SelectActorRange(EditorContext& context, uint64_t anchorActorID, uint64_t targetActorID,
                                               const std::vector<uint64_t>& orderedActorIDs) const {
    Scene* scene = context.GetInspectorScene();
    if (!scene || anchorActorID == 0 || targetActorID == 0)
        return false;
    Actor* targetActor = scene->FindByID(targetActorID);
    if (!scene->FindByID(anchorActorID) || !targetActor)
        return false;
    ...
    if (anchorIt == ordered.end() || targetIt == ordered.end())
        return false;
    ...
    context.GetSelection().Select(EditorSelectObject::MakeActor(targetActor->GetHandle(), targetActor->GetID(), world),
                                  EditorSelectionMode::Add);
    return context.GetSelection().IsSelected(targetActorID, world);
}

bool EditorSelectionOperator::SelectActorSubtree(EditorContext& context, uint64_t actorID, bool includeRoot) const {
    Scene* scene = context.GetInspectorScene();
...
EditorSelectionSnapshot EditorSelectionOperator::GetSelectionSnapshot(const EditorContext& context) const {
