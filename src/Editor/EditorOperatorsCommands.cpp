...
bool EditorCommandOperator::ExecuteAction(EditorContext& context, const std::string& actionID) const {
    EditorActionRegistry* actions = context.GetActionRegistry();
    return actions && actions->Execute(actionID, context);
}

uint64_t EditorCommandOperator::CreateActor(EditorContext& context, const std::string& name) const {
...
uint64_t EditorCommandOperator::CreateChildActor(EditorContext& context, const std::string& name,
...
uint64_t EditorCommandOperator::CreateUIActor(EditorContext& context, const std::string& presetID,
                                              uint64_t parentActorID) const {
    if (!context.CanEditScene() || !context.GetScene() || !context.GetCommandStack()) {
        return 0;
    }
    ...
    if (!context.GetCommandStack()->ExecuteCommand(
            EditorUndoUtil::MakeSceneSnapshotCommand("Create UI Actor", before, after, oldSelection, newSelection),
            context)) {
        return 0;
    }
    return newSelection;
...
uint64_t EditorCommandOperator::CreateEmptyParent(EditorContext& context, uint64_t actorID) const {
    Scene* scene = context.GetScene();
    ...
    if (!context.GetCommandStack()->ExecuteCommand(
            EditorUndoUtil::MakeSceneSnapshotCommand("Create Empty Parent", before, after, beforeSelection,
                                                     newParentID),
            context)) {
        return 0;
    }
...
bool EditorCommandOperator::DuplicateActorSubtree(EditorContext& context, uint64_t actorID) const {
    Scene* scene = context.GetScene();
    Actor* actor = scene ? scene->FindByID(actorID) : nullptr;
    ...
    if (!ActorSubtreeSerializer::Serialize(*actor, nodes, &error)) {
        if (!error.empty())
            Logger::Warn("[Editor] Duplicate actor failed: ", error);
        return false;
    }
    ...
    if (const uint64_t nextID = NextSiblingID(*actor))
        nextSibling = scene->FindByID(nextID);
    Actor* clone = InstantiatePrefabNodesAsCopy(*scene, nodes, actor->GetParent(), nextSibling, &error, firstCloneID);
    if (!clone) {
        SceneSerializer::LoadFromString(*scene, before);
        if (!error.empty())
            Logger::Warn("[Editor] Duplicate actor failed: ", error);
    ...
    return stack->ExecuteCommand(
        EditorUndoUtil::MakeSceneSnapshotCommand("Duplicate Actor", before, after, beforeSelection, cloneID), context);
}

bool IsSelectedActorDescendant(const std::vector<uint64_t>& selectedIDs, const Actor& actor) {
    for (Actor* parent = actor.GetParent(); parent; parent = parent->GetParent()) {
        if (std::find(selectedIDs.begin(), selectedIDs.end(), parent->GetID()) != selectedIDs.end()) {
            return true;
        }
    }
    return false;
...
std::vector<uint64_t> OrderedSelectedActorRoots(Scene& scene, const std::vector<uint64_t>& selectedIDs) {
    std::vector<uint64_t> roots;
    roots.reserve(selectedIDs.size());
    scene.ForEach([&](Actor& actor) {
        const uint64_t actorID = actor.GetID();
        if (std::find(selectedIDs.begin(), selectedIDs.end(), actorID) == selectedIDs.end()) {
            return;
        }
        if (IsSelectedActorDescendant(selectedIDs, actor))
            return;
...
bool EditorCommandOperator::DuplicateSelection(EditorContext& context) const {
    const EditorSelectionSnapshot selection = EditorSelectionOperator{}.GetSelectionSnapshot(context);
    ...
    if (selectedIDs.size() <= 1)
        return DuplicateActorSubtree(context, selection.actorID);
    ...
    for (uint64_t actorID : rootIDs) {
        ...
        if (!ActorSubtreeSerializer::Serialize(*actor, nodes, &error)) {
            SceneSerializer::LoadFromString(*scene, before);
            if (!error.empty())
                Logger::Warn("[Editor] Duplicate actors failed: ", error);
            return false;
        }
        ...
        const uint64_t firstCloneID = FindNextActorID(*scene);
        Actor* clone =
            InstantiatePrefabNodesAsCopy(*scene, nodes, actor->GetParent(), nextSibling, &error, firstCloneID);
        if (!clone) {
            SceneSerializer::LoadFromString(*scene, before);
            if (!error.empty())
                Logger::Warn("[Editor] Duplicate actors failed: ", error);
    ...
    SceneSerializer::LoadFromString(*scene, before);
    return stack->ExecuteCommand(
        EditorUndoUtil::MakeSceneSnapshotCommand("Duplicate Actors", before, after, beforeSelection, afterSelection),
        context);
}

bool EditorCommandOperator::RenameActor(EditorContext& context, uint64_t actorID, const std::string& name) const {
    Scene* scene = context.GetScene();
    Actor* actor = scene ? scene->FindByID(actorID) : nullptr;
    if (!actor || !context.GetCommandStack() || !context.CanEditScene())
        return false;
    if (actor->GetName() == name)
        return false;
    return context.GetCommandStack()->ExecuteCommand(EditorUndoUtil::MakeSetNameCommand(*actor, name), context);
}
...
bool EditorCommandOperator::DeleteActor(EditorContext& context, uint64_t actorID) const {
    ...
    if (!actor || !context.GetCommandStack() || !context.CanEditScene())
        return false;
    return context.GetCommandStack()->ExecuteCommand(EditorUndoUtil::MakeDestroyActorCommand(*actor), context);
}
...
bool EditorCommandOperator::DeleteSelection(EditorContext& context) const {
    ...
    if (selection.hasActor && context.GetSelection().GetActorIDs().size() > 1) {
        ...
        SceneSerializer::LoadFromString(*scene, before);
        return stack->ExecuteCommand(
            EditorUndoUtil::MakeSceneSnapshotCommand("Delete Actors", before, after, beforeSelection, 0), context);
    }
    if (selection.hasActor)
        return DeleteActor(context, selection.actorID);
    if (selection.hasAsset)
        return EditorAssetOperator{}.DeleteAsset(context, selection.assetPath);
    return false;
}

bool EditorCommandOperator::RenameSelection(EditorContext& context, const std::string& name) const {
    const EditorSelectionSnapshot selection = EditorSelectionOperator{}.GetSelectionSnapshot(context);
    if (selection.hasActor)
        return RenameActor(context, selection.actorID, name);
    if (selection.hasAsset)
        return EditorAssetOperator{}.RenameAsset(context, selection.assetPath, name);
    return false;
}
...
bool EditorCommandOperator::CopySelection(EditorContext& context) const {
    ...
    Scene* scene = context.GetScene();
    if (!scene || !context.GetSelection().HasActor())
        return false;
    ...
    for (uint64_t actorID : rootIDs) {
        ...
        if (!ActorSubtreeSerializer::Serialize(*actor, nodes, &error)) {
            if (!error.empty())
                Logger::Warn("[Editor] Copy actors failed: ", error);
            return false;
        }
...
bool EditorCommandOperator::CopyAssets(EditorContext& context, const std::vector<std::string>& paths) const {
    if (!m_Clipboard)
        return false;
    ...
    for (const std::string& path : paths) {
        ...
        if (std::find(validPaths.begin(), validPaths.end(), normalized) == validPaths.end()) {
...
bool EditorCommandOperator::PasteSelection(EditorContext& context) const {
    ...
    if (m_Clipboard->GetKind() == EditorClipboardService::Kind::Asset) {
        ...
        const std::filesystem::path targetFolder =
            selection.hasAsset ? ResolveEditorAssetPath(context, selection.assetPath).parent_path()
                               : context.GetContentRoot();
        return PasteAssetToFolder(context, targetFolder.string());
    }
    if (m_Clipboard->GetKind() != EditorClipboardService::Kind::Actors)
        return false;
    ...
    if (!LoadClipboardRoots(m_Clipboard->GetActorJson(), roots))
        return false;
    ...
    for (const std::vector<PrefabNode>& nodes : roots) {
        const uint64_t firstCloneID = FindNextActorID(*scene);
        Actor* pasted = InstantiatePrefabNodesAsCopy(*scene, nodes, parent, nextSibling, &error, firstCloneID);
        if (!pasted) {
            SceneSerializer::LoadFromString(*scene, before);
            if (!error.empty())
                Logger::Warn("[Editor] Paste actors failed: ", error);
            return false;
        }
    ...
    SceneSerializer::LoadFromString(*scene, before);
    return stack->ExecuteCommand(
        EditorUndoUtil::MakeSceneSnapshotCommand(pastedIDs.size() > 1 ? "Paste Actors" : "Paste Actor", before, after,
                                                 beforeSelection, pastedIDs.back()),
        context);
}

bool EditorCommandOperator::PasteAssetToFolder(EditorContext& context, const std::string& targetFolder) const {
    if (!m_Clipboard || m_Clipboard->GetKind() != EditorClipboardService::Kind::Asset ||
        m_Clipboard->GetAssetPaths().empty()) {
        return false;
    }
    bool pastedAny = false;
    for (const std::string& path : m_Clipboard->GetAssetPaths()) {
        pastedAny = EditorAssetOperator{}.CopyAssetToFolder(context, path, targetFolder) || pastedAny;
    }
    return pastedAny;
}

bool EditorCommandOperator::SetActorActive(EditorContext& context, uint64_t actorID, bool active) const {
    Scene* scene = context.GetScene();
    Actor* actor = scene ? scene->FindByID(actorID) : nullptr;
    if (!actor || !context.GetCommandStack() || !context.CanEditScene())
        return false;
    if (actor->IsActiveSelf() == active)
        return false;
    return context.GetCommandStack()->ExecuteCommand(EditorUndoUtil::MakeSetActiveCommand(*actor, active), context);
}

bool EditorCommandOperator::SetActorTag(EditorContext& context, uint64_t actorID, const std::string& tag) const {
    Scene* scene = context.GetScene();
    Actor* actor = scene ? scene->FindByID(actorID) : nullptr;
    EditorCommandStack* stack = context.GetCommandStack();
    if (!actor || !stack || !context.CanEditScene())
...
bool EditorCommandOperator::SetActorLayer(EditorContext& context, uint64_t actorID, uint32_t layer) const {
    Scene* scene = context.GetScene();
    Actor* actor = scene ? scene->FindByID(actorID) : nullptr;
    EditorCommandStack* stack = context.GetCommandStack();
    if (!actor || !stack || !context.CanEditScene())
...
bool EditorCommandOperator::SetActorEditorFlags(EditorContext& context, uint64_t actorID, uint32_t flags) const {
    Scene* scene = context.GetScene();
    Actor* actor = scene ? scene->FindByID(actorID) : nullptr;
    EditorCommandStack* stack = context.GetCommandStack();
    if (!actor || !stack || !context.CanEditScene())
...
bool EditorCommandOperator::SetActorStatic(EditorContext& context, uint64_t actorID, bool isStatic) const {
    Scene* scene = context.GetScene();
    Actor* actor = scene ? scene->FindByID(actorID) : nullptr;
    if (!actor)
        return false;
...
bool EditorCommandOperator::SetSceneMainCameraHint(EditorContext& context, uint64_t actorID) const {
    Scene* scene = context.GetScene();
    EditorCommandStack* stack = context.GetCommandStack();
    if (!scene || !stack || !context.CanEditScene())
        return false;
...
bool EditorCommandOperator::SetSceneAmbientIntensity(EditorContext& context, float intensity) const {
    Scene* scene = context.GetScene();
    EditorCommandStack* stack = context.GetCommandStack();
    if (!scene || !stack || !context.CanEditScene())
        return false;
...
bool EditorCommandOperator::SetActorsActive(EditorContext& context, const std::vector<uint64_t>& actorIDs,
                                            bool active) const {
    Scene* scene = context.GetScene();
    EditorCommandStack* stack = context.GetCommandStack();
...
bool EditorCommandOperator::SetActorsTag(EditorContext& context, const std::vector<uint64_t>& actorIDs,
    ...
    for (uint64_t actorID : actorIDs) {
        ...
        if (actor && actor->GetTag() != tag)
            before.emplace_back(actorID, actor->GetTag());
    }
    if (before.empty())
        return false;
...
bool EditorCommandOperator::SetActorsLayer(EditorContext& context, const std::vector<uint64_t>& actorIDs,
    ...
    for (uint64_t actorID : actorIDs) {
        ...
        if (actor && actor->GetLayer() != layer)
            before.emplace_back(actorID, actor->GetLayer());
    }
    if (before.empty())
        return false;
...
bool EditorCommandOperator::SetActorsEditorFlags(EditorContext& context, const std::vector<uint64_t>& actorIDs,
...
bool EditorCommandOperator::SetActorsStatic(EditorContext& context, const std::vector<uint64_t>& actorIDs,
                                            bool isStatic) const {
    Scene* scene = context.GetScene();
    EditorCommandStack* stack = context.GetCommandStack();
    if (!scene || !stack || !context.CanEditScene() || actorIDs.empty())
...
bool EditorCommandOperator::SetActorsPosition(EditorContext& context, const std::vector<uint64_t>& actorIDs,
...
bool EditorCommandOperator::SetActorsRotation(EditorContext& context, const std::vector<uint64_t>& actorIDs,
...
bool EditorCommandOperator::SetActorsScale(EditorContext& context, const std::vector<uint64_t>& actorIDs,
...
bool EditorCommandOperator::MoveActor(EditorContext& context, uint64_t actorID, uint64_t afterParentID,
    ...
    if (beforeParentID == afterParentID && beforeNextSiblingID == afterNextSiblingID)
        return false;
    return context.GetCommandStack()->ExecuteCommand(
        EditorUndoUtil::MakeMoveActorCommand(*actor, beforeParentID, beforeNextSiblingID, afterParentID,
                                             afterNextSiblingID),
        context);
}
