...
public:
    ...
    bool SelectActorRange(EditorContext& context, uint64_t anchorActorID,
                          uint64_t targetActorID,
                          const std::vector<uint64_t>& orderedActorIDs = {}) const;
    bool SelectActorSubtree(EditorContext& context, uint64_t actorID,
                            bool includeRoot = true) const;
...
public:
    explicit EditorCommandOperator(EditorClipboardService* clipboard = nullptr)
    ...
    uint64_t CreateChildActor(EditorContext& context, const std::string& name,
                              uint64_t parentActorID) const;
    uint64_t CreateUIActor(EditorContext& context, const std::string& presetID,
                           uint64_t parentActorID = 0) const;
    ...
    bool SetActorTag(EditorContext& context, uint64_t actorID,
                     const std::string& tag) const;
    bool SetActorLayer(EditorContext& context, uint64_t actorID,
                       uint32_t layer) const;
    bool SetActorEditorFlags(EditorContext& context, uint64_t actorID,
                             uint32_t flags) const;
    bool SetActorStatic(EditorContext& context, uint64_t actorID,
                        bool isStatic) const;
    ...
    bool SetActorsActive(EditorContext& context, const std::vector<uint64_t>& actorIDs,
                         bool active) const;
    bool SetActorsTag(EditorContext& context, const std::vector<uint64_t>& actorIDs,
                      const std::string& tag) const;
    bool SetActorsLayer(EditorContext& context, const std::vector<uint64_t>& actorIDs,
                        uint32_t layer) const;
    bool SetActorsEditorFlags(EditorContext& context,
                              const std::vector<uint64_t>& actorIDs,
                              uint32_t flags) const;
    bool SetActorsStatic(EditorContext& context,
                         const std::vector<uint64_t>& actorIDs,
                         bool isStatic) const;
    bool SetActorsPosition(EditorContext& context, const std::vector<uint64_t>& actorIDs,
                           const Vec3& position) const;
    bool SetActorsRotation(EditorContext& context, const std::vector<uint64_t>& actorIDs,
                           const Vec3& rotation) const;
    bool SetActorsScale(EditorContext& context, const std::vector<uint64_t>& actorIDs,
...
public:
    bool AddComponent(EditorContext& context, uint64_t actorID,
                      const std::string& typeName,
                      const nlohmann::json& initialData = nlohmann::json::object()) const;
    bool AddComponents(EditorContext& context,
                       const std::vector<uint64_t>& actorIDs,
                       const std::string& typeName,
                       const nlohmann::json& initialData = nlohmann::json::object()) const;
    bool RemoveComponent(EditorContext& context, uint64_t actorID,
                         const std::string& typeName) const;
    bool RemoveComponent(EditorContext& context, Actor& actor,
                         const Component& component) const;
    bool RemoveComponents(EditorContext& context,
                          const std::vector<uint64_t>& actorIDs,
                          const std::string& typeName) const;
    bool SetComponentPropertyForActors(EditorContext& context,
                                       const std::vector<uint64_t>& actorIDs,
                                       const std::string& typeName,
                                       const std::string& propertyName,
                                       const nlohmann::json& value) const;
    bool SetJson(EditorContext& context, uint64_t actorID,
                 const std::string& typeName,
                 const nlohmann::json& beforeJson,
                 const nlohmann::json& afterJson,
                 const std::string& label = {}) const;
    bool SetProperty(EditorContext& context, Actor& actor,
...
public:
    ...
    bool CommitComponentProperty(EditorContext& context, Actor& actor,
                                 const std::string& componentType,
                                 const std::string& propertyName,
                                 const nlohmann::json& beforeJson,
                                 const nlohmann::json& afterJson) const;
...
public:
    ...
    bool RenameFolder(EditorContext& context, const std::string& folderPath,
    ...
    bool RenameAsset(EditorContext& context, const std::string& path,
                     const std::string& newNameOrPath) const;
    bool MoveAsset(EditorContext& context, const std::string& path,
                   const std::string& targetFolder) const;
    bool MoveFolder(EditorContext& context, const std::string& folderPath,
                    const std::string& targetFolder) const;
    bool CopyAssetToFolder(EditorContext& context, const std::string& path,
                           const std::string& targetFolder) const;
    ...
    std::vector<SceneReferenceInfo> FindSceneReferences(
        EditorContext& context, const std::string& path) const;
    std::vector<SceneReferenceInfo> FindProjectSceneReferences(
        EditorContext& context, const std::string& path) const;
    size_t RetargetSceneReferences(EditorContext& context,
                                   const std::string& oldPath,
                                   const std::string& newPath) const;
    size_t RetargetProjectSceneReferences(EditorContext& context,
                                          const std::string& oldPath,
                                          const std::string& newPath) const;
    ...
    bool ReimportWithSettings(EditorContext& context, const std::string& uuid,
...
public:
    ...
    std::vector<OverrideInfo> GetOverrides(EditorContext& context,
    ...
    uint64_t InstantiatePrefab(EditorContext& context, const std::string& path,
                               uint64_t parentActorID = 0,
    ...
    bool ApplyOverride(EditorContext& context, uint64_t actorID,
                       size_t overrideIndex) const;
    bool RevertOverride(EditorContext& context, uint64_t actorID,
                        size_t overrideIndex) const;
    bool ApplyOverride(EditorContext& context, uint64_t actorID,
...
public:
    ...
    bool FrameTarget(EditorContext& context, const Vec3& target,
                     float radius = 1.0f) const;
    bool FrameDirection(EditorContext& context, SceneViewDirection direction) const;
    bool OrbitAroundSelection(EditorContext& context, float yawDegrees,
                              float pitchDegrees) const;
    bool ToggleSceneProjection(EditorContext& context) const;
    bool DropModel(EditorContext& context, const std::string& path,
