...
bool CopyTemplateTree(const fs::path& source, const fs::path& destination, std::string* error) {
    ...
    for (fs::recursive_directory_iterator it(source, fs::directory_options::skip_permission_denied, ec), end;
        ...
        if (ec || !it->is_regular_file(ec))
            continue;
        if (target.filename() == ProjectConfig::kFileName)
            continue;
...
EditorWorkspace::EditorWorkspace(fs::path settingsPath, fs::path templateRoot)
    : m_SettingsPath(settingsPath.empty() ? DefaultSettingsPath() : std::move(settingsPath)),
      m_TemplateRoot(templateRoot.empty() ? FindTemplateRootFromCurrentPath()
...
bool EditorWorkspace::Load(std::string* error) {
    ...
    m_PanelState.clear();
    std::error_code ec;
    if (!fs::exists(m_SettingsPath, ec))
        return true;
    try {
        ...
        m_UserUiScale = Editor::UI::EditorUIScaleManager::ClampUserScale(json.value("userUiScale", m_UserUiScale));
        m_EditorThemeId =
            Editor::UI::EditorThemeManager::NormalizeThemeID(json.value("editorThemeId", m_EditorThemeId));
...
void EditorWorkspace::SetPanelStateValue(const std::string& panelID, const std::string& key, std::string value) {
    if (panelID.empty() || key.empty())
        return;
    m_PanelState[panelID][key] = std::move(value);
}

std::optional<std::string> EditorWorkspace::GetPanelStateValue(const std::string& panelID,
                                                               const std::string& key) const {
    const auto panel = m_PanelState.find(panelID);
    if (panel == m_PanelState.end())
        return std::nullopt;
    const auto value = panel->second.find(key);
    if (value == panel->second.end())
        return std::nullopt;
    return value->second;
}
...
bool EditorWorkspace::CreateProject(const fs::path& projectRoot, const std::string& projectName, std::string* error) {
    if (error)
        error->clear();
    ...
    if (!m_TemplateRoot.empty()) {
        if (!CopyTemplateTree(m_TemplateRoot, root, error))
            return false;
    } else {
        ...
        nlohmann::json scene = {{"name", "Main"}, {"nextID", 1}, {"actors", nlohmann::json::array()}};
    ...
    if (!config.Open(root, true, error))
        return false;
    config.SetName(projectName);
    config.GetPublishSettings().target = PublishTargets::kDefaultTargetId;
    if (!config.SetInputConfigPath(InputActionMap::kDefaultProjectPath, error))
        return false;
    const fs::path startupScene = root / "Content" / "Scenes" / "Main.scene.json";
    if (!config.SetStartupScene(startupScene, error) || !config.Save(error))
        return false;
