...
EditorContext::EditorContext(Scene* scene) : m_SceneOverride(scene) {
}

EditorContext::EditorContext(SceneRenderLayer* sceneLayer, IRenderContext* renderContext, IWindow* window,
                             Engine* engine)
...
void EditorContext::SetSceneViewMode(EditorWorldViewMode mode) {
    if (mode == EditorWorldViewMode::PlayWorldInspect && (!m_SceneLayer || !m_SceneLayer->GetPlayScene())) {
    ...
    if (m_SceneViewMode == mode) {
        if (m_SceneLayer) {
            m_SceneLayer->SetSceneViewportUsesSimulationScene(mode == EditorWorldViewMode::PlayWorldInspect);
        }
        return;
    }
    ...
    if (targetActor) {
        const EditorSelectionWorldKind world = mode == EditorWorldViewMode::PlayWorldInspect
                                                   ? EditorSelectionWorldKind::Play
                                                   : EditorSelectionWorldKind::Editor;
        m_Selection.Select(EditorSelectObject::MakeActor(targetActor->GetHandle(), targetActor->GetID(), world));
    } else {
        m_Selection.Clear();
    }
}
...
void EditorContext::RefreshSceneViewMode() {
    if (m_SceneViewMode == EditorWorldViewMode::PlayWorldInspect && (!m_SceneLayer || !m_SceneLayer->GetPlayScene())) {
        SetSceneViewMode(EditorWorldViewMode::EditorWorld);
        return;
    }
    if (m_SceneLayer) {
        m_SceneLayer->SetSceneViewportUsesSimulationScene(m_SceneViewMode == EditorWorldViewMode::PlayWorldInspect);
    }
}

Scene* EditorContext::GetSceneViewScene() const {
    if (m_SceneViewMode == EditorWorldViewMode::PlayWorldInspect) {
        if (Scene* playScene = GetPlayScene())
            return playScene;
    }
...
Scene* EditorContext::GetInspectorScene() const {
    const EditorSelectObject& selected = m_Selection.GetPrimaryObject();
    if (selected.IsActor() && selected.GetWorldKind() == EditorSelectionWorldKind::Play) {
        if (Scene* playScene = GetPlayScene())
            return playScene;
    }
    return GetEditorScene();
...
bool EditorContext::CanEditScene() const {
    return m_SceneOverride ||
           (m_SceneLayer && m_SceneLayer->IsEditing() && m_SceneViewMode == EditorWorldViewMode::EditorWorld);
}
...
bool EditorContext::IsInspectingPlayWorld() const {
    return m_SceneViewMode == EditorWorldViewMode::PlayWorldInspect && GetPlayScene() != nullptr;
}
...
void EditorContext::SetPanelFocusRequestHandler(std::function<void(std::string_view)> handler) {
