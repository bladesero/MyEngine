...
public:
    ...
    virtual bool CanDraw(const EditorSelectObject& object, const EditorContext& context) const = 0;
...
public:
    void Register(std::unique_ptr<EditorInspectorSection> section) {
        ...
        std::stable_sort(m_Sections.begin(), m_Sections.end(),
                         [](const auto& left, const auto& right) { return left->GetOrder() < right->GetOrder(); });
    }

    const std::vector<std::unique_ptr<EditorInspectorSection>>& GetSections() const { return m_Sections; }
