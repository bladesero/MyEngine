...
{
    if (value == "cppOnly") return EditorScriptCorePanelMode::CppOnly;
    if (value == "scriptOnlyDebug") return EditorScriptCorePanelMode::ScriptOnlyDebug;
...
EditorScriptConfig EditorScriptConfig::LoadFromFile(const std::filesystem::path& path,
                                                    std::string* error)
{
    ...
    std::error_code ec;
    if (!std::filesystem::is_regular_file(path, ec) || ec) return config;
    ...
    try {
        ...
        config.allowProjectOverrideCore = json.value("allowProjectOverrideCore",
                                                     config.allowProjectOverrideCore);
        ...
        config.enableInspectorExtensions = json.value("enableInspectorExtensions",
                                                      config.enableInspectorExtensions);
        config.enableContextMenuExtensions = json.value("enableContextMenuExtensions",
        ...
        if (json.contains("enabledCorePanels")) {
            ...
            for (const auto& value : json["enabledCorePanels"]) {
                if (value.is_string()) config.enabledCorePanels.insert(value.get<std::string>());
            }
        }
