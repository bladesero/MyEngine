...
bool UI_IconButton(const std::string& id, const std::string& icon,
                   const std::string& tooltip, bool enabled)
{
    EditorContext* context = Context();
    if (!context) return false;
    return Editor::UI::EditorWidgets::IconButton(
        *context, id.c_str(), icon.c_str(), tooltip.c_str(), enabled);
}
...
bool UI_ToolbarActionButtonEx(const std::string& actionID, const std::string& icon,
                              int variant, bool sameLineAfter)
{
    EditorContext* context = Context();
    if (!context) return false;
    ...
    return Editor::UI::EditorWidgets::ToolbarActionButton(
        *context, actionID.c_str(), icon.c_str(),
        static_cast<Editor::UI::EditorWidgetVariant>(variant), sameLineAfter);
}
...
void UI_InlineMessage(int type, const std::string& text)
{
    Editor::UI::EditorWidgets::InlineMessage(
        static_cast<Editor::UI::EditorNotificationType>(type), text.c_str());
}
...
uint64_t Selection_GetActorId()
{
    ...
    return context && operators
        ? operators->Selection().GetSelectionSnapshot(*context).actorID
        : 0;
}
...
void Selection_SelectActor(uint64_t actorID)
{
    EditorContext* context = Context();
    EditorOperators* operators = context ? context->GetOperators() : nullptr;
    if (context && operators) operators->Selection().SelectActor(*context, actorID);
}
...
void Selection_SelectActorMode(uint64_t actorID, int mode)
{
    EditorContext* context = Context();
    EditorOperators* operators = context ? context->GetOperators() : nullptr;
    if (!context || !operators) return;
    EditorSelectionIntentMode selectionMode = EditorSelectionIntentMode::Replace;
    if (mode == 1) selectionMode = EditorSelectionIntentMode::Add;
    else if (mode == 2) selectionMode = EditorSelectionIntentMode::Toggle;
...
void Selection_SelectAsset(const std::string& path)
{
    EditorContext* context = Context();
    EditorOperators* operators = context ? context->GetOperators() : nullptr;
    if (context && operators) operators->Selection().SelectAsset(*context, path);
}
...
void Selection_Clear()
{
    EditorContext* context = Context();
    EditorOperators* operators = context ? context->GetOperators() : nullptr;
    if (context && operators) operators->Selection().Clear(*context);
}
...
std::string Selection_GetAssetPath()
{
    ...
    EditorOperators* operators = context ? context->GetOperators() : nullptr;
    return context && operators
        ? operators->Selection().GetSelectionSnapshot(*context).assetPath
        : std::string{};
}
...
std::string Scene_GetRootActors()
{
    ...
    for (Actor* actor : scene->GetRootActors()) {
        if (!actor || actor->GetParent()) continue;
        if (!first) out << "\n";
        first = false;
        out << actor->GetID();
    }
...
bool Components_SetJson(uint64_t actorID, const std::string& type, const std::string& jsonText)
{
    ...
    try {
        ...
        return operators->Transactions().CommitSceneSnapshot(
            *context, "Script Component Edit", before, after, actorID, actorID);
    } catch (...) {
        return false;
    }
}
...
void Assets_Refresh()
{
    EditorContext* context = Context();
    EditorOperators* operators = context ? context->GetOperators() : nullptr;
    if (context && operators) operators->Assets().Refresh(*context);
}
...
bool Commands_MoveActor(uint64_t actorID, uint64_t parentID, uint64_t nextSiblingID)
{
    ...
    EditorOperators* operators = context ? context->GetOperators() : nullptr;
    return context && operators &&
        operators->Commands().MoveActor(*context, actorID, parentID, nextSiblingID);
}
...
bool Assets_Rename(const std::string& path, const std::string& newNameOrPath)
{
    ...
    EditorOperators* operators = context ? context->GetOperators() : nullptr;
    return context && operators &&
        operators->Assets().RenameAsset(*context, path, newNameOrPath);
}
...
bool DragDrop_BeginSource(const std::string& type, const std::string& payload,
                          const std::string& label)
{
#if defined(MYENGINE_ENABLE_IMGUI)
    if (type.empty() || payload.empty()) return false;
    if (!ImGui::BeginDragDropSource()) return false;
    ImGui::SetDragDropPayload(type.c_str(), payload.data(),
...
{
#if defined(MYENGINE_ENABLE_IMGUI)
    if (type.empty() || !ImGui::BeginDragDropTarget()) return {};
    std::string result;
    if (const ImGuiPayload* payload = ImGui::AcceptDragDropPayload(type.c_str())) {
        if (payload->Data && payload->DataSize > 0) {
            result.assign(static_cast<const char*>(payload->Data),
...
bool DragDrop_ApplyActorDrop(uint64_t actorID, uint64_t parentID, uint64_t nextSiblingID)
{
    ...
    EditorOperators* operators = context ? context->GetOperators() : nullptr;
    return context && operators &&
        operators->DragDrop().ApplyActorDrop(*context, actorID, parentID, nextSiblingID);
}
...
bool DragDrop_ApplyAssetDrop(const std::string& assetPath, const std::string& targetPath)
{
    ...
    EditorOperators* operators = context ? context->GetOperators() : nullptr;
    return context && operators &&
        operators->DragDrop().ApplyAssetDrop(*context, assetPath, targetPath);
}

bool Transaction_BeginSnapshot(const std::string& label)
{
    EditorContext* context = Context();
    if (!context || !context->GetScene()) return false;
    ...
    operators->Transactions().BeginSnapshot(
        g_PanelTransactions[g_ActivePanelID], label.c_str(), beforeJson, selection);
    return true;
}
...
bool Transaction_CommitIfChanged()
{
    EditorContext* context = Context();
    EditorOperators* operators = context ? context->GetOperators() : nullptr;
    if (!context || !operators) return false;
    return operators->Transactions().CommitIfChanged(
        *context, g_PanelTransactions[g_ActivePanelID]);
}
...
void Transaction_Cancel()
{
    EditorContext* context = Context();
    EditorOperators* operators = context ? context->GetOperators() : nullptr;
    if (operators) operators->Transactions().Cancel(g_PanelTransactions[g_ActivePanelID]);
}
...
bool Viewport_SetSceneViewportRect(float x, float y, float width, float height, bool hovered)
{
    EditorContext* context = Context();
    EditorOperators* operators = context ? context->GetOperators() : nullptr;
    return context && operators && operators->Viewport().SetSceneViewportRect(
        *context, {x, y, width, height}, hovered);
}
...
bool Viewport_SetGameViewportRect(float x, float y, float width, float height)
{
    EditorContext* context = Context();
    EditorOperators* operators = context ? context->GetOperators() : nullptr;
    return context && operators && operators->Viewport().SetGameViewportRect(
...
{
    if (filter.empty() || actor.GetName().find(filter) != std::string::npos) return true;
    for (const Actor* child : actor.GetChildren()) {
        if (child && ActorSubtreeMatches(*child, filter)) return true;
    }
    return false;
}
...
std::string Hierarchy_GetRows(const std::string& filter)
{
    ...
    const auto appendActor = [&](auto&& self, Actor* actor, int depth) -> void {
        ...
        for (Actor* child : actor->GetChildren()) self(self, child, depth + 1);
    };
    for (Actor* root : scene->GetRootActors()) {
        if (root && !root->GetParent()) appendActor(appendActor, root, 0);
    }
    return rows.dump();
}
...
std::string Inspector_GetComponentTypes(uint64_t actorID)
{
    ...
    actor->ForEachComponent([&](Component& component) {
        types.push_back(component.GetTypeName());
    });
    return types.dump();
}
...
std::string AssetBrowser_GetAssets(const std::string& folder, bool recursive,
                                   const std::string& filter)
{
    ...
    for (const EditorAssetInfo& asset : registry->GetAssetsInFolder(folder, recursive)) {
        if (!filter.empty() && asset.relativePath.find(filter) == std::string::npos) continue;
...
std::string Log_GetRows()
{
    ...
    if (!logs) return "[]";
    nlohmann::json rows = nlohmann::json::array();
    for (const std::string& line : logs->Snapshot()) rows.push_back(line);
    return rows.dump();
}
    ...
    if (which == "game") {
        if (GameViewport* viewport = context.GetGameViewport()) {
            ...
            if (EditorOperators* operators = context.GetOperators()) {
                operators->Viewport().SetGameViewportRect(
                    context, {local.x, local.y, imageSize.x, imageSize.y});
            }
...
void RegisterEditorUIFacade(void* scriptEngine)
{
    ...
    Check(engine->RegisterGlobalFunction("void Text(const string &in)",
        asFUNCTION(UI_Text), asCALL_CDECL));
    Check(engine->RegisterGlobalFunction("bool Button(const string &in)",
        asFUNCTION(UI_Button), asCALL_CDECL));
    Check(engine->RegisterGlobalFunction("bool MenuItem(const string &in, bool enabled = true)",
        asFUNCTION(UI_MenuItem), asCALL_CDECL));
    Check(engine->RegisterGlobalFunction("bool BeginChild(const string &in, float width = 0, float height = 0, bool border = false)",
        asFUNCTION(UI_BeginChild), asCALL_CDECL));
    Check(engine->RegisterGlobalFunction("void EndChild()",
        asFUNCTION(UI_EndChild), asCALL_CDECL));
    Check(engine->RegisterGlobalFunction("void BeginGroup()",
        asFUNCTION(UI_BeginGroup), asCALL_CDECL));
    Check(engine->RegisterGlobalFunction("void EndGroup()",
        asFUNCTION(UI_EndGroup), asCALL_CDECL));
    Check(engine->RegisterGlobalFunction("bool IconButton(const string &in, const string &in, const string &in, bool enabled = true)",
    ...
    Check(engine->RegisterGlobalFunction("bool ToolbarActionButtonEx(const string &in, const string &in, int variant = 0, bool sameLineAfter = true)",
        asFUNCTION(UI_ToolbarActionButtonEx), asCALL_CDECL));
    Check(engine->RegisterGlobalFunction("void Separator()",
        asFUNCTION(UI_Separator), asCALL_CDECL));
    Check(engine->RegisterGlobalFunction("void SameLine()",
        asFUNCTION(UI_SameLine), asCALL_CDECL));
    Check(engine->RegisterGlobalFunction("bool BeginTable(const string &in, int)",
        asFUNCTION(UI_BeginTable), asCALL_CDECL));
    Check(engine->RegisterGlobalFunction("void EndTable()",
        asFUNCTION(UI_EndTable), asCALL_CDECL));
    Check(engine->RegisterGlobalFunction("void TableNextRow()",
        asFUNCTION(UI_TableNextRow), asCALL_CDECL));
    Check(engine->RegisterGlobalFunction("void TableSetColumnIndex(int)",
        asFUNCTION(UI_TableSetColumnIndex), asCALL_CDECL));
    Check(engine->RegisterGlobalFunction("void TableHeader(const string &in)",
        asFUNCTION(UI_TableHeader), asCALL_CDECL));
    ...
    Check(engine->RegisterGlobalFunction("void TreePop()",
        asFUNCTION(UI_TreePop), asCALL_CDECL));
    ...
    Check(engine->RegisterGlobalFunction("void Tooltip(const string &in)",
        asFUNCTION(UI_Tooltip), asCALL_CDECL));
    ...
    Check(engine->RegisterGlobalFunction("void BeginPropertyGrid(const string &in)",
        asFUNCTION(UI_BeginPropertyGrid), asCALL_CDECL));
    Check(engine->RegisterGlobalFunction("void EndPropertyGrid()",
        asFUNCTION(UI_EndPropertyGrid), asCALL_CDECL));
    Check(engine->RegisterGlobalFunction("bool BeginPropertyRow(const string &in)",
        asFUNCTION(UI_BeginPropertyRow), asCALL_CDECL));
    Check(engine->RegisterGlobalFunction("void EndPropertyRow()",
        asFUNCTION(UI_EndPropertyRow), asCALL_CDECL));
    Check(engine->RegisterGlobalFunction("bool InputText(const string &in, string &inout)",
        asFUNCTION(UI_InputText), asCALL_CDECL));
    Check(engine->RegisterGlobalFunction("bool Checkbox(const string &in, bool &inout)",
        asFUNCTION(UI_Checkbox), asCALL_CDECL));
    ...
    Check(engine->RegisterGlobalFunction("bool DragVec3(const string &in, float &inout, float &inout, float &inout, float speed = 0.1f)",
    ...
    Check(engine->RegisterGlobalFunction("void BeginDisabled(bool disabled = true)",
        asFUNCTION(UI_BeginDisabled), asCALL_CDECL));
    Check(engine->RegisterGlobalFunction("void EndDisabled()",
        asFUNCTION(UI_EndDisabled), asCALL_CDECL));
    ...
    Check(engine->RegisterGlobalFunction("void PopStyleColor(int count = 1)",
        asFUNCTION(UI_PopStyleColor), asCALL_CDECL));
    Check(engine->RegisterGlobalFunction("bool ImageViewport(const string &in)",
        asFUNCTION(UI_ImageViewport), asCALL_CDECL));
    Check(engine->RegisterGlobalFunction("void InlineMessage(int, const string &in)",
        asFUNCTION(UI_InlineMessage), asCALL_CDECL));
...
void RegisterEditorContextBindings(void* scriptEngine)
{
    ...
    Check(engine->RegisterGlobalFunction("bool ExecuteAction(const string &in)", asFUNCTION(Editor_ExecuteAction), asCALL_CDECL));
    ...
    Check(engine->RegisterGlobalFunction("void SelectActor(uint64, int)", asFUNCTION(Selection_SelectActorMode), asCALL_CDECL));
    Check(engine->RegisterGlobalFunction("void SelectAsset(const string &in)", asFUNCTION(Selection_SelectAsset), asCALL_CDECL));
    ...
    Check(engine->RegisterGlobalFunction("bool Duplicate(const string &in)", asFUNCTION(Assets_Duplicate), asCALL_CDECL));
    ...
    Check(engine->RegisterGlobalFunction("string Accept(const string &in)",
        asFUNCTION(DragDrop_Accept), asCALL_CDECL));
    ...
    Check(engine->RegisterGlobalFunction("bool BeginSnapshot(const string &in)",
        asFUNCTION(Transaction_BeginSnapshot), asCALL_CDECL));
    Check(engine->RegisterGlobalFunction("bool CommitIfChanged()",
        asFUNCTION(Transaction_CommitIfChanged), asCALL_CDECL));
    Check(engine->RegisterGlobalFunction("void Cancel()",
        asFUNCTION(Transaction_Cancel), asCALL_CDECL));
    ...
    Check(engine->RegisterGlobalFunction("bool GetBool(const string &in, bool)",
        asFUNCTION(PanelState_GetBool), asCALL_CDECL));
    Check(engine->RegisterGlobalFunction("void SetBool(const string &in, bool)",
        asFUNCTION(PanelState_SetBool), asCALL_CDECL));
    Check(engine->RegisterGlobalFunction("float GetFloat(const string &in, float)",
        asFUNCTION(PanelState_GetFloat), asCALL_CDECL));
    Check(engine->RegisterGlobalFunction("void SetFloat(const string &in, float)",
        asFUNCTION(PanelState_SetFloat), asCALL_CDECL));
    engine->SetDefaultNamespace("Hierarchy");
    Check(engine->RegisterGlobalFunction("string GetRows(const string &in)",
        asFUNCTION(Hierarchy_GetRows), asCALL_CDECL));
    engine->SetDefaultNamespace("Inspector");
    Check(engine->RegisterGlobalFunction("string GetSelection()",
        asFUNCTION(Inspector_GetSelection), asCALL_CDECL));
    Check(engine->RegisterGlobalFunction("string GetComponentTypes(uint64)",
        asFUNCTION(Inspector_GetComponentTypes), asCALL_CDECL));
    engine->SetDefaultNamespace("AssetBrowser");
    Check(engine->RegisterGlobalFunction("string GetFolders()",
        asFUNCTION(AssetBrowser_GetFolders), asCALL_CDECL));
    ...
    Check(engine->RegisterGlobalFunction("string GetRows()",
        asFUNCTION(Log_GetRows), asCALL_CDECL));
    Check(engine->RegisterGlobalFunction("void Clear()",
        asFUNCTION(Log_Clear), asCALL_CDECL));
    Check(engine->RegisterGlobalFunction("bool GetAutoScroll()",
        asFUNCTION(Log_GetAutoScroll), asCALL_CDECL));
    Check(engine->RegisterGlobalFunction("void SetAutoScroll(bool)",
        asFUNCTION(Log_SetAutoScroll), asCALL_CDECL));
    engine->SetDefaultNamespace("Profiler");
    Check(engine->RegisterGlobalFunction("string GetRows()",
        asFUNCTION(Profiler_GetRows), asCALL_CDECL));
    Check(engine->RegisterGlobalFunction("string GetFrameStats()",
        asFUNCTION(Profiler_GetFrameStats), asCALL_CDECL));
    Check(engine->RegisterGlobalFunction("void Clear()",
        asFUNCTION(Profiler_Clear), asCALL_CDECL));
    Check(engine->RegisterGlobalFunction("bool GetEnabled()",
        asFUNCTION(Profiler_GetEnabled), asCALL_CDECL));
    Check(engine->RegisterGlobalFunction("void SetEnabled(bool)",
        asFUNCTION(Profiler_SetEnabled), asCALL_CDECL));
    engine->SetDefaultNamespace("");
}
