...
{
    EditorSelectObject object;
    if (path.empty()) return object;
    object.m_Type = EditorSelectObjectType::Asset;
    object.m_AssetPath = std::filesystem::path(std::move(path))
...
void EditorSelection::Select(EditorSelectObject object, EditorSelectionMode mode)
{
    ...
    if (mode == EditorSelectionMode::Toggle && found != m_MultiActorIDs.end()) {
        ...
        if (found == m_MultiActorIDs.end()) m_MultiActorIDs.push_back(actorID);
        m_PrimaryObject = std::move(object);
    }
    PublishIfChanged(previous, previousActorIDs);
}

EditorSelection::ListenerID EditorSelection::SubscribeSelectionChanged(
...
void EditorSelection::Validate(Scene& scene)
{
    ...
    m_MultiActorIDs.erase(
        std::remove_if(m_MultiActorIDs.begin(), m_MultiActorIDs.end(),
...
{
    if (!m_PrimaryObject.IsActor()) return nullptr;
    if (m_PrimaryObject.GetActorHandle().IsValid()) {
        if (Actor* actor = scene.TryGetActor(m_PrimaryObject.GetActorHandle())) return actor;
    }
    return m_PrimaryObject.GetActorID()
...
const Actor* EditorSelection::ResolveActor(const Scene& scene) const
{
    if (!m_PrimaryObject.IsActor()) return nullptr;
    if (m_PrimaryObject.GetActorHandle().IsValid()) {
        if (const Actor* actor = scene.TryGetActor(m_PrimaryObject.GetActorHandle())) return actor;
    }
    return m_PrimaryObject.GetActorID()
        ? scene.FindByID(m_PrimaryObject.GetActorID()) : nullptr;
}
...
void EditorSelection::PublishIfChanged(
    const EditorSelectObject& previous,
    const std::vector<uint64_t>& previousActorIDs)
{
    if (previous == m_PrimaryObject && previousActorIDs == m_MultiActorIDs) return;
