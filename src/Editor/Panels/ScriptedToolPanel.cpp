...
{
    switch (area) {
        case EditorScriptPanelArea::Top: return "top";
        case EditorScriptPanelArea::Left: return "left";
        case EditorScriptPanelArea::Right: return "right";
        case EditorScriptPanelArea::BottomLeft: return "bottomLeft";
        case EditorScriptPanelArea::BottomCenter: return "bottomCenter";
...
ScriptedToolPanel::ScriptedToolPanel(EditorScriptPanelSpec spec)
    : EditorPanel(std::move(spec.id), std::move(spec.title)),
...
void ScriptedToolPanel::DrawContent()
{
    ...
    if (!context || !domain || !domain->IsLoaded()) return;

    std::string error;
    const std::string stateKey = "tool:" + GetID();
    if (!domain->ExecuteExtension(m_Spec.callback, stateKey, *context, &error) &&
        !error.empty()) {
