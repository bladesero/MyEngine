...
const char* IconForAssetType(EditorAssetType type)
{
    switch (type) {
        ...
        case EditorAssetType::Particle: return EditorIcons::Asset;
        case EditorAssetType::Navigation: return EditorIcons::Scene;
...
{
    switch (index) {
        case 1: return EditorAssetType::Model;
        case 2: return EditorAssetType::Texture;
        case 3: return EditorAssetType::Material;
        case 4: return EditorAssetType::Scene;
        case 5: return EditorAssetType::Prefab;
        case 6: return EditorAssetType::Script;
        case 7: return EditorAssetType::Shader;
        case 8: return EditorAssetType::Audio;
        case 9: return EditorAssetType::UI;
        case 10: return EditorAssetType::Particle;
        case 11: return EditorAssetType::Navigation;
        default: return EditorAssetType::Unknown;
    }
}
...
{
    switch (filter) {
        case 1: return asset.imported && asset.importState == AssetImportState::Ready;
        case 2: return asset.imported && asset.importState == AssetImportState::Importing;
        case 3: return asset.imported && asset.importState == AssetImportState::Failed;
        case 4: return asset.imported && asset.importState == AssetImportState::Stale;
        case 5: return asset.imported && asset.importState == AssetImportState::MissingSource;
        case 6: return asset.imported;
        case 7: return !asset.imported;
        default: return true;
    }
}

bool AssetMatchesBrowserFilters(const EditorAssetInfo& asset, const char* textFilter,
                                int typeFilter, int importStateFilter,
                                bool diagnosticsOnly)
{
    if (textFilter && textFilter[0] &&
        asset.relativePath.find(textFilter) == std::string::npos) {
        return false;
    }
    const EditorAssetType type = AssetTypeFromFilterIndex(typeFilter);
    if (type != EditorAssetType::Unknown && asset.type != type) return false;
    if (!AssetMatchesImportFilter(asset, importStateFilter)) return false;
    if (diagnosticsOnly && asset.diagnostics.empty()) return false;
    return true;
}
...
std::string AssetParentFolderForBrowser(const EditorContext& context,
                                        const std::string& assetPath)
{
    ...
    if (StartsWithPath(parentGeneric, "Content") ||
        StartsWithPath(parentGeneric, "SourceAssets")) {
        return parentGeneric;
    }

    auto relativeToRoot = [&](const fs::path& root,
                              const char* browserRoot) -> std::string {
        if (root.empty()) return {};
        std::error_code error;
        const fs::path relative = fs::relative(parent, root.lexically_normal(), error);
        if (error || relative.empty() || IsPathTraversalRelative(relative)) return {};
        const std::string relativeGeneric = relative.generic_string();
        if (relativeGeneric.empty() || relativeGeneric == ".") return browserRoot;
        return std::string(browserRoot) + "/" + relativeGeneric;
    };

    if (std::string folder = relativeToRoot(context.GetContentRoot(), "Content");
        !folder.empty()) {
        return folder;
    }
...
{
    if (actionID == "asset.createMaterial") return "material";
    if (actionID == "asset.createTexture") return "texture";
    if (actionID == "asset.createPrefab") return "prefab";
    if (actionID == "asset.createAngelScript") return "as";
    if (actionID == "asset.createLua") return "lua";
    if (actionID == "asset.createShader") return "shader";
    if (actionID == "asset.createUI") return "ui";
    if (actionID == "asset.createScene") return "scene";
    return nullptr;
}
...
const char* TemplateFallbackFolder(const char* templateID)
{
    ...
    if (std::strcmp(templateID, "material") == 0) return "Materials";
    if (std::strcmp(templateID, "texture") == 0) return "Textures";
    if (std::strcmp(templateID, "prefab") == 0) return "Prefabs";
    return "";
}

int ParseWorkspaceInt(const EditorWorkspace& workspace, const std::string& panelID,
...
bool ParseWorkspaceBool(const EditorWorkspace& workspace, const std::string& panelID,
...
size_t CountProjectSceneReferencesForAssets(EditorContext& context,
...
size_t CountProjectSceneReferencesForFolder(EditorContext& context,
                                            EditorOperators* operators,
                                            const std::string& folder)
{
    auto* registry = context.GetAssetRegistry();
    if (!operators || !registry || folder.empty()) return 0;
    size_t count = 0;
    for (const auto& asset : registry->GetAssetsInFolder(
             std::filesystem::path(folder).lexically_normal().generic_string(),
             true, EditorAssetType::Unknown)) {
        count += operators->Assets()
            .FindProjectSceneReferences(context, asset.relativePath)
            .size();
    }
    return count;
}

std::string AppendRetargetReferenceWarning(std::string message,
                                           size_t currentSceneReferenceCount,
...
bool AssetBrowserPanel::HandleEditorAction(EditorContext& context,
                                           std::string_view actionID)
{
    ...
    if (const char* templateID = TemplateIDForAssetAction(actionID)) {
        EnsureSelectedFolder();
        return RequestCreateAssetFromTemplateInFolder(
            context, m_SelectedFolder, templateID);
    }

    if (actionID == "asset.open") {
        auto* operators = context.GetOperators();
        if (!operators || !context.GetSelection().HasAsset()) return false;
    ...
    if (actionID == "asset.reveal") {
        ...
        EnsureSelectedFolder();
        if (m_SelectedFolder.empty()) return false;
    ...
    if (actionID == "edit.selectAll") {
        ...
        for (const auto& asset : registry->GetAssetsInFolder(
                 m_SelectedFolder, m_RecursiveAssets,
                 AssetTypeFromFilterIndex(m_TypeFilter))) {
            if (!AssetMatchesBrowserFilters(asset, m_Filter, m_TypeFilter,
                                            m_ImportStateFilter,
                                            m_DiagnosticsOnly)) {
                continue;
            }
    ...
    if (actionID == "edit.duplicate") {
        if (ActiveSelectedAssetPaths(context).empty()) return false;
    ...
    if (actionID == "edit.copy") {
        ...
        if (selectedPaths.empty()) return false;
        auto* operators = context.GetOperators();
        const bool copied = operators &&
            operators->Commands().CopyAssets(context, selectedPaths);
        SetOperationMessage(
            copied ? (selectedPaths.size() == 1
                         ? "Copied asset: " +
                               std::filesystem::path(selectedPaths.front())
                                   .filename().string()
                         : "Copied " + std::to_string(selectedPaths.size()) +
                               " asset(s)")
    ...
    if (actionID != "edit.rename") return false;
    if (context.GetSelection().HasAsset()) {
...
bool AssetBrowserPanel::CanHandleEditorAction(
    const EditorContext& context, std::string_view actionID) const
{
    ...
    if (actionID == "asset.move") {
        ...
        const std::filesystem::path targetFolder =
            CurrentContentDirectory("").lexically_normal();
        if (targetFolder.empty()) return false;
        return std::any_of(selectedPaths.begin(), selectedPaths.end(),
            [&](const std::string& path) {
                return std::filesystem::path(path)
                    .lexically_normal()
                    .parent_path()
                    .lexically_normal() != targetFolder;
            });
    }
    ...
    if (actionID == "edit.paste") {
        auto* operators = context.GetOperators();
        return operators && operators->Commands().HasAssetClipboard() &&
...
void AssetBrowserPanel::DeleteSelectedAsset() {
    ...
    SetOperationMessage(
        "Deleted asset: " + std::filesystem::path(path).filename().string(),
        false);
...
void AssetBrowserPanel::DeleteSelectedAssets() {
    ...
    SetOperationMessage(
        deletedCount > 0
            ? "Deleted " + std::to_string(deletedCount) + " asset(s)"
            : "Failed to delete selected asset(s)",
        deletedCount == 0);
}

void AssetBrowserPanel::DrawDeleteReferenceWarning(
    const std::vector<std::string>& paths)
{
#if defined(MYENGINE_ENABLE_IMGUI)
    auto* context = GetContext();
    auto* operators = context ? context->GetOperators() : nullptr;
    if (!operators || paths.empty()) return;
    ...
    for (const std::string& path : paths) {
        const auto references =
            operators->Assets().FindSceneReferences(*context, path);
        const auto projectReferences =
    ...
    const std::string warning =
        std::to_string(totalReferences) +
        " current/project scene reference(s) will become unresolved.";
    ...
    for (const ReferenceRow& row : rows) {
        ImGui::BulletText("%s -> %s%s%s / %s %s",
                          ...
                          row.reference.scenePath.empty()
                              ? ""
                              : row.reference.scenePath.c_str(),
                          row.reference.scenePath.empty() ? "" : " / ",
                          row.reference.actorName.empty()
                              ? "(unnamed actor)"
                              : row.reference.actorName.c_str(),
                          row.reference.componentType.c_str(),
                          row.reference.jsonPath.c_str());
    }
    if (totalReferences > rows.size()) {
        ImGui::TextDisabled("...and %zu more reference(s)",
                            totalReferences - rows.size());
    }
#else
    (void)paths;
#endif
...
std::vector<std::string> AssetBrowserPanel::CollectAssetPathsForFolder(
    const EditorContext& context, const std::string& folderPath) const
{
    std::vector<std::string> paths;
    auto* registry = context.GetAssetRegistry();
    if (!registry || folderPath.empty()) return paths;
    for (const auto& asset : registry->GetAssetsInFolder(
...
void AssetBrowserPanel::RequestDeleteFolder(const std::string& folderPath) {
    if (folderPath.empty() || IsRootFolder(folderPath)) return;
    m_PendingDelete = false;
...
void AssetBrowserPanel::DeleteSelectedFolder() {
    auto* context = GetContext();
    const std::string folderPath = !m_PendingFolderDeletePath.empty()
        ? m_PendingFolderDeletePath
        : m_SelectedFolder;
    if (!context || folderPath.empty() || IsRootFolder(folderPath)) return;
    auto* operators = context->GetOperators();
...
void AssetBrowserPanel::DuplicateSelectedAsset() {
    ...
    SetOperationMessage(
        "Duplicated asset: " + std::filesystem::path(path).filename().string(),
        false);
}
...
void AssetBrowserPanel::ReimportSelectedAssets() {
    ...
    if (reimportedCount > 0) {
        ...
        if (skippedCount > 0) {
            message += "; skipped " + std::to_string(skippedCount) +
                " non-imported asset(s)";
        }
        SetOperationMessage(std::move(message), false);
    } else {
        SetOperationMessage(
            skippedCount > 0
                ? "No imported assets selected for reimport"
                : "Failed to reimport selected asset(s)",
            true);
    }
}

bool AssetBrowserPanel::MoveSelectedAssetsToFolder(
    EditorContext& context, const std::string& targetFolderPath) {
    auto* operators = context.GetOperators();
    const std::filesystem::path rawTargetFolder =
        std::filesystem::path(targetFolderPath).lexically_normal();
    const std::filesystem::path targetFolder =
        rawTargetFolder.is_absolute()
            ? rawTargetFolder
            : FolderPathToAbsolute(targetFolderPath, "").lexically_normal();
    const auto selectedPaths = ActiveSelectedAssetPaths(context);
    ...
    for (const std::string& path : selectedPaths) {
        ...
        const std::filesystem::path newPath =
            (targetFolder / selectedPath.filename()).lexically_normal();
        const size_t referenceCount =
            CountSceneReferencesForAssets(context, operators, {selectedPath.string()});
        const size_t projectReferenceCount =
            CountProjectSceneReferencesForAssets(context, operators, {selectedPath.string()});
        if (operators->Assets().MoveAsset(
                context, selectedPath.string(), targetFolder.string())) {
            ++movedCount;
            ...
            if (referenceCount > 0 || projectReferenceCount > 0) {
                pendingRetargets.push_back(
                    {selectedPath.string(), newPath.string(), referenceCount,
    ...
    m_LastPrimaryAssetPath =
        context.GetSelection().HasAsset()
            ? NormalizePanelAssetPath(context.GetSelection().GetAssetPath())
            : std::string{};
    std::string message = movedCount > 0
        ? "Moved " + std::to_string(movedCount) + " asset(s) into folder: " +
              targetFolder.filename().string()
    ...
    if (movedCount > 0) {
        message = AppendRetargetReferenceWarning(std::move(message),
                                                 movedReferenceCount,
                                                 movedProjectReferenceCount);
    }
...
void AssetBrowserPanel::RenameSelectedAsset() {
    ...
    auto* operators = context->GetOperators();
    const size_t referenceCount =
        CountSceneReferencesForAssets(*context, operators, {path});
    const size_t projectReferenceCount =
        CountProjectSceneReferencesForAssets(*context, operators, {path});
    if (!operators || !operators->Assets().RenameAsset(*context, path, dst.string())) {
        ...
        SetOperationMessage(
            AppendRetargetReferenceWarning("Renamed asset to: " + newName,
                                           referenceCount,
                                           projectReferenceCount),
            false);
        if (referenceCount > 0 || projectReferenceCount > 0) {
            SetPendingRetargets({{src.string(), dst.string(), referenceCount,
                                  projectReferenceCount}});
        }
    }
    m_PendingRename = false;
...
void AssetBrowserPanel::RenameSelectedFolder() {
    ...
    const std::filesystem::path renamed =
        std::filesystem::path(m_PendingFolderRenamePath).parent_path() / newName;
    auto pendingRetargets = BuildPendingRetargetsForFolder(
        *context, operators, m_PendingFolderRenamePath, renamed.generic_string());
    ...
    if (!operators || !operators->Assets().RenameFolder(
            *context, m_PendingFolderRenamePath, newName)) {
        const std::string message =
            "Failed to rename folder: " + m_PendingFolderRenamePath;
        Logger::Warn("[Editor] ", message);
        SetOperationMessage(message, true);
    } else {
        m_SelectedFolder = renamed.generic_string();
        SetOperationMessage(
            AppendRetargetReferenceWarning("Renamed folder to: " + newName,
                                           referenceCount,
                                           projectReferenceCount),
            false);
        if (referenceCount > 0 || projectReferenceCount > 0) {
...
bool AssetBrowserPanel::StartRenameSelectedAsset() {
    ...
    if (const std::string assetFolder = AssetParentFolderForBrowser(*context, path);
        !assetFolder.empty()) {
...
void AssetBrowserPanel::SelectAssetRow(
    EditorContext& context, const std::vector<EditorAssetInfo>& visibleAssets,
    size_t index, bool toggle, bool range) {
    if (index >= visibleAssets.size()) return;
    const std::string path =
        NormalizePanelAssetPath(visibleAssets[index].absolutePath.string());
    auto* operators = context.GetOperators();

    if (range && !m_LastPrimaryAssetPath.empty()) {
        auto findVisible = [&](const std::string& value) {
            return std::find_if(visibleAssets.begin(), visibleAssets.end(),
        ...
        if (anchor != visibleAssets.end()) {
            const size_t anchorIndex =
                static_cast<size_t>(std::distance(visibleAssets.begin(), anchor));
            const size_t first = std::min(anchorIndex, index);
            const size_t last = std::max(anchorIndex, index);
            m_SelectedAssetPaths.clear();
            for (size_t row = first; row <= last; ++row) {
                m_SelectedAssetPaths.push_back(
                    NormalizePanelAssetPath(visibleAssets[row].absolutePath.string()));
            }
            if (operators) operators->Selection().SelectAsset(context, path);
            else context.GetSelection().SelectAssetPath(path);
    ...
    if (toggle) {
        auto selected = std::find(m_SelectedAssetPaths.begin(),
                                  m_SelectedAssetPaths.end(), path);
        if (selected != m_SelectedAssetPaths.end()) {
            m_SelectedAssetPaths.erase(selected);
            if (m_SelectedAssetPaths.empty()) {
                if (operators) operators->Selection().Clear(context);
                else context.GetSelection().Clear();
                m_LastPrimaryAssetPath.clear();
                return;
            }
            const std::string next = m_SelectedAssetPaths.back();
            if (operators) operators->Selection().SelectAsset(context, next);
            else context.GetSelection().SelectAssetPath(next);
...
void AssetBrowserPanel::SelectVisibleAssets(
    EditorContext& context, const std::vector<EditorAssetInfo>& visibleAssets) {
    m_SelectedAssetPaths.clear();
    if (visibleAssets.empty()) return;
    m_SelectedAssetPaths.reserve(visibleAssets.size());
    for (const EditorAssetInfo& asset : visibleAssets) {
        m_SelectedAssetPaths.push_back(
            NormalizePanelAssetPath(asset.absolutePath.string()));
    }
...
bool AssetBrowserPanel::IsAssetSelected(const std::string& path) const {
    ...
    return std::find(m_SelectedAssetPaths.begin(), m_SelectedAssetPaths.end(),
                     normalized) != m_SelectedAssetPaths.end();
}

std::vector<std::string> AssetBrowserPanel::ActiveSelectedAssetPaths(
    const EditorContext& context) const {
    if (!context.GetSelection().HasAsset()) return {};
    const std::string primary =
        NormalizePanelAssetPath(context.GetSelection().GetAssetPath());
    if (primary.empty()) return {};
    if (m_SelectedAssetPaths.empty()) return {primary};
    if (std::find(m_SelectedAssetPaths.begin(), m_SelectedAssetPaths.end(),
                  primary) == m_SelectedAssetPaths.end()) {
        return {primary};
    }
...
void AssetBrowserPanel::SyncAssetSelectionFromContext(const EditorContext& context) {
    ...
    if (m_LastPrimaryAssetPath.empty()) {
        m_LastPrimaryAssetPath = primary;
        if (m_SelectedAssetPaths.empty()) m_SelectedAssetPaths.push_back(primary);
        return;
    }
...
bool AssetBrowserPanel::RequestCreateFolderInFolder(
    EditorContext& context, const std::string& parentFolder,
    bool selectCreated) {
    if (parentFolder.empty()) return false;
    const std::filesystem::path folderPath =
        FolderPathToAbsolute(parentFolder, "") / "New Folder";
    auto* operators = context.GetOperators();
    const bool created = operators &&
        operators->Assets().CreateFolder(context, folderPath.string());
    if (created && selectCreated) {
        m_SelectedFolder =
            (std::filesystem::path(parentFolder) / folderPath.filename())
                .lexically_normal()
                .generic_string();
        EnsureSelectedFolder();
        SaveWorkspaceState();
    ...
    SetOperationMessage(
        created ? "Created folder: " + folderPath.filename().string()
                : "Failed to create folder: " + folderPath.string(),
        !created);
    return created;
}

bool AssetBrowserPanel::RequestCreateAssetFromTemplateInFolder(
    EditorContext& context, const std::string& folderPath,
    const char* templateID) {
    if (folderPath.empty() || !templateID || templateID[0] == '\0') return false;
    auto* operators = context.GetOperators();
    const std::filesystem::path targetPath =
        TemplateTargetDirectoryInFolder(folderPath, templateID);
    const bool created = operators &&
        operators->Assets().CreateAssetFromTemplate(
            context, targetPath.string(), templateID);
    EnsureSelectedFolder();
    SetOperationMessage(
        created ? "Created asset from template: " + targetPath.filename().string()
                : "Failed to create asset from template: " + targetPath.string(),
        !created);
    return created;
}

bool AssetBrowserPanel::RequestPasteAssetsToFolder(
    EditorContext& context, const std::string& targetFolder) {
    if (targetFolder.empty()) return false;
    auto* operators = context.GetOperators();
    const std::filesystem::path rawTargetFolder =
        std::filesystem::path(targetFolder).lexically_normal();
    const std::filesystem::path resolvedTargetFolder =
        rawTargetFolder.is_absolute()
            ? rawTargetFolder
            : FolderPathToAbsolute(targetFolder, "").lexically_normal();
    const bool pasted = operators &&
        operators->Commands().PasteAssetToFolder(
            context, resolvedTargetFolder.string());
    SetOperationMessage(
        pasted ? "Pasted asset into folder: " + resolvedTargetFolder.string()
               : "Failed to paste asset into folder: " +
                     resolvedTargetFolder.string(),
        !pasted);
    return pasted;
}
...
bool AssetBrowserPanel::RequestOpenAsset(const std::string& path) {
    auto* context = GetContext();
    if (!context || path.empty()) return false;
    ...
    SetOperationMessage(
        opened ? "Opened asset: " + std::filesystem::path(path).filename().string()
...
bool AssetBrowserPanel::RequestRevealPath(const std::string& path) {
    ...
    SetOperationMessage(
        revealed ? "Revealed in Explorer: " + std::filesystem::path(path).filename().string()
...
bool AssetBrowserPanel::RequestValidateAssets() {
    ...
    if (!context) return false;
    auto* operators = context->GetOperators();
    const bool requested = operators &&
        operators->Commands().ExecuteAction(*context, "asset.validate");
    EnsureSelectedFolder();
    std::string message = "Failed to validate assets.";
    if (requested) {
        if (auto* imports = context->GetService<EditorImportService>()) {
            SetValidationSummary(imports->GetValidationSummaryText(),
                                 imports->HasValidationIssues());
...
bool AssetBrowserPanel::OpenPendingSceneAsset(bool discardUnsavedChanges) {
    auto* context = GetContext();
    if (!context || m_PendingSceneOpenPath.empty()) return false;
    SceneRenderLayer* layer = context->GetSceneLayer();
    if (!layer) return false;
    if (discardUnsavedChanges) layer->ClearDirty();
    auto* operators = context->GetOperators();
    const bool opened = operators &&
        operators->Assets().OpenAsset(*context, m_PendingSceneOpenPath);
    if (!opened && discardUnsavedChanges) layer->MarkDirty();
    if (opened) {
        SetOperationMessage(
            "Opened asset: " +
            std::filesystem::path(m_PendingSceneOpenPath).filename().string(),
            false);
        m_PendingSceneOpenPath.clear();
        m_PendingSceneOpenPopup = false;
    } else {
        SetOperationMessage(
            "Failed to open asset: " + m_PendingSceneOpenPath, true);
    }
    return opened;
}
...
void AssetBrowserPanel::DrawPendingSceneOpenModal() {
#if defined(MYENGINE_ENABLE_IMGUI)
    auto* context = GetContext();
    if (!context || m_PendingSceneOpenPath.empty()) return;
    SceneRenderLayer* layer = context->GetSceneLayer();
    if (!layer) return;
    ...
    if (ImGui::Button("Save")) {
        if (layer->HasFilePath()) {
            if (layer->SaveScene()) OpenPendingSceneAsset(false);
...
AssetBrowserPanel::BuildPendingRetargetsForFolder(
    EditorContext& context, EditorOperators* operators,
    const std::string& oldFolder, const std::string& newFolder) const {
    std::vector<PendingAssetRetarget> retargets;
    auto* registry = context.GetAssetRegistry();
    if (!operators || !registry || oldFolder.empty() || newFolder.empty() ||
        oldFolder == newFolder) {
        return retargets;
    }

    const std::filesystem::path oldRoot =
        std::filesystem::path(oldFolder).lexically_normal();
    const std::filesystem::path newRoot =
        std::filesystem::path(newFolder).lexically_normal();
    for (const auto& asset : registry->GetAssetsInFolder(
             oldRoot.generic_string(), true, EditorAssetType::Unknown)) {
        const std::filesystem::path oldPath =
            std::filesystem::path(asset.relativePath).lexically_normal();
        std::filesystem::path suffix = oldPath.lexically_relative(oldRoot);
        if (suffix.empty() ||
            suffix.generic_string().find("..") != std::string::npos) {
            suffix = oldPath.filename();
        }
        const std::filesystem::path newPath =
            (newRoot / suffix).lexically_normal();
        const size_t referenceCount =
            operators->Assets().FindSceneReferences(context,
                                                    oldPath.generic_string()).size();
        const size_t projectReferenceCount =
            operators->Assets().FindProjectSceneReferences(
                context, oldPath.generic_string()).size();
        if (referenceCount > 0 || projectReferenceCount > 0) {
            retargets.push_back(
                {oldPath.generic_string(), newPath.generic_string(),
...
void AssetBrowserPanel::SetPendingRetargets(
    std::vector<PendingAssetRetarget> retargets) {
    m_PendingAssetRetargets.clear();
    for (auto& retarget : retargets) {
        if (retarget.oldPath.empty() || retarget.newPath.empty() ||
            retarget.oldPath == retarget.newPath ||
            (retarget.referenceCount == 0 &&
             retarget.projectReferenceCount == 0)) {
            continue;
        }
...
bool AssetBrowserPanel::ExecutePendingRetargets() {
    auto* context = GetContext();
    auto* operators = context ? context->GetOperators() : nullptr;
    if (!context || !operators || m_PendingAssetRetargets.empty()) return false;
    ...
    for (const auto& retarget : retargets) {
        ...
        if (retarget.referenceCount > 0) {
            currentSceneChangedCount += operators->Assets().RetargetSceneReferences(
                *context, retarget.oldPath, retarget.newPath);
        }
        if (retarget.projectReferenceCount > 0) {
            projectSceneChangedCount +=
                operators->Assets().RetargetProjectSceneReferences(
                    *context, retarget.oldPath, retarget.newPath);
        }
    }
    ...
    SetOperationMessage(
        changedCount > 0
            ? "Retargeted " + std::to_string(currentSceneChangedCount) +
                  " current scene reference(s) and " +
                  std::to_string(projectSceneChangedCount) +
                  " project scene reference(s)"
...
void AssetBrowserPanel::LoadWorkspaceState() {
    ...
    m_ImportStateFilter = std::clamp(
        ParseWorkspaceInt(*workspace, GetID(), "importStateFilter", m_ImportStateFilter), 0, 7);
...
std::filesystem::path AssetBrowserPanel::FolderPathToAbsolute(
    const std::string& folderPath, const char* fallback) const {
    auto* context = GetContext();
    if (!context) return {};
    const auto projectRoot = context->GetContentRoot().parent_path();
    if (StartsWithPath(folderPath, "Content")) {
        const std::string suffix = folderPath == "Content"
            ? std::string{}
            : folderPath.substr(std::string("Content/").size());
        return suffix.empty()
            ? context->GetContentRoot()
            : context->GetContentRoot() / std::filesystem::path(suffix);
    }
    if (StartsWithPath(folderPath, "SourceAssets")) {
        const std::string suffix = folderPath == "SourceAssets"
            ? std::string{}
            : folderPath.substr(std::string("SourceAssets/").size());
        return suffix.empty()
            ? projectRoot / "SourceAssets"
...
std::filesystem::path AssetBrowserPanel::TemplateTargetDirectoryInFolder(
    const std::string& folderPath, const char* templateID) const {
    auto* context = GetContext();
    if (!context) return {};
...
std::filesystem::path AssetBrowserPanel::TemplateTargetDirectory(
...
void AssetBrowserPanel::DrawContent(){
    ...
    auto* operators = context->GetOperators();
    auto refreshAssets = [&]() {
        if (operators) return operators->Assets().Refresh(*context);
        EditorAssetOperator assetOperator;
    ...
    if(EditorWidgets::IconButton(*context, "ReimportAllAssets", EditorIcons::Refresh, "Reimport All")){
        std::vector<std::string> failures;const bool reimported=operators&&operators->Assets().ReimportAll(*context,&failures);if(!failures.empty()){Logger::Warn("[Editor] Reimport failures: ",failures.front());SetOperationMessage("Reimport completed with failures: "+failures.front(),true);}else{SetOperationMessage(reimported?"Reimported all assets":"Failed to reimport assets",!reimported);}EnsureSelectedFolder();
    }ImGui::SameLine();
    ...
    if (!m_OperationMessage.empty()) {
        ...
        if (!m_PendingAssetRetargets.empty()) {
            ...
            ImGui::EndDisabled();
            ImGui::SameLine();
            ImGui::TextDisabled("%zu current / %zu project scene reference(s)",
                                referenceCount, projectReferenceCount);
            ImGui::SameLine();
    ...
    if (m_PendingDelete) {
        ...
        if (selectedForDelete.empty()) {
            ...
            const std::string deleteMessage =
                "Delete " + std::to_string(selectedForDelete.size()) +
                " selected asset(s)?";
            EditorWidgets::InlineMessage(
    ...
    if (m_PendingFolderDelete && !m_PendingFolderDeletePath.empty()) {
        const std::string folderDeleteMessage =
            "Delete folder and all contained assets: " +
            m_PendingFolderDeletePath + "?";
        EditorWidgets::InlineMessage(
            Editor::UI::EditorNotificationType::Warning,
            folderDeleteMessage.c_str());
        const auto folderDeletePaths =
            CollectAssetPathsForFolder(*context, m_PendingFolderDeletePath);
    ...
    const char* typeItems[] = {
        "All Types", "Model", "Texture", "Material", "Scene", "Prefab",
        "Script", "Shader", "Audio", "UI", "Particle", "Navigation"
    };
    ImGui::SetNextItemWidth(120.0f);
    ImGui::Combo("##AssetTypeFilter", &m_TypeFilter,
                 typeItems, static_cast<int>(std::size(typeItems)));
    ImGui::SameLine();
    const char* importItems[] = {
        "All States", "Ready", "Importing", "Failed", "Stale",
        "Missing Source", "Imported", "Not Imported"
    };
    ImGui::SetNextItemWidth(140.0f);
    ImGui::Combo("##ImportStateFilter", &m_ImportStateFilter,
                 importItems, static_cast<int>(std::size(importItems)));
    ImGui::SameLine();
    ...
    if (ImGui::BeginChild("##AssetFolderTree", ImVec2(220.0f, 0), true)) {
        ...
        std::function<void(const EditorAssetFolderInfo&)> drawFolder =
            [&](const EditorAssetFolderInfo& folder) {
                ...
                ImGui::SameLine();
                if (m_PendingFolderRename &&
                    m_PendingFolderRenamePath == folder.relativePath) {
                    ImGui::SetKeyboardFocusHere();
                    if (ImGui::InputText("##folderRename", m_FolderRenameBuffer,
                ...
                if (ImGui::BeginDragDropTarget()) {
                    if (const ImGuiPayload* payload = ImGui::AcceptDragDropPayload(kAssetPayload)) {
                        if (operators) {
                            ...
                            const std::filesystem::path targetFolder =
                                FolderPathToAbsolute(folder.relativePath, "")
                                    .lexically_normal();
                            const size_t referenceCount =
                                CountSceneReferencesForAssets(
                                    *context, operators, {source});
                            const size_t projectReferenceCount =
                                CountProjectSceneReferencesForAssets(
                                    *context, operators, {source});
                            const std::filesystem::path newPath =
                                (targetFolder /
                                 std::filesystem::path(source).filename())
                                    .lexically_normal();
                            if (!operators->Assets().MoveAsset(
                                    *context, source, targetFolder.string())) {
                                SetOperationMessage(
                                    "Failed to move asset into folder: " +
                                    folder.relativePath, true);
                            } else {
                                SetOperationMessage(
                                    AppendRetargetReferenceWarning(
                                        "Moved asset into folder: " +
                                            folder.relativePath,
                                        referenceCount,
                                        projectReferenceCount),
                                    false);
                    ...
                    if (const ImGuiPayload* payload = ImGui::AcceptDragDropPayload(kFolderPayload)) {
                        ...
                        if (operators && source != folder.relativePath &&
                            !StartsWithPath(folder.relativePath, source)) {
                            const std::filesystem::path newFolder =
                                (std::filesystem::path(folder.relativePath) /
                                 std::filesystem::path(source).filename())
                                    .lexically_normal();
                            auto pendingRetargets = BuildPendingRetargetsForFolder(
                                *context, operators, source,
                                newFolder.generic_string());
                            size_t referenceCount = 0;
                            size_t projectReferenceCount = 0;
                            for (const auto& retarget : pendingRetargets) {
                                referenceCount += retarget.referenceCount;
                                projectReferenceCount +=
                                    retarget.projectReferenceCount;
                            }
                            if (!operators->Assets().MoveFolder(
                                    *context, source, folder.relativePath)) {
                                SetOperationMessage(
                                    "Failed to move folder into folder: " +
                                    folder.relativePath, true);
                            } else {
                                SetOperationMessage(
                                    AppendRetargetReferenceWarning(
                ...
                if (ImGui::BeginPopup("##FolderCtx")) {
                    ...
                    ImGui::EndDisabled();
                    const bool canPasteIntoFolder =
                        operators && operators->Commands().HasAssetClipboard();
                    ...
                    if (ImGui::BeginMenu("Create")) {
                        if (ImGui::MenuItem("Material")) {
                            RequestCreateAssetFromTemplateInFolder(
                                *context, folder.relativePath, "material");
                        }
                        if (ImGui::MenuItem("Default Texture")) {
                            RequestCreateAssetFromTemplateInFolder(
                                *context, folder.relativePath, "texture");
                        }
                        if (ImGui::MenuItem("Prefab")) {
                            RequestCreateAssetFromTemplateInFolder(
                                *context, folder.relativePath, "prefab");
                        }
                        ImGui::Separator();
                        if (ImGui::MenuItem("AngelScript")) {
                            RequestCreateAssetFromTemplateInFolder(
                                *context, folder.relativePath, "as");
                        }
                        if (ImGui::MenuItem("Lua Script")) {
                            RequestCreateAssetFromTemplateInFolder(
                                *context, folder.relativePath, "lua");
                        }
                        if (ImGui::MenuItem("Shader")) {
                            RequestCreateAssetFromTemplateInFolder(
                                *context, folder.relativePath, "shader");
                        }
                        if (ImGui::MenuItem("UI Document")) {
                            RequestCreateAssetFromTemplateInFolder(
                                *context, folder.relativePath, "ui");
                        }
                        if (ImGui::MenuItem("Scene")) {
                            RequestCreateAssetFromTemplateInFolder(
                                *context, folder.relativePath, "scene");
                        }
                        ImGui::EndMenu();
                    }
    ...
    std::vector<EditorAssetInfo> visibleAssets;
    const auto folderAssets = registry->GetAssetsInFolder(
        m_SelectedFolder, m_RecursiveAssets,
        AssetTypeFromFilterIndex(m_TypeFilter));
    ...
    if (visibleDiagnosticCount > 0) {
        const std::string diagnosticsHeader =
            "Diagnostics Issues (" + std::to_string(visibleDiagnosticCount) + ")";
        if (ImGui::CollapsingHeader(diagnosticsHeader.c_str(),
                                    ImGuiTreeNodeFlags_DefaultOpen)) {
            const float issueListHeight =
                std::min(160.0f, 26.0f * static_cast<float>(visibleDiagnosticCount + 1));
            if (ImGui::BeginChild("##AssetDiagnosticsIssues",
                                  ImVec2(0.0f, issueListHeight), true)) {
                for (const auto& asset : visibleAssets) {
                    if (asset.diagnostics.empty()) continue;
                    ImGui::PushID(asset.relativePath.c_str());
                    const bool open = ImGui::TreeNodeEx(
                        "AssetDiagnosticsGroup",
                        ImGuiTreeNodeFlags_DefaultOpen,
                        "%s", asset.relativePath.c_str());
                    if (open) {
                        for (size_t diagnosticIndex = 0;
                             diagnosticIndex < asset.diagnostics.size();
                             ++diagnosticIndex) {
                            const auto& diagnostic = asset.diagnostics[diagnosticIndex];
                            ImGui::PushID(static_cast<int>(diagnosticIndex));
                            const std::string issueLabel =
                                diagnostic.severity + ": " + diagnostic.message;
                            if (ImGui::Selectable(issueLabel.c_str(), false)) {
                                if (operators) {
                                    operators->Selection().SelectAsset(
                                        *context, asset.absolutePath.string());
                                }
    ...
    for(size_t assetIndex = 0; assetIndex < visibleAssets.size(); ++assetIndex){
        ...
        if(ImGui::Selectable(label.c_str(),selected)){
            ...
            SelectAssetRow(*context, visibleAssets, assetIndex,
        ...
        if (ImGui::BeginPopup("##AssetItemCtx")) {
            ...
            const std::string assetFolder =
                std::filesystem::path(asset.relativePath)
                    ...
                    .generic_string();
            if (asset.imported &&
                ImGui::Selectable(hasMultiSelection ? "Reimport Selected" : "Reimport")) {
                    ...
                    SetOperationMessage(
                        copied
                            ? (selectedPaths.size() == 1
                                   ? "Copied asset: " +
                                         asset.absolutePath.filename().string()
                                   : "Copied " +
                                         std::to_string(selectedPaths.size()) +
                                         " asset(s)")
                               : "Failed to copy selected asset",
                        !copied);
                }
            ...
            if (ImGui::Selectable("Move Selected Here")) {
                MoveSelectedAssetsToFolder(
                    *context, assetFolder.empty() ? m_SelectedFolder : assetFolder);
            }
            ImGui::EndDisabled();
            if (ImGui::Selectable("Paste Into Folder")) {
                RequestPasteAssetsToFolder(
                    *context, assetFolder.empty() ? m_SelectedFolder : assetFolder);
            }
            if (ImGui::Selectable("Delete")) { RequestDeleteSelectedAssets(); }
            if (ImGui::Selectable("Duplicate")) { DuplicateSelectedAssets(); }
    ...
    if (ImGui::BeginPopup("##AssetCtxMenu")) {
        if (ImGui::Selectable("Refresh")) { refreshAssets(); EnsureSelectedFolder(); SetOperationMessage("Refreshed asset registry", false); }
        ...
        if (ImGui::Selectable("Copy Selected")) {
            const bool copied = operators &&
                operators->Commands().CopyAssets(*context, emptyContextSelection);
            SetOperationMessage(
                copied ? "Copied " +
                             std::to_string(emptyContextSelection.size()) +
                             " asset(s)"
        ...
        if (ImGui::BeginMenu("Create")) {
            if (ImGui::MenuItem("Material")) {
                RequestCreateAssetFromTemplateInFolder(
                    *context, m_SelectedFolder, "material");
            }
            if (ImGui::MenuItem("Default Texture")) {
                RequestCreateAssetFromTemplateInFolder(
                    *context, m_SelectedFolder, "texture");
            }
            ImGui::Separator();
            if (ImGui::MenuItem("AngelScript")) {
                RequestCreateAssetFromTemplateInFolder(
                    *context, m_SelectedFolder, "as");
            }
            if (ImGui::MenuItem("Lua Script")) {
                RequestCreateAssetFromTemplateInFolder(
                    *context, m_SelectedFolder, "lua");
            }
            if (ImGui::MenuItem("Shader")) {
                RequestCreateAssetFromTemplateInFolder(
                    *context, m_SelectedFolder, "shader");
            }
            if (ImGui::MenuItem("Prefab")) {
                RequestCreateAssetFromTemplateInFolder(
                    *context, m_SelectedFolder, "prefab");
            }
            if (ImGui::MenuItem("UI Document")) {
                RequestCreateAssetFromTemplateInFolder(
                    *context, m_SelectedFolder, "ui");
            }
            if (ImGui::MenuItem("Scene")) {
                RequestCreateAssetFromTemplateInFolder(
                    *context, m_SelectedFolder, "scene");
            }
            ImGui::EndMenu();
            EnsureSelectedFolder();
        }
