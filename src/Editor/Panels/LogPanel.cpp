...
void LogPanel::DrawContent(){
    ...
    if (ImGui::Checkbox("Auto-scroll", &autoScroll)) logs->SetAutoScroll(autoScroll);
    ImGui::Separator();
    ...
    if (logFont) ImGui::PushFont(logFont);
    for (const std::string& line : logs->Snapshot()) {
        const bool error = line.find("ShaderCompileError") != std::string::npos ||
            line.find("[Error]") != std::string::npos;
        if (error) ImGui::PushStyleColor(ImGuiCol_Text, {1, 0.42f, 0.38f, 1});
        ImGui::TextUnformatted(line.c_str());
        if (error) ImGui::PopStyleColor();
    }
    if (logs->IsAutoScroll() && logs->ConsumeScrollRequest()) ImGui::SetScrollHereY(1);
    if (logFont) ImGui::PopFont();
    ImGui::EndChild();
#endif
}
