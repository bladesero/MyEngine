...
const char* ComponentCategory(const std::string& type)
{
    if (type == "MeshRenderer" || type == "SkinnedMeshRenderer" ||
        type == "Camera" || type == "Light" || type == "PostProcess") {
        return "Rendering";
    }
...
bool CommitInspectorActorName(EditorContext& context, Actor& actor,
                              const char* name)
{
    const std::string afterName = name ? name : "";
    if (afterName == actor.GetName()) return false;
...
bool CommitInspectorActorActive(EditorContext& context, Actor& actor,
                                bool active)
{
    if (active == actor.IsActiveSelf()) return false;
    if (auto* operators = context.GetOperators()) {
...
bool CommitInspectorActorTag(EditorContext& context, Actor& actor,
                             const char* tag)
{
    const std::string afterTag = tag ? tag : "";
    if (afterTag == actor.GetTag()) return false;
    if (auto* operators = context.GetOperators()) {
...
bool CommitInspectorActorLayer(EditorContext& context, Actor& actor,
                               uint32_t layer)
{
    if (layer == actor.GetLayer()) return false;
    if (auto* operators = context.GetOperators()) {
...
bool CommitInspectorActorStatic(EditorContext& context, Actor& actor,
                                bool isStatic)
{
    if (isStatic == actor.IsStatic()) return false;
    if (auto* operators = context.GetOperators()) {
...
std::vector<Actor*> ResolveSelectedActors(EditorContext& context, Scene& scene)
{
    ...
    for (uint64_t actorID : context.GetSelection().GetActorIDs()) {
        if (Actor* actor = scene.FindByID(actorID)) actors.push_back(actor);
    }
    return actors;
}
...
bool CommonActiveState(const std::vector<Actor*>& actors, bool& active)
{
    ...
    for (const Actor* actor : actors) {
        if (!actor || actor->IsActiveSelf() != active) return false;
    }
    return true;
}
...
bool CommonTagValue(const std::vector<Actor*>& actors, std::string& tag)
{
    ...
    for (const Actor* actor : actors) {
        if (!actor || actor->GetTag() != tag) return false;
    }
    return true;
}
...
bool CommonLayerValue(const std::vector<Actor*>& actors, uint32_t& layer)
{
    ...
    for (const Actor* actor : actors) {
        if (!actor || actor->GetLayer() != layer) return false;
    }
    return true;
}
...
bool CommonStaticState(const std::vector<Actor*>& actors, bool& isStatic)
{
    ...
    for (const Actor* actor : actors) {
        if (!actor || actor->IsStatic() != isStatic) return false;
    }
    return true;
}
...
std::vector<uint64_t> ActorIDsFor(const std::vector<Actor*>& actors)
{
    ...
    for (const Actor* actor : actors) {
        if (actor) ids.push_back(actor->GetID());
    }
    return ids;
}
...
{
    if (actors.empty() || !actors.front()) return {};

    std::set<std::string> common;
    actors.front()->ForEachComponent([&](Component& component) {
        common.insert(component.GetTypeName());
    });
    ...
    for (size_t index = 1; index < actors.size() && !common.empty(); ++index) {
        ...
        for (auto it = common.begin(); it != common.end();) {
            if (!actor->HasComponentType(*it)) it = common.erase(it);
            else ++it;
        }
    }
...
bool CompatiblePropertyType(const nlohmann::json& a, const nlohmann::json& b)
{
    ...
    if (IsEditableVec3Json(a) && IsEditableVec3Json(b)) return true;
...
std::vector<CommonComponentProperty> CommonComponentProperties(
    const std::vector<Actor*>& actors, const std::string& typeName)
{
    if (actors.empty() || typeName.empty()) return {};
    const Component* firstComponent =
        actors.front() ? actors.front()->GetComponentByTypeName(typeName) : nullptr;
    if (!firstComponent) return {};

    nlohmann::json firstData = nlohmann::json::object();
    firstComponent->Serialize(firstData);
    if (!firstData.is_object()) return {};

    std::vector<CommonComponentProperty> properties;
    for (const auto& item : firstData.items()) {
        if (!IsEditableCommonProperty(item.value())) continue;
        ...
        for (size_t index = 1; index < actors.size(); ++index) {
            const Component* component = actors[index]
                ? actors[index]->GetComponentByTypeName(typeName) : nullptr;
            if (!component) {
                presentOnAll = false;
                break;
            }
            ...
            if (!data.is_object() || found == data.end() ||
                !CompatiblePropertyType(property.value, *found) ||
                !IsEditableCommonProperty(*found)) {
                presentOnAll = false;
                break;
            }
            if (*found != property.value) property.hasCommonValue = false;
        }

        if (presentOnAll) properties.push_back(std::move(property));
    }
    return properties;
}
...
void InspectorPanel::OnSelectionChanged(const EditorSelectionChangedEvent& event)
{
    ...
    std::fill(std::begin(m_MultiAddComponentSearch),
              std::end(m_MultiAddComponentSearch), '\0');
...
void InspectorPanel::DrawContent()
{
    ...
    const bool multiActorSelection = m_SelectedObject.IsActor() &&
        context->GetSelection().GetActorIDs().size() > 1;
    if (multiActorSelection) {
        ...
        ImGui::Text("Active: %s",
                    hasCommonActive ? (commonActive ? "On" : "Off") : "Mixed");
        ...
        if (!m_MultiActorTagDirty) {
            std::fill(std::begin(m_MultiActorTagBuffer),
                      std::end(m_MultiActorTagBuffer), '\0');
            if (hasCommonTag) {
                const size_t count = std::min(
                    commonTag.size(), sizeof(m_MultiActorTagBuffer) - 1);
                std::memcpy(m_MultiActorTagBuffer, commonTag.data(), count);
            }
        }
        if (!hasCommonTag) ImGui::TextDisabled("Tag: Mixed");
        if (ImGui::InputText("Tag", m_MultiActorTagBuffer,
                             sizeof(m_MultiActorTagBuffer))) {
            m_MultiActorTagDirty = true;
        }
        if (ImGui::IsItemDeactivatedAfterEdit()) {
            if (auto* operators = context->GetOperators()) {
                operators->Commands().SetActorsTag(
                    *context, actorIDs, m_MultiActorTagBuffer);
            }
            m_MultiActorTagDirty = false;
        }
        ...
        if (!hasCommonLayer) ImGui::TextDisabled("Layer: Mixed");
        ...
        ImGui::Text("Static: %s",
                    hasCommonStatic ? (commonStatic ? "On" : "Off") : "Mixed");
        ...
        ImGui::Separator();
        const std::vector<std::string> commonComponents =
            CommonComponentTypes(actors);
        if (ImGui::TreeNodeEx("Common Components",
            ...
            if (commonComponents.empty()) {
                ...
                for (const std::string& typeName : commonComponents) {
                    ...
                    if (ImGui::SmallButton("Remove")) {
                        if (auto* operators = context->GetOperators()) {
                            operators->Components().RemoveComponents(
                                *context, actorIDs, typeName);
                        }
                    }
                    if (open) {
                        ...
                        if (properties.empty()) {
                            ...
                            for (const CommonComponentProperty& property : properties) {
                                ...
                                if (property.value.is_boolean()) {
                                ...
                                } else if (property.value.is_number_integer() ||
                                           property.value.is_number_unsigned()) {
                                    ...
                                    float values[3] = {
                                        property.value[0].get<float>(),
                                        property.value[1].get<float>(),
        ...
        if (ImGui::BeginCombo("##MultiAddComponent", "Add Component to Selection...")) {
            ...
            ImGui::InputTextWithHint("##MultiAddComponentSearch",
                                     "Search components...",
                                     m_MultiAddComponentSearch,
                                     sizeof(m_MultiAddComponentSearch));
            ImGui::Separator();

            std::vector<std::string> types =
                ComponentRegistry::Get().GetRegisteredTypes();
            ...
            for (const char* category : kCategories) {
                ...
                if (!hasVisible || !ImGui::BeginMenu(category)) continue;
                for (const std::string& type : types) {
                    if (std::strcmp(ComponentCategory(type), category) != 0 ||
                        !ComponentMatchesFilter(type, m_MultiAddComponentSearch)) {
                        continue;
                    }
                    const bool existsOnAny = std::any_of(
                        actors.begin(), actors.end(),
                        [&](const Actor* selectedActor) {
                            return selectedActor &&
                                selectedActor->HasComponentType(type);
                        });
                    if (existsOnAny) ImGui::BeginDisabled();
                    const std::string label = ComponentDisplayName(type);
                    if (ImGui::Selectable(label.c_str()) && !existsOnAny) {
                        if (auto* operators = context->GetOperators()) {
                            operators->Components().AddComponents(
                                *context, actorIDs, type, nlohmann::json::object());
                        }
                    }
                    if (ImGui::IsItemHovered()) ImGui::SetTooltip("%s", type.c_str());
                    if (existsOnAny) ImGui::EndDisabled();
                }
                ImGui::EndMenu();
            }
            ImGui::EndCombo();
    ...
    if(actor && actor->IsPrefabRoot()){
        ...
        auto refreshPrefabOverrides = [&]() {
            prefabOverrides = operators
                ? operators->Prefabs().GetOverrides(*context, actor->GetID())
        ...
        if(ImGui::Button("Apply All")){
            ...
            setPrefabOperationMessage(
                refreshed ? "Applied prefab overrides" : "Failed to apply prefab overrides",
                !refreshed);
            directCommand = directCommand || refreshed;
        }
        ImGui::SameLine();if(ImGui::Button("Revert All")){refreshed=operators->Prefabs().RevertAll(*context,actor->GetID());setPrefabOperationMessage(refreshed ? "Reverted prefab overrides" : "Failed to revert prefab overrides", !refreshed);directCommand=directCommand||refreshed;}
        ImGui::EndDisabled();
        ImGui::SameLine();ImGui::BeginDisabled(!operators);if(ImGui::Button("Unpack")){refreshed=operators->Prefabs().Unpack(*context,actor->GetID());setPrefabOperationMessage(refreshed ? "Unpacked prefab instance" : "Failed to unpack prefab instance", !refreshed);directCommand=directCommand||refreshed;}ImGui::EndDisabled();
        ImGui::SameLine();
        ImGui::BeginDisabled(!operators);
        if(ImGui::Button("Select Source")){
            const std::string sourcePath =
                PrefabSystem::ResolvePrefabPath(actor->GetPrefabAssetPath()).string();
            const bool selectedSource =
                operators->Selection().SelectAsset(*context, sourcePath);
            if (selectedSource) {
                context->RequestPanelFocus("inspector");
            }
            setPrefabOperationMessage(
                selectedSource ? "Selected prefab source: " +
        ...
        if (!m_PrefabOperationMessage.empty()) {
            Editor::UI::EditorWidgets::InlineMessage(
                m_PrefabOperationMessageIsError
                    ? Editor::UI::EditorNotificationType::Error
                    : Editor::UI::EditorNotificationType::Success,
                m_PrefabOperationMessage.c_str());
        }
        if(refreshed){actor=context->GetSelection().ResolveActor(*scene);if(!actor){ImGui::EndDisabled();return;}refreshPrefabOverrides();}
        if (actor && operators) {
            const auto& overrides = prefabOverrides;
            const std::string header =
                "Overrides (" + std::to_string(overrides.size()) + ")";
            if (ImGui::TreeNodeEx(header.c_str(), ImGuiTreeNodeFlags_DefaultOpen)) {
                if (overrides.empty()) {
                    ImGui::TextDisabled("No overrides");
                } else {
                    ImGui::InputTextWithHint("##PrefabOverrideFilter", "Filter overrides...",
                                             m_PrefabOverrideFilter,
                    ...
                    auto matchesOverrideFilter = [&](const auto& item) {
                        ...
                        if (m_PrefabOverrideFilter[0] == '\0') return true;
                    ...
                    ImGui::TextDisabled("%d shown / %d total - %d ready, %d blocked",
                                        visibleOverrideCount,
                                        static_cast<int>(overrides.size()),
                                        visibleReadyCount,
                                        visibleBlockedCount);
                    ...
                    if (visibleOverrideCount == 0) {
                        ...
                        for (const auto& item : overrides) {
                            if (!matchesOverrideFilter(item)) continue;
                            if (item.category != currentCategory) {
                                currentCategory = item.category;
                                auto categoryState =
                                    m_PrefabOverrideCategoryOpen.try_emplace(
                                        currentCategory, true).first;
                                currentCategoryOpen = categoryState->second;
                                ImGui::TableNextRow();
                                ImGui::TableSetColumnIndex(0);
                                const std::string groupLabel =
                                    std::string(currentCategoryOpen ? "v " : "> ") +
                                    currentCategory + " (" +
                                    std::to_string(categoryCounts[currentCategory]) + ")";
                                ImGui::PushID(("PrefabOverrideGroup" + currentCategory).c_str());
                                if (ImGui::Selectable(groupLabel.c_str(), false,
                            ...
                            if (!currentCategoryOpen) continue;
                            ...
                            if (ImGui::SmallButton("Apply")) {
                                refreshed = context->GetOperators()->Prefabs().ApplyOverride(
                                    *context, actor->GetID(), item.index);
                                setPrefabOperationMessage(
                                    refreshed
                                        ? "Applied prefab override: " + item.label
                            ...
                            if (ImGui::SmallButton("Revert")) {
                                refreshed = context->GetOperators()->Prefabs().RevertOverride(
                                    *context, actor->GetID(), item.index);
                                setPrefabOperationMessage(
                                    refreshed
                                        ? "Reverted prefab override: " + item.label
                        ...
                        if (refreshed) {
                            ...
                            if (!actor) { ImGui::TreePop(); ImGui::EndDisabled(); return; }
                        }
                    }
                }
                ImGui::TreePop();
    ...
    if (canEditSelection && actor) {
        ...
        } else if (!m_ActorNameDirty &&
                   std::string(m_ActorNameBuffer) != actor->GetName()) {
            CopyActorNameToBuffer(*actor, m_ActorNameBuffer, sizeof(m_ActorNameBuffer));
        }
        if (ImGui::InputText("Name", m_ActorNameBuffer,
                             sizeof(m_ActorNameBuffer))) {
            m_ActorNameDirty = true;
        }
        if (ImGui::IsItemDeactivatedAfterEdit()) {
            const bool renamed = CommitInspectorActorName(
                *context, *actor, m_ActorNameBuffer);
            directCommand = directCommand || renamed;
            actor = context->GetSelection().ResolveActor(*scene);
            if (actor) {
                m_ActorNameEditID = actor->GetID();
                CopyActorNameToBuffer(*actor, m_ActorNameBuffer,
                                      sizeof(m_ActorNameBuffer));
            }
            m_ActorNameDirty = false;
        }
        ...
        if (actor) {
            ...
            } else if (!m_ActorTagDirty &&
                       std::string(m_ActorTagBuffer) != actor->GetTag()) {
                CopyActorTagToBuffer(*actor, m_ActorTagBuffer, sizeof(m_ActorTagBuffer));
            }
            if (ImGui::InputText("Tag", m_ActorTagBuffer,
                                 sizeof(m_ActorTagBuffer))) {
                m_ActorTagDirty = true;
            }
            if (ImGui::IsItemDeactivatedAfterEdit()) {
                const bool changed = CommitInspectorActorTag(
                    *context, *actor, m_ActorTagBuffer);
                directCommand = directCommand || changed;
                actor = context->GetSelection().ResolveActor(*scene);
                if (actor) {
                    m_ActorTagEditID = actor->GetID();
                    CopyActorTagToBuffer(*actor, m_ActorTagBuffer,
                                         sizeof(m_ActorTagBuffer));
                }
                m_ActorTagDirty = false;
            }
