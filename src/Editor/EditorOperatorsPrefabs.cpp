...
std::vector<EditorPrefabOperator::OverrideInfo> EditorPrefabOperator::GetOverrides(EditorContext& context,
    ...
    if (!PrefabSystem::BuildOverrides(*actor, overrides, &error)) {
        if (!error.empty())
            Logger::Warn("[Editor] Prefab override list failed: ", error);
        return result;
    }
    ...
    for (size_t index = 0; index < overrides.size(); ++index) {
        ...
        if (!supported) {
            info.diagnostic = info.kind.empty() ? "Unsupported prefab override kind"
    ...
    std::stable_sort(result.begin(), result.end(), [](const OverrideInfo& left, const OverrideInfo& right) {
...
bool EditorPrefabOperator::ApplyAll(EditorContext& context, uint64_t actorID) const {
    ...
    if (!scene || !stack || !actor || !actor->IsPrefabRoot() || !context.CanEditScene()) {
        RecordPrefabOperatorEvent(context, "Apply Prefab", actorID, ElapsedOperatorMs(start), false);
        return false;
    }
    ...
    if (!PrefabSystem::ApplyAll(*actor, &error)) {
        if (!error.empty())
            Logger::Warn("[Editor] Apply Prefab failed: ", error);
        RecordPrefabOperatorEvent(context, "Apply Prefab", actorID, ElapsedOperatorMs(start), false,
                                  "prefab=" + prefabPath.string());
        return false;
    }
    ...
    if (!WriteFileContent(prefabPath, prefabBefore) || !SceneSerializer::LoadFromString(*scene, sceneBefore)) {
        RecordPrefabOperatorEvent(context, "Apply Prefab", actorID, ElapsedOperatorMs(start), false,
                                  "prefab=" + prefabPath.string());
        return false;
    }

    auto applyState = [prefabPath, actorID](EditorContext& value, const std::string& prefabContent,
        ...
        if (targetScene->FindByID(actorID))
            value.GetSelection().SelectActorID(actorID);
        else
            value.GetSelection().Clear();
        if (EditorAssetRegistry* registry = value.GetAssetRegistry())
            registry->Refresh();
        return true;
    };
    ...
    const bool ok = stack->ExecuteCommand(std::make_unique<LambdaEditorCommand>(
                                          ...
                                          context);
    RecordPrefabOperatorEvent(context, "Apply Prefab", actorID, ElapsedOperatorMs(start), ok,
                              "prefab=" + prefabPath.string());
    return ok;
}
...
bool EditorPrefabOperator::RevertAll(EditorContext& context, uint64_t actorID) const {
    ...
    Actor* actor = scene ? scene->FindByID(actorID) : nullptr;
    if (!actor) {
        RecordPrefabOperatorEvent(context, "Revert Prefab", actorID, ElapsedOperatorMs(start), false);
        return false;
    }
    const bool ok = CommitPrefabSceneSnapshot(context, *actor, "Revert Prefab", [](Actor& value, std::string* error) {
        return PrefabSystem::RevertAll(value, error);
    });
    RecordPrefabOperatorEvent(context, "Revert Prefab", actorID, ElapsedOperatorMs(start), ok);
    return ok;
}
...
bool EditorPrefabOperator::Unpack(EditorContext& context, uint64_t actorID) const {
    ...
    Actor* actor = scene ? scene->FindByID(actorID) : nullptr;
    if (!actor) {
        RecordPrefabOperatorEvent(context, "Unpack Prefab", actorID, ElapsedOperatorMs(start), false);
        return false;
    }
    const bool ok = CommitPrefabSceneSnapshot(context, *actor, "Unpack Prefab", [](Actor& value, std::string* error) {
        return PrefabSystem::Unpack(value, error);
    });
    RecordPrefabOperatorEvent(context, "Unpack Prefab", actorID, ElapsedOperatorMs(start), ok);
    return ok;
}

bool EditorPrefabOperator::CreatePrefabFromActor(EditorContext& context, uint64_t actorID) const {
    const auto start = EditorOperatorClock::now();
    Scene* scene = context.GetScene();
    EditorCommandStack* stack = context.GetCommandStack();
    Actor* actor = scene ? scene->FindByID(actorID) : nullptr;
    if (!scene || !stack || !actor || !context.CanEditScene()) {
        RecordPrefabOperatorEvent(context, "Create Prefab", actorID, ElapsedOperatorMs(start), false);
        return false;
    }
    ...
    const std::filesystem::path prefabPath =
        EditorImportService::MakeUniqueContentPath(directory, actor->GetName(), ".prefab.json");
    std::string error;
    if (!PrefabSystem::SaveSubtree(*actor, prefabPath, &error)) {
        if (!error.empty())
            Logger::Warn("[Editor] Prefab creation failed: ", error);
        RecordPrefabOperatorEvent(context, "Create Prefab", actorID, ElapsedOperatorMs(start), false,
                                  "prefab=" + prefabPath.string());
        return false;
    }
    ...
    const std::string metaContent = std::filesystem::exists(metaPath) ? ReadFileContent(metaPath) : std::string{};
    ...
    if (!instance) {
        if (!error.empty())
            Logger::Warn("[Editor] Prefab instance creation failed: ", error);
        SceneSerializer::LoadFromString(*scene, sceneBefore);
        RemoveAssetFileAndMeta(prefabPath);
        RecordPrefabOperatorEvent(context, "Create Prefab", actorID, ElapsedOperatorMs(start), false,
                                  "prefab=" + prefabPath.string());
    ...
    auto applyState = [prefabPath, metaPath, prefabContent, metaContent, hadMeta,
                       actorID](EditorContext& value, const std::string& sceneJson, bool writePrefab) {
        Scene* targetScene = value.GetScene();
        if (!targetScene)
            return false;
        if (writePrefab) {
            if (!WriteFileContent(prefabPath, prefabContent))
                return false;
            if (hadMeta && !WriteFileContent(metaPath, metaContent))
                return false;
        }
        if (!SceneSerializer::LoadFromString(*targetScene, sceneJson))
            return false;
        if (!writePrefab) {
            RemoveAssetFileAndMeta(prefabPath);
        }
    ...
    const bool ok = stack->ExecuteCommand(
        std::make_unique<LambdaEditorCommand>(
            "Create Prefab",
            [applyState, sceneAfter](EditorContext& value) { return applyState(value, sceneAfter, true); },
            [applyState, sceneBefore](EditorContext& value) { return applyState(value, sceneBefore, false); }),
        context);
    RecordPrefabOperatorEvent(context, "Create Prefab", actorID, ElapsedOperatorMs(start), ok,
                              "prefab=" + prefabPath.string());
    return ok;
}

uint64_t EditorPrefabOperator::InstantiatePrefab(EditorContext& context, const std::string& path,
                                                 uint64_t parentActorID, const std::optional<Transform>& rootTransform,
                                                 const char* commandName) const {
    const auto start = EditorOperatorClock::now();
    Scene* scene = context.GetScene();
    EditorCommandStack* stack = context.GetCommandStack();
    if (!scene || !stack || !context.CanEditScene() || path.empty()) {
        RecordPrefabOperatorEvent(context, "Instantiate Prefab", 0, ElapsedOperatorMs(start), false, "prefab=" + path);
        return 0;
    }
    ...
    if (!stack->ExecuteCommand(
            EditorUndoUtil::MakeSceneSnapshotCommand(commandName ? commandName : "Instantiate Prefab", before, after,
                                                     oldSelection, newID),
            context)) {
        RecordPrefabOperatorEvent(context, "Instantiate Prefab", newID, ElapsedOperatorMs(start), false,
                                  "prefab=" + path);
        return 0;
    }
    RecordPrefabOperatorEvent(context, "Instantiate Prefab", newID, ElapsedOperatorMs(start), true, "prefab=" + path);
    return newID;
}

size_t EditorPrefabOperator::SelectInstances(EditorContext& context, const std::string& path) const {
    Scene* scene = context.GetScene();
    if (!scene || path.empty())
        return 0;

    const std::filesystem::path targetPath = PrefabSystem::ResolvePrefabPath(path).lexically_normal();
    ...
    scene->ForEach([&](Actor& actor) {
        ...
        if (actorPath == targetPath)
            rootIDs.push_back(actor.GetID());
    });
    if (rootIDs.empty())
        return 0;
...
bool EditorPrefabOperator::ApplyOverride(EditorContext& context, uint64_t actorID, size_t overrideIndex) const {
    const auto start = EditorOperatorClock::now();
    Actor* actor = ResolveEditablePrefabActor(context, actorID);
    EditorCommandStack* stack = context.GetCommandStack();
    if (!actor || !stack || !context.CanEditScene()) {
        RecordPrefabOperatorEvent(context, "Apply Prefab Override", actorID, ElapsedOperatorMs(start), false,
    ...
    if (!ApplySinglePrefabOverrideNow(context, actorID, overrideIndex, &error)) {
        if (!error.empty())
            Logger::Warn("[Editor] Apply Prefab Override failed: ", error);
    ...
    const bool ok = stack->ExecuteCommand(
        ...
        context);
    RecordPrefabOperatorEvent(context, "Apply Prefab Override", actorID, ElapsedOperatorMs(start), ok,
                              "overrideIndex=" + std::to_string(overrideIndex));
    return ok;
}

bool EditorPrefabOperator::RevertOverride(EditorContext& context, uint64_t actorID, size_t overrideIndex) const {
    const auto start = EditorOperatorClock::now();
    Actor* actor = ResolveEditablePrefabActor(context, actorID);
    if (!actor) {
        RecordPrefabOperatorEvent(context, "Revert Prefab Override", actorID, ElapsedOperatorMs(start), false,
                                  "overrideIndex=" + std::to_string(overrideIndex));
        return false;
    }
    const bool ok = CommitPrefabSceneSnapshot(
        context, *actor, "Revert Prefab Override", [actorID, overrideIndex, &context](Actor&, std::string* error) {
            return RevertSinglePrefabOverrideNow(context, actorID, overrideIndex, error);
        });
    RecordPrefabOperatorEvent(context, "Revert Prefab Override", actorID, ElapsedOperatorMs(start), ok,
                              "overrideIndex=" + std::to_string(overrideIndex));
    return ok;
}
