...
void CSEffectsComposite(uint3 dispatchThreadId : SV_DispatchThreadID)
{
    ...
    g_Output[pixel] = float4(max(hdr + gi + specularCorrection, 0.0f), 1.0f);
}

float GatherReactiveMask(int2 pixel)
{
    float reactive = 0.0f;
    [unroll]
    for (int y = -1; y <= 1; ++y)
    ...
    for (int x = -1; x <= 1; ++x)
    {
        int2 samplePixel = clamp(pixel + int2(x, y), int2(0, 0), int2(g_FullSize) - 1);
        reactive = max(reactive, 1.0f - saturate(g_Current.Load(int3(samplePixel, 0)).a));
    }
    return reactive;
}
