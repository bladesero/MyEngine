cbuffer EnvironmentMipmapConstants : register(b0)
{
    float4 g_FaceInfo;
};

TextureCube<float4> g_SourceCube : register(t0);
...
float4 PSMain(VSOut input) : SV_TARGET
{
    ...
    float sourceMip = g_FaceInfo.y;
    return g_SourceCube.SampleLevel(
        g_SourceSampler, DirectionFromCubeFace(face, uv), sourceMip);
}
