...
bool TestPhysicsBroadPhaseTriggersAndSleep() {
    ...
    return Check(body->IsSleeping() && body->GetVelocity().LengthSq() < 1e-6f,
                 "resting rigid body did not enter sleep state (sleeping=" + std::to_string(body->IsSleeping()) +
                     ", speedSq=" + std::to_string(body->GetVelocity().LengthSq()) +
                     ", angularSpeedSq=" + std::to_string(body->GetAngularVelocity().LengthSq()) + ")");
}
...
bool TestRaycastAndCharacterController() {
    ...
    return Check(
        controller->IsGrounded() && player->GetTransform().position.x > 2.0f &&
            player->GetTransform().position.y > 0.9f,
        "character controller did not move and settle on ground (grounded=" + std::to_string(controller->IsGrounded()) +
            ", position=" + std::to_string(player->GetTransform().position.x) + "," +
...
bool TestCharacterControllerContactStability() {
    ...
    for (int i = 0; i < 180; ++i)
        scene.OnUpdate(1.0f / 60.0f);
    if (!Check(controller->IsGrounded(), "character did not settle before stability sampling"))
        return false;
    ...
    if (!Check(maximumY - minimumY < 0.005f && groundedFrames == 180 &&
               ...
               "resting character contact jittered or lost grounded state"))
        return false;

    if (!Check(controller->Jump(), "grounded character rejected jump request"))
        return false;
    ...
    if (!Check(becameAirborne && landedAgain, "character did not leave and reacquire stable ground after jumping"))
        return false;
    ...
    return Check(player->GetTransform().position.x < 1.3f && player->GetTransform().position.z > 1.0f &&
...
bool TestCharacterControllerDynamicBodyStability() {
    ...
    for (int i = 0; i < 240; ++i) {
        scene.OnUpdate(1.0f / 60.0f);
        if (i >= 60 && controller->IsGrounded())
            ++groundedFrames;
        maximumVerticalSpeed = (std::max)(maximumVerticalSpeed, std::abs(controller->GetActualVelocity().y));
    }
    const Vec3 playerPosition = player->GetTransform().position;
    const Vec3 boxPosition = box->GetTransform().position;
    if (!Check(std::isfinite(playerPosition.x) && std::isfinite(playerPosition.y) && std::isfinite(boxPosition.x) &&
                   std::isfinite(boxPosition.y) && boxPosition.x > 2.1f && groundedFrames > 170 &&
                   maximumVerticalSpeed < 1.0f,
               "character interaction with a dynamic collider was unstable"))
        return false;
    ...
    if (!Check(riderController->IsGrounded(), "character did not settle on moving platform"))
        return false;
    ...
    return Check(riderController->IsGrounded() && platform->GetWorldPosition().x > 1.1f &&
                     rider->GetWorldPosition().x > 0.9f && std::abs(rider->GetWorldPosition().y - 1.25f) < 0.02f,
...
bool TestJoltRigidBodyExtensions() {
    ...
    for (int i = 0; i < 30; ++i)
        scene.OnUpdate(1.0f / 60.0f);
    if (!Check(std::fabs(dynamic->GetWorldPosition().x) < 0.01f && dynamic->GetWorldPosition().y > 0.2f,
               "Jolt impulse or linear axis lock failed"))
        return false;
    if (!Check(std::fabs(body->GetAngularVelocity().y) > 0.05f, "Jolt torque did not produce angular velocity"))
        return false;
    ...
    scene.OnUpdate(1.0f / 60.0f);
    if (!Check(NearlyEqual(kinematic->GetWorldPosition().x, 2.0f, 0.02f), "kinematic target was not applied"))
        return false;
    ...
    if (!Check(NearlyEqual(child->GetTransform().position.x, 2.0f, 0.02f),
               "world-space teleport was not converted to parent-local transform"))
        return false;
...
bool TestRigidBodyJsonCompatibility() {
    ...
    if (!Check(roundTrip.GetBodyType() == BodyType::Kinematic && NearlyEqual(roundTrip.GetAngularVelocity().z, 3.0f) &&
                   NearlyEqual(roundTrip.GetAngularDamping(), 0.7f) &&
                   roundTrip.GetCollisionDetectionMode() == CollisionDetectionMode::Continuous,
               "new rigid-body fields failed JSON round trip"))
        return false;

    RigidBodyComponent legacy;
    legacy.Deserialize(nlohmann::json{{"bodyType", "static"}, {"mass", 4.0f}, {"velocity", {1.0f, 0.0f, 0.0f}}});
    return Check(legacy.GetBodyType() == BodyType::Static && NearlyEqual(legacy.GetMass(), 4.0f) &&
...
MYENGINE_REGISTER_TEST("Physics", "TestCharacterControllerDynamicBodyStability",
                       TestCharacterControllerDynamicBodyStability);
