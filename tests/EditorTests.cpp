...
bool TestEditorCommandStackAndSelection() {
    ...
    auto hasCommandEvent = [&](const char* operation, const char* commandName) {
        return std::any_of(commandEvents.begin(), commandEvents.end(),
                           [&](const EditorProfilerEvent& event) {
                               return event.category == "EditorCommand" &&
                                      event.name == operation &&
                                      event.details.find(std::string("command=") + commandName) !=
                                          std::string::npos &&
                                      event.details.find(";success=true") != std::string::npos &&
                                      event.details.find(";dirty=scene") != std::string::npos;
                           });
    };
    if (!Check(hasCommandEvent("Execute", "Add One") &&
               hasCommandEvent("Execute", "Add Two") &&
               hasCommandEvent("Undo", "Add Two") &&
               hasCommandEvent("Redo", "Add Two"),
...
bool TestEditorOperatorsSelectionAndCommands() {
    ...
    if (!Check(operators.Selection().SelectActorRange(
            context, rangeFirst->GetID(), rangeChild->GetID()),
            "operator range selection failed")) return false;
    if (!Check(context.GetSelection().GetMultiCount() == 3 &&
               ...
               "operator range selection did not follow hierarchy order")) return false;

    const std::vector<uint64_t> visibleOrder {
        rangeThird->GetID(), rangeSecond->GetID(), rangeFirst->GetID()
    };
    if (!Check(operators.Selection().SelectActorRange(
            context, rangeThird->GetID(), rangeFirst->GetID(), visibleOrder),
            "operator custom-order range selection failed")) return false;
    if (!Check(context.GetSelection().GetMultiCount() == 3 &&
               context.GetSelection().IsSelected(rangeThird->GetID()) &&
               context.GetSelection().IsSelected(rangeSecond->GetID()) &&
               context.GetSelection().IsSelected(rangeFirst->GetID()) &&
               !context.GetSelection().IsSelected(rangeChild->GetID()) &&
               context.GetSelection().GetActorID() == rangeFirst->GetID(),
               "operator range selection did not honor visible order")) return false;
    if (!Check(!operators.Selection().SelectActorRange(
            context, 999999, rangeFirst->GetID(), visibleOrder),
            "operator range selection accepted invalid anchor")) return false;
    ...
    if (!Check(operators.Selection().SelectActorSubtree(
                   context, subtreeRoot->GetID(), false),
               "operator select children failed")) return false;
    if (!Check(context.GetSelection().GetMultiCount() == 3 &&
               ...
               "operator select children did not select only descendants")) return false;
    if (!Check(operators.Selection().SelectActorSubtree(
                   context, subtreeRoot->GetID(), true) &&
               context.GetSelection().GetMultiCount() == 4 &&
               context.GetSelection().IsSelected(subtreeRoot->GetID()),
               "operator select subtree did not include root")) return false;
    if (!Check(!operators.Selection().SelectActorSubtree(
                   context, subtreeGrandchild->GetID(), false),
               "operator select children accepted actor without children")) return false;

    auto executeSceneSetting = [&](std::unique_ptr<IEditorCommand> command,
                                   const char* failure) {
    ...
    if (!executeSceneSetting(std::make_unique<LambdaEditorCommand>(
            ...
            "scene settings name command failed")) return false;
    if (!Check(scene.GetName() == "EditedScene",
               "scene settings name command did not apply")) return false;
    if (!Check(stack.Undo(context) && scene.GetName() == "EditorOperators",
               "scene settings name command undo failed")) return false;
    if (!Check(stack.Redo(context) && scene.GetName() == "EditedScene",
               "scene settings name command redo failed")) return false;
    ...
    if (!executeSceneSetting(std::make_unique<LambdaEditorCommand>(
            ...
            "scene settings gravity command failed")) return false;
    if (!Check(scene.GetPhysicsWorld().GetGravity().y == -4.0f,
               "scene settings gravity command did not apply")) return false;
    if (!Check(stack.Undo(context) &&
               scene.GetPhysicsWorld().GetGravity().y == originalGravity.y,
               "scene settings gravity command undo failed")) return false;
    if (!Check(stack.Redo(context) &&
               scene.GetPhysicsWorld().GetGravity().y == -4.0f,
               "scene settings gravity command redo failed")) return false;

    Actor* sceneCameraActor = scene.CreateActor("SceneHintCamera");
    sceneCameraActor->AddComponent<CameraComponent>();
    if (!Check(operators.Commands().SetSceneMainCameraHint(
                   context, sceneCameraActor->GetID()),
               "scene settings main camera hint command failed")) return false;
    if (!Check(scene.GetMainCameraHintActorID() == sceneCameraActor->GetID(),
               "scene settings main camera hint did not apply")) return false;
    if (!Check(stack.Undo(context) && scene.GetMainCameraHintActorID() == 0,
               "scene settings main camera hint undo failed")) return false;
    if (!Check(stack.Redo(context) &&
               scene.GetMainCameraHintActorID() == sceneCameraActor->GetID(),
               "scene settings main camera hint redo failed")) return false;

    if (!Check(operators.Commands().SetSceneAmbientIntensity(context, 0.35f),
               "scene settings ambient intensity command failed")) return false;
    if (!Check(scene.GetAmbientIntensity() == 0.35f,
               "scene settings ambient intensity did not apply")) return false;
    if (!Check(stack.Undo(context) && scene.GetAmbientIntensity() == 1.0f,
               "scene settings ambient intensity undo failed")) return false;
    if (!Check(stack.Redo(context) && scene.GetAmbientIntensity() == 0.35f,
               "scene settings ambient intensity redo failed")) return false;
    if (!Check(operators.Commands().SetSceneAmbientIntensity(context, -1.0f) &&
               scene.GetAmbientIntensity() == 0.0f,
               "scene settings ambient intensity should clamp negative values")) return false;
    ...
    if (!Check(operators.Commands().SetActorTag(context, actorID, "Player"),
               "operator tag edit failed")) return false;
    if (!Check(actor->GetTag() == "Player",
               "operator tag edit did not apply")) return false;
    if (!Check(stack.Undo(context) && actor->GetTag().empty(),
               "operator tag edit undo failed")) return false;
    if (!Check(stack.Redo(context) && actor->GetTag() == "Player",
               "operator tag edit redo failed")) return false;

    if (!Check(operators.Commands().SetActorLayer(context, actorID, 7),
               "operator layer edit failed")) return false;
    if (!Check(actor->GetLayer() == 7,
               "operator layer edit did not apply")) return false;
    if (!Check(stack.Undo(context) && actor->GetLayer() == 0,
               "operator layer edit undo failed")) return false;
    if (!Check(stack.Redo(context) && actor->GetLayer() == 7,
               "operator layer edit redo failed")) return false;

    if (!Check(operators.Commands().SetActorStatic(context, actorID, true),
               "operator static edit failed")) return false;
    if (!Check(actor->IsStatic(),
               "operator static edit did not apply")) return false;
    if (!Check(stack.Undo(context) && !actor->IsStatic(),
               "operator static edit undo failed")) return false;
    if (!Check(stack.Redo(context) && actor->IsStatic(),
               "operator static edit redo failed")) return false;
    ...
    if (!Check(SceneSerializer::LoadFromString(staticRoundTrip, staticSceneJson),
               "scene static flag round-trip failed to load")) return false;
    Actor* staticLoaded = staticRoundTrip.FindByID(actorID);
    if (!Check(staticLoaded && staticLoaded->IsStatic(),
               "scene static flag round-trip did not preserve editor flags")) return false;
    ...
    if (!Check(operators.Commands().SetActorsActive(context, batchIDs, true),
               "operator batch active edit failed")) return false;
    if (!Check(batchA->IsActiveSelf() && batchB->IsActiveSelf(),
               "operator batch active edit did not apply")) return false;
    if (!Check(stack.Undo(context) && batchA->IsActiveSelf() && !batchB->IsActiveSelf(),
               "operator batch active undo failed")) return false;
    if (!Check(stack.Redo(context) && batchA->IsActiveSelf() && batchB->IsActiveSelf(),
               "operator batch active redo failed")) return false;

    if (!Check(operators.Commands().SetActorsTag(context, batchIDs, "Enemy"),
               "operator batch tag edit failed")) return false;
    if (!Check(batchA->GetTag() == "Enemy" && batchB->GetTag() == "Enemy",
               "operator batch tag edit did not apply")) return false;
    if (!Check(stack.Undo(context) && batchA->GetTag() == "A" && batchB->GetTag() == "B",
               "operator batch tag undo failed")) return false;
    if (!Check(stack.Redo(context) && batchA->GetTag() == "Enemy" && batchB->GetTag() == "Enemy",
               "operator batch tag redo failed")) return false;

    if (!Check(operators.Commands().SetActorsLayer(context, batchIDs, 4),
               "operator batch layer edit failed")) return false;
    if (!Check(batchA->GetLayer() == 4 && batchB->GetLayer() == 4,
               "operator batch layer edit did not apply")) return false;
    if (!Check(stack.Undo(context) && batchA->GetLayer() == 1 && batchB->GetLayer() == 2,
               "operator batch layer undo failed")) return false;
    if (!Check(stack.Redo(context) && batchA->GetLayer() == 4 && batchB->GetLayer() == 4,
               "operator batch layer redo failed")) return false;

    if (!Check(operators.Commands().SetActorsStatic(context, batchIDs, false),
               "operator batch static edit failed")) return false;
    if (!Check(!batchA->IsStatic() && !batchB->IsStatic(),
               "operator batch static edit did not apply")) return false;
    if (!Check(stack.Undo(context) && !batchA->IsStatic() && batchB->IsStatic(),
               "operator batch static undo failed")) return false;
    if (!Check(stack.Redo(context) && !batchA->IsStatic() && !batchB->IsStatic(),
               "operator batch static redo failed")) return false;

    batchA->GetTransform().position = {1.0f, 2.0f, 3.0f};
    batchB->GetTransform().position = {-1.0f, -2.0f, -3.0f};
    if (!Check(operators.Commands().SetActorsPosition(
                   context, batchIDs, {9.0f, 8.0f, 7.0f}),
               "operator batch position edit failed")) return false;
    if (!Check(NearlyEqual(batchA->GetTransform().position.x, 9.0f) &&
               NearlyEqual(batchB->GetTransform().position.y, 8.0f),
               "operator batch position edit did not apply")) return false;
    if (!Check(stack.Undo(context) &&
               NearlyEqual(batchA->GetTransform().position.x, 1.0f) &&
               NearlyEqual(batchB->GetTransform().position.y, -2.0f),
               "operator batch position undo failed")) return false;
    if (!Check(stack.Redo(context) &&
               NearlyEqual(batchA->GetTransform().position.z, 7.0f) &&
               NearlyEqual(batchB->GetTransform().position.x, 9.0f),
               "operator batch position redo failed")) return false;

    batchA->GetTransform().rotation = {10.0f, 20.0f, 30.0f};
    batchB->GetTransform().rotation = {-10.0f, -20.0f, -30.0f};
    if (!Check(operators.Commands().SetActorsRotation(
                   context, batchIDs, {45.0f, 90.0f, 135.0f}),
               "operator batch rotation edit failed")) return false;
    if (!Check(NearlyEqual(batchA->GetTransform().rotation.x, 45.0f) &&
               NearlyEqual(batchB->GetTransform().rotation.z, 135.0f),
               "operator batch rotation edit did not apply")) return false;
    if (!Check(stack.Undo(context) &&
               NearlyEqual(batchA->GetTransform().rotation.y, 20.0f) &&
               NearlyEqual(batchB->GetTransform().rotation.x, -10.0f),
               "operator batch rotation undo failed")) return false;
    if (!Check(stack.Redo(context) &&
               NearlyEqual(batchA->GetTransform().rotation.z, 135.0f) &&
               NearlyEqual(batchB->GetTransform().rotation.y, 90.0f),
               "operator batch rotation redo failed")) return false;

    batchA->GetTransform().scale = {1.0f, 2.0f, 3.0f};
    batchB->GetTransform().scale = {4.0f, 5.0f, 6.0f};
    if (!Check(operators.Commands().SetActorsScale(
                   context, batchIDs, {2.0f, 2.0f, 2.0f}),
               "operator batch scale edit failed")) return false;
    if (!Check(NearlyEqual(batchA->GetTransform().scale.x, 2.0f) &&
               NearlyEqual(batchB->GetTransform().scale.z, 2.0f),
               "operator batch scale edit did not apply")) return false;
    if (!Check(stack.Undo(context) &&
               NearlyEqual(batchA->GetTransform().scale.y, 2.0f) &&
               NearlyEqual(batchB->GetTransform().scale.x, 4.0f),
               "operator batch scale undo failed")) return false;
    if (!Check(stack.Redo(context) &&
               NearlyEqual(batchA->GetTransform().scale.z, 2.0f) &&
               NearlyEqual(batchB->GetTransform().scale.y, 2.0f),
               "operator batch scale redo failed")) return false;

    std::vector<uint64_t> idsBeforeMultiDuplicate;
    scene.ForEach([&](Actor& value) {
        idsBeforeMultiDuplicate.push_back(value.GetID());
    });
    ...
    if (!Check(operators.Commands().DuplicateSelection(context),
               "operator multi-duplicate selection failed")) return false;
    if (!Check(scene.ActorCount() == countBeforeMultiDuplicate + 3,
               "operator multi-duplicate did not duplicate selected root subtrees")) return false;
    std::vector<uint64_t> multiDuplicateCloneIDs;
    scene.ForEach([&](Actor& value) {
        if (std::find(idsBeforeMultiDuplicate.begin(), idsBeforeMultiDuplicate.end(),
                      value.GetID()) == idsBeforeMultiDuplicate.end()) {
            multiDuplicateCloneIDs.push_back(value.GetID());
        }
    });
    if (!Check(multiDuplicateCloneIDs.size() == 3,
               "operator multi-duplicate produced unexpected clone count")) return false;
    ...
    if (!Check(foundParentClone,
               "operator multi-duplicate did not preserve selected subtree")) return false;
    ...
    if (!Check(std::find(multiDuplicateCloneIDs.begin(), multiDuplicateCloneIDs.end(),
               ...
               "operator multi-duplicate did not select a cloned actor")) return false;
    if (!Check(stack.Undo(context) &&
               scene.ActorCount() == countBeforeMultiDuplicate &&
               !scene.FindByID(multiDuplicateCloneIDs.front()),
               "operator multi-duplicate undo failed")) return false;
    if (!Check(stack.Redo(context) &&
               scene.ActorCount() == countBeforeMultiDuplicate + 3 &&
               scene.FindByID(selectedCloneAfterMultiDuplicate),
               "operator multi-duplicate redo failed")) return false;

    std::vector<uint64_t> idsBeforeMultiPaste;
    scene.ForEach([&](Actor& value) {
        idsBeforeMultiPaste.push_back(value.GetID());
    });
    ...
    if (!copyParent || !copyChild || !copySibling ||
        !copyChild->AddComponent<BoxColliderComponent>()) {
    ...
    if (!Check(operators.Commands().CopySelection(context),
               "operator multi-copy selection failed")) return false;
    if (!Check(operators.Commands().HasActorClipboard() &&
               !operators.Commands().HasAssetClipboard(),
               "operator actor copy did not set actor clipboard kind")) return false;
    context.GetSelection().SelectActorID(copySiblingID);
    if (!Check(operators.Commands().PasteSelection(context),
               "operator multi-paste selection failed")) return false;
    if (!Check(scene.ActorCount() == countBeforeMultiPaste + 3,
               "operator multi-paste did not paste copied root subtrees")) return false;
    std::vector<uint64_t> multiPasteCloneIDs;
    scene.ForEach([&](Actor& value) {
        if (std::find(idsBeforeMultiPaste.begin(), idsBeforeMultiPaste.end(),
                      value.GetID()) == idsBeforeMultiPaste.end()) {
            multiPasteCloneIDs.push_back(value.GetID());
        }
    });
    if (!Check(multiPasteCloneIDs.size() == 3,
               "operator multi-paste produced unexpected clone count")) return false;
    ...
    for (uint64_t cloneID : multiPasteCloneIDs) {
        ...
        if (pasted && pasted->GetChildren().size() == 1) {
            foundPastedParentClone = pasted->GetChildren()[0]
                ->GetComponent<BoxColliderComponent>() != nullptr;
        }
    }
    if (!Check(foundPastedParentClone,
               "operator multi-paste did not preserve copied subtree")) return false;
    const uint64_t selectedCloneAfterMultiPaste = context.GetSelection().GetActorID();
    if (!Check(std::find(multiPasteCloneIDs.begin(), multiPasteCloneIDs.end(),
                         selectedCloneAfterMultiPaste) != multiPasteCloneIDs.end(),
               "operator multi-paste did not select a pasted actor")) return false;
    if (!Check(stack.Undo(context) &&
               scene.ActorCount() == countBeforeMultiPaste &&
               !scene.FindByID(multiPasteCloneIDs.front()),
               "operator multi-paste undo failed")) return false;
    if (!Check(stack.Redo(context) &&
               scene.ActorCount() == countBeforeMultiPaste + 3 &&
               scene.FindByID(selectedCloneAfterMultiPaste),
               "operator multi-paste redo failed")) return false;
    ...
    if (!Check(operators.Commands().DeleteSelection(context),
               "operator multi-delete selection failed")) return false;
    if (!Check(!scene.FindByID(deleteParentID) &&
               !scene.FindByID(deleteChildID) &&
               !scene.FindByID(deleteSiblingID) &&
               !context.GetSelection().HasActor(),
               "operator multi-delete did not remove selected actor set")) return false;
    if (!Check(stack.Undo(context) &&
               scene.FindByID(deleteParentID) &&
               scene.FindByID(deleteChildID) &&
               scene.FindByID(deleteSiblingID) &&
               scene.FindByID(deleteChildID)->GetParent() == scene.FindByID(deleteParentID) &&
               context.GetSelection().GetActorID() == deleteSiblingID,
               "operator multi-delete undo did not restore hierarchy and selection")) return false;
    if (!Check(stack.Redo(context) &&
               !scene.FindByID(deleteParentID) &&
               !scene.FindByID(deleteChildID) &&
               !scene.FindByID(deleteSiblingID),
               "operator multi-delete redo failed")) return false;

    actor = scene.FindByID(actorID);
    if (!Check(actor != nullptr,
               "operator multi-delete snapshot did not preserve unrelated actor")) return false;
    ...
    if (!Check(operators.Commands().DuplicateActorSubtree(context, actorID),
               "operator duplicate subtree failed")) return false;
    const uint64_t cloneID = context.GetSelection().GetActorID();
    Actor* clone = scene.FindByID(cloneID);
    if (!Check(clone && cloneID != actorID,
               "operator duplicate did not select the cloned root")) return false;
    if (!Check(clone->GetChildren().size() == 1,
               "operator duplicate did not preserve subtree child")) return false;
    if (!Check(clone->GetChildren()[0]->GetComponent<BoxColliderComponent>() != nullptr,
               "operator duplicate did not preserve subtree component")) return false;
    if (!Check(stack.Undo(context) && scene.FindByID(cloneID) == nullptr,
               "operator duplicate undo failed")) return false;
    if (!Check(stack.Redo(context) && scene.FindByID(cloneID) != nullptr,
               "operator duplicate redo failed")) return false;

    actor = scene.FindByID(actorID);
    if (!Check(actor != nullptr,
               "operator duplicate snapshot did not restore source actor")) return false;
    if (!Check(operators.Components().AddComponent(context, actorID, "BoxCollider"),
               "component operator add failed")) return false;
    actor = scene.FindByID(actorID);
    if (!Check(actor && actor->GetComponent<BoxColliderComponent>() != nullptr,
               "component operator add did not apply")) return false;
    if (!Check(stack.Undo(context) && actor->GetComponent<BoxColliderComponent>() == nullptr,
               "component operator add undo failed")) return false;
    if (!Check(stack.Redo(context) && actor->GetComponent<BoxColliderComponent>() != nullptr,
               "component operator add redo failed")) return false;
    if (!Check(operators.Components().RemoveComponent(context, actorID, "BoxCollider"),
               "component operator remove failed")) return false;
    if (!Check(actor->GetComponent<BoxColliderComponent>() == nullptr,
               "component operator remove did not apply")) return false;
    if (!Check(stack.Undo(context) && actor->GetComponent<BoxColliderComponent>() != nullptr,
               "component operator remove undo failed")) return false;
    ...
    if (!Check(!operators.Components().RemoveComponents(
                   context,
                   {batchComponentAID, batchComponentMissingID},
                   "BoxCollider"),
               "component operator batch remove allowed missing component")) return false;
    if (!Check(operators.Components().RemoveComponents(
                   context,
                   {batchComponentAID, batchComponentBID},
                   "BoxCollider"),
               "component operator batch remove failed")) return false;
    batchComponentA = scene.FindByID(batchComponentAID);
    batchComponentB = scene.FindByID(batchComponentBID);
    if (!Check(batchComponentA && batchComponentB &&
               !batchComponentA->HasComponentType("BoxCollider") &&
               !batchComponentB->HasComponentType("BoxCollider"),
               "component operator batch remove did not apply")) return false;
    if (!Check(stack.Undo(context) &&
               scene.FindByID(batchComponentAID)->HasComponentType("BoxCollider") &&
               scene.FindByID(batchComponentBID)->HasComponentType("BoxCollider"),
               "component operator batch remove undo failed")) return false;
    if (!Check(stack.Redo(context) &&
               !scene.FindByID(batchComponentAID)->HasComponentType("BoxCollider") &&
               !scene.FindByID(batchComponentBID)->HasComponentType("BoxCollider"),
               "component operator batch remove redo failed")) return false;
    ...
    if (!Check(batchAddA && batchAddB && batchAddExisting &&
               batchAddExisting->AddComponent<BoxColliderComponent>(),
               "component operator batch add setup failed")) return false;
    const std::vector<uint64_t> batchAddIDs {
        batchAddA->GetID(), batchAddB->GetID()
    };
    if (!Check(!operators.Components().AddComponents(
                   context,
                   {batchAddA->GetID(), batchAddExisting->GetID()},
                   "BoxCollider"),
               "component operator batch add allowed existing component")) return false;
    if (!Check(operators.Components().AddComponents(
                   context, batchAddIDs, "BoxCollider"),
               "component operator batch add failed")) return false;
    if (!Check(scene.FindByID(batchAddIDs[0])->HasComponentType("BoxCollider") &&
               ...
               "component operator batch add did not apply")) return false;
    if (!Check(stack.Undo(context) &&
               !scene.FindByID(batchAddIDs[0])->HasComponentType("BoxCollider") &&
               !scene.FindByID(batchAddIDs[1])->HasComponentType("BoxCollider"),
               "component operator batch add undo failed")) return false;
    if (!Check(stack.Redo(context) &&
               scene.FindByID(batchAddIDs[0])->HasComponentType("BoxCollider") &&
               scene.FindByID(batchAddIDs[1])->HasComponentType("BoxCollider"),
               "component operator batch add redo failed")) return false;

    Actor* batchPropertyA = scene.CreateActor("BatchPropertyA");
    Actor* batchPropertyB = scene.CreateActor("BatchPropertyB");
    BoxColliderComponent* batchBoxA =
        batchPropertyA ? batchPropertyA->AddComponent<BoxColliderComponent>() : nullptr;
    BoxColliderComponent* batchBoxB =
        batchPropertyB ? batchPropertyB->AddComponent<BoxColliderComponent>() : nullptr;
    if (!Check(batchPropertyA && batchPropertyB && batchBoxA && batchBoxB,
               "component operator batch property setup failed")) return false;
    batchBoxA->SetHalfExtents({1.0f, 1.0f, 1.0f});
    batchBoxB->SetHalfExtents({2.0f, 2.0f, 2.0f});
    const std::vector<uint64_t> batchPropertyIDs {
        batchPropertyA->GetID(), batchPropertyB->GetID()
    };
    if (!Check(operators.Components().SetComponentPropertyForActors(
                   context, batchPropertyIDs, "BoxCollider", "halfExtents",
                   nlohmann::json::array({3.0f, 4.0f, 5.0f})),
               "component operator batch property edit failed")) return false;
    ...
    if (!Check(batchBoxA && batchBoxB &&
               NearlyEqual(batchBoxA->GetHalfExtents().x, 3.0f) &&
               NearlyEqual(batchBoxB->GetHalfExtents().z, 5.0f),
               "component operator batch property edit did not apply")) return false;
    if (!Check(stack.Undo(context),
               "component operator batch property undo command failed")) return false;
    batchBoxA = scene.FindByID(batchPropertyIDs[0])->GetComponent<BoxColliderComponent>();
    batchBoxB = scene.FindByID(batchPropertyIDs[1])->GetComponent<BoxColliderComponent>();
    if (!Check(batchBoxA && batchBoxB &&
               NearlyEqual(batchBoxA->GetHalfExtents().x, 1.0f) &&
               NearlyEqual(batchBoxB->GetHalfExtents().y, 2.0f),
               "component operator batch property undo failed")) return false;
    if (!Check(stack.Redo(context),
               "component operator batch property redo command failed")) return false;
    batchBoxA = scene.FindByID(batchPropertyIDs[0])->GetComponent<BoxColliderComponent>();
    batchBoxB = scene.FindByID(batchPropertyIDs[1])->GetComponent<BoxColliderComponent>();
    if (!Check(batchBoxA && batchBoxB &&
               NearlyEqual(batchBoxA->GetHalfExtents().y, 4.0f) &&
               NearlyEqual(batchBoxB->GetHalfExtents().z, 5.0f),
               "component operator batch property redo failed")) return false;

    actor = scene.FindByID(actorID);
    if (!Check(actor != nullptr,
               "component operator batch remove snapshot did not restore source actor")) {
        return false;
    }
    BoxColliderComponent* box = actor->GetComponent<BoxColliderComponent>();
    if (!Check(box != nullptr, "component operator property setup missing collider")) return false;
    ...
    if (!Check(operators.Components().SetProperty(context, *actor, "BoxCollider",
                                                   "halfExtents", beforeCollider, afterCollider),
               "component operator property edit failed")) return false;
    if (!Check(box->GetHalfExtents().x == 2.0f && box->GetLayer() == 8,
               "component operator property edit did not apply")) return false;
    if (!Check(stack.Undo(context) && box->GetHalfExtents().x == 0.5f &&
               box->GetLayer() == 1,
               "component operator property edit undo failed")) return false;
    if (!Check(stack.Redo(context) && box->GetHalfExtents().x == 2.0f &&
               box->GetLayer() == 8,
               "component operator property edit redo failed")) return false;

    auto verifyPropertyEdit = [&](Component& component, const char* componentType,
                                  const char* propertyName, auto&& edit,
                                  auto&& isApplied, auto&& isRestored,
        ...
        if (!Check(operators.Components().SetProperty(context, *actor, componentType,
                                                       propertyName, before, after),
                   std::string(failurePrefix) + " set property failed")) return false;
        if (!Check(isApplied(),
                   std::string(failurePrefix) + " property did not apply")) return false;
        if (!Check(stack.Undo(context) && isRestored(),
                   std::string(failurePrefix) + " property undo failed")) return false;
        if (!Check(stack.Redo(context) && isApplied(),
                   std::string(failurePrefix) + " property redo failed")) return false;
        return true;
    };
    ...
    if (!Check(light && camera && post,
               "render component property setup failed")) return false;
    if (!verifyPropertyEdit(*light, "Light", "intensity",
            [&] { light->SetIntensity(7.0f); },
            [&] { return light->GetIntensity() == 7.0f; },
            [&] { return light->GetIntensity() == 3.0f; },
            "light")) return false;
    if (!verifyPropertyEdit(*camera, "Camera", "fovYDegrees",
            [&] { camera->SetFovYDegrees(75.0f); },
            [&] { return camera->GetFovYDegrees() == 75.0f; },
            [&] { return camera->GetFovYDegrees() == 60.0f; },
            "camera")) return false;
    if (!verifyPropertyEdit(*post, "PostProcess", "exposure",
            [&] { post->SetExposure(2.5f); },
            [&] { return post->GetExposure() == 2.5f; },
            [&] { return post->GetExposure() == 1.0f; },
            "post process")) return false;

    auto* canvas = actor->AddComponent<UICanvasComponent>();
    auto* rect = actor->AddComponent<UIRectTransformComponent>();
    if (!Check(canvas && rect,
               "ui component property setup failed")) return false;
    if (!verifyPropertyEdit(*canvas, "UICanvas", "sortOrder",
            [&] { canvas->SetSortOrder(12); },
            [&] { return canvas->GetSortOrder() == 12; },
            [&] { return canvas->GetSortOrder() == 0; },
            "ui canvas")) return false;
    if (!verifyPropertyEdit(*rect, "UIRectTransform", "anchorMin",
            [&] { rect->GetRect().anchorMin = {0.25f, 0.5f}; },
            [&] { return rect->GetRect().anchorMin.x == 0.25f &&
                         rect->GetRect().anchorMin.y == 0.5f; },
            [&] { return rect->GetRect().anchorMin.x == 0.0f &&
                         rect->GetRect().anchorMin.y == 0.0f; },
            "ui rect transform")) return false;
...
bool TestEditorCommandOperatorCreateUIActor() {
    ...
    if (!Check(operators.Commands().CreateUIActor(context, "missing") == 0,
               "operator accepted invalid UI actor preset")) return false;

    const uint64_t canvasID = operators.Commands().CreateUIActor(context, "canvas");
    Actor* canvas = scene.FindByID(canvasID);
    if (!Check(canvas && canvas->GetName() == "UI Canvas" &&
               canvas->GetComponent<UICanvasComponent>() &&
               canvas->GetComponent<UIRectTransformComponent>(),
               "operator create UI canvas failed")) return false;
    if (!Check(context.GetSelection().GetActorID() == canvasID,
               "operator create UI canvas did not select new actor")) return false;
    if (!Check(stack.Undo(context) && scene.FindByID(canvasID) == nullptr,
               "operator create UI canvas undo failed")) return false;
    if (!Check(stack.Redo(context) && scene.FindByID(canvasID) != nullptr,
               "operator create UI canvas redo failed")) return false;
    ...
    if (!Check(button != nullptr, "operator create UI button child missing")) return false;
    canvas = scene.FindByID(canvasID);
    if (!Check(canvas != nullptr, "operator create UI button lost canvas parent")) return false;
    if (!Check(button->GetParent() == canvas,
               "operator create UI button parent mismatch")) return false;
    if (!Check(button->GetComponent<UIRectTransformComponent>() != nullptr,
               "operator create UI button missing rect transform")) return false;
    if (!Check(button->GetComponent<UIButtonComponent>() != nullptr,
               "operator create UI button missing button component")) return false;
    if (!Check(stack.Undo(context) && scene.FindByID(buttonID) == nullptr &&
               scene.FindByID(canvasID) != nullptr,
               "operator create UI button undo failed")) return false;
    if (!Check(stack.Redo(context), "operator create UI button redo failed")) return false;
    button = scene.FindByID(buttonID);
    canvas = scene.FindByID(canvasID);
    if (!Check(button && canvas && button->GetParent() == canvas &&
               button->GetComponent<UIButtonComponent>(),
               "operator create UI button redo did not restore child")) return false;
...
bool TestEditorCommandOperatorCreateEmptyParent() {
    ...
    if (!Check(parent && parent->GetName() == "Empty Parent" && child &&
               child->GetParent() == parent &&
               context.GetSelection().GetActorID() == parentID,
               "operator create empty parent failed")) return false;
    if (!Check(roots.size() >= 3 &&
               roots[0]->GetID() == firstID &&
               roots[1]->GetID() == parentID &&
               roots[2]->GetID() == thirdID,
               "operator create empty parent did not preserve sibling slot")) return false;

    if (!Check(stack.Undo(context), "operator create empty parent undo failed")) return false;
    child = scene.FindByID(selectedID);
    roots = scene.GetRootActors();
    if (!Check(child && child->GetParent() == nullptr &&
               scene.FindByID(parentID) == nullptr &&
               roots.size() >= 3 &&
               roots[0]->GetID() == firstID &&
               roots[1]->GetID() == selectedID &&
               roots[2]->GetID() == thirdID,
               "operator create empty parent undo did not restore hierarchy")) return false;

    if (!Check(stack.Redo(context), "operator create empty parent redo failed")) return false;
    parent = scene.FindByID(parentID);
    child = scene.FindByID(selectedID);
    roots = scene.GetRootActors();
    if (!Check(parent && child && child->GetParent() == parent &&
               roots.size() >= 3 &&
               roots[1]->GetID() == parentID,
               "operator create empty parent redo did not restore hierarchy")) return false;
...
bool TestEditorCommandOperatorCreateChildActor() {
    ...
    if (!Check(operators.Commands().CreateChildActor(context, "Orphan", 999999) == 0,
               "operator accepted invalid parent for child actor")) return false;

    const uint64_t childID =
        operators.Commands().CreateChildActor(context, "Child", parentID);
    Actor* child = scene.FindByID(childID);
    parent = scene.FindByID(parentID);
    if (!Check(child && parent && child->GetName() == "Child" &&
               child->GetParent() == parent &&
               context.GetSelection().GetActorID() == childID,
               "operator create child actor failed")) return false;
    if (!Check(parent->GetChildren().size() == 1 &&
               parent->GetChildren().front() == child,
               "operator create child actor did not attach to parent")) return false;

    if (!Check(stack.Undo(context) && scene.FindByID(childID) == nullptr &&
               scene.FindByID(parentID) != nullptr,
               "operator create child actor undo failed")) return false;
    if (!Check(stack.Redo(context), "operator create child actor redo failed")) return false;
    child = scene.FindByID(childID);
    parent = scene.FindByID(parentID);
    if (!Check(child && parent && child->GetParent() == parent,
               "operator create child actor redo did not restore parent")) return false;
...
bool TestEditorCommandOperatorHierarchyOrganization() {
    ...
    if (!Check(operators.Commands().MoveActorUp(context, secondID),
               "operator move actor up failed")) return false;
    std::vector<Actor*> roots = scene.GetRootActors();
    if (!Check(roots.size() >= 3 &&
               roots[0]->GetID() == secondID &&
               roots[1]->GetID() == firstID &&
               roots[2]->GetID() == thirdID,
               "operator move actor up did not reorder roots")) return false;
    if (!Check(stack.Undo(context), "operator move actor up undo failed")) return false;
    roots = scene.GetRootActors();
    if (!Check(roots.size() >= 3 &&
               roots[0]->GetID() == firstID &&
               roots[1]->GetID() == secondID &&
               roots[2]->GetID() == thirdID,
               "operator move actor up undo did not restore roots")) return false;

    if (!Check(operators.Commands().MoveActorDown(context, secondID),
               "operator move actor down failed")) return false;
    roots = scene.GetRootActors();
    if (!Check(roots.size() >= 3 &&
               roots[0]->GetID() == firstID &&
               roots[1]->GetID() == thirdID &&
               roots[2]->GetID() == secondID,
               "operator move actor down did not reorder roots")) return false;
    if (!Check(stack.Undo(context), "operator move actor down undo failed")) return false;

    Actor* parent = scene.CreateActor("Parent");
    Actor* child = scene.CreateActor("Child", parent);
    const uint64_t parentID = parent->GetID();
    const uint64_t childID = child->GetID();
    if (!Check(operators.Commands().UnparentActor(context, childID),
               "operator unparent actor failed")) return false;
    Actor* reloadedChild = scene.FindByID(childID);
    roots = scene.GetRootActors();
    if (!Check(reloadedChild && reloadedChild->GetParent() == nullptr &&
               roots.size() >= 5 &&
               roots[3]->GetID() == parentID &&
               roots[4]->GetID() == childID,
               "operator unparent actor did not move child after parent")) return false;
    if (!Check(stack.Undo(context), "operator unparent actor undo failed")) return false;
    reloadedChild = scene.FindByID(childID);
    Actor* reloadedParent = scene.FindByID(parentID);
    if (!Check(reloadedChild && reloadedParent &&
               reloadedChild->GetParent() == reloadedParent,
               "operator unparent actor undo did not restore parent")) return false;
...
bool TestEditorViewportOperatorFrameSelected() {
    ...
    if (!Check(!operators.Viewport().FrameSelected(context),
               "frame selected should fail without actor selection")) return false;
    ...
    if (!Check(viewport != nullptr, "scene viewport missing for frame selected test")) return false;

    const Vec3 before = viewport->GetCamera().GetPosition();
    if (!Check(operators.Viewport().FrameSelected(context),
               "frame selected operator failed with actor selection")) return false;
    ...
    if (!Check((viewport->GetCamera().GetTarget() - target).Length() < 1e-3f &&
               ...
               "frame selected did not move camera to selected actor")) return false;

    viewport->ToggleProjectionMode();
    if (!Check(operators.Viewport().FrameSelected(context) &&
               viewport->IsOrthographic() &&
               viewport->GetCamera().GetOrthoWidth() >= 2.0f,
               "frame selected did not preserve orthographic projection")) return false;
    return true;
}
...
bool TestEditorStatusBarTextAndActionRouting() {
    ...
    if (!Check(Editor::UI::EditorStatusBar::FormatEditorModeText(context) == "Edit" &&
               ...
               "status bar editor state mismatch")) return false;
    ...
    if (!Check(Editor::UI::EditorStatusBar::FormatSceneText(layerContext) == "Untitled *" &&
               ...
               "status bar dirty scene state mismatch")) return false;
    if (!Check(sceneLayer.BeginPlay() &&
               Editor::UI::EditorStatusBar::FormatEditorModeText(layerContext) == "Play",
               "status bar play state mismatch")) return false;
    sceneLayer.PausePlay();
    if (!Check(Editor::UI::EditorStatusBar::FormatEditorModeText(layerContext) == "Paused",
               "status bar paused state mismatch")) return false;
    sceneLayer.StopPlay();
...
bool TestEditorPanelSelectAllActionRouting() {
    ...
    if (!Check(hierarchy.CanHandleEditorAction(context, "edit.rename") &&
               ...
               "scene hierarchy did not expose or handle actor rename intent")) return false;
    if (!Check(hierarchy.CanHandleEditorAction(context, "edit.selectAll"),
               "scene hierarchy capability did not expose select all")) return false;
    if (!Check(hierarchy.HandleEditorAction(context, "edit.selectAll"),
               "scene hierarchy did not handle select all")) return false;
    if (!Check(context.GetSelection().HasActor() &&
               context.GetSelection().GetActorID() == second->GetID() &&
               context.GetSelection().GetMultiCount() == 2 &&
               context.GetSelection().IsSelected(first->GetID()) &&
               context.GetSelection().IsSelected(second->GetID()),
               "scene hierarchy select all did not select every actor")) return false;
    if (!Check(hierarchy.HandleEditorAction(context, "hierarchy.expandAll") &&
               ...
               "scene hierarchy did not handle expand/collapse all")) return false;

    namespace fs = std::filesystem;
    const auto root = fs::temp_directory_path() /
        ("myengine_panel_select_all_" + std::to_string(
            std::chrono::steady_clock::now().time_since_epoch().count()));
    ...
    if (!Check(browser.CanHandleEditorAction(context, "asset.createFolder") &&
               browser.HandleEditorAction(context, "asset.createFolder") &&
               fs::is_directory(createdFolder),
               "asset browser did not expose or handle asset.createFolder")) {
        fs::remove_all(root, ec);
        return false;
    }
    if (!Check(stack.Undo(context) && !fs::exists(createdFolder),
               "asset browser create folder action did not undo")) {
        fs::remove_all(root, ec);
        return false;
    }
    ...
    const fs::path prefabTemplatePath =
        content / "Prefabs" / "NewPrefab.prefab.json";
    ...
    if (!Check(prefabMeta.has_value() &&
               PrefabAsset::Load(prefabTemplatePath, prefabTemplate, &prefabError) &&
               prefabTemplate.uuid == prefabMeta->uuid &&
               prefabTemplate.rootLocalId == "root" &&
               prefabTemplate.nodes.size() == 1 &&
               prefabTemplate.nodes.front().name == "NewPrefab",
    ...
    const fs::path multiDuplicateMaterial =
        content / "Materials" / "Selection_Copy.mat";
    const fs::path multiDuplicateScript =
        content / "Tools" / "MoveMe_Copy.as";
    if (!Check(fs::exists(multiDuplicateMaterial) &&
               fs::exists(multiDuplicateScript),
               "asset browser multi-select duplicate did not duplicate visible assets")) {
        fs::remove_all(root, ec);
        return false;
    }
    if (!Check(stack.Undo(context) && stack.Undo(context) &&
               !fs::exists(multiDuplicateMaterial) &&
               !fs::exists(multiDuplicateScript),
    ...
    if (!Check(fs::exists(duplicatePath),
               "asset browser duplicate action did not create copy")) {
        fs::remove_all(root, ec);
        return false;
    }
    context.GetSelection().SelectAssetPath(duplicatePath.string());
    if (!Check(browser.HandleEditorAction(context, "edit.delete") &&
    ...
    if (!Check(browser.HandleEditorAction(context, "edit.copy"),
               "asset browser did not handle copy action")) {
        fs::remove_all(root, ec);
        return false;
    }
    ...
    if (!Check(browser.HandleEditorAction(context, "edit.paste"),
               "asset browser did not handle copy/paste actions")) {
        fs::remove_all(root, ec);
        return false;
    }
    const fs::path pastedPath = content / "Selection.mat";
    if (!Check(fs::exists(pastedPath) &&
               fs::exists(AssetMeta::MetaPathFor(pastedPath.string())),
...
bool TestSceneHierarchyOrganizationActions() {
    ...
    if (!Check(hierarchy.CanHandleEditorAction(context, "edit.copy") &&
               ...
               "scene hierarchy capability did not reflect selected actor edit state")) return false;
    if (!Check(hierarchy.HandleEditorAction(context, "edit.copy"),
               "scene hierarchy did not handle actor copy action")) return false;
    if (!Check(operators.Commands().HasActorClipboard(),
               "scene hierarchy copy did not populate actor clipboard")) return false;
    if (!Check(hierarchy.CanHandleEditorAction(context, "edit.paste"),
               "scene hierarchy capability did not expose actor paste after copy")) return false;
    if (!Check(hierarchy.HandleEditorAction(context, "edit.duplicate"),
               "scene hierarchy did not handle actor duplicate action")) return false;
    const uint64_t duplicateID = context.GetSelection().GetActorID();
    if (!Check(duplicateID != secondID && scene.FindByID(duplicateID),
               "scene hierarchy duplicate action did not create selected clone")) return false;
    stack.Undo(context);
    if (!Check(!scene.FindByID(duplicateID) && scene.FindByID(secondID),
               "scene hierarchy duplicate undo did not remove clone")) return false;
    if (!Check(hierarchy.HandleEditorAction(context, "edit.paste"),
               "scene hierarchy did not handle actor paste action")) return false;
    const uint64_t pastedID = context.GetSelection().GetActorID();
    if (!Check(pastedID != secondID && scene.FindByID(pastedID),
               "scene hierarchy paste action did not create selected actor copy")) return false;
    stack.Undo(context);
    if (!Check(!scene.FindByID(pastedID) && scene.FindByID(secondID),
               "scene hierarchy paste undo did not remove pasted actor")) return false;

    context.GetSelection().SelectActorID(thirdID);
    if (!Check(hierarchy.HandleEditorAction(context, "edit.delete"),
               "scene hierarchy did not handle actor delete action")) return false;
    if (!Check(!scene.FindByID(thirdID),
               "scene hierarchy delete action did not remove selected actor")) return false;
    stack.Undo(context);
    if (!Check(scene.FindByID(thirdID),
               "scene hierarchy delete undo did not restore actor")) return false;

    context.GetSelection().SelectActorID(secondID);
    if (!Check(hierarchy.HandleEditorAction(context, "hierarchy.moveUp"),
               "scene hierarchy did not move selected actor up")) return false;
    auto roots = scene.GetRootActors();
    if (!Check(roots.size() >= 3 && roots[0]->GetID() == secondID &&
               roots[1]->GetID() == firstID && roots[2]->GetID() == thirdID,
               "move up did not reorder root actors")) return false;
    stack.Undo(context);
    roots = scene.GetRootActors();
    if (!Check(roots.size() >= 3 && roots[0]->GetID() == firstID &&
               roots[1]->GetID() == secondID && roots[2]->GetID() == thirdID,
               "move up undo did not restore root order")) return false;
    stack.Redo(context);
    roots = scene.GetRootActors();
    if (!Check(roots.size() >= 3 && roots[0]->GetID() == secondID &&
               roots[1]->GetID() == firstID && roots[2]->GetID() == thirdID,
               "move up redo did not restore moved order")) return false;
    stack.Undo(context);

    context.GetSelection().SelectActorID(secondID);
    if (!Check(hierarchy.HandleEditorAction(context, "hierarchy.moveDown"),
               "scene hierarchy did not move selected actor down")) return false;
    roots = scene.GetRootActors();
    if (!Check(roots.size() >= 3 && roots[0]->GetID() == firstID &&
               roots[1]->GetID() == thirdID && roots[2]->GetID() == secondID,
               "move down did not reorder root actors")) return false;
    stack.Undo(context);

    context.GetSelection().SelectActorID(secondID);
    if (!Check(hierarchy.HandleEditorAction(context, "hierarchy.createEmptyParent"),
               "scene hierarchy did not create empty parent")) return false;
    Actor* newParent = scene.FindByID(context.GetSelection().GetActorID());
    Actor* reloadedSecond = scene.FindByID(secondID);
    if (!Check(newParent && newParent->GetName() == "Empty Parent" &&
               reloadedSecond && reloadedSecond->GetParent() == newParent,
               "create empty parent did not wrap selected actor")) return false;
    stack.Undo(context);
    if (!Check(scene.FindByID(secondID) && !scene.FindByID(secondID)->GetParent() &&
               scene.FindByName("Empty Parent") == nullptr,
               "create empty parent undo did not restore hierarchy")) return false;
    stack.Redo(context);
    newParent = scene.FindByID(context.GetSelection().GetActorID());
    if (!Check(newParent && scene.FindByID(secondID) &&
               scene.FindByID(secondID)->GetParent() == newParent,
               "create empty parent redo did not restore hierarchy")) return false;
    stack.Undo(context);
    ...
    if (!Check(hierarchy.CanHandleEditorAction(context, "hierarchy.selectSubtree") &&
               ...
               "scene hierarchy did not expose or handle select subtree")) return false;
    if (!Check(context.GetSelection().GetMultiCount() == 4 &&
               context.GetSelection().IsSelected(parentID) &&
               context.GetSelection().IsSelected(childID) &&
               context.GetSelection().IsSelected(grandchildID) &&
               context.GetSelection().IsSelected(siblingID),
               "select subtree did not select root and descendants")) return false;
    context.GetSelection().SelectActorID(parentID);
    if (!Check(hierarchy.HandleEditorAction(context, "hierarchy.selectChildren") &&
               context.GetSelection().GetMultiCount() == 3 &&
               !context.GetSelection().IsSelected(parentID) &&
               context.GetSelection().IsSelected(childID) &&
               context.GetSelection().IsSelected(grandchildID) &&
               context.GetSelection().IsSelected(siblingID),
               "select children did not select only descendants")) return false;
    ...
    if (!Check(hierarchy.CanHandleEditorAction(context, "hierarchy.selectParent") &&
               ...
               context.GetSelection().GetActorID() == parentID,
               "scene hierarchy did not select parent")) return false;
    if (!Check(std::string(stack.GetUndoName()) == undoNameBeforeSelectionTools,
               "select parent should not enter the command stack")) return false;
    ...
    if (!Check(hierarchy.CanHandleEditorAction(context, "hierarchy.selectNextSibling") &&
               ...
               "scene hierarchy did not select next sibling")) return false;
    if (!Check(std::string(stack.GetUndoName()) == undoNameBeforeSelectionTools,
               "select next sibling should not enter the command stack")) return false;
    ...
    if (!Check(hierarchy.CanHandleEditorAction(context, "hierarchy.selectPreviousSibling") &&
               ...
               context.GetSelection().GetActorID() == childID,
               "scene hierarchy did not select previous sibling")) return false;
    if (!Check(std::string(stack.GetUndoName()) == undoNameBeforeSelectionTools,
               "select previous sibling should not enter the command stack")) return false;
...
bool TestEditorInspectorSelectionRouting() {
    ...
    const bool modelRouted = accepts("modelAsset", modelObject)
        && !accepts("genericAsset", modelObject)
    ...
    const bool jsonRouted = accepts("genericAsset", jsonObject)
        && !accepts("modelAsset", jsonObject)
        && !accepts("prefabAsset", jsonObject)
        && !accepts("audioAsset", jsonObject)
        && !accepts("materialAsset", jsonObject)
        && !accepts("textureAsset", jsonObject);
    const EditorSelectObject prefabObject =
        EditorSelectObject::MakeAsset(prefabPath.string());
    const bool prefabRouted = accepts("prefabAsset", prefabObject)
        && !accepts("genericAsset", prefabObject)
        && !accepts("modelAsset", prefabObject)
        && !accepts("audioAsset", prefabObject)
        && !accepts("materialAsset", prefabObject)
        && !accepts("textureAsset", prefabObject);
    const EditorSelectObject audioObject =
        EditorSelectObject::MakeAsset(audioPath.string());
    const bool audioRouted = accepts("audioAsset", audioObject)
        && !accepts("genericAsset", audioObject)
        && !accepts("prefabAsset", audioObject)
        && !accepts("modelAsset", audioObject)
...
bool TestEditorProfilerBufferAndSourceContracts() {
    ...
    const std::string editorOperators = readSource("src/Editor/EditorOperators.cpp");
    ...
    if (!Check(!editorLayer.empty() && !editorCommand.empty() &&
               !editorOperators.empty() && !editorImportService.empty() &&
               !assetRegistry.empty() && !statusBar.empty() &&
               !profilerPanel.empty() && !sceneSerializer.empty() &&
               !editingWorkflows.empty(),
    ...
    if (!Check(statusBar.find("Mode: %s | Scene: %s | Selected: %s | Project: %s") != std::string::npos &&
               ...
               statusBar.find("renderContext") == std::string::npos &&
               statusBar.find(" FPS") == std::string::npos,
               "status bar still exposes performance or renderer data")) return false;
    if (!Check(profilerPanel.find("stats.frameNumber") != std::string::npos &&
               ...
               "profiler panel is missing frame or renderer performance data")) return false;
    ...
    if (!Check(assetRegistry.find("database.ValidateAgainstProject(m_Root.parent_path(), validationReport)") != std::string::npos &&
               ...
               "EditorAssetRegistry does not surface AssetDatabase validation issues as asset diagnostics")) return false;
    if (!Check(editorCommand.find("RecordCommandEvent") != std::string::npos &&
               ...
               "EditorCommandStack does not profile editor command execute/undo/redo events")) return false;
    if (!Check(editorOperators.find("RecordAssetOperatorEvent") != std::string::npos &&
               ...
               "EditorAssetOperator does not profile asset editing/open/reimport events")) return false;
    if (!Check(editorOperators.find("RecordPrefabOperatorEvent") != std::string::npos &&
               ...
               "EditorPrefabOperator does not profile prefab apply/revert/create events")) return false;
    if (!Check(editorImportService.find("RecordImportEvent") != std::string::npos &&
               ...
               "EditorImportService does not profile import/reimport events")) return false;
    if (!Check(editorImportService.find("EditorImportService::RefreshValidation") != std::string::npos &&
               ...
               "EditorImportService does not profile asset validation events")) return false;
    if (!Check(editorImportService.find("EditorImportService::GetValidationSummaryText") != std::string::npos &&
               ...
               "EditorImportService does not centralize asset validation summary text")) return false;
    if (!Check(editorImportService.find("EditorImportService::HasValidationIssues") != std::string::npos &&
               ...
               "EditorImportService does not expose validation summary severity")) return false;
    if (!Check(editingWorkflows.find("EditorCommand") != std::string::npos &&
               ...
               "editor editing workflow docs do not document profiler categories")) return false;
...
bool TestEditorProjectAndAssetRegistry() {
    ...
    const size_t importSuccessCount = static_cast<size_t>(std::count_if(
        importEvents.begin(), importEvents.end(),
        [](const EditorProfilerEvent& event) {
            return event.category == "EditorImport" &&
                   event.name == "Import" &&
                   event.details.find(";success=true") != std::string::npos &&
                   event.details.find("artifact=") != std::string::npos &&
                   event.details.find("uuid=") != std::string::npos;
        }));
    if (!Check(importSuccessCount == 2,
               "editor import service did not record profiler events for both imports")) return false;
...
bool TestEditorAssetOperatorCommandsAndWatch() {
    ...
    if (!Check(imageActor && imageComponent,
               "asset scene reference setup failed")) {
        fs::remove_all(root, error);
        return false;
    }
    const uint64_t imageActorID = imageActor->GetID();
    imageComponent->source = "Content/Textures/SceneReferenced.tex";
    const auto sceneReferences =
        operators.Assets().FindSceneReferences(context, sceneTexturePath.string());
    if (!Check(sceneReferences.size() == 1 &&
               sceneReferences.front().actorID == imageActor->GetID() &&
               sceneReferences.front().actorName == "Scene Image" &&
               sceneReferences.front().componentType == "UIImage" &&
               sceneReferences.front().jsonPath == "/source",
               "asset operator did not find scene component asset reference")) {
        fs::remove_all(root, error);
        return false;
    }
    if (!Check(operators.Assets().FindSceneReferences(
                   context, (content / "Textures" / "Unused.tex").string()).empty(),
    ...
    const fs::path projectScenePath =
        content / "Scenes" / "Referencer.scene.json";
    ...
    const auto projectSceneReferences =
        operators.Assets().FindProjectSceneReferences(
            context, sceneTexturePath.string());
    if (!Check(projectSceneReferences.size() == 1 &&
               projectSceneReferences.front().scenePath ==
                   "Content/Scenes/Referencer.scene.json" &&
    ...
    if (!Check(operators.Assets().RetargetProjectSceneReferences(
                   context, sceneTexturePath.string(), retargetTexturePath.string()) == 1,
    ...
    }
    {
        ...
        if (!Check(contentText.find("Content/Textures/SceneRetargeted.tex") !=
                       std::string::npos,
    ...
    if (!Check(stack.Undo(context),
               "asset operator project scene retarget undo command failed")) {
        fs::remove_all(root, error);
        return false;
    }
    {
        ...
        const std::string contentText{
            std::istreambuf_iterator<char>(in),
            std::istreambuf_iterator<char>()};
        if (!Check(contentText.find("Content/Textures/SceneReferenced.tex") !=
                       std::string::npos,
    ...
    if (!Check(operators.Assets().RetargetSceneReferences(
                   context, sceneTexturePath.string(), retargetTexturePath.string()) == 1,
    ...
    if (!Check(imageComponent &&
               imageComponent->source == "Content/Textures/SceneRetargeted.tex",
               "asset operator retarget did not update component asset path")) {
        fs::remove_all(root, error);
        return false;
    }
    if (!Check(stack.Undo(context),
               "asset operator retarget scene references undo command failed")) {
        fs::remove_all(root, error);
        return false;
    }
    imageActor = scene.FindByID(imageActorID);
    ...
    if (!Check(fs::exists(copyPath) &&
               fs::exists(AssetMeta::MetaPathFor(copyPath.string())) &&
    ...
    if (!Check(fs::exists(secondCopyPath) &&
               fs::exists(AssetMeta::MetaPathFor(secondCopyPath.string())) &&
               secondCopyMeta.has_value() &&
               firstCopyMeta->uuid != secondCopyMeta->uuid &&
               std::filesystem::path(context.GetSelection().GetAssetPath()).lexically_normal() ==
...

    {
        ...
        auto material = MaterialAsset::CreateDefaultAtPath(
            materialEditPath.string(), "OperatorMaterial");
        ...
        MaterialModifier modifier(
            materialEditPath.string(), "Modify Material Test",
            [source = handle.Get()](MaterialAsset& target) {
                target.ReloadFrom(*source);
                return true;
            });
        if (!Check(modifier.Modify(context),
                   "material modifier command failed")) {
            fs::remove_all(root, error);
            return false;
        }
        auto readMaterial = [&]() {
            return LoadMaterialAssetFromFile(materialEditPath.string());
        };
        auto saved = readMaterial();
        if (!Check(saved && saved->IsTwoSided() &&
                   NearlyEqual(saved->GetFloat("Metallic"), 0.75f),
        ...
        if (!Check(saved && !saved->IsTwoSided() &&
                   NearlyEqual(saved->GetFloat("Metallic"), 0.0f) &&
    ...
    if (!Check(operators.Assets().CreateFolder(context, toolsFolder.string()) &&
               fs::is_directory(toolsFolder),
               "asset operator create folder failed")) {
        fs::remove_all(root, error);
        return false;
    }
    if (!Check(stack.Undo(context) && !fs::exists(toolsFolder),
               "asset operator create folder undo failed")) {
        fs::remove_all(root, error);
        return false;
    }
    if (!Check(stack.Redo(context) && fs::is_directory(toolsFolder),
               "asset operator create folder redo failed")) {
        fs::remove_all(root, error);
        return false;
    }
    if (!Check(!operators.Assets().MoveFolder(context, content.string(),
    ...
    if (!Check(operators.Assets().MoveFolder(context, toolsFolder.string(),
                                             (content / "Materials").string()),
    ...
    if (!Check(fs::is_directory(movedToolsFolder),
               "asset operator move folder did not move directory")) {
        fs::remove_all(root, error);
        return false;
    }
    if (!Check(operators.Assets().MoveFolder(context, movedToolsFolder.string(),
                                             content.string()),
               "asset operator move folder back to root failed")) {
        fs::remove_all(root, error);
        return false;
    }
    if (!Check(fs::is_directory(toolsFolder) &&
               !fs::exists(movedToolsFolder),
               "asset operator move folder back to root did not move directory")) {
        fs::remove_all(root, error);
        return false;
    }
    if (!Check(stack.Undo(context) && fs::is_directory(movedToolsFolder) &&
               !fs::exists(toolsFolder),
    ...
    if (!Check(!operators.Assets().MoveFolder(context, toolsFolder.string(),
                                              (content / "Materials").string()) &&
               fs::is_directory(toolsFolder) &&
               fs::is_directory(movedToolsFolder),
    ...
    if (!Check(!operators.Assets().RenameFolder(context, toolsFolder.string(), "ExistingTools") &&
               fs::is_directory(toolsFolder) &&
               fs::is_directory(existingToolsName),
    ...
    if (!Check(fs::is_directory(renamedToolsFolder),
               "asset operator rename folder did not move directory")) {
        fs::remove_all(root, error);
        return false;
    }
    if (!Check(stack.Undo(context) && fs::is_directory(toolsFolder),
               "asset operator rename folder undo failed")) {
        fs::remove_all(root, error);
        return false;
    }
    ...
    if (!Check(AssetMeta::Save(dbFolderAssetMeta),
               "asset operator database folder fixture meta save failed")) {
        fs::remove_all(root, error);
        return false;
    }
    {
        ...
        record.type = "script";
        if (!Check(database.Open(databasePath) &&
    ...
    if (!Check(operators.Assets().MoveFolder(context, dbFolder.string(),
                                             (content / "Materials").string()),
               "asset operator move folder with database record failed")) {
        fs::remove_all(root, error);
        return false;
    }
    const fs::path movedDbFolderAsset =
        content / "Materials" / "DatabaseTracked" / "Tracked.as";
    {
        AssetDatabase database;
        database.Open(databasePath);
        const AssetRecord* record = database.FindByUuid(dbFolderAssetMeta.uuid);
        if (!Check(record &&
                   fs::path(record->sourcePath).lexically_normal() ==
                       movedDbFolderAsset.lexically_normal(),
    ...
    }
    {
        ...
        if (!Check(record &&
                   fs::path(record->sourcePath).lexically_normal() ==
                       dbFolderAsset.lexically_normal(),
    ...
    if (!Check(AssetMeta::Save(dbDeleteAssetMeta),
               "asset operator database delete fixture meta save failed")) {
        fs::remove_all(root, error);
        return false;
    }
    {
    ...
    if (!Check(operators.Assets().DeleteAsset(context, dbDeleteAsset.string()) &&
               !fs::exists(dbDeleteAsset) &&
    ...
    if (!Check(fs::exists(scriptPath),
               "asset operator script template did not create file")) {
        fs::remove_all(root, error);
        return false;
    }
    const auto scriptMeta = AssetMeta::Load(scriptPath.string());
    if (!Check(scriptMeta.has_value(),
               "asset operator script template did not create readable meta")) {
        fs::remove_all(root, error);
        return false;
    }
    {
    ...
    if (!Check(!operators.Assets().RenameAsset(context, scriptPath.string(),
                                               conflictingScriptPath.string()) &&
               fs::exists(scriptPath) &&
               fs::exists(conflictingScriptPath),
    ...
    if (!Check(AssetMeta::Save(metaOnly) &&
               !operators.Assets().MoveAsset(context, scriptPath.string(),
                                             (content / "Conflicts").string()) &&
               fs::exists(scriptPath) &&
               !fs::exists(metaOnlyConflict) &&
    ...
    std::string materialContent((std::istreambuf_iterator<char>(materialInput)),
                                std::istreambuf_iterator<char>());
    materialInput.close();
    ...
    if (!Check(fs::exists(templatedTexturePath) &&
    ...
    if (!Check(operators.Assets().DeleteFolder(context, toolsFolder.string()) &&
               !fs::exists(toolsFolder) &&
               !fs::exists(scriptPath) &&
    ...
    if (!Check(operators.Assets().MoveAsset(context, scriptPath.string(),
                                            (content / "Materials").string()),
    ...
    if (!Check(operators.Commands().CopySelection(context),
               "asset operator command copy asset selection failed")) {
        fs::remove_all(root, error);
        return false;
    }
    ...
    if (!Check(fs::exists(pastedScript) &&
    ...
    if (!Check(fs::exists(pastedScriptCopy) &&
               fs::exists(AssetMeta::MetaPathFor(pastedScriptCopy.string())) &&
               pastedMeta.has_value() &&
               pastedCopyMeta.has_value() &&
               pastedMeta->uuid != pastedCopyMeta->uuid &&
               std::filesystem::path(context.GetSelection().GetAssetPath()).lexically_normal() ==
    ...
    if (!Check(operators.Commands().CopyAssets(
                   context, {scriptPath.string(), extraScript.string()}),
    ...
    if (!Check(fs::exists(batchScript) &&
               fs::exists(AssetMeta::MetaPathFor(batchScript.string())) &&
               fs::exists(batchExtra) &&
               fs::exists(AssetMeta::MetaPathFor(batchExtra.string())) &&
               std::filesystem::path(context.GetSelection().GetAssetPath()).lexically_normal() ==
    ...
    if (!Check(stack.Undo(context) && stack.Undo(context) &&
               !fs::exists(batchScript) && !fs::exists(batchExtra),
    ...
    if (!Check(importService.Import(textureSource.string()),
               "asset operator import settings setup import failed")) {
        importService.OnDetach();
    ...
    if (!Check(operators.Assets().ReimportWithSettings(
                   context, textureUuid, nearestSettings.dump()),
    ...
    if (!Check(stack.Redo(context) &&
               readSettings().find("\"nearest\"") != std::string::npos,
    ...
    auto hasAssetEvent = [&](const char* operation, const char* detailNeedle = nullptr) {
        return std::any_of(assetEvents.begin(), assetEvents.end(),
                           [&](const EditorProfilerEvent& event) {
                               return event.category == "EditorAsset" &&
                                      event.name == operation &&
                                      event.details.find(";success=true") != std::string::npos &&
                                      (!detailNeedle ||
                                       event.details.find(detailNeedle) != std::string::npos);
                           });
    };
    if (!Check(hasAssetEvent("Duplicate Asset", "target=") &&
               hasAssetEvent("Delete Asset") &&
               hasAssetEvent("Create Folder") &&
               hasAssetEvent("Move Folder", "targetFolder=") &&
               hasAssetEvent("Create Asset From Template", "template=material") &&
               hasAssetEvent("Copy Asset", "target=") &&
               hasAssetEvent("Move Asset", "targetFolder=") &&
...
bool TestEditorAssetOperatorOpenSceneAsset() {
    namespace fs = std::filesystem;
    const auto root = fs::temp_directory_path() /
        ("myengine_editor_open_scene_asset_" + std::to_string(
            std::chrono::steady_clock::now().time_since_epoch().count()));
    ...
    context.SetPanelFocusRequestHandler([&](std::string_view panelID) {
        focusRequests.emplace_back(panelID);
    });
    ...
    if (!Check(layer.IsDirty() &&
               !operators.Assets().OpenAsset(context, scenePath.string()) &&
               layer.GetEditorScene().FindByName("Scratch") != nullptr &&
               context.GetSelection().HasActor() &&
               stack.CanUndo() &&
               project.GetLastScenePath() != scenePath.string(),
    ...
    if (!Check(operators.Assets().OpenAsset(context, scenePath.string()),
               "asset operator did not open scene asset")) {
        fs::remove_all(root, error);
        return false;
    }

    const bool loadedScene =
        layer.GetEditorScene().GetName() == "OpenedFromAsset" &&
        layer.GetEditorScene().FindByName("LoadedActor") != nullptr;
    const bool clearedEditorState =
        !context.GetSelection().HasActor() &&
        !context.GetSelection().HasAsset() &&
        !stack.CanUndo() &&
        !stack.CanRedo() &&
    ...
    if (!Check(operators.Assets().OpenAsset(context, prefabPath.string()) &&
               context.GetSelection().HasAsset() &&
               fs::path(context.GetSelection().GetAssetPath()).lexically_normal() ==
                   prefabPath.lexically_normal() &&
               !focusRequests.empty() &&
               focusRequests.back() == "inspector",
...
bool TestEditorPrefabOverrideOperatorSingleApplyRevert() {
    namespace fs = std::filesystem;
    const auto root = fs::temp_directory_path() /
        ("myengine_editor_prefab_override_" + std::to_string(
            std::chrono::steady_clock::now().time_since_epoch().count()));
    ...
    std::string error;
    if (!Check(PrefabSystem::SaveSubtree(*sourceRoot, prefabPath, &error),
    ...
    if (!Check(instance && !instance->GetChildren().empty(),
               "prefab instantiate failed: " + error)) {
        cleanup();
        return false;
    }
    ...
    if (!Check(overrides.front().category == "Actor" &&
               overrides.front().target.find("Actor ") == 0 &&
               overrides.front().property == "name" &&
               overrides.front().valuePreview == "EditedWheel" &&
    ...
    if (!Check(instance && !instance->GetChildren().empty() &&
               instance->GetChildren().front()->GetName() == "Wheel",
    ...
    if (!Check(!overrides.empty() &&
               operators.Prefabs().ApplyOverride(context, rootID, overrides.front().index),
    ...
    auto hasPrefabEvent = [&](const char* operation, const char* detailNeedle = nullptr) {
        return std::any_of(prefabEvents.begin(), prefabEvents.end(),
                           [&](const EditorProfilerEvent& event) {
                               return event.category == "EditorPrefab" &&
                                      event.name == operation &&
                                      event.details.find(";success=true") != std::string::npos &&
                                      (!detailNeedle ||
                                       event.details.find(detailNeedle) != std::string::npos);
...
bool TestEditorPrefabOverrideDisplayModelSorting() {
    namespace fs = std::filesystem;
    const auto root = fs::temp_directory_path() /
        ("myengine_editor_prefab_override_display_" + std::to_string(
            std::chrono::steady_clock::now().time_since_epoch().count()));
    std::error_code errorCode;
    fs::create_directories(root / "Content" / "Prefabs", errorCode);
    const fs::path previousProjectRoot = AssetManager::Get().GetProjectRoot();
    ...
    const fs::path prefabPath = root / "Content" / "Prefabs" / "Vehicle.prefab.json";
    std::string error;
    if (!Check(sourceRoot && sourceChild &&
               PrefabSystem::SaveSubtree(*sourceRoot, prefabPath, &error),
    ...
    Actor* instance = PrefabSystem::Instantiate(scene, prefabPath, {}, &error);
    if (!Check(instance && !instance->GetChildren().empty(),
               "prefab display instantiate failed: " + error)) {
        cleanup();
        return false;
    }
    ...
    if (!Check(overrides.size() >= 2,
               "prefab display model did not expose multiple override categories")) {
        cleanup();
        return false;
    }
    if (!Check(overrides.front().category == "Actor" &&
    ...
    if (!Check(std::is_sorted(
                   ...
                   [](const EditorPrefabOperator::OverrideInfo& left,
                      const EditorPrefabOperator::OverrideInfo& right) {
...
bool TestEditorPrefabOverrideUnsupportedPersistedKindIsBlocked() {
    namespace fs = std::filesystem;
    const auto root = fs::temp_directory_path() /
        ("myengine_editor_prefab_override_unsupported_" + std::to_string(
            std::chrono::steady_clock::now().time_since_epoch().count()));
    std::error_code errorCode;
    fs::create_directories(root / "Content" / "Prefabs", errorCode);
    const fs::path previousProjectRoot = AssetManager::Get().GetProjectRoot();
    ...
    if (!Check(overrides.size() == 1 &&
               overrides.front().kind == "FutureOverrideKind" &&
               overrides.front().category == "Unsupported" &&
               !overrides.front().canApply &&
               !overrides.front().canRevert &&
               overrides.front().diagnostic.find("FutureOverrideKind") !=
                   std::string::npos,
               "unsupported persisted prefab override was not surfaced as blocked")) {
        cleanup();
        return false;
    }
    if (!Check(!operators.Prefabs().ApplyOverride(context, instanceID,
                                                  overrides.front().index) &&
               !operators.Prefabs().RevertOverride(context, instanceID,
                                                   overrides.front().index) &&
               !stack.CanUndo(),
...
bool TestEditorPrefabOperatorCreateAndInstantiateCommands() {
    namespace fs = std::filesystem;
    const auto root = fs::temp_directory_path() /
        ("myengine_editor_prefab_create_" + std::to_string(
            std::chrono::steady_clock::now().time_since_epoch().count()));
    std::error_code errorCode;
    ...
    if (!Check(operators.Prefabs().CreatePrefabFromActor(context, actorID),
               "prefab create operator command failed")) {
        cleanup();
        return false;
    }
    actor = scene.FindByID(actorID);
    ...
    if (!Check(!prefabPath.empty() && fs::exists(prefabPath),
               "prefab create command did not write prefab asset")) {
        cleanup();
        return false;
    }
    ...
    const uint64_t instantiatedID =
        operators.Prefabs().InstantiatePrefab(context, prefabPath.string(), 0, placement,
                                             "Instantiate Prefab Test");
    ...
    if (!Check(operators.Prefabs().SelectInstances(context, prefabPath.string()) == 2 &&
               context.GetSelection().GetMultiCount() == 2 &&
               context.GetSelection().IsSelected(actorID) &&
    ...
    if (!Check(stack.Undo(context) &&
               scene.GetRootActors().size() == rootCountBeforeInstantiate,
    ...
    if (!Check(hasPrefabEvent("Create Prefab", "prefab=") &&
               hasPrefabEvent("Instantiate Prefab", "prefab="),
...
bool TestEditorPrefabOperatorRemovedActorOverrideApplyRevert() {
    namespace fs = std::filesystem;
    const auto root = fs::temp_directory_path() /
        ("myengine_editor_prefab_remove_actor_" + std::to_string(
            std::chrono::steady_clock::now().time_since_epoch().count()));
    std::error_code errorCode;
    fs::create_directories(root / "Content" / "Prefabs", errorCode);
    const fs::path previousProjectRoot = AssetManager::Get().GetProjectRoot();
    ...
    Actor* instance = PrefabSystem::Instantiate(scene, prefabPath, {}, &error);
    if (!Check(instance && !instance->GetChildren().empty(),
               "prefab removed actor instantiate failed: " + error)) {
        cleanup();
        return false;
    }
    const uint64_t rootID = instance->GetID();
    ...
    auto overrides = operators.Prefabs().GetOverrides(context, rootID);
    auto removed = std::find_if(overrides.begin(), overrides.end(),
        [&](const EditorPrefabOperator::OverrideInfo& item) {
            return item.kind == "RemoveActorSubtree" &&
                item.localId == removedLocalID && item.canApply && item.canRevert;
        });
    if (!Check(removed != overrides.end(),
               "prefab removed actor override was not exposed as apply/revert row")) {
        cleanup();
        return false;
    }
    ...
    overrides = operators.Prefabs().GetOverrides(context, rootID);
    removed = std::find_if(overrides.begin(), overrides.end(),
        [&](const EditorPrefabOperator::OverrideInfo& item) {
            return item.kind == "RemoveActorSubtree" && item.localId == removedLocalID;
        });
    if (!Check(removed != overrides.end() &&
               operators.Prefabs().ApplyOverride(context, rootID, removed->index),
    ...
    if (!Check(PrefabAsset::Load(prefabPath, appliedAsset, &error) &&
               std::none_of(appliedAsset.nodes.begin(), appliedAsset.nodes.end(),
                            [&](const PrefabNode& node) {
                                return node.localId == removedLocalID;
                            }),
...
bool TestEditorPrefabOperatorAddedActorOverrideApplyRevert() {
    namespace fs = std::filesystem;
    const auto root = fs::temp_directory_path() /
        ("myengine_editor_prefab_add_actor_" + std::to_string(
            std::chrono::steady_clock::now().time_since_epoch().count()));
    std::error_code errorCode;
    fs::create_directories(root / "Content" / "Prefabs", errorCode);
    const fs::path previousProjectRoot = AssetManager::Get().GetProjectRoot();
    ...
    if (!Check(added && tip && instance->GetChildren().size() == 1 &&
               added->GetChildren().size() == 1,
    ...
    auto overrides = operators.Prefabs().GetOverrides(context, rootID);
    auto addedOverride = std::find_if(overrides.begin(), overrides.end(),
        [](const EditorPrefabOperator::OverrideInfo& item) {
            return item.kind == "AddActorSubtree" &&
                item.canApply && item.canRevert;
        });
    if (!Check(addedOverride != overrides.end(),
               "prefab added actor override was not exposed as apply/revert row")) {
        cleanup();
        return false;
    }
    ...
    instance = scene.FindByID(rootID);
    if (!Check(instance && instance->GetChildren().empty(),
               "prefab added actor revert did not remove added subtree")) {
        cleanup();
        return false;
    }
    ...
    overrides = operators.Prefabs().GetOverrides(context, rootID);
    addedOverride = std::find_if(overrides.begin(), overrides.end(),
        [](const EditorPrefabOperator::OverrideInfo& item) {
            return item.kind == "AddActorSubtree";
        });
    if (!Check(addedOverride != overrides.end() &&
    ...
    if (!Check(PrefabAsset::Load(prefabPath, appliedAsset, &error) &&
               std::any_of(appliedAsset.nodes.begin(), appliedAsset.nodes.end(),
                           [](const PrefabNode& node) {
                               return node.name == "Antenna";
                           }) &&
               std::any_of(appliedAsset.nodes.begin(), appliedAsset.nodes.end(),
                           [](const PrefabNode& node) {
                               return node.name == "Tip";
                           }),
    ...
    if (!Check(PrefabAsset::Load(prefabPath, restoredAsset, &error) &&
               std::none_of(restoredAsset.nodes.begin(), restoredAsset.nodes.end(),
                            [](const PrefabNode& node) {
                                return node.name == "Antenna" || node.name == "Tip";
                            }) &&
               !operators.Prefabs().GetOverrides(context, rootID).empty(),
...
bool TestEditorPrefabOperatorAddedComponentOverrideApplyRevert() {
    namespace fs = std::filesystem;
    const auto root = fs::temp_directory_path() /
        ("myengine_editor_prefab_add_component_" + std::to_string(
            std::chrono::steady_clock::now().time_since_epoch().count()));
    std::error_code errorCode;
    fs::create_directories(root / "Content" / "Prefabs", errorCode);
    const fs::path previousProjectRoot = AssetManager::Get().GetProjectRoot();
    ...
    if (!Check(collider != nullptr,
               "prefab added component setup could not add collider")) {
        cleanup();
        return false;
    }
    ...
    auto overrides = operators.Prefabs().GetOverrides(context, rootID);
    auto added = std::find_if(overrides.begin(), overrides.end(),
        [](const EditorPrefabOperator::OverrideInfo& item) {
            return item.kind == "AddComponent" &&
                item.componentType == "BoxCollider" &&
                item.canApply && item.canRevert;
        });
    if (!Check(added != overrides.end(),
               "prefab added component override was not exposed as apply/revert row")) {
        cleanup();
        return false;
    }
    ...
    overrides = operators.Prefabs().GetOverrides(context, rootID);
    added = std::find_if(overrides.begin(), overrides.end(),
        [](const EditorPrefabOperator::OverrideInfo& item) {
            return item.kind == "AddComponent" &&
                item.componentType == "BoxCollider";
        });
    if (!Check(added != overrides.end() &&
               operators.Prefabs().ApplyOverride(context, rootID, added->index),
...
bool TestEditorPrefabOperatorRemovedComponentOverrideApplyRevert() {
    namespace fs = std::filesystem;
    const auto root = fs::temp_directory_path() /
        ("myengine_editor_prefab_remove_component_" + std::to_string(
            std::chrono::steady_clock::now().time_since_epoch().count()));
    std::error_code errorCode;
    fs::create_directories(root / "Content" / "Prefabs", errorCode);
    const fs::path previousProjectRoot = AssetManager::Get().GetProjectRoot();
    ...
    BoxColliderComponent* sourceCollider =
        sourceRoot ? sourceRoot->AddComponent<BoxColliderComponent>() : nullptr;
    if (sourceCollider) sourceCollider->SetHalfExtents({5.0f, 6.0f, 7.0f});
    const fs::path prefabPath = root / "Content" / "Prefabs" / "Trigger.prefab.json";
    std::string error;
    if (!Check(sourceRoot && sourceCollider &&
               PrefabSystem::SaveSubtree(*sourceRoot, prefabPath, &error),
    ...
    auto removed = std::find_if(overrides.begin(), overrides.end(),
        [](const EditorPrefabOperator::OverrideInfo& item) {
            return item.kind == "RemoveComponent" &&
                item.componentType == "BoxCollider" &&
                item.canApply && item.canRevert;
        });
    if (!Check(removed != overrides.end(),
               "prefab removed component override was not exposed as apply/revert row")) {
        cleanup();
        return false;
    }
...
bool TestProductionAssetDatabaseAndImportPipeline() {
    ...
    const bool registrySawValidation = std::any_of(
        assets.begin(), assets.end(),
        [&first](const EditorAssetInfo& info) {
            return info.uuid == first.record.uuid &&
                std::any_of(info.diagnostics.begin(), info.diagnostics.end(),
    ...
    if (!Check(registrySawValidation,
               "asset registry did not surface AssetDatabase validation diagnostics")) return false;
...
void DrawActorMenu() {}
    ...
    if (!Check(!registry.GetDiagnostics().empty(),
               "rejected core extension registration did not emit diagnostics")) {
        fs::remove_all(root, ec);
        return false;
    }
...
void RunFacadeMetadata()
{
    ...
    if (canUndo) stack.Undo(context);
    collider->Serialize(colliderJson);
    const bool undone = colliderJson["halfExtents"][0].get<float>() == 1.0f;

    const bool metadataOk =
        domain.ExecuteExtension("RunFacadeMetadata", "tool:facade.metadata", context, &error);
    (void)metadataOk;
    const bool hasActor =
...
bool TestEditorOperatorSourceContracts() {
    ...
    const std::string inspectorSections = readSource(root / "src/Editor/InspectorSections.cpp");
    ...
    if (!Check(!contextHeader.empty() && !panelHeader.empty() && !panelSource.empty() &&
               !operatorsSource.empty() &&
               !operatorsHeader.empty() && !assetBrowser.empty() &&
               !hierarchyPanel.empty() && !viewportPanel.empty() &&
               !inspectorPanel.empty() && !sceneViewportHeader.empty() &&
               !sceneViewportSource.empty() && !gameViewportSource.empty() &&
               !inspectorSections.empty() && !editorLayer.empty() &&
               !editorWorkspace.empty() &&
               !editorCommand.empty() && !shortcutMap.empty() && !uiFacade.empty() &&
               !sceneHeader.empty() && !actorHeader.empty() && !sceneSerializer.empty() &&
               !sceneLighting.empty() && !prefabAsset.empty() && !prefabSystem.empty() &&
               !actorSubtreeSerializer.empty(),
               "operator source contract files were not found")) return false;
    auto countSubstring = [](const std::string& source,
    ...
    if (!Check(panelHeader.find("HandleEditorAction") != std::string::npos &&
               ...
               "EditorPanel does not expose focused action routing")) return false;
    if (!Check(editorWorkspace.find("\"panelState\"") != std::string::npos &&
               ...
               "EditorWorkspace does not persist panel-local state")) return false;
    if (!Check(operatorsHeader.find("DuplicateActorSubtree") != std::string::npos &&
               ...
               operatorsSource.find("EditorComponentOperator::SetComponentPropertyForActors") != std::string::npos &&
               ...
               operatorsSource.find("RemoveAssetDatabaseRecords(commandContext, databaseRecords)") != std::string::npos &&
               operatorsSource.find("RestoreAssetDatabaseRecords(commandContext, databaseRecords)") != std::string::npos &&
    ...
    if (!Check(assetBrowser.find("registry->WatchForChanges()") == std::string::npos &&
               ...
               assetBrowser.find("if (ImGui::Selectable(\"Paste Into Folder\")) {\n"
                                 "                        RequestPasteAssetsToFolder(*context, folder.relativePath);") != std::string::npos &&
               ...
               assetBrowser.find("operators->Commands().PasteAssetToFolder(\n"
                                 "            context, resolvedTargetFolder.string())") != std::string::npos &&
               assetBrowser.find("std::filesystem::path(asset.relativePath)") != std::string::npos &&
               assetBrowser.find(".parent_path()") != std::string::npos &&
               countSubstring(assetBrowser,
                              "assetFolder.empty() ? m_SelectedFolder : assetFolder") >= 2 &&
               assetBrowser.find("ImGui::Selectable(\"Move Selected Here\")") != std::string::npos &&
               assetBrowser.find("operators->Commands().PasteAssetToFolder(*context, m_SelectedFolder)") == std::string::npos &&
               ...
               assetBrowser.find("if (ImGui::Selectable(\"Delete\")) { RequestDeleteSelectedAssets(); }") != std::string::npos &&
               ...
               assetBrowser.find("RequestCreateFolderInFolder(*context, folder.relativePath, true)") != std::string::npos &&
               assetBrowser.find("if (ImGui::BeginPopup(\"##FolderCtx\")) {\n"
                                 "                    if (ImGui::Selectable(\"Refresh\"))") != std::string::npos &&
               assetBrowser.find("const std::string folderPath = !m_PendingFolderDeletePath.empty()") != std::string::npos &&
               ...
               assetBrowser.find("AppendRetargetReferenceWarning(\"Renamed asset to: \" + newName") != std::string::npos &&
               assetBrowser.find("AppendRetargetReferenceWarning(\"Renamed folder to: \" + newName") != std::string::npos &&
               assetBrowser.find("AppendRetargetReferenceWarning(\n"
                                 "                                        \"Moved asset into folder: \"") != std::string::npos &&
               assetBrowser.find("AppendRetargetReferenceWarning(\n"
                                 "                                        \"Moved folder into folder: \"") != std::string::npos &&
               ...
               assetBrowser.find("operators->Commands().ExecuteAction(*context, \"asset.validate\")") != std::string::npos &&
               ...
               assetBrowser.find("GetAssetsInFolder(\n"
                                 "                 m_SelectedFolder, m_RecursiveAssets") != std::string::npos &&
               ...
               assetBrowser.find("SaveWorkspaceState();\n"
                                 "    SetOperationMessage(\"Opened folder: \" + m_SelectedFolder") != std::string::npos &&
               ...
               assetBrowser.find("if (!opened && discardUnsavedChanges) layer->MarkDirty();") != std::string::npos &&
               ...
               assetBrowser.find("RequestCreateFolderInFolder(context, m_SelectedFolder, false)") != std::string::npos &&
               assetBrowser.find("RequestCreateFolderInFolder(*context, folder.relativePath, true)") != std::string::npos &&
               assetBrowser.find("RequestCreateAssetFromTemplateInFolder(\n"
                                 "                                *context, folder.relativePath, \"material\")") != std::string::npos &&
               assetBrowser.find("RequestCreateAssetFromTemplateInFolder(\n"
                                 "                                *context, folder.relativePath, \"scene\")") != std::string::npos &&
               assetBrowser.find("m_SelectedFolder =\n"
                                 "            (std::filesystem::path(parentFolder) / folderPath.filename())") != std::string::npos &&
               assetBrowser.find("SaveWorkspaceState();") != std::string::npos &&
               ...
               countSubstring(assetBrowser,
                              "(IsFocused() && !m_SelectedFolder.empty() &&\n"
                              "             !IsRootFolder(m_SelectedFolder))") >= 2 &&
               ...
               assetBrowser.find("operators->Commands().CopyAssets(*context, emptyContextSelection)") != std::string::npos &&
               ...
               assetBrowser.find("operators->Assets().MoveAsset(\n"
                                 "                                    *context, source, targetFolder.string())") != std::string::npos &&
               assetBrowser.find("!StartsWithPath(folder.relativePath, source)") != std::string::npos &&
               assetBrowser.find("if (!IsRootFolder(folder.relativePath)) {\n"
                                 "                        if (const ImGuiPayload* payload = "
                                 "ImGui::AcceptDragDropPayload(kFolderPayload))") == std::string::npos,
               "AssetBrowser still bypasses operator watch/selection/create/open paths")) return false;
    if (!Check(operatorsSource.find("EditorAssetOperator::OpenAsset") != std::string::npos &&
               ...
               operatorsSource.find("if (type != EditorAssetType::Unknown) return false") != std::string::npos &&
               ...
               "asset open/reveal workflow should use operator-owned Shell API helpers")) return false;
    if (!Check(editorCommand.find("CaptureAssetDatabaseRecordsForPath") != std::string::npos &&
               editorCommand.find("m_RemovedDatabaseRecords = CaptureAssetDatabaseRecordsForPath") != std::string::npos &&
               editorCommand.find("RemoveAssetDatabaseRecords(context, m_RemovedDatabaseRecords)") != std::string::npos &&
               editorCommand.find("RestoreAssetDatabaseRecords(context, m_RemovedDatabaseRecords)") != std::string::npos,
               "asset delete commands must remove and restore AssetDatabase records")) return false;
    if (!Check(hierarchyPanel.find("CanHandleEditorAction") != std::string::npos &&
               ...
               hierarchyPanel.find("CreateActor(*context, \"Actor\");\n"
                                   "                if (id) operators->Commands().MoveActor") ==
                   std::string::npos &&
               ...
               hierarchyPanel.find("MakeSceneSnapshotCommand(\n"
                                   "            \"Create Empty Parent\"") == std::string::npos &&
               ...
               "SceneHierarchyPanel still bypasses operator duplicate/delete paths")) return false;
    if (!Check(operatorsHeader.find("CreateChildActor") != std::string::npos &&
               ...
               "child actor creation is not centralized in EditorCommandOperator")) return false;
    if (!Check(operatorsHeader.find("CreateEmptyParent") != std::string::npos &&
               ...
               "empty parent creation is not centralized in EditorCommandOperator")) return false;
    if (!Check(operatorsHeader.find("UnparentActor") != std::string::npos &&
               ...
               "hierarchy organization commands are not centralized in EditorCommandOperator")) return false;
    if (!Check(operatorsHeader.find("CreateUIActor") != std::string::npos &&
               ...
               "UI actor preset creation is not centralized in EditorCommandOperator")) return false;
    if (!Check(hierarchyPanel.find("Prefabs().CreatePrefabFromActor") != std::string::npos &&
               ...
               "prefab create/drop paths are not centralized in EditorPrefabOperator")) return false;
    if (!Check(inspectorPanel.find("Prefabs().ApplyAll") != std::string::npos &&
               ...
               inspectorPanel.find("ImGui::BeginDisabled(!operators || !hasPrefabOverrides)") != std::string::npos &&
               inspectorPanel.find("ImGui::BeginDisabled(!operators);if(ImGui::Button(\"Unpack\")") != std::string::npos &&
               ...
               inspectorPanel.find("directCommand = refreshed") == std::string::npos &&
               inspectorPanel.find("directCommand=refreshed") == std::string::npos &&
               inspectorPanel.find("directCommand = directCommand || refreshed") != std::string::npos &&
               inspectorPanel.find("directCommand=directCommand||refreshed") != std::string::npos &&
               ...
               "InspectorPanel still bypasses prefab or actor-name operator paths")) return false;
    if (!Check(actorHeader.find("GetEditorFlags") != std::string::npos &&
               ...
               "actor static/editor flags are not preserved across scene, prefab, and subtree serialization")) return false;
    if (!Check(inspectorSections.find("RemoveComponentByType(context") != std::string::npos &&
               ...
               inspectorSections.find("ImGui::SetNextWindowViewport(ImGui::GetWindowViewport()->ID)") != std::string::npos &&
               ...
               inspectorPanel.find("ImGui::SetNextWindowViewport(ImGui::GetWindowViewport()->ID)") != std::string::npos &&
               ...
               "InspectorSections do not route component edits through operators")) return false;
    if (!Check(inspectorSections.find("CommitSceneNameEdit") != std::string::npos &&
               ...
               "Scene Settings still bypass EditorCommandOperator edit helpers")) return false;
    if (!Check(inspectorSections.find("DrawAssetMetadataHeader") != std::string::npos &&
               ...
               inspectorSections.find("database.ValidateAgainstProject(projectRoot, validationReport)") != std::string::npos &&
               ...
               "asset inspector does not expose metadata/diagnostics through operator-backed UI")) return false;
    if (!Check(editorCommand.find("ModifyAssetCommand::Undo") != std::string::npos &&
               ...
               "asset modification commands do not refresh loaded assets on execute/undo")) return false;
    if (!Check(editorCommand.find("ModifyAssetsCommand::Execute") != std::string::npos &&
               ...
               "batch asset modification command is missing execute/undo support")) return false;
    if (!Check(editorCommand.find("ShouldMarkSceneDirty") != std::string::npos &&
               ...
               editorCommand.find("if (markSceneDirty) context.MarkSceneDirty()") != std::string::npos,
               "resource commands still mark the edited scene dirty")) return false;
    if (!Check(operatorsHeader.find("ReimportWithSettings") != std::string::npos &&
               ...
               "asset import settings edits do not route through undoable operator command")) return false;
    if (!Check(editorLayer.find("\"edit.delete\"") != std::string::npos &&
               ...
               editorLayer.find("panel && panel->IsVisible() &&\n"
                                "            panel->HandleEditorAction") != std::string::npos &&
               ...
               "global edit actions or shortcuts are not registered")) return false;
    if (!Check(operatorsHeader.find("FrameSelected") != std::string::npos &&
               ...
               "viewport camera controls are not centralized in EditorViewportOperator")) return false;
    if (!Check(viewportPanel.find("viewport.FrameDirection") == std::string::npos &&
               ...
               "ViewportPanel still mutates scene viewport camera or model-drop scene data directly")) return false;
    if (!Check(sceneViewportHeader.find("FrameTarget") != std::string::npos &&
               sceneViewportSource.find("Editor/") == std::string::npos,
               "runtime SceneViewport controller is missing FrameTarget or depends on Editor")) return false;
...
MYENGINE_REGISTER_TEST("Editor", "TestEditorCommandOperatorCreateEmptyParent", TestEditorCommandOperatorCreateEmptyParent);
MYENGINE_REGISTER_TEST("Editor", "TestEditorCommandOperatorCreateChildActor", TestEditorCommandOperatorCreateChildActor);
MYENGINE_REGISTER_TEST("Editor", "TestEditorCommandOperatorHierarchyOrganization", TestEditorCommandOperatorHierarchyOrganization);
...
MYENGINE_REGISTER_TEST("Editor", "TestEditorPrefabOperatorAddedComponentOverrideApplyRevert", TestEditorPrefabOperatorAddedComponentOverrideApplyRevert);
MYENGINE_REGISTER_TEST("Editor", "TestEditorPrefabOperatorRemovedComponentOverrideApplyRevert", TestEditorPrefabOperatorRemovedComponentOverrideApplyRevert);
...
MYENGINE_REGISTER_TEST("Editor", "TestEditorScriptValidationAndComponentMetadataFacade", TestEditorScriptValidationAndComponentMetadataFacade);
